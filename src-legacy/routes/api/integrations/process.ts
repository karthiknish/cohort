...
import { claimNextSyncJob, completeSyncJob, failSyncJob, getAdIntegration, updateIntegrationStatus, writeMetricsBatch } from '@/lib/ads-admin'
...
import { IntegrationTokenError, isTokenExpiringSoon, refreshGoogleAccessToken, refreshMetaAccessToken, refreshTikTokAccessToken, refreshLinkedInAccessToken } from '@/lib/integration-token-refresh'
...
async function updateProviderStatus(options: { userId: string; providerId: string; clientId?: string | null; status: 'pending' | 'success' | 'error'; message?: string | null }) {
  if (options.providerId === 'google-analytics') {
    await updateGoogleAnalyticsStatus({ userId: options.userId, clientId: options.clientId ?? null, status: options.status, message: options.message ?? null })
    return
  }
...
const handlers = adaptApiHandler(
  { bodySchema: processSchema, querySchema: processQuerySchema, rateLimit: 'sensitive' },
    ...
    try {
      ...
      if (!integration || !integration.accessToken) {
        await Promise.all([
          ...
          integration ? updateProviderStatus({ userId: targetUserId, providerId: job.providerId, clientId, status: 'error', message: 'Missing credentials' }) : Promise.resolve(),
        ])
      ...
      switch (job.providerId) {
        case 'google': {
          ...
          if (typeof accountId !== 'string' || accountId.trim().length === 0) {
            ...
            await updateProviderStatus({ userId: targetUserId, providerId: job.providerId, clientId, status: 'error', message: 'Account ID missing. Please reconnect integration.' })
          ...
          if (developerToken.length === 0) {
            await failSyncJob({ userId: targetUserId, jobId: job.id, message: 'Google Ads developer token is missing. Set GOOGLE_ADS_DEVELOPER_TOKEN and reconnect Google Ads.' })
            await updateProviderStatus({ userId: targetUserId, providerId: job.providerId, clientId, status: 'error', message: 'Google Ads developer token is missing.' })
          ...
          metrics = await fetchGoogleAdsMetrics({
            accessToken: googleAccessToken, developerToken, customerId: accountId, loginCustomerId, managerCustomerId,
            timeframeDays: job.timeframeDays,
            refreshAccessToken: async () => { googleAccessToken = await refreshGoogleAccessToken({ userId: targetUserId, clientId }); return googleAccessToken },
          })
          break
        }
        case 'facebook': {
          const accountId = integration.accountId
          if (typeof accountId !== 'string' || accountId.trim().length === 0) {
            await failSyncJob({ userId: targetUserId, jobId: job.id, message: 'Meta Ads account not configured. Please reconnect your Meta Ads integration.' })
            await updateIntegrationStatus({ userId: targetUserId, providerId: job.providerId, clientId, status: 'error', message: 'Account ID missing. Please reconnect integration.' })
            return { jobId: job.id, providerId: job.providerId, metricsCount: 0, skipped: true, reason: 'missing_account_id' }
          }
          ...
          metrics = await fetchMetaAdsMetrics({
            accessToken: metaAccessToken, adAccountId: accountId, timeframeDays: job.timeframeDays,
            refreshAccessToken: async () => { metaAccessToken = await refreshMetaAccessToken({ userId: targetUserId, clientId }); return metaAccessToken },
          })
          break
        }
        case 'linkedin': {
          ...
          metrics = await fetchLinkedInAdsMetrics({
            accessToken: linkedInAccessToken, accountId, timeframeDays: job.timeframeDays, maxRetries: 3,
            refreshAccessToken: async () => { linkedInAccessToken = await refreshLinkedInAccessToken({ userId: targetUserId, clientId }); return linkedInAccessToken },
          })
          break
        }
        case 'tiktok': {
          ...
          metrics = await fetchTikTokAdsMetrics({
            accessToken: tiktokAccessToken, advertiserId, timeframeDays: job.timeframeDays,
            refreshAccessToken: async () => { tiktokAccessToken = await refreshTikTokAccessToken({ userId: targetUserId, clientId }); return tiktokAccessToken },
          })
          break
        }
        case 'google-analytics': {
          const result = await syncGoogleAnalyticsMetrics({ userId: targetUserId, clientId, days: job.timeframeDays, requestId: req.headers.get('x-request-id') })
      ...
      // Run alert evaluation inline (was after() in Next.js - now fires synchronously for non-critical work)
      if (clientId) {
        try {
          let recipientEmail: string | null = null
          const convex = createConvexAdminClient({
            auth: { uid: targetUserId, email: null, name: null, claims: { provider: 'better-auth' }, isCron: false },
          })
          if (convex) {
            try {
              ...
              logger.error('[integrations/process] failed to fetch recipient email', queryError, { userId: targetUserId, requestId: req.headers.get('x-request-id') })
            }
          }
          if (recipientEmail) {
            const workspaceId = await resolveWorkspaceIdForUser(targetUserId)
            await processWorkspaceAlerts({ userId: targetUserId, workspaceId, clientId, recipientEmail })
          }
        } catch (alertError) {
          logger.error('[integrations/process] alert evaluation failed', alertError, { userId: targetUserId, requestId: req.headers.get('x-request-id') })
        }
      }

      return { jobId: job.id, providerId: job.providerId, metricsCount }
    } catch (error: unknown) {
      logger.error('[integrations/process] error', error, { requestId: req.headers.get('x-request-id'), userId: resolvedUserId, jobId: activeJob?.id })
...
interface SyncJobErrorLike { message?: string; userId?: string; jobId?: string; providerId?: string }
