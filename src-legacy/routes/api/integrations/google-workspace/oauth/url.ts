...
import { buildGoogleWorkspaceOAuthUrl, createGoogleWorkspaceOAuthState, resolveGoogleWorkspaceOAuthCredentials, resolveGoogleWorkspaceOAuthRedirectUri } from '@/services/google-workspace'
...
const handlers = adaptApiHandler(
  { querySchema, rateLimit: 'standard' },
  async (_req, { auth, query }) => {
    ...
    const state = createGoogleWorkspaceOAuthState({ state: auth.uid, redirect })
    const url = buildGoogleWorkspaceOAuthUrl({ clientId: googleClientId, redirectUri, state })
    return { url }
  },
)

export const Route = createFileRoute('/api/integrations/google-workspace/oauth/url')({
  server: { handlers },
})
