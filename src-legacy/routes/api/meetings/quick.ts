...
const handlers = adaptApiHandler(
  { workspace: 'required' as const, bodySchema: quickMeetingSchema, rateLimit: 'sensitive' },
  async (req, { auth, workspace, body }) => {
    ...
    const now = Date.now()
    const startTimeMs = now + 60000
    const endTimeMs = now + Math.max(10, durationMinutes) * 60000
    ...
    const calendarEvent = await createGoogleCalendarEvent({
      accessToken, title, description, startTimeMs, endTimeMs, timezone, attendeeEmails, meetingUrl: roomUrl,
    })
    const meeting = await createMeetingRecord({
      userId: auth.uid, userEmail: auth.email, title, description, startTimeMs, endTimeMs, timezone,
    ...
    const notificationSummary = await notifyMeetingScheduledEmails({
      recipientEmails: attendeeEmails, meetingTitle: title, meetingStartIso, meetingEndIso,
      meetingTimezone: timezone, organizerName: auth.name ?? auth.email ?? 'Cohorts',
...
export const Route = createFileRoute('/api/meetings/quick')({
  server: { handlers },
})
