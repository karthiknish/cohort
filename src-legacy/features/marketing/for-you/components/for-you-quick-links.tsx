...
function QuickLinkCard({ link }: {
    ...
    return (<Link href={link.href} className="group flex min-w-0 flex-1 flex-col gap-3 rounded-xl border border-border/60 bg-card p-4 shadow-sm transition-[border-color,box-shadow] hover:border-border hover:shadow-md">
...
export function ForYouQuickLinks() {
    return (<section aria-labelledby="for-you-quick-links-heading" className="mb-10">
      <h2 id="for-you-quick-links-heading" className="mb-3 text-base font-semibold text-foreground">
        Quick links
      </h2>
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {QUICK_LINKS.map((link) => (<QuickLinkCard key={link.href} link={link}/>))}
      </div>
    </section>);
}
