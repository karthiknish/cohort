...
import { applySurfaceTransform, findPreviewView, getNextPreviewViewId, INITIAL_METRIC_ID, INITIAL_VIEW_ID, isPreviewViewId, resetSurfaceTransform, type PreviewView, } from './dashboard-preview-data';
import { DashboardPreviewAgentPanel, DashboardPreviewChrome, DashboardPreviewFooterHints, DashboardPreviewMetricDetail, DashboardPreviewMetricPicker, DashboardPreviewQueuePanel, DashboardPreviewViewRail, } from './dashboard-preview-sections';
export function DashboardPreview() {
    ...
    const currentView = findPreviewView(activeViewId);
    const currentMetric = currentView.metrics.find((metric) => metric.id === activeMetricId) ?? currentView.metrics[0];
    useEffect(() => {
    ...
    const handleViewSelect = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    const handleMetricSelect = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    return (<div className="relative mx-auto w-full max-w-280">
      ...
      <div className="relative perspective-[1800px]" onMouseEnter={handlePreviewMouseEnter} onMouseLeave={handlePreviewMouseLeave} onMouseMove={handlePreviewMouseMove}>
        <div ref={surfaceRef} className="relative overflow-hidden rounded-[28px] border border-border/60 bg-background/95 shadow-2xl backdrop-blur-xl transition-transform duration-300 ease-out">
          ...
          <div className="grid gap-4 p-4 sm:p-6 lg:grid-cols-[90px_minmax(0,1fr)] lg:gap-6">
            ...
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1.55fr)_minmax(280px,0.85fr)] lg:gap-5">
              <div className="space-y-4">
                <DashboardPreviewMetricPicker currentView={currentView} currentMetric={currentMetric} onMetricSelect={handleMetricSelect}/>
