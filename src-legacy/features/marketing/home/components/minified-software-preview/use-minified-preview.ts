'use client';
import { startTransition, useCallback, useEffect, useMemo, useRef, useState, type KeyboardEvent, type MouseEvent, type RefObject, } from 'react';
...
import { getNextPreviewTabId, INITIAL_PREVIEW_TAB, isPreviewTabId, PREVIEW_TAB_ORDER, PREVIEW_TABS, } from './tabs';
...
export function useMinifiedPreview(): MinifiedPreviewController {
    ...
    const tab = PREVIEW_TABS.find((t) => t.id === activeTabId) ?? INITIAL_PREVIEW_TAB;
    ...
    const focusTabButton = (id: PreviewTabId) => {
        requestAnimationFrame(() => {
            const tabButton = document.getElementById(`preview-tab-d-${id}`) ??
                document.getElementById(`preview-tab-m-${id}`);
            tabButton?.focus();
        });
    };
    const selectTab = (id: PreviewTabId) => {
        if (id === activeTabId)
            return;
    ...
    const handleMetricClick = (e: MouseEvent<HTMLButtonElement>) => {
