...
export function buildActivityHubModel({ selectedClientName, activities, taskSummary, tasks, proposals, projects, meetings, metrics, integrationSummary, googleAnalyticsStatus, googleWorkspaceConnected, nowMs = Date.now(), }: BuildActivityHubModelArgs): ActivityHubModel {
    ...
    const proposalCounts = proposals.reduce((acc, proposal) => {
        acc.total += 1;
        if (proposal.status === 'ready')
            acc.ready += 1;
        if (proposal.status === 'sent')
            acc.sent += 1;
        if (proposal.status === 'draft' || proposal.status === 'in_progress')
            acc.drafting += 1;
        return acc;
    ...
    if (unreadActivities.length > 0) {
        priorityItems.push({
            ...
            href: '/for-you', badge: 'For you', tone: unreadActivities.length > 5 ? 'warning' : 'neutral',
        });
    }
