...
function dayOverDayPercent(chartData: AnalyticsChartPoint[], date: string, key: keyof Pick<AnalyticsChartPoint, 'users' | 'sessions' | 'revenue' | 'conversions' | 'conversionRate'>): number | null {
...
function DayChange({ value }: {
    ...
    return (<span className={cn('inline-flex items-center gap-1 text-[11px] font-medium', isUp && 'text-success', isDown && 'text-destructive', !isUp && !isDown && 'text-muted-foreground')}>
...
function TooltipRow({ label, value }: {
    ...
    return (<div className="flex items-center justify-between gap-6 text-xs">
...
function TooltipTip({ children }: {
    ...
    return (<p className="mt-2 border-t border-border/60 pt-2 text-[11px] leading-relaxed text-muted-foreground">
      {children}
    </p>);
}
function TooltipPanel({ title, children }: {
    ...
    return (<div className="grid min-w-[12rem] gap-1.5 rounded-lg border border-border/60 bg-popover px-3 py-2.5 text-xs text-popover-foreground shadow-lg">
      <p className="text-sm font-semibold text-foreground">{title}</p>
      {children}
    </div>);
}
export function AnalyticsUsersSessionsTooltip({ active, payload, chartData, }: AnalyticsChartTooltipProps & {
    ...
    return (<TooltipPanel title={formatTooltipDate(point.date)}>
...
export function AnalyticsRevenueTooltip({ active, payload, chartData, formatRevenue, }: AnalyticsChartTooltipProps & {
    chartData: AnalyticsChartPoint[];
...
export function AnalyticsConversionsTooltip({ active, payload, chartData, formatRevenue, }: AnalyticsChartTooltipProps & {
    chartData: AnalyticsChartPoint[];
    formatRevenue: (amount: number | null | undefined) => string;
}) {
    if (!active || !payload?.length)
...
export function AnalyticsConversionRateTooltip({ active, payload, chartData, }: AnalyticsChartTooltipProps & {
    chartData: AnalyticsChartPoint[];
}) {
    if (!active || !payload?.length)
        return null;
