...
function channelMembersDialogFormReducer(state: ChannelMembersDialogFormState, action: ChannelMembersDialogFormAction): ChannelMembersDialogFormState {
    switch (action.type) {
...
function ChannelMembersDialogForm({ channel, teamMembers, onOpenChange, onSave, onDelete, }: {
    ...
    return (<>
      <DialogContent className="sm:max-w-2xl">
          ...
          <div className="rounded-2xl border border-border/70 bg-muted/20 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                {channel.description ? (<p className="mt-1 text-sm text-muted-foreground">{channel.description}</p>) : (<p className="mt-1 text-sm text-muted-foreground">No description yet.</p>)}
              </div>
        ...
        <DialogFooter>
          {channel.isCustom && onDelete ? (<Button variant="destructive" onClick={handleOpenDeleteConfirm} disabled={saving || deleting} className="mr-auto">
      ...
      <ConfirmDialog open={confirmDeleteOpen} onOpenChange={handleConfirmDeleteOpenChange} title="Delete channel?" description={`This will permanently remove #${channel.name} and its message history for everyone.`} confirmLabel={deleting ? 'Deleting...' : 'Delete channel'} variant="destructive" isLoading={deleting} onConfirm={handleDelete}/>
    </>);
}
export function ChannelMembersDialog({ open, onOpenChange, channel, teamMembers, onSave, onDelete, }: ChannelMembersDialogProps) {
    const dialogKey = `${channel?.id ?? 'no-channel'}:${open ? 'open' : 'closed'}`;
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      {channel ? (<ChannelMembersDialogForm key={dialogKey} channel={channel} teamMembers={teamMembers} onOpenChange={onOpenChange} onSave={onSave} onDelete={onDelete}/>) : null}
    </Dialog>);
}
