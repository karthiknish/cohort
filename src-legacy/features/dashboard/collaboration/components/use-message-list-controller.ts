...
export function useMessageListController({ messages, hasMore, isLoading, onLoadMore, onToggleReaction, reactionPendingByMessage = EMPTY_REACTION_PENDING_BY_MESSAGE, renderers: renderersProp, focusMessageId, focusThreadId, typingIndicatorText, }: UseMessageListControllerArgs) {
    ...
    useEffect(() => {
        ...
        if (prependSnapshotRef.current) {
            ...
            const conversationSwitched = previousFirst !== null &&
                previousLast !== null &&
                firstId !== null &&
                lastId !== null &&
                previousFirst !== firstId &&
                previousLast !== lastId;
            const appendedAtBottom = previousLast !== null && previousFirst === firstId && previousLast !== lastId;
    ...
    useEffect(() => {
        const resolvedFocusId = (typeof focusMessageId === 'string' && focusMessageId.trim().length > 0
            ? focusMessageId.trim()
            : null) ??
