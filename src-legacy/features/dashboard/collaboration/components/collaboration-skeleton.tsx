      ...
      <Card className="border-muted/60 bg-background">
        <CardHeader className="border-b border-muted/40 pb-4">
          <div className="flex flex-wrap items-center gap-3">
            {tabSlots.map((slot) => (<Skeleton key={slot} className="h-6 w-40"/>))}
          </div>
        </CardHeader>
        <CardContent className="flex flex-col gap-4 p-0 lg:flex-row">
          <div className="w-full max-w-xs border-r border-muted/30 p-4">
            ...
            <div className="mt-4 space-y-3">
              {folderSlots.map((slot) => (<div key={slot} className="rounded-md border border-muted/60 bg-muted/10 p-3">
          ...
          <div className="flex-1 space-y-4 p-4">
            ...
            <div className="space-y-3">
              {messageSlots.map((slot) => (<div key={slot} className="flex items-start gap-3">
          ...
          <div className="w-full max-w-xs space-y-4 border-l border-muted/30 p-4">
            ...
            <div className="space-y-3">
              {sidebarSlots.map((slot) => (<div key={slot} className="rounded-md border border-muted/60 bg-muted/10 p-3">
            ...
            <Skeleton className="h-5 w-40"/>
            <div className="space-y-3">
              {insightSlots.map((slot) => (<div key={slot} className="rounded-md border border-muted/60 bg-muted/10 p-3">
                  <Skeleton className="h-4 w-32"/>
                  <div className="mt-2 grid grid-cols-3 gap-2">
                    {insightSlots.map((innerSlot) => (<Skeleton key={innerSlot} className="h-3 w-full"/>))}
                  </div>
                </div>))}
            </div>
          </div>
