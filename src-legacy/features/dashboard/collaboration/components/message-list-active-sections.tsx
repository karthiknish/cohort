...
import { MessageListJumpToLatest, MessageListScrollBody, } from './message-list-scroll-sections';
...
export function MessageListActiveBody({ variant = 'dm', currentUserId, showAvatars = true, editingMessageId, deletingMessageId, updatingMessageId, ...controllerArgs }: MessageListActiveBodyProps) {
    ...
    return (<div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
      <MessageListScrollBody scrollRef={scrollRef} messagesEndRef={messagesEndRef} isChannel={isChannel} hasMore={controllerArgs.hasMore} isLoading={controllerArgs.isLoading} groupedMessages={groupedMessages} typingIndicatorText={controllerArgs.typingIndicatorText} onScroll={handleScroll} onLoadMore={requestLoadOlder} groupedMessagesProps={groupedMessagesProps}/>
