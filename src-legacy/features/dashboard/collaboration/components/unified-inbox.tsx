...
import { ChannelConversationPane, type ChannelParticipant, ConversationListPane, DirectMessageConversationPane, EmptyConversationPane, } from './unified-inbox-sections';
...
type UnifiedInboxChannelPaneProps = {
    ...
    onOpenChannelMessage?: (messageId: string, options?: {
...
export function UnifiedInbox({ currentUserId, sidebar, channelPane, directMessagePane, manageChannel, }: UnifiedInboxProps) {
    ...
    const listState = ({ canLoadMore, loading: isCurrentChannelLoading, loadingMore });
    const searchState = ({
        active: inbox.isChannelSearchActive,
        searching: searchingMessages,
    });
    const composerState = ({ sending, uploading });
    return (<div className="flex min-h-0 flex-1 flex-col overflow-hidden max-lg:min-h-[min(72dvh,640px)] lg:h-[640px] lg:flex-row">
      <ConversationListPane className={cn(inbox.hasActiveConversation && 'max-lg:hidden')} channelCount={inbox.channelCount} dmCount={inbox.dmCount} filteredItems={inbox.filteredItems} isLoading={inbox.isLoading} isSelected={inbox.isSelected} onNewDM={onNewDM} onSearchQueryChange={inbox.setSearchQuery} onSelectItem={inbox.handleSelectItem} onSourceFilterChange={inbox.setSourceFilter} searchQuery={inbox.searchQuery} sourceFilter={inbox.sourceFilter} totalUnread={inbox.totalUnread}/>

      <div className={cn('flex min-h-0 min-w-0 flex-1 flex-col border-muted/40 max-lg:border-t lg:border-t-0', !inbox.hasActiveConversation && 'max-lg:hidden')}>
        {selectedChannel ? (<ChannelConversationPane listState={listState} searchState={searchState} composerState={composerState} channelMessages={channelMessages} channelMessagesForPane={inbox.channelMessagesForPane as CollaborationMessage[]} channelParticipants={channelParticipants} mentionParticipants={mentionParticipants} currentUserId={currentUserId} currentUserRole={currentUserRole} deepLinkMessageId={deepLinkMessageId ?? null} deepLinkThreadId={deepLinkThreadId ?? null} messageDeletingId={messageDeletingId} messageInput={messageInput} messageSearchQuery={messageSearchQuery} messageUpdatingId={messageUpdatingId} onAddAttachments={onAddAttachments} onBackToInbox={inbox.handleBackToInbox} onComposerBlur={onComposerBlur} onComposerFocus={onComposerFocus} onDeleteMessage={onDeleteMessage} onEditMessage={onEditMessage} onLoadMore={onLoadMore} onLoadMoreThreadReplies={onLoadMoreThreadReplies} onLoadThreadReplies={onLoadThreadReplies} onMarkThreadAsRead={onMarkThreadAsRead} onMessageInputChange={onMessageInputChange} onMessageSearchChange={onMessageSearchChange} onRemoveAttachment={onRemoveAttachment} onSendMessage={onSendMessage} onShareToPlatform={onShareToPlatform} onCreateTask={onCreateTask} onForwardMessage={onForwardMessage} onCreatePoll={onCreatePoll} onExportChannel={onExportChannel} onOpenChannelMessage={onOpenChannelMessage} onToggleReaction={onToggleReaction} pendingAttachments={pendingAttachments} reactionPendingByMessage={reactionPendingByMessage} searchHighlights={searchHighlights} onClearDeepLink={onClearDeepLink} messagesError={messagesError} onRetryMessages={onRetryMessages} selectedChannel={selectedChannel} sharedFiles={sharedFiles} canManageMembers={canManageSelectedChannel} onManageMembers={onManageSelectedChannel} threadErrorsByRootId={threadErrorsByRootId} threadLoadingByRootId={threadLoadingByRootId} threadMessagesByRootId={threadMessagesByRootId} threadNextCursorByRootId={threadNextCursorByRootId} threadUnreadCountsByRootId={threadUnreadCountsByRootId} typingIndicatorText={inbox.typingIndicatorText} channelUnreadCount={channelUnreadCount} onMarkChannelRead={onMarkChannelRead} markChannelReadPending={markChannelReadPending} workspaceId={workspaceId} isAdmin={isAdmin}/>) : selectedDM ? (<DirectMessageConversationPane mentionParticipants={mentionParticipants} currentUserId={currentUserId} dmDeleteMessage={dmDeleteMessage} dmEditMessage={dmEditMessage} dmHasMoreMessages={dmHasMoreMessages} dmIsLoadingMessages={dmIsLoadingMessages} dmIsLoadingMore={dmIsLoadingMore} dmIsSending={dmIsSending} dmLoadMoreMessages={dmLoadMoreMessages} dmMessageInput={inbox.dmMessageInput} dmMessageSearchQuery={dmMessageSearchQuery} dmMessagesForPane={inbox.dmMessagesForPane as DirectMessage[]} dmMuteConversation={dmMuteConversation} dmSearchHighlights={dmSearchHighlights} dmSearchingMessages={dmSearchingMessages} dmToggleReaction={dmToggleReaction} handleSendDirectMessage={inbox.handleSendDirectMessage} isDmSearchActive={inbox.isDmSearchActive} onAddAttachments={onAddAttachments} onArchiveConversation={dmArchiveConversation} onBackToInbox={inbox.handleBackToInbox} onDmMessageSearchChange={onDmMessageSearchChange} onRemoveAttachment={onRemoveAttachment} pendingAttachments={pendingAttachments} selectedDM={selectedDM} setActiveDmMessageInput={inbox.setActiveDmMessageInput} uploading={uploading} onStartNewDM={onStartNewDM} messagesError={dmMessagesError} onRetryMessages={onDmRetryMessages} onShareToPlatform={onShareToPlatform} onComposerFocus={dmHandleComposerFocus} onComposerBlur={dmHandleComposerBlur} typingIndicatorText={inbox.typingIndicatorText} onCreateTask={dmOnCreateTask} currentUserRole={dmCurrentUserRole} workspaceId={dmWorkspaceId} deepLinkMessageId={dmDeepLinkMessageId} onClearDeepLink={dmOnClearDeepLink}/>) : (<EmptyConversationPane channelCount={inbox.channelCount} dmCount={inbox.dmCount} onNewDM={onNewDM}/>)}
      </div>
    </div>);
}
