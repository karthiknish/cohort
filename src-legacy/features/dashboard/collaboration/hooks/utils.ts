...
import type { CollaborationAttachment, CollaborationMention, CollaborationMessageFormat, CollaborationReaction, } from '@/types/collaboration';
import { COLLABORATION_REACTION_SET } from '@/constants/collaboration-reactions';
import type { Channel } from '../types';
import { MAX_ATTACHMENT_SIZE, ALLOWED_ATTACHMENT_EXTENSIONS, ALLOWED_ATTACHMENT_MIME_TYPES, } from './constants';
 ...
 */
export function validateAttachments(files: FileList | File[], currentCount: number, maxAttachments: number): AttachmentValidationResult {
...
export function sanitizeReaction(input: unknown): CollaborationReaction | null {
    ...
    const validUserIds = Array.from(new Set(userIdsRaw.filter((value): value is string => typeof value === 'string' && value.trim().length > 0)));
