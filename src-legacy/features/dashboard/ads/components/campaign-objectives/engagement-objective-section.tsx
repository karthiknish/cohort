...
function engagementResourcesReducer(state: EngagementResourcesState, action: EngagementResourcesAction): EngagementResourcesState {
    switch (action.type) {
        case 'setPosts':
...
export function EngagementObjectiveSection({ formData, onChange, disabled, metaContext, }: ObjectiveComponentProps) {
    const [resources, dispatch] = useReducer(engagementResourcesReducer, undefined, createInitialEngagementResourcesState);
    ...
    const handlePostSelect = (postId: string) => onChange({ postId });
    const handleEventSelect = (eventId: string) => onChange({ eventId });
    useEffect(() => {
    ...
    return (<div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Select value={formData.engagementType} onValueChange={handleEngagementTypeChange} disabled={disabled}>
              <SelectTrigger id="engagement-type">
                <SelectValue placeholder="Select engagement type"/>
              </SelectTrigger>
              <SelectContent>
                {ENGAGEMENT_TYPES.map((type) => (<SelectItem key={type.value} value={type.value}>
      ...
      {formData.engagementType === 'POST_ENGAGEMENT' ? (<Card>
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-base">
          ...
          <CardContent className="space-y-3">
            {!canUseMetaApi ? (<Alert>
              ...
              </Alert>) : resources.loadingPosts ? (<div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Loader2 className="size-4 animate-spin" aria-hidden/>
                Loading posts…
              </div>) : resources.posts.length > 0 ? (<div className="grid gap-2 max-h-48 overflow-y-auto">
                {resources.posts.map((post) => (<ResourceOptionButton key={post.id} disabled={Boolean(disabled)} isSelected={formData.postId === post.id} onSelectResource={handlePostSelect} resourceId={post.id} title={formatPostPreview(post)} subtitle={post.id}/>))}
      ...
      {formData.engagementType === 'EVENT_RESPONSES' ? (<Card>
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-base">
          ...
          <CardContent className="space-y-3">
            {!canUseMetaApi ? (<Alert>
              ...
              </Alert>) : resources.loadingEvents ? (<div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Loader2 className="size-4 animate-spin" aria-hidden/>
                Loading events…
              </div>) : resources.events.length > 0 ? (<div className="grid gap-2 max-h-48 overflow-y-auto">
                {resources.events.map((event) => (<ResourceOptionButton key={event.id} disabled={Boolean(disabled)} isSelected={formData.eventId === event.id} onSelectResource={handleEventSelect} resourceId={event.id} title={event.name} subtitle={formatEventWhen(event.startTime) ?? event.id}/>))}
              </div>) : (<p className="text-xs text-muted-foreground">
      ...
      {formData.engagementType === 'PAGE_ENGAGEMENT' ? (<Card>
...
function ResourceOptionButton({ title, subtitle, disabled, isSelected, resourceId, onSelectResource, }: {
    ...
    return (<button type="button" onClick={handleSelectResource} disabled={disabled} className={`flex items-center justify-between rounded-lg border p-3 text-left motion-chromatic ${isSelected ? 'border-primary/30 bg-primary/10' : 'border-border hover:border-primary/50'}`}>
