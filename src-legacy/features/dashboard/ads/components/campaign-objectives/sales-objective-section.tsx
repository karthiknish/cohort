...
export function SalesObjectiveSection({ formData, onChange, disabled, metaContext }: ObjectiveComponentProps) {
    ...
    const [pixelState, dispatchPixels] = useReducer((_: PixelPickerState, value: PixelPickerState) => value, { pixels: [], loading: false });
    const [catalogState, dispatchCatalogs] = useReducer((_: CatalogPickerState, value: CatalogPickerState) => value, { catalogs: [], loading: false });
    const [setState, dispatchSets] = useReducer((_: ProductSetPickerState, value: ProductSetPickerState) => value, { sets: [], loading: false });
            ...
            dispatchCatalogs({
                catalogs: Array.isArray(rows)
                    ? rows.map((row) => ({
                        ...
                        productCount: typeof row.productCount === 'number' ? row.productCount : undefined,
                    }))
                    : [],
                loading: false,
            });
    ...
    const handleSalesModeChange = (value: string) => {
    ...
    const handleConversionEventChange = (value: string) => onChange({ conversionEvent: value });
    const handlePixelChange = (value: string) => onChange({ pixelId: value });
    const handleManualPixelChange = (event: React.ChangeEvent<HTMLInputElement>) => onChange({ pixelId: event.target.value });
    const handleCatalogChange = (value: string) => onChange({ productCatalogId: value, productSetId: undefined });
    const handleProductSetChange = (value: string) => onChange({ productSetId: value === '__all__' ? undefined : value });
    const handleManualCatalogChange = (event: React.ChangeEvent<HTMLInputElement>) => onChange({ productCatalogId: event.target.value, productSetId: undefined });
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Select value={salesMode} onValueChange={handleSalesModeChange} disabled={disabled}>
          ...
          {salesMode === 'pixel' ? (<>
              <div className="space-y-2">
                ...
                <Select value={formData.conversionEvent} onValueChange={handleConversionEventChange} disabled={disabled}>
                  <SelectTrigger id="conversion-event">
                    <SelectValue placeholder="Select conversion event"/>
                  </SelectTrigger>
                  <SelectContent>
                    {CONVERSION_EVENTS.map((event) => (<SelectItem key={event.value} value={event.value}>
              ...
              <div className="space-y-2">
                ...
                {canLoadMeta ? (pixelState.loading ? (<div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <Loader2 className="size-4 animate-spin" aria-hidden/>
                      Loading pixels…
                    </div>) : pixelState.pixels.length > 0 ? (<Select value={formData.pixelId || undefined} onValueChange={handlePixelChange} disabled={disabled}>
                      <SelectTrigger id="pixel-id">
                        <SelectValue placeholder="Select a pixel"/>
                      </SelectTrigger>
                      <SelectContent>
                        {pixelState.pixels.map((pixel) => (<SelectItem key={pixel.id} value={pixel.id}>
                            {pixel.name}
                          </SelectItem>))}
                      </SelectContent>
                    </Select>) : null) : null}
                <Input id="pixel-id-manual" placeholder="Or enter pixel ID manually" value={formData.pixelId || ''} onChange={handleManualPixelChange} disabled={disabled}/>
              </div>
            </>) : (<>
              <div className="space-y-2">
                ...
                {canLoadMeta ? (catalogState.loading ? (<div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <Loader2 className="size-4 animate-spin" aria-hidden/>
                      Loading catalogs…
                    </div>) : catalogState.catalogs.length > 0 ? (<Select value={formData.productCatalogId || undefined} onValueChange={handleCatalogChange} disabled={disabled}>
                      <SelectTrigger id="product-catalog-id">
                        <SelectValue placeholder="Select a catalog"/>
                      </SelectTrigger>
                      <SelectContent>
                        {catalogState.catalogs.map((catalog) => (<SelectItem key={catalog.id} value={catalog.id}>
                            {catalog.name}
                ...
                <Input id="product-catalog-id-manual" placeholder="Or enter catalog ID manually" value={formData.productCatalogId || ''} onChange={handleManualCatalogChange} disabled={disabled}/>
              </div>

              {formData.productCatalogId && canLoadMeta ? (<div className="space-y-2">
                  <Label htmlFor="product-set-id">Product set (optional)</Label>
                  {setState.loading ? (<div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <Loader2 className="size-4 animate-spin" aria-hidden/>
                      Loading product sets…
                    </div>) : setState.sets.length > 0 ? (<Select value={formData.productSetId ?? '__all__'} onValueChange={handleProductSetChange} disabled={disabled}>
                      ...
                      <SelectContent>
                        ...
                        {setState.sets.map((set) => (<SelectItem key={set.id} value={set.id}>
                            {set.name}
