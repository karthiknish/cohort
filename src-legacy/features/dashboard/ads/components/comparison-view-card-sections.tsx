...
function TrendIndicator({ delta, invertTrend }: {
    ...
    if (delta === null || Math.abs(delta) < 0.0001)
        return <Minus className="size-4 text-muted-foreground" aria-label="No change"/>;
...
function ProviderComparisonView({ currency, providerCurrencies, providers }: {
    ...
    if (providers.length === 0)
        return <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-muted/60 p-10 text-center text-sm text-muted-foreground"><p>No provider data available for comparison.</p></div>;
...
export function ComparisonViewCardShell({ currency, onTabChange, periodComparison, providerComparison, providerCurrencies, selectedTab }: {
    currency?: string;
    onTabChange: (value: 'period' | 'platform') => void;
    periodComparison: PeriodComparison | null;
    providerComparison: ProviderComparison[];
    providerCurrencies?: Record<string, string>;
    selectedTab: 'period' | 'platform';
}) {
