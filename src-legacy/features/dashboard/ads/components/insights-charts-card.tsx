...
import { getProviderDisplayName, pickDefaultInsightsTab, resolveChartProviderKey, resolveInsightsChartCurrency, tabHasChartData, type InsightsTabId, } from './insights-chart-utils';
import { InsightsFunnelPanel } from './insights-funnel-panel';
import { BenchmarkComparisonChart, EfficiencyRadarChart, ProviderComparisonChart, SpendTrendChart, } from './insights-themed-charts';
import { InsightsChartsEmptyState, InsightsChartsHeader, InsightsChartsLoadingState, InsightsChartPanel, InsightsChartsTabs, } from './insights-charts-card-sections';
...
export function InsightsChartsCard({ analysis, currency, providerCurrencies, loading = false, hasConnections = false, metricsDisplayState, }: InsightsChartsCardProps) {
    ...
    const funnelChartKeys = (analysis ? Object.keys(analysis.chartData.funnelCharts) : []);
    const activeProvider = resolveChartProviderKey(selectedProvider, funnelChartKeys);
    ...
    if (!analysis || analysis.summaries.length === 0) {
        return (<InsightsChartsEmptyState hasConnections={hasConnections} metricsDisplayState={metricsDisplayState}/>);
    }
    ...
    return (<MotionCard className={ADS_PAGE_THEME.surfaceCard}>
      ...
      <InsightsChartsTabs activeTab={activeTab} onTabChange={setActiveTab} tabAvailability={tabAvailability}>
        <InsightsChartPanel value="comparison" title="Spend vs Revenue by Platform" description="Compare financial performance across connected platforms">
          <ProviderComparisonChart currency={chartCurrency} data={analysis.chartData.providerComparison}/>
        </InsightsChartPanel>
        <InsightsChartPanel value="efficiency" title="Efficiency Breakdown" description="Multi-dimensional performance analysis">
          <EfficiencyRadarChart data={analysis.chartData.efficiencyBreakdown} providerId={activeProvider} providerLabel={providerLabel}/>
        </InsightsChartPanel>
        <InsightsChartPanel value="trends" title="Spend Trend Analysis" description="Historical spend with trend line">
          <SpendTrendChart currency={chartCurrency} data={analysis.chartData.trendCharts} providerId={activeProvider} providerLabel={providerLabel}/>
        </InsightsChartPanel>
        <InsightsChartPanel value="funnel" title="Conversion Funnel" description="Impressions → Clicks → Conversions drop-off analysis">
          <InsightsFunnelPanel stages={funnelStages} analysis={funnelAnalysis} providerLabel={providerLabel}/>
        </InsightsChartPanel>
        <InsightsChartPanel value="benchmarks" title="Industry Benchmarks" description="How you compare to industry averages">
          <BenchmarkComparisonChart data={analysis.chartData.benchmarkCharts} providerId={activeProvider} providerLabel={providerLabel}/>
        </InsightsChartPanel>
      </InsightsChartsTabs>
    </MotionCard>);
}
