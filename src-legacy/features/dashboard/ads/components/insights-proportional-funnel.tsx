...
import type { ProportionalFunnelStage } from './insights-proportional-funnel-types';
...
function TrapezoidSegment({ stage, nextWidthPct, segmentHeight, }: {
    ...
    const containerStyle = ({ height: segmentHeight });
    return (<div className="relative flex w-full justify-center" style={containerStyle}>
      <svg className="size-full max-w-md" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden>
        <polygon points={`${50 - top / 2},0 ${50 + top / 2},0 ${50 + bottom / 2},100 ${50 - bottom / 2},100`} fill={stage.fill} stroke="hsl(var(--border))" strokeWidth={0.6} vectorEffect="non-scaling-stroke"/>
      </svg>
    </div>);
}
...
export function InsightsProportionalFunnel({ stages, className, }: {
    ...
    return (<div className={cn('space-y-4', className)}>
      ...
      <figure className="mx-auto flex w-full max-w-lg flex-col gap-1" aria-label="Conversion funnel chart">
        {layoutStages.map((stage, index) => {
            const nextWidth = index < layoutStages.length - 1
