...
import { CreativeComparisonDialog, CreativesCardContent, CreativesCardHeader, CreativesDisconnectedState, } from './creatives-card-sections';
const creative = {
    providerId: 'facebook',
...
describe('creatives card sections', () => {
    it('renders the disconnected state and card header', () => {
        ...
        const headerMarkup = renderToStaticMarkup(<CreativesCardHeader loading={false} onCompare={vi.fn()} onLoad={vi.fn()} providerName="Meta Ads" selectedCount={2} summary={headerSummary}/>);
    ...
    it('renders the creatives content table', () => {
        const markup = renderToStaticMarkup(<CreativesCardContent creatives={[creative]} onToggleSelected={vi.fn()} selectedIds={new Set(['creative-1'])} summary={contentSummary}/>);
    ...
    it('renders the comparison dialog', () => {
        const markup = renderToStaticMarkup(<CreativeComparisonDialog creatives={[creative]} onOpenChange={vi.fn()} onPromote={vi.fn()} open={true} providerName="Meta Ads" selectedIds={new Set(['creative-1'])}/>);
