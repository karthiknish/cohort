...
function CreativeImage({ src, alt, fallbackSrc }: {
    src?: string;
    alt?: string;
    fallbackSrc?: string;
}) {
    ...
    const imageStyle: CSSProperties = { imageRendering: 'crisp-edges' };
    ...
    return <div className="relative size-full"><NextImage src={currentSrc} alt={alt || ''} fill unoptimized sizes="(max-width: 768px) 100vw, 320px" className="object-cover" style={imageStyle} onError={handleImageError}/></div>;
}
function getTypeIcon(type: string) {
    const normalized = type.toLowerCase();
    if (normalized.includes('lead'))
        return <span className="text-xs font-medium">LG</span>;
    if (normalized.includes('carousel'))
        return <GalleryHorizontal className="size-4" aria-hidden/>;
    if (normalized.includes('dynamic_product'))
        return <ShoppingBag className="size-4" aria-hidden/>;
    if (normalized.includes('dynamic_creative'))
        return <Layers className="size-4" aria-hidden/>;
    if (normalized.includes('boosted') || normalized.includes('page_post'))
        return <Link2 className="size-4" aria-hidden/>;
    if (normalized.includes('video'))
        return <Video className="size-4"/>;
    if (normalized.includes('image') || normalized.includes('display'))
...
export function CreativesDisconnectedState({ providerName }: {
    providerName: string;
}) {
...
export function CreativesCardHeader({ loading, onCompare, onLoad, providerName, selectedCount, summary }: {
    loading: boolean;
    onCompare: () => void;
    onLoad: () => void;
    providerName: string;
    ...
    return <CardHeader className="flex flex-row items-center justify-between gap-y-0 pb-2"><div><CardTitle className="text-lg">Creatives</CardTitle><CardDescription>{summary ? `${summary.total} creatives` : `View ${providerName} ad creatives`}</CardDescription></div><div className="flex gap-2">{selectedCount > 1 ? <Button variant="default" size="sm" onClick={onCompare}><BarChart2 className="mr-2 size-4"/>Compare &amp; Test ({selectedCount})</Button> : null}<Button variant="outline" size="sm" onClick={onLoad} disabled={loading}><span className={`mr-2 inline-flex size-4 items-center justify-center ${loading ? 'animate-spin' : ''}`}>↻</span>Load Creatives</Button></div></CardHeader>;
}
export function CreativesCardContent({ creatives, onToggleSelected, selectedIds, summary }: {
    creatives: Creative[];
    onToggleSelected: (creativeId: string) => void;
    ...
    const toggleSelectedHandlers = Object.fromEntries(creatives.map((creative) => [creative.creativeId, () => onToggleSelected(creative.creativeId)])) as Record<string, () => void>;
    if (creatives.length === 0)
        return <CardContent><p className="text-sm text-muted-foreground">Click &quot;Load Creatives&quot; to view your ad creatives.</p></CardContent>;
