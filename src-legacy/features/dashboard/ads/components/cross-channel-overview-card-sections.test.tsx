...
vi.mock('@/features/dashboard/home/components/performance-chart', () => ({
    PerformanceChart: ({ currency, metrics }: {
        currency?: string;
        metrics: Array<{
            providerId: string;
        }>;
    }) => <div>chart:{currency}:{metrics.length}:{metrics[0]?.providerId}</div>,
}));
...
describe('cross channel overview card sections', () => {
    it('renders the header controls and filter chips', () => {
        const markup = renderToStaticMarkup(<CrossChannelOverviewHeader availableProviders={['google']} dateRange={DATE_RANGE} hasMetricData={true} hasProviderFilter={true} onDateRangeChange={vi.fn()} onExport={vi.fn()} onToggleProvider={vi.fn()} selectedProviders={['google']}/>);
    ...
    it('renders summary cards and the chart', () => {
        const markup = renderToStaticMarkup(<CrossChannelOverviewContent currency="GBP" chartMetrics={[{ id: '1', providerId: 'google', date: '2024-01-01', spend: 120, impressions: 1000, clicks: 45, conversions: 4, revenue: 380 }]} metricsLoading={false} summaryCards={[{ id: 'spend', label: 'Total Spend', value: '£120', helper: 'Connected platforms in this date range' }]}/>);
