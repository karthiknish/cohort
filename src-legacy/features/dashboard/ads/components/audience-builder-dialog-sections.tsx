...
function AudienceStepButton({ activeTab, index, onSelectStep, step, totalSteps, }: {
    ...
    return (<div className="flex items-center">
      <button type="button" onClick={onSelectAudienceStep} className={cn('flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium transition-colors', activeTab === step.id
...
function AudienceInput({ className, id, onEnter, onValueChange, placeholder, value, }: {
    ...
    const onAudienceInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
...
function AudienceTextarea({ id, onValueChange, placeholder, rows, value, }: {
    ...
    const onAudienceTextareaChange = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
        onValueChange(event.target.value);
    };
...
function SegmentBadge({ segment, index, onRemoveSegment, }: {
    ...
    return (<Badge key={segment} variant="secondary" className="gap-1 rounded-lg">
      {segment}
...
function InterestBadge({ interest, onAddInterest, }: {
    ...
    return (<Badge variant="outline" asChild>
      <button type="button" onClick={onAddSuggestedInterest} className="cursor-pointer rounded-lg transition-colors hover:border-primary hover:bg-accent/10" aria-label={`Add interest: ${interest}`}>
...
function SelectedInterestBadge({ interest, onRemoveInterest, }: {
    ...
    return (<Badge className="gap-1 rounded-lg">
      {interest}
...
function AgePresetButton({ active, onAgePreset, preset, }: {
    ...
    return (<button type="button" onClick={onSelectAgePreset} className={cn('rounded-lg border px-4 py-2 text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2', active ? 'border-primary bg-primary text-primary-foreground' : 'hover:bg-muted')} aria-label={`Select age range: ${preset.label}`} aria-pressed={active}>
      {preset.label}
    </button>);
}
function GenderButton({ formData, gender, onGenderToggle, onResetGenders, }: {
    ...
    return (<button type="button" onClick={onSelectGender} className={cn('flex-1 rounded-lg border px-4 py-3 text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2', gender === 'All' && formData.genders.length === 0
            ? 'border-primary bg-primary text-primary-foreground'
            : isSelected
                ? 'border-primary bg-primary text-primary-foreground'
                : 'hover:bg-muted')} aria-label={`Select gender: ${gender}`} aria-pressed={isSelected}>
      {gender}
    </button>);
}
export function AudienceBuilderDialogHeader({ activeTab, completionSteps, onSelectStep, providerId, }: {
    activeTab: string;
    completionSteps: CompletionStep[];
    onSelectStep: (tab: string) => void;
    providerId: string;
}) {
    return (<DialogHeader className="border-b bg-gradient-to-r from-primary/5 to-transparent p-6 pb-4">
      ...
      <div className="mt-4 flex items-center gap-2">
        {completionSteps.map((step, index) => (<AudienceStepButton key={step.id} activeTab={activeTab} index={index} onSelectStep={onSelectStep} step={step} totalSteps={completionSteps.length}/>))}
      </div>
    </DialogHeader>);
}
export function AudienceBuilderDialogTabs({ activeTab, formData, newInterest, newSegment, onAddInterest, onAddSegment, onAgePreset, onDescriptionChange, onGenderToggle, onInterestInputChange, onLocationRemove, onLocationSelect, onNameChange, onRemoveInterest, onRemoveSegment, onResetGenders, onSegmentInputChange, providerId, setActiveTab, }: {
    ...
    return (<ScrollArea className="max-h-[50vh] flex-1">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsContent value="basics" className="m-0 space-y-4 p-6">
          <FormField id="aud-name" label={<>
                Audience Name <span className="text-destructive">*</span>
              </>}>
            <AudienceInput id="aud-name" placeholder="e.g. Website Visitors - Last 30 Days" value={formData.name} onValueChange={onNameChange} className="h-11"/>
          </FormField>

          <FormField id="aud-desc" label="Description" description="Describe your target audience and campaign goals.">
            <AudienceTextarea id="aud-desc" placeholder="Describe your target audience and campaign goals…" value={formData.description} onValueChange={onDescriptionChange} rows={3}/>
          </FormField>
        </TabsContent>
        ...
        <TabsContent value="locations" className="m-0 space-y-4 p-6">
          ...
          <LocationMap selectedLocations={formData.locations} onLocationSelect={onLocationSelect} onLocationRemove={onLocationRemove} height="280px" interactive={true} showSearch={true} showSelectedList={true}/>
        </TabsContent>
        ...
        <TabsContent value="targeting" className="m-0 space-y-6 p-6">
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <AudienceInput placeholder={providerId === 'linkedin' ? 'Enter job title or skill' : 'Enter interest or keyword'} value={newSegment} onValueChange={onSegmentInputChange} onEnter={onAddSegment} className="h-10"/>
            ...
            <div className="flex min-h-[32px] flex-wrap gap-1.5">
              {formData.segments.length === 0 ? (<p className="py-1 text-xs italic text-muted-foreground">No segments added yet</p>) : (formData.segments.map((segment, index) => (<SegmentBadge key={segment} segment={segment} index={index} onRemoveSegment={onRemoveSegment}/>)))}
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <AudienceInput placeholder="Add custom interest…" value={newInterest} onValueChange={onInterestInputChange} onEnter={handleAddInterest} className="h-10"/>
              <Button type="button" size="sm" onClick={handleAddInterest} disabled={!newInterest.trim()} className="h-10 px-4" aria-label="Add interest">
                <Plus className="size-4"/>
              </Button>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {SUGGESTED_INTERESTS.flatMap((interest) => !formData.interests.includes(interest)
            ? [<InterestBadge key={interest} interest={interest} onAddInterest={onAddInterest}/>]
            : [])}
              </div>
            </div>

            {formData.interests.length > 0 ? (<div className="flex flex-wrap gap-1.5 rounded-lg border bg-muted/30 p-3">
                {formData.interests.map((interest) => (<SelectedInterestBadge key={interest} interest={interest} onRemoveInterest={onRemoveInterest}/>))}
              </div>) : null}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {AGE_PRESETS.map((preset) => (<AgePresetButton key={preset.label} preset={preset} active={formData.ageMin === preset.min && formData.ageMax === preset.max} onAgePreset={onAgePreset}/>))}
            </div>
            {formData.ageMin || formData.ageMax ? (<p className="text-sm text-muted-foreground">
          ...
          <div className="space-y-3">
            <Label>Gender</Label>
            <div className="flex gap-2">
              {(['Male', 'Female', 'All'] as const).map((gender) => (<GenderButton key={gender} gender={gender} formData={formData} onGenderToggle={onGenderToggle} onResetGenders={onResetGenders}/>))}
            </div>
          </div>
        </TabsContent>
      </Tabs>
...
export function AudienceBuilderDialogFooter({ completedCount, loading, onCancel, onCreate, totalSteps, }: {
    ...
    return (<DialogFooter className="border-t bg-muted/30 p-4">
      <div className="flex w-full items-center justify-between">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <div className={cn('flex size-6 items-center justify-center rounded-full text-xs font-medium', completedCount >= 2 ? 'bg-primary text-primary-foreground' : 'bg-muted')}>
            {completedCount}
          </div>
          <span>
