...
export function CampaignControlHeader({ icon: Icon, eyebrow = 'Campaign settings', title, description, actions, stats, className, }: Props) {
    return (<CardHeader className={cn('border-b border-border/50 pb-5', className)}>
      <div className="flex flex-col gap-4">
        ...
        {stats && stats.length > 0 ? (<div className="flex flex-wrap gap-2 sm:gap-3">
            {stats.map((stat) => (<div key={stat.label} className={ADS_PAGE_THEME.controlStatChip}>
