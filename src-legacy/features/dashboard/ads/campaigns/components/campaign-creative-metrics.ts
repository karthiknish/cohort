...
export function getMetricsForAd(ad: CampaignAd, adMetrics: Record<string, CreativePerformanceMetrics | undefined>): CreativePerformanceMetrics | null {
    return adMetrics[ad.creativeId] ?? deriveCreativeMetrics(ad.metrics) ?? null;
}
export function computeCreativeTotals(ads: CampaignAd[], adMetrics: Record<string, CreativePerformanceMetrics | undefined>): CreativeTotals {
...
export function sortCreativesByMetric(ads: CampaignAd[], adMetrics: Record<string, CreativePerformanceMetrics | undefined>, sortKey: CreativeSortKey): CampaignAd[] {
    const sorted = [...ads];
    sorted.sort((a, b) => {
        if (sortKey === 'name') {
...
export function resolveCreativeInsights(ads: CampaignAd[], adMetrics: Record<string, CreativePerformanceMetrics | undefined>): Map<string, CreativeInsightKind> {
    ...
    for (const { ad, metrics } of withMetrics) {
        ...
        const needsReview = metrics.spend >= 25 &&
    ...
    if (topSpendId)
        insights.set(topSpendId, 'top-spend');
    if (bestRoasId && bestRoasId !== topSpendId)
        insights.set(bestRoasId, 'best-roas');
    if (highCtrId && !insights.has(highCtrId))
        insights.set(highCtrId, 'high-ctr');
    return insights;
}
export const CREATIVE_INSIGHT_LABELS: Record<CreativeInsightKind, {
    label: string;
    className: string;
}> = {
    'top-spend': {
