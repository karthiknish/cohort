...
export function CreateCreativeLeadFormSection({ workspaceId, clientId, campaignObjective, pageId, leadFormId, disabled, onLeadFormIdChange, }: Props) {
    ...
    if (!isLeadsCampaign)
        return null;
    return (<div className="space-y-3 rounded-lg border border-border/60 bg-muted/20 p-4">
      <div>
      ...
      {!pageId ? (<Alert>
        ...
        </Alert>) : state.loading ? (<div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Loader2 className="size-4 animate-spin" aria-hidden/>
          Loading forms…
        </div>) : state.forms.length > 0 ? (<div className="grid gap-2 max-h-40 overflow-y-auto">
          {state.forms.map((form) => (<LeadFormOption key={form.id} form={form} disabled={Boolean(disabled)} isSelected={leadFormId === form.id} onSelect={handleSelect}/>))}
...
function LeadFormOption({ form, disabled, isSelected, onSelect, }: {
    ...
    return (<button type="button" onClick={handleSelectLeadForm} disabled={disabled} className={`flex items-center justify-between rounded-lg border p-3 text-left text-sm motion-chromatic ${isSelected ? 'border-info/30 bg-info/10' : 'border-border hover:border-info/50'}`}>
