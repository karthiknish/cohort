...
import { buildPlacementDetailDraftFromSource, togglePlacementDraftValue, type MetaPlacementDetailDraft, } from '@/lib/meta-placement-positions';
...
import { buildBaseMetaTargetingPayload, persistAdSetTargeting, type AudienceControlDispatch, } from './audience-control-targeting-persist';
type DraftState = {
...
export function useAudienceControlTargetingHandlers({ dispatch, aggregatedData, drafts: { draftInterests, draftLocations, draftAudiences, draftDemographics, draftPlacements, draftPlacementDetail, editingSection, }, canEditMetaTargeting, workspaceId, activeAdSetId, clientId, updateAdSetTargeting, fetchTargeting, }: UseAudienceControlTargetingHandlersArgs) {
    const audiencesForPayload = draftAudiences ?? aggregatedData.audiences;
    const handleToggleEditing = (section: string) => {
        ...
        if (section === 'placements' && editingSection !== 'placements' && !draftPlacements) {
            dispatch({
                ...
                value: aggregatedData.placements.length > 0
    ...
    const handleAddInterestDraft = (interest: {
        ...
        dispatch({
            ...
            updater: (prev) => {
                ...
                if (base.some((item) => item.id === interest.id || item.name === interest.name))
                    return base;
                return [...base, interest];
            },
        });
    };
    const handleRemoveInterestDraft = (interestName: string) => {
        dispatch({
            type: 'updateDraftInterests',
            updater: (prev) => {
                const base = prev ?? aggregatedData.interests;
    ...
    const handleAddLocationDraft = (item: MetaTargetingSearchResult) => {
        dispatch({
            ...
            updater: (prev) => {
                ...
                if (base.included.some((loc) => loc.id === item.id))
                    return base;
                return {
                    ...base,
                    included: [
    ...
    const handleRemoveLocationDraft = (locationId: string) => {
        dispatch({
            type: 'updateDraftLocations',
            updater: (prev) => {
                const base = prev ?? {
    ...
    const handleAddAudienceDraft = (audience: {
        ...
        dispatch({
            ...
            updater: (prev) => {
                ...
                if (base.included.some((item) => item.id === audience.id))
                    return base;
                return {
                    ...base,
    ...
    const handleRemoveAudienceDraft = (audienceId: string) => {
        dispatch({
            type: 'updateDraftAudiences',
            updater: (prev) => {
                const base = prev ?? aggregatedData.audiences;
    ...
    const handlePersistPlacements = async () => {
        const nextPlatforms = draftPlacements ??
    ...
    const handleTogglePlatformDraft = (platformId: string) => {
        dispatch({
    ...
    const handleTogglePlacementPositionDraft = (field: keyof MetaPlacementDetailDraft, positionId: string) => {
        dispatch({
    ...
    const handleDraftDemographicsChange = (updater: (prev: {
        ageMin: number;
        ageMax: number;
        genders: string[];
    }) => {
