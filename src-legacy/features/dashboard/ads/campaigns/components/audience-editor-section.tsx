...
export function AudienceEditorSection({ aggregatedData, expandedSections, toggleSection, editingSection, onToggleEditing, canEditTargeting, workspaceId, clientId, onAddInterest, onRemoveInterest, onSaveTargeting, savingTargeting, }: AudienceEditorSectionProps) {
    const [newInterest, setNewInterest] = useState('');
    const isEditing = editingSection === 'interests';
    const handleToggleInterests = () => {
        toggleSection('interests');
    };
    const handleEditInterests = (event: React.MouseEvent<HTMLButtonElement>) => {
        event.stopPropagation();
        onToggleEditing('interests');
    };
    const handleNewInterestChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const editButton = (<TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <Button variant={isEditing ? 'default' : 'ghost'} size="icon" className="size-8" onClick={handleEditInterests} aria-pressed={isEditing} aria-label="Edit interests">
    ...
    return (<TargetingCollapsiblePanel sectionId="interests" icon={Heart} title="Interests" count={aggregatedData.interests.length} expanded={expandedSections.has('interests')} onToggle={handleToggleInterests} headerActions={editButton}>
      <div className="space-y-3">
        {isEditing ? (canEditTargeting && workspaceId ? (<MetaTargetingSearchCombobox workspaceId={workspaceId} clientId={clientId} mode="interests" disabled={savingTargeting} onSelect={handleMetaInterestSelect}/>) : (<div className="flex gap-2">
              <Input placeholder="Add interest (ID or name)…" value={newInterest} onChange={handleNewInterestChange} className="h-9 flex-1 text-sm"/>
        ...
        {aggregatedData.interests.length > 0 ? (<InterestGroups interests={aggregatedData.interests} removable={isEditing} onRemove={handleRemoveInterest}/>) : (<p className="py-2 text-center text-xs text-muted-foreground">
            No interests configured.{isEditing ? ' Add one above.' : ''}
          </p>)}

        {isEditing && onSaveTargeting ? (<div className="flex flex-col gap-2 border-t border-border/60 pt-3">
            {canEditTargeting ? (<p className="text-xs text-muted-foreground">
                Search Meta interests above, then save to the selected ad set.
              </p>) : null}
            <Button size="sm" className="w-full sm:w-auto" disabled={savingTargeting} onClick={handleSaveTargetingClick}>
...
function InterestGroups({ interests, removable, onRemove, }: {
    ...
    if (categories.length === 1 && (categories[0] === 'General' || categories[0] === 'interest')) {
        ...
        {interests.map((interest) => (<AudienceInterestBadge key={interest.id} name={interest.name} removable={removable} onRemove={onRemove}/>))}
      </div>);
    }
    return (<div className="space-y-3">
      {categories.map((category) => (<div key={category}>
          ...
          <div className="flex flex-wrap gap-1.5">
            {categorized[category]!.map((interest) => (<AudienceInterestBadge key={interest.id} name={interest.name} removable={removable} onRemove={onRemove}/>))}
          </div>
        </div>))}
    </div>);
}
function AudienceInterestBadge({ name, removable, onRemove, }: {
    ...
    return (<Badge variant="outline" className="group text-xs">
      <Heart className="mr-1 size-3 text-accent" aria-hidden/>
      {name}
      {removable ? (<button type="button" onClick={handleRemove} className="ml-1 rounded-sm opacity-0 transition-opacity hover:text-destructive group-hover:opacity-100" aria-label={`Remove ${name}`}>
