...
export function ProposalDraftPanel({ draftId, autosaveStatus, stepContent, onBack, onNext, isFirstStep, isLastStep, currentStep, totalSteps, isSubmitting, validationMessages, }: ProposalDraftPanelProps) {
    ...
    return (<div className="flex min-h-0 flex-1 flex-col gap-4">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain pr-0.5 [-webkit-overflow-scrolling:touch]">
        <ProposalDraftContentShell stepContent={stepContent} validationMessages={validationMessages}/>
      </div>

      <ProposalDraftFooter currentStep={currentStep} isFirstStep={isFirstStep} isLastStep={isLastStep} isSubmitting={isSubmitting} onBack={onBack} onNext={onNext} totalSteps={totalSteps}/>
    </div>);
}
