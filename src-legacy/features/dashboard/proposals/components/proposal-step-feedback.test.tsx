...
describe('ProposalStepFeedback', () => {
    it('renders nothing when there are no validation errors', () => {
        const markup = renderToStaticMarkup(<ProposalStepFeedback validationMessages={[]}/>);
        expect(markup).toBe('');
    });
    it('renders validation guidance when required fields are still missing', () => {
        const markup = renderToStaticMarkup(<ProposalStepFeedback validationMessages={['Company name is required.', 'Industry is required.']}/>);
