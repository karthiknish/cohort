...
import { canDragProjectKanbanCard, projectKanbanReducer, resolveProjectKanbanMoveTarget, } from './project-kanban-logic';
...
describe('project kanban helpers', () => {
    ...
    it('blocks drag while a status update is pending or project is deleted', () => {
        ...
        expect(canDragProjectKanbanCard({ ...baseProject, deletedAt: new Date().toISOString() }, new Set())).toBe(false);
    });
    it('tracks drag-over state in the reducer', () => {
        const started = projectKanbanReducer({ draggedProject: null, dragOverStatus: null, boardAnnouncement: '' }, { type: 'startDrag', draggedProject: { id: 'project-1', sourceStatus: 'active' } });
