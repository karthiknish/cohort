...
export const STATUS_DOT_STYLES = Object.fromEntries(PROJECT_STATUSES.map((status) => [status, { backgroundColor: STATUS_ACCENT_COLORS[status] }])) as Record<ProjectStatus, {
    backgroundColor: string;
}>;
export const STATUS_ICONS: Record<ProjectStatus, React.ComponentType<{
    className?: string;
}>> = {
...
export function projectMatchesQuery(project: Pick<ProjectRecord, 'name' | 'description' | 'clientName' | 'tags'>, rawQuery: string): boolean {
...
export function buildTaskCountsByProject(tasks: Array<{
...
export function mergeProjectTaskCounts(project: ProjectRecord, counts: Record<string, {
    taskCount: number;
    openTaskCount: number;
}>): ProjectRecord {
...
export function filterProjectsByQuery<T extends Pick<ProjectRecord, 'name' | 'description' | 'clientName' | 'tags'>>(projects: T[], rawQuery: string): T[] {
    const query = rawQuery.trim();
    if (!query)
        return projects;
    return projects.filter((project) => projectMatchesQuery(project, query));
}
export function projectMatchesContext(project: Pick<ProjectRecord, 'id' | 'name'>, projectId?: string | null, projectName?: string | null): boolean {
...
export function computeTimelineRange(projects: ProjectRecord[], milestones: {
