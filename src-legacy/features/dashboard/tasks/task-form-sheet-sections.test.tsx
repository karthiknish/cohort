...
describe('task form sheet sections', () => {
    it('renders shared sheet fields with helper text', () => {
        const markup = renderToStaticMarkup(<TaskSheetFields ids={sharedIds} formState={formState} setFormState={vi.fn()} disabled={false} mentionableUsers={sharedMentionableUsers} titlePlaceholder="e.g. Prepare Q4 campaign brief" clientPlaceholder="Select a client from the dashboard" projectPlaceholder="Open tasks from a project to link them automatically" clientHelpText="Switch clients in the header to change assignment." projectHelpText="Start from a project view to attach tasks here." dueDateLayout="compact"/>);
    ...
    it('renders the empty attachments helper', () => {
        const markup = renderToStaticMarkup(<TaskSheetAttachmentsSection disabled={false} pendingAttachments={[]} onAddAttachments={vi.fn()} onRemoveAttachment={vi.fn()} fileInputRef={emptyFileInputRef}/>);
