    ...
    return (<>
        ...
        <div className="grid grid-cols-2 gap-3.5">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button id="quick-task-due" type="button" variant="outline" className={cn(TASKS_THEME.selectTrigger, 'w-full justify-start text-left font-normal', !dueDate && 'text-muted-foreground')} disabled={isLoading} aria-label="Due date, open calendar">
                  <CalendarIcon className="mr-2 size-4 shrink-0"/>
                {dueDate ? format(parseISO(dueDate), 'PPP') : <span>Pick a date</span>}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar mode="single" selected={dueDate ? parseISO(dueDate) : undefined} disabled={isTaskDueDateDisabled} onSelect={onDateSelect} initialFocus/>
            </PopoverContent>
          </Popover>
          </TaskFormField>
        </div>
      ...
      <TaskFormSection title="Attachments">
        <div className="flex items-center justify-between gap-2">
          ...
          <Button type="button" variant="outline" size="sm" className="h-8 shrink-0 gap-1.5" disabled={isLoading} onClick={handleOpenFileDialog}>
            <Paperclip className="size-3.5"/>
            Attach
          </Button>
        </div>
        <input ref={fileInputRef} type="file" multiple aria-label="Attach files to task" className="hidden" onChange={handleFilesChange}/>
        {pendingAttachments.length > 0 ? (<PendingAttachmentsList attachments={pendingAttachments} uploading={isLoading} onRemove={onRemoveAttachment}/>) : null}
      </TaskFormSection>

      {error ? <TaskModalError message={error}/> : null}

      <div className={TASKS_THEME.dialog.footer}>
        <TaskModalActions onCancel={onCancel} submitLabel="Create task" loadingLabel="Creating…" isLoading={isLoading} submitDisabled={!title.trim()}/>
      </div>
    </>);
}
