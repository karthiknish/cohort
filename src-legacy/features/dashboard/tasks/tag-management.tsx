...
export function TagManagement({ availableTags, selectedTags, onTagsChange, onTagCreate, className, }: TagManagementProps) {
    ...
    return (<div className={cn('flex flex-wrap items-center gap-2', className)}>
      {/* Selected tags */}
      {selectedTags.map((tag) => (<SelectedTagBadge key={tag} tag={tag} onRemove={handleRemoveTag}/>))}
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button variant="outline" size="sm" className="h-7 gap-1 text-xs">
            ...
            Add Tag
            {selectedTags.length > 0 && (<span className="ml-1 text-muted-foreground">({selectedTags.length})</span>)}
          </Button>
        </PopoverTrigger>
...
export function TagFilter({ availableTags, selectedTag, onTagChange, className, }: {
    availableTags: string[];
    ...
    return (<Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="outline" size="sm" className={cn('gap-1.5', selectedTag && 'border-accent/50', className)}>
      ...
      <PopoverContent className="w-48 p-2" align="start">
        <div className="space-y-0.5">
          <TagFilterOptionButton label="All tags" selected={!selectedTag} tag={null} selectedTag={selectedTag} onTagChange={onTagChange}/>
          {availableTags.map((tag) => (<TagFilterOptionButton key={tag} label={tag} selected={selectedTag === tag} dotClassName={getTagBgColor(tag)} tag={tag} selectedTag={selectedTag} onTagChange={onTagChange}/>))}
        </div>
      </PopoverContent>
    </Popover>);
}
function SelectedTagBadge({ tag, onRemove, }: {
    tag: string;
...
function AvailableTagButton({ tag, selected, onToggle, }: {
    tag: string;
    selected: boolean;
    ...
    return (<button type="button" onClick={onToggleTagFilter} className={cn('w-full flex items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm transition-colors', selected ? 'bg-accent/10 text-primary' : 'hover:bg-muted')}>
      <div className={cn('size-2 rounded-full shrink-0', getTagDotColor(tag))}/>
      <span className="flex-1 truncate">{tag}</span>
      {selected ? <span className="text-xs">✓</span> : null}
    </button>);
}
function TagFilterOptionButton({ label, selected, dotClassName, tag, selectedTag, onTagChange, }: {
    label: string;
    selected: boolean;
    dotClassName?: string;
    tag: string | null;
    ...
    return (<button type="button" onClick={onSelectTag} className={cn('flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm transition-colors', selected ? 'bg-muted' : 'hover:bg-muted')}>
      {dotClassName ? <div className={cn('size-2 shrink-0 rounded-full', dotClassName)}/> : null}
