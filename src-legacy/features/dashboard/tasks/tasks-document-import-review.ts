...
export function taskNeedsAssigneeReview(task: ProposedImportTask): boolean {
    ...
    if (task.documentAssigneeNames.length > 0) {
        if (task.assignmentStatus === 'unassigned')
            return true;
        if (task.assignmentStatus === 'resolved' && task.assignedToUserIds.length === 0)
            return true;
    }
...
export function buildImportReviewDescription(documentSummary: string | null, tasks: ProposedImportTask[]): string {
    if (documentSummary)
...
export function resolveImportAssigneeUserIds(value: string, participants: Parameters<typeof resolveAssigneeUserIds>[1]): string[] {
