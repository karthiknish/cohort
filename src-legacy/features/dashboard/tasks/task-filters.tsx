...
export function TaskFilters({ searchQuery, onSearchChange, selectedStatus, onStatusChange, selectedAssignee, onAssigneeChange, assigneeOptions, showAssigneeFilter, sortField, onSortFieldChange, sortDirection, onSortDirectionToggle, hasActiveFilters = false, onClearFilters, }: TaskFiltersProps) {
    ...
    return (<div className={TASKS_THEME.filterBar}>
        ...
        <Popover open={filtersOpen} onOpenChange={setFiltersOpen}>
          ...
          <PopoverContent align="end" className="w-72 space-y-3 p-4">
            ...
            <div className="flex gap-2">
              ...
              <Button type="button" variant="outline" size="icon" className="size-9 shrink-0" onClick={onSortDirectionToggle} aria-label={sortDirection === 'asc' ? 'Sort descending' : 'Sort ascending'}>
                {sortDirection === 'asc' ? (<ArrowUp className="size-4"/>) : (<ArrowDown className="size-4"/>)}
              </Button>
            </div>
            {hasActiveFilters && onClearFilters ? (<Button type="button" variant="ghost" size="sm" className="h-9 w-full" onClick={onClearFilters}>
                Clear filters
              </Button>) : null}
          </PopoverContent>
        </Popover>
