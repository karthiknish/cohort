...
export function TaskListLoadingState({ viewMode }: {
    viewMode: 'grid' | 'list';
}) {
    if (viewMode === 'grid') {
        return (<>
        {['task-skeleton-1', 'task-skeleton-2', 'task-skeleton-3', 'task-skeleton-4', 'task-skeleton-5', 'task-skeleton-6'].map((skeletonKey) => (<Skeleton key={skeletonKey} className="h-44 w-full rounded-2xl"/>))}
      </>);
    }
...
export function TaskListErrorState({ error, loading, onRefresh, viewMode, }: {
    error: string;
    loading: boolean;
    onRefresh: () => void;
    viewMode: 'grid' | 'list';
}) {
    return (<div className={viewMode === 'grid' ? 'col-span-full' : ''}>
...
export function TaskListEmptyState({ emptyStateMessage, showEmptyStateFiltered, viewMode, onClearFilters, onCreateTask, }: {
    ...
    return (<div className={viewMode === 'grid' ? 'col-span-full p-4' : 'p-4'}>
...
export function TaskListLoadMore({ loadingMore, onLoadMore, viewMode, }: {
    loadingMore: boolean;
    onLoadMore: () => void;
    viewMode: 'grid' | 'list';
}) {
    return (<div className={viewMode === 'grid' ? 'col-span-full border-t border-border/80 px-4 py-3' : 'border-t border-border/80 px-4 py-3'}>
...
export function TaskListItems({ onDelete, onEdit, onOpen, onQuickStatusChange, onSelectToggle, pendingStatusUpdates, selectedTaskIds, tasks, viewMode, }: {
    ...
    if (viewMode === 'grid') {
        return (<>
        {tasks.map((task) => (<div key={task.id} className="h-full">
            <TaskCard task={task} variant="grid" isPendingUpdate={pendingStatusUpdates.has(task.id)} onOpen={onOpen} onEdit={onEdit} onDelete={onDelete} onQuickStatusChange={onQuickStatusChange} selected={selectedTaskIds?.has(task.id)} onSelectToggle={onSelectToggle}/>
          </div>))}
      </>);
    }
