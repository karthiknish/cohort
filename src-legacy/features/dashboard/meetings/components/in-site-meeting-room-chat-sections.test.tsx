...
import { MeetingChatFloatingDock, MeetingChatLauncherButton, MeetingChatMessageItem, MeetingChatPanel, } from './in-site-meeting-room-chat-sections';
...
describe('meeting room chat sections', () => {
    ...
    it('renders the open panel states', () => {
        const markup = renderToStaticMarkup(<MeetingChatPanel attachmentAccept=".png,.pdf" canSend={true} chatMessages={[]} draft="@a" fileInputRef={sharedFileInputRef} highlightedMentionIndex={0} isSending={false} localAvatarUrl={null} maxAttachments={10} mentionLabels={[]} mentionResults={sharedMentionResults} messageEndRef={sharedMessageEndRef} onAttachmentSelection={panelHandler} onClose={panelHandler} onComposerBlur={panelHandler} onDraftChange={panelHandler} onKeyDown={panelHandler} onMentionMouseDown={panelHandler} onRemoveAttachment={panelHandler} onSelectMention={panelHandler} onSend={panelHandler} pendingAttachments={sharedPendingAttachments} showMentionResults={true} textareaRef={sharedTextareaRef} uploadingFiles={false}/>);
    ...
    it('renders the floating dock launcher and panel branches', () => {
        const closedMarkup = renderToStaticMarkup(<MeetingChatFloatingDock compact={false} isOpen={false} onOpen={launcherOpen} unreadCount={2} panelProps={dockPanelProps}/>);
        const openMarkup = renderToStaticMarkup(<MeetingChatFloatingDock compact={false} isOpen={true} onOpen={launcherOpen} unreadCount={0} panelProps={dockPanelProps}/>);
