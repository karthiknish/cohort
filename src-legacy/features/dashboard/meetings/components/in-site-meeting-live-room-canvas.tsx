...
function shouldRetryAutoCapture(error: string | null): boolean {
    ...
    return !(normalized.includes('denied') ||
...
export function InSiteMeetingLiveRoomCanvas(props: LiveRoomCanvasProps) {
    ...
    }, [autoCaptureEnabled, onInterimTranscriptChange, trimmedTranscript]);
    useEffect(() => {
    ...
    return (<LiveRoomCanvasShell aiStatusLabel={aiStatusLabel} captureLabel={captureLabel} compact={compact} isSupported={isSupported} layoutContext={layoutContext} meetingTitle={meetingTitle} roomViewportRef={roomViewportRef} shouldUseAssertiveLiveRegion={shouldUseAssertiveLiveRegion}>
      <LiveRoomCanvasViewport aiStatusLabel={aiStatusLabel} autoSyncing={autoSyncing} canMinimize={canMinimize} captureLabel={captureLabel} compact={compact} finalizingSession={finalizingSession} isMinimized={isMinimized} notesProcessingState={notesProcessingState} onToggleMinimize={onToggleMinimize} onTogglePictureInPicture={onTogglePictureInPicture} pipActive={pipActive} pipSupported={pipSupported} recordingPrompt={<MeetingRecordingPromptCard canRecord captureError={error} captureSupported={isSupported} compact inRoom recordingEnabled={transcriptRecordingEnabled} onEnableRecording={onEnableTranscriptRecording}/>} showRecordingPrompt={!transcriptRecordingEnabled} tracks={tracks} transcriptProcessingState={transcriptProcessingState}/>
    </LiveRoomCanvasShell>);
}
