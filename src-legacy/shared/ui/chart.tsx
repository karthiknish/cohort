...
import { Legend, ResponsiveContainer, Tooltip } from "@/shared/ui/recharts-dynamic";
...
function ChartContainer({ id, className, children, config, ...props }: React.ComponentProps<"div"> & {
    ...
    return (<ChartContext.Provider value={contextValue}>
      <div data-slot="chart" data-chart={chartId} className={cn("[&_.recharts-cartesian-axis-tick_text]:fill-muted-foreground [&_.recharts-cartesian-grid_line]:stroke-border/50 [&_.recharts-curve.recharts-tooltip-cursor]:stroke-border [&_.recharts-polar-grid_line]:stroke-border/50 [&_.recharts-polar-grid-circle]:stroke-border/50 [&_.recharts-radial-bar-background-sector]:fill-muted [&_.recharts-rectangle.recharts-tooltip-cursor]:fill-muted [&_.recharts-reference-line_line]:stroke-border/50 flex aspect-video justify-center text-xs [&_.recharts-dot[stroke='#fff']]:stroke-transparent [&_.recharts-layer]:outline-hidden [&_.recharts-sector]:outline-hidden [&_.recharts-sector[stroke='#fff']]:stroke-transparent [&_.recharts-surface]:outline-hidden", className)} {...props}>
...
function ChartTooltip(props: TooltipProps<ValueType, NameType>) {
    const TooltipComponent = Tooltip as ComponentType<TooltipProps<ValueType, NameType>>;
    return <TooltipComponent {...props}/>;
}
function ChartTooltipContent({ active, payload, className, indicator = "dot", hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey, }: TooltipProps<ValueType, NameType> & React.ComponentProps<"div"> & {
    hideLabel?: boolean;
    ...
    if (!hideLabel) {
        ...
        const value = !labelKey && typeof label === "string"
            ...
            : itemConfig?.label;
        if (labelFormatter) {
            tooltipLabel = (<div className={cn("font-medium", labelClassName)}>
...
function ChartLegend(props: LegendProps) {
    const LegendComponent = Legend as ComponentType<LegendProps>;
    return <LegendComponent {...props}/>;
}
function ChartLegendContent({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey, }: React.ComponentProps<"div"> & Pick<LegendProps, "payload" | "verticalAlign"> & {
    hideIcon?: boolean;
    nameKey?: string;
}) {
    const { config } = useChart();
    ...
    return (<div className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
      {payload.flatMap((item) => {
            ...
            return [(<div key={item.value} className={cn("[&>svg]:text-muted-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3")}>
              {itemConfig?.icon && !hideIcon ? (<itemConfig.icon />) : (<div className="size-2 shrink-0 rounded-[2px]" style={getChartLegendMarkerStyle(item.color)}/>)}
              {itemConfig?.label}
            </div>)];
        })}
    </div>);
