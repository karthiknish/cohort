...
export function AutoRefreshControls({ onRefresh, disabled = false, isRefreshing = false, className, defaultInterval = 'off', }: AutoRefreshProps) {
    ...
    return (<div className={cn('flex items-center gap-2', className)}>
      <Select value={interval} onValueChange={handleIntervalChange} disabled={disabled}>
        ...
        <SelectContent>
          {INTERVAL_OPTIONS.map((option) => (<SelectItem key={option.value} value={option.value}>
              {option.label}
            </SelectItem>))}
        </SelectContent>
      </Select>

      <Button variant="outline" size="icon" className="size-8" onClick={handleManualRefresh} disabled={disabled || isRefreshing} aria-label={isRefreshing ? 'Refreshing' : 'Refresh now'}>
