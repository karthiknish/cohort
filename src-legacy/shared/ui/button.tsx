...
function hasAccessibleButtonLabel(props: React.ComponentProps<'button'>): boolean {
    if (typeof props['aria-label'] === 'string' && props['aria-label'].trim().length > 0)
        return true;
    if (typeof props['aria-labelledby'] === 'string' && props['aria-labelledby'].trim().length > 0)
        return true;
    if (typeof props.title === 'string' && props.title.trim().length > 0)
        return true;
    return childrenContainAccessibleText(props.children);
}
function Button({ className, variant, size, asChild = false, ...props }: React.ComponentProps<"button"> & ButtonVariantProps & {
    asChild?: boolean;
}) {
