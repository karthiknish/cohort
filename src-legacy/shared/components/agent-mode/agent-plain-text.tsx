...
function parsePlainAgentContent(text: string): ParsedBlock[] {
    ...
    while (index < lines.length) {
        ...
        while (index < lines.length) {
            ...
            if (!next || /^[-*•]\s+/.test(next) || /^[A-Za-z][A-Za-z0-9\s/&]+:\s+/.test(next))
                break;
...
export function AgentPlainText({ text, mentionLabels, mentionClassName }: AgentPlainTextProps) {
    ...
    if (blocks.length <= 1 && blocks[0]?.type === 'paragraph') {
        return (<AgentMentionText text={text} mentionLabels={mentionLabels} mentionClassName={mentionClassName}/>);
    }
    return (<div className="space-y-3 text-sm leading-relaxed text-foreground">
      {blocks.map((block) => {
            if (block.type === 'list') {
                const listKey = `list-${block.items.join('\u0000')}`;
                return (<ul key={listKey} className="ml-1 list-disc space-y-1 pl-4 text-foreground/95">
            ...
            if (block.type === 'metrics') {
                ...
                return (<div key={metricsKey} className="grid gap-2 sm:grid-cols-2">
              {block.pairs.map((pair) => (<div key={pair.label} className="rounded-lg border border-border/50 bg-muted/20 px-2.5 py-2">
            ...
            return (<p key={paragraphKey} className="text-pretty">
            <AgentMentionText text={block.lines.join(' ')} mentionLabels={mentionLabels} mentionClassName={mentionClassName}/>
          </p>);
        })}
    </div>);
}
