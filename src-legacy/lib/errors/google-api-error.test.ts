...
import { isGoogleInsufficientScopeError, isGoogleRateLimitError, parseGoogleApiErrorBody, } from './google-api-error';
describe('google-api-error', () => {
    it('detects insufficient scope from structured error payload', () => {
        const payload = parseGoogleApiErrorBody(JSON.stringify({
            error: {
                code: 403,
