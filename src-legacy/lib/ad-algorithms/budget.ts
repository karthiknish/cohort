...
// =============================================================================
import type { AdMetricsSummary, EnrichedMetricsSummary, BudgetAllocation, AlgorithmicInsight, } from './types';
import { enrichSummaryWithMetrics } from './efficiency';
/**
 * Budget allocation strategies
 */
export type AllocationStrategy = 'maximize_roas' // Shift budget to highest ROAS platforms
 ...
 */
export function calculateOptimalAllocation(summaries: AdMetricsSummary[], strategy: AllocationStrategy = 'balanced', totalBudget?: number): BudgetAllocation[] {
 ...
 */
function generateAllocationReason(summary: EnrichedMetricsSummary, changePercent: number, strategy: AllocationStrategy): string {
 ...
 */
export function generateBudgetInsights(summaries: AdMetricsSummary[]): AlgorithmicInsight[] {
    ...
    const highSpendLowRoas = enriched.filter(s => s.totalSpend > 1000 && s.averageRoaS < 1.5);
 ...
 */
export function projectBudgetImpact(allocations: BudgetAllocation[], summaries: AdMetricsSummary[]): {
