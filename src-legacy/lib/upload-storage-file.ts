...
async function uploadWithProgress(url: string, file: File, onProgress?: (progress: {
    loaded: number;
    total: number;
}) => void): Promise<void> {
 ...
 */
export async function uploadStorageFile({ file, contentType, generateUploadUrl, syncMetadata, onProgress, }: UploadStorageFileOptions): Promise<string> {
    ...
    const resolvedFile = file instanceof File
        ? file
 ...
 */
export async function uploadStorageFileWithPublicUrl({ getPublicUrl, ...uploadOptions }: UploadStorageFileWithUrlOptions): Promise<{
    storageId: string;
    url: string;
}> {
