...
export function getPreviewAgentModeResponse(input: string, context: PreviewAgentContext): PreviewAgentResponse {
    ...
    if (normalized.includes('create task')
    ...
    if (normalized.includes('schedule a meeting')
    ...
    if (normalized.includes('send message')
    ...
    if (normalized.includes('generate weekly report')
