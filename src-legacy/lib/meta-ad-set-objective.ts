...
export type MetaAdSetEngagementType = 'POST_ENGAGEMENT' | 'PAGE_ENGAGEMENT' | 'EVENT_RESPONSES' | 'OFFER_CLAIMS';
...
export function validateMetaAdSetObjective(campaignObjective: string | null | undefined, fields: MetaAdSetObjectiveFields): string[] {
    const normalized = normalizeMetaCampaignObjective(campaignObjective);
    const errors: string[] = [];
    ...
    if (normalized === 'OUTCOME_ENGAGEMENT') {
        ...
        if ((engagementType === 'PAGE_ENGAGEMENT'
...
export function buildMetaAdSetPromotedObject(campaignObjective: string | null | undefined, fields: MetaAdSetObjectiveFields): Record<string, string> | undefined {
    const normalized = normalizeMetaCampaignObjective(campaignObjective);
    if (normalized === 'OUTCOME_SALES') {
        const mode = fields.salesOptimizationMode ?? (fields.productCatalogId ? 'catalog' : 'pixel');
...
export function resolveMetaAdSetObjectiveGoals(campaignObjective: string | null | undefined, fields: MetaAdSetObjectiveFields): {
