...
function isPersistableAttachment(attachment: AgentAttachmentContext): attachment is AgentAttachmentContext & {
    extractionStatus: AgentStoredAttachment['extractionStatus'];
} {
    return attachment.extractionStatus !== 'extracting';
}
export function serializeAgentAttachmentsForStorage(attachments: AgentAttachmentContext[]): AgentStoredAttachment[] {
...
export function toAgentRequestAttachmentContext(attachments: AgentAttachmentContext[]): Array<{
    name: string;
    mimeType: string;
    sizeLabel: string;
    excerpt: string;
...
export async function hydrateAgentAttachmentUrls(attachments: AgentAttachmentContext[] | undefined, getPublicUrl: (args: {
    storageId: string;
}) => Promise<{
    url: string | null;
    ...
    const hydrated = await Promise.all(attachments.map(async (attachment) => {
        if (!attachment.storageId?.trim())
            return attachment;
        if (attachment.url?.trim() && attachment.url !== 'about:blank')
            return attachment;
...
export function agentAttachmentsToChatMedia(attachments: AgentAttachmentContext[]): Array<{
    name: string;
    url: string;
    type?: string | null;
    size?: string | null;
...
async function readZipDocumentAttachment(file: File, extension: string): Promise<string> {
    ...
    const collectParts = async (index: number, parts: string[]): Promise<string[]> => {
        if (index >= matchingNames.length)
            return parts;
        if (parts.join(' ').length >= MAX_ATTACHMENT_TEXT_LENGTH)
            return parts;
        const entryName = matchingNames[index];
        if (!entryName)
            return collectParts(index + 1, parts);
...
export function getPdfUploadSizeError(file: File): string | null {
    ...
    if (!isPdf || file.size <= MAX_PDF_UPLOAD_BYTES)
        return null;
    return 'PDF is too large for import. Try a file under 750 KB or save as .docx.';
}
export async function buildAgentAttachmentContext(file: File, options?: {
