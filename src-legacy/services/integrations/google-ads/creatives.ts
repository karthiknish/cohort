...
function compactTextValues(items?: Array<{
    text?: string | null;
}>): string[] {
...
function compactAssetValues(items?: Array<{
    asset?: string | null;
}>): string[] {
    return items?.flatMap((item) => {
...
export function buildGoogleCreativesGaql(options: BuildGoogleCreativesGaqlOptions): string {
    ...
    let filters = '';
    if (campaignId)
        filters += ` AND campaign.id = ${campaignId}`;
    if (adGroupId)
        filters += ` AND ad_group.id = ${adGroupId}`;
...
function firstVideoAssetResourceFromRow(row: GoogleAdsResult): string | undefined {
    ...
    const fromApp = appCamel?.youtubeVideos?.map((v) => v.asset?.trim()).find(Boolean)
...
function videoAssetResourceByAdId(rows: GoogleAdsResult[]): Map<string, string> {
    ...
    for (const row of rows) {
        const adId = (row.adGroupAd as {
            ad?: {
                id?: string;
            };
        } | undefined)?.ad?.id?.trim();
...
async function resolveYoutubeVideoIdsFromAssetResources(options: {
    ...
    await Promise.all(chunks.map(async (chunk) => {
        ...
        for (const row of assetRows) {
            ...
            const yid = (asset?.youtubeVideoAsset?.youtubeVideoId
                ?? asset?.youtube_video_asset?.youtube_video_id)?.trim();
            if (rn && yid) {
                out.set(rn, yid);
            }
        }
...
export function buildGooglePmaxAssetGroupsGaql(options: {
    campaignId: string;
}): string {
...
function mapPmaxAssetGroupRow(row: GoogleAdsResult): GoogleCreative {
    ...
    return {
        ...
        status: (assetGroup?.status === 'REMOVED' ? 'REMOVED' : (assetGroup?.status ?? 'ENABLED')) as 'ENABLED' | 'PAUSED' | 'REMOVED',
...
export async function fetchGoogleCreatives(options: {
    ...
    const { accessToken, developerToken, customerId, campaignId, adGroupId, loginCustomerId, statusFilter = ['ENABLED', 'PAUSED', 'REMOVED'], maxRetries = 3, maxSearchPages = 10, includePerformanceMaxAssetGroups = true, resolveYoutubeVideoAssets = true, maxYoutubeAssetLookupChunks = 5, } = options;
    ...
    const fromAds = rows.map((row) => {
        const adGroupAd = row.adGroupAd as {
            ad?: {
                ...
                demandGenMultiAssetAd?: {
                    ...
                    businessName?: string;
                    callToActionText?: string;
                    tallPortraitMarketingImages?: Array<{
                        asset?: string;
                    }>;
                    portraitMarketingImages?: Array<{
                        asset?: string;
                    }>;
        ...
        const tallPortraitImages = compactAssetValues(ad?.demandGenMultiAssetAd?.tallPortraitMarketingImages);
        const portraitImages = compactAssetValues(ad?.demandGenMultiAssetAd?.portraitMarketingImages);
        const squareImages = compactAssetValues(ad?.demandGenMultiAssetAd?.squareMarketingImages);
        const carouselCardAssets = compactAssetValues(ad?.demandGenCarouselAd?.carouselCards);
        const demandGenVideoLogoImages = compactAssetValues(ad?.demandGenVideoResponsiveAd?.logoImages);
        const videoId = ad?.videoAd?.video?.id;
        const imageUrl = ad?.imageAd?.imageUrl
    ...
    const shouldFetchPmax = Boolean(campaignId?.trim())
