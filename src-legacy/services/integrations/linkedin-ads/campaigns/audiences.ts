...
type IncludeFacetKind = 'ageRange' | 'gender' | 'industry' | 'companySize' | 'title' | 'function' | 'seniority' | 'skill' | 'location' | 'interest' | 'audience';
...
export async function fetchLinkedInAudienceTargeting(options: {
    ...
    const { accessToken, accountId, campaignId, maxRetries = 3, } = options;
...
export async function createLinkedInAudience(options: {
    ...
    const { accessToken, accountId, name, description, maxRetries = 3, } = options;
