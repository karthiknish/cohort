...
import { enrichMetaCreativesWithObjectStoryMedia, needsObjectStoryMediaEnrichment, } from './post-enrichment';
...
describe('needsObjectStoryMediaEnrichment', () => {
    ...
    it('returns false when imageUrl is present', () => {
        expect(needsObjectStoryMediaEnrichment(creative({ imageUrl: 'https://cdn.example/img.jpg' }))).toBe(false);
    });
...
describe('enrichMetaCreativesWithObjectStoryMedia', () => {
    ...
    it('enriches boosted video posts with thumbnail from page post', async () => {
        ...
        expect(postEnrichment.postEnrichmentRuntime.fetchMetaObjectStoryMedia).toHaveBeenCalledWith(expect.objectContaining({ objectStoryId: '111_222', accessToken: 'token' }));
