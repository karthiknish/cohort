...
import { DASHBOARD_NAVIGATION_GROUPS, type NavigationGroup, } from '@/lib/dashboard-navigation';
export type { AuthRole };
export type DashboardCapability = 'analytics.view' | 'proposals.view' | 'proposals.manage' | 'agency.ads' | 'agency.socials' | 'admin.directory';
...
export function can(role: string | null | undefined, capability: DashboardCapability): boolean {
...
export function agencyOnlyPrefixes(): readonly string[] {
    return ROUTE_ACCESS.flatMap((entry) => entry.capability === 'agency.ads' || entry.capability === 'agency.socials'
        ? [entry.prefix]
        : []);
}
...
export function navItemsForRole(role: string | null | undefined, groups: NavigationGroup[] = DASHBOARD_NAVIGATION_GROUPS): NavigationGroup[] {
    const normalized = normalizeAuthRole(role);
...
export function accessDeniedContentForCapability(capability: DashboardCapability, role: AuthRole): AccessDeniedContent {
...
export function accessDeniedContentForPath(pathname: string, role: string | null | undefined): AccessDeniedContent | null {
    const normalized = normalizeAuthRole(role);
    for (const entry of ROUTE_ACCESS) {
