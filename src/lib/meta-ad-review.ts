...
function parseAdReviewFeedback(feedback: unknown, messages: string[]) {
  ...
  const global = (feedback as { global?: Record<string, unknown> }).global
...
function parseIssuesInfo(issuesInfo: unknown, messages: string[]) {
  ...
  for (const issue of issuesInfo) {
    ...
    const row = issue as { error_summary?: string; error_message?: string }
    const text = [row.error_summary, row.error_message].filter(Boolean).join(' — ')
    if (text) pushUniqueMessage(messages, text)
  }
}

export function summarizeMetaAdReview(
  effectiveStatus: string,
  adReviewFeedback?: unknown,
  issuesInfo?: unknown,
): MetaAdReviewSummary {
