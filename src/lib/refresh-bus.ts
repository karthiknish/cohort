'use client';
export type DashboardRefreshReason = 'manual-dashboard-refresh' | 'task-mutated' | 'project-mutated' | 'unknown';
...
function getEventTarget(): EventTarget | null {
    ...
    const anyWindow = window as unknown as {
        __cohortsRefreshTarget?: EventTarget;
    };
...
export function emitDashboardRefresh(detail: Omit<DashboardRefreshEventDetail, 'at'> & {
    at?: number;
} = { reason: 'unknown' }) {
    if (typeof window === 'undefined')
        return;
