 ...
 */
export async function fetchWithRetry<T = unknown>(url: string, init: RequestInit, options: FetchWithRetryOptions = {}): Promise<FetchWithRetryResult<T>> {
    const { maxRetries = 3, baseDelayMs = 1000, maxDelayMs = 10000, getAuthToken, onRetry, onFinalFailure, signal, timeoutMs, } = options;
    let attempts = 0;
 ...
 */
export async function postWithRetry<T = unknown>(url: string, body: unknown, token: string, options: FetchWithRetryOptions = {}): Promise<FetchWithRetryResult<T>> {
 ...
 */
export async function getWithRetry<T = unknown>(url: string, token: string, options: FetchWithRetryOptions = {}): Promise<FetchWithRetryResult<T>> {
    return fetchWithRetry<T>(url, {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${token}`,
