...
export type AgentMessageLifecycle =
  | 'queued'
  | 'sending'
  | 'sent'
  | 'failed'
...
export function buildCompletedStepsFromResponse(
...
export function operationRiskLevel(
  operation?: string | null,
  action?: string | null,
): 'read' | 'navigate' | 'write' | 'destructive' {
  if (action === 'navigate') return 'navigate'
  if (!operation) return 'read'
...
export function upsertAgentMessage<T extends AgentMessageUpsert>(
  messages: T[],
  message: T,
): T[] {
...
export function filterMessagesForAgentContext<T extends { lifecycle?: AgentMessageLifecycle }>(
  messages: T[],
  limit: number,
): T[] {
  return messages
...
export function parsePendingConfirmationFromStored(
  executeResult: Record<string, unknown> | null | undefined,
  params: Record<string, unknown> | null | undefined,
  agentMessageId?: string,
): AgentPendingConfirmation | null {
  const data =
    executeResult && typeof executeResult.data === 'object' && !Array.isArray(executeResult.data)
      ? (executeResult.data as Record<string, unknown>)
      : executeResult

  const pending =
    data && typeof data.pendingConfirmation === 'object' && !Array.isArray(data.pendingConfirmation)
  ...
  if (params?._pendingConfirmation === true) {
    const operation =
      typeof params._operation === 'string'
        ? params._operation
    ...
    if (operation) {
      ...
      return {
        confirmationId:
          typeof params._confirmationId === 'string' ? params._confirmationId : (agentMessageId ?? operation),
        operation,
        params: cleanParams,
        agentMessageId,
      }
...
export function countUserMessagesByClientId(
  messages: Array<{ type: string; clientId: string }>,
): Map<string, number> {
