import {
  GeminiAIService,
  resolveGeminiApiKey,
  type GeminiContentGenerationOptions,
...
export function buildTranscriptExcerptForNotes(transcriptText: string): { text: string; truncated: boolean } {
...
export function buildMeetingNotesUserPrompt(transcriptText: string, options?: { retryInvalidFormat?: boolean }): string {
...
export function validateAndNormalizeMeetingNotes(raw: string): { ok: true; notes: string } | { ok: false; reason: string } {
...
export async function generateConciseMeetingNotes(
  transcriptText: string,
): Promise<{ summary: string; model: string; truncated: boolean } | null> {
  ...
  const firstAttempt = await gemini.generateContentWithOptions(
    buildMeetingNotesUserPrompt(transcriptText),
    MEETING_NOTES_GENERATION_OPTIONS,
  )
  ...
  const retryAttempt = await gemini.generateContentWithOptions(
    buildMeetingNotesUserPrompt(transcriptText, { retryInvalidFormat: true }),
    MEETING_NOTES_GENERATION_OPTIONS,
  )
