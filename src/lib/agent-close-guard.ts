...
export type AgentCloseBlockerReason = 'unsent-message' | 'attachments' | 'processing' | 'pending-confirmation';
...
export function getAgentCloseBlockers(state: AgentCloseGuardState): AgentCloseBlockerReason[] {
    ...
    const hasPendingConfirmation = state.messages.some((message) => message.metadata?.pendingConfirmation != null);
