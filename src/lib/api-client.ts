...
import { getPreviewClients, getPreviewActivity, getPreviewNotifications, isPreviewModeEnabled, } from '@/lib/preview-data';
    ...
    const executeRequest = async (attempt = 0): Promise<T> => {
        try {
            ...
            if (isEnvelope && 'data' in payloadRecord) {
                ...
                if (isDeduplicatable)
                    void responseCache.set(cacheKey, result, RESPONSE_CACHE_TTL_MS);
                return result;
            }
            // Backward compatibility: return payload directly
            if (isDeduplicatable)
                void responseCache.set(cacheKey, payload as T, RESPONSE_CACHE_TTL_MS);
            return payload as T;
        }
        catch (error) {
