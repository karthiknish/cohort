...
export function createProposalDeckRequest(formData: ProposalFormData, instructions: string, options?: {
    themeId?: string;
...
export function createProposalTemplateDeckRequest(formData: ProposalFormData, instructions: string, templateDeckId: string, options?: {
    themeId?: string;
    ...
    const prompt = [
        ...
        .filter(Boolean)
        .join('\n');
    return createTemplateRequest(templateDeckId, prompt.slice(0, 100000), {
        themeId: options?.themeId,
        exportAs: ['pptx', 'pdf'],
        folderIds: resolvePresentationFolderIds(),
        sharingOptions: {
...
export function extractExternalDeckId(payload: Record<string, unknown>): string | null {
    ...
    const webUrl = typeof payload.gammaUrl === 'string'
        ? payload.gammaUrl
...
export function sanitizeDeckProviderWarnings(warnings: string[] | undefined): string[] {
    if (!warnings?.length)
        return [];
    return warnings.map((warning) => warning.replace(/\bGamma\b/gi, 'Presentation'));
}
