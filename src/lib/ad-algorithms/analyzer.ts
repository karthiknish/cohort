import type { MetricDataPoint, AdMetricsSummary, EnrichedMetricsSummary, AlgorithmicInsight, TrendResult, FunnelAnalysis, BenchmarkComparison, ProviderComparisonData, EfficiencyBreakdown, BudgetAllocation, } from './types';
...
import { generateEfficiencyInsights, generateCreativeInsights, generateAudienceInsights, generateTrendInsights, generateFunnelInsights, generateBenchmarkInsights, combineInsights, } from './insights';
/**
 ...
 */
function buildSummary(providerId: string, dataPoints: MetricDataPoint[], accountId?: string | null): AdMetricsSummary {
 ...
 */
export function analyzeAdPerformance(dataPoints: MetricDataPoint[]): PerformanceAnalysis {
    // Group by provider
    ...
    const globalEfficiencyScore = totalSpend > 0
        ? Math.round(summaries.reduce((sum, s) => sum + s.efficiencyScore * s.totalSpend, 0) / totalSpend)
        : 0;
    ...
    const chartData = {
        ...
        trendCharts: {} as Record<string, {
            date: string;
            actual: number;
            trend: number;
        }[]>,
        funnelCharts: {} as Record<string, {
            name: string;
            value: number;
            fill: string;
            dropOff: number;
        }[]>,
        benchmarkCharts: {} as Record<string, {
            metric: string;
            current: number;
            benchmark: number;
            percentile: number;
