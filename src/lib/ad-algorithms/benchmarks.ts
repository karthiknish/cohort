...
// =============================================================================
import type { AdMetricsSummary, IndustryBenchmarks, BenchmarkComparison, } from './types';
/**
 ...
 */
function calculatePercentile(value: number, benchmark: number, higherIsBetter: boolean = true): number {
    if (benchmark === 0)
        return 50;
    ...
    if (higherIsBetter) {
        ...
        if (ratio < 1)
            return Math.round(25 + (ratio - 0.5) * 50);
        if (ratio < 2)
            return Math.round(50 + (ratio - 1) * 40);
        ...
        if (ratio > 1)
            return Math.round(25 + (2 - ratio) * 25);
        if (ratio > 0.5)
            return Math.round(50 + (1 - ratio) * 40);
 ...
 */
export function compareToBenchmark(metricName: string, value: number, benchmark: number, higherIsBetter: boolean = true): BenchmarkComparison {
 ...
 */
export function getBenchmarkChartData(comparisons: BenchmarkComparison[]): {
    metric: string;
    current: number;
    benchmark: number;
    percentile: number;
 ...
 */
export function crossPlatformBenchmarkAnalysis(summaries: AdMetricsSummary[]): {
    providerId: string;
