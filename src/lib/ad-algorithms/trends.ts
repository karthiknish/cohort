...
// =============================================================================
import type { MetricDataPoint, TrendResult, AnomalyPoint, } from './types';
/**
...
function linearRegression(points: number[]): {
    ...
    if (n < 2)
        return { slope: 0, intercept: points[0] || 0, r2: 0 };
...
function detectAnomalies(points: {
    date: string;
    value: number;
}[], threshold: number = 2.0): AnomalyPoint[] {
    if (points.length < 5)
        return [];
...
function detectSeasonality(points: {
    ...
    if (points.length < 14)
        return false; // Need at least 2 weeks
 ...
 */
export function analyzeTrend(dataPoints: MetricDataPoint[], metricKey: 'spend' | 'revenue' | 'clicks' | 'conversions' | 'impressions'): TrendResult {
    // Sort by date
    ...
    // Determine direction
    const direction: 'up' | 'down' | 'stable' = slope > 0.01 ? 'up' : slope < -0.01 ? 'down' : 'stable';
 ...
 */
export function getTrendChartData(dataPoints: MetricDataPoint[], metricKey: 'spend' | 'revenue' | 'clicks' | 'conversions' | 'impressions' | 'roas'): {
    date: string;
    actual: number;
    trend: number;
    forecast?: number;
