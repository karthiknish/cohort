...
import type { AdMetricsSummary, AlgorithmicInsight, TrendResult, FunnelAnalysis, BenchmarkComparison, } from './types';
...
export function generateEfficiencyInsights(summary: AdMetricsSummary): AlgorithmicInsight[] {
    ...
    const { totalSpend, totalRevenue, totalClicks, totalConversions, averageRoaS, efficiencyScore, roi, } = enriched;
    if (totalSpend === 0)
        return [];
 ...
 */
export function generateTrendInsights(trends: Record<string, TrendResult>, providerId: string): AlgorithmicInsight[] {
    const insights: AlgorithmicInsight[] = [];
    const providerName = formatProviderName(providerId);
    // ROAS trend
 ...
 */
export function generateFunnelInsights(analysis: FunnelAnalysis, providerId: string): AlgorithmicInsight[] {
    const insights: AlgorithmicInsight[] = [];
    const providerName = formatProviderName(providerId);
 ...
 */
export function generateBenchmarkInsights(comparisons: BenchmarkComparison[], providerId: string): AlgorithmicInsight[] {
    const insights: AlgorithmicInsight[] = [];
    const providerName = formatProviderName(providerId);
 ...
 */
export function combineInsights(allInsights: AlgorithmicInsight[]): AlgorithmicInsight[] {
