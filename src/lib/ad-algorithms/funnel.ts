import type { AdMetricsSummary, FunnelStage, FunnelAnalysis, } from './types';
 ...
 */
function generateFunnelRecommendations(stages: FunnelStage[], summary: AdMetricsSummary): string[] {
    ...
    // CTR recommendations
    if (ctr < 0.5) {
        recommendations.push('Your CTR is very low (<0.5%). Consider refreshing ad creatives, improving headlines, or refining audience targeting.');
    }
    else if (ctr < 1.0) {
        recommendations.push('CTR is below average. Test different ad formats, images, or copy variations to improve engagement.');
    }
    // Conversion rate recommendations
    if (convRate < 1.0 && summary.totalClicks > 100) {
        recommendations.push('Conversion rate is low despite traffic. Audit your landing page for load speed, mobile responsiveness, and clear CTAs.');
    }
    else if (convRate < 2.0 && summary.totalClicks > 500) {
        recommendations.push('Consider A/B testing landing page elements like headlines, form length, and trust signals to improve conversions.');
    }
    ...
    if (cpa > 100 && summary.totalConversions > 0) {
        recommendations.push(`Your CPA ($${cpa.toFixed(2)}) is high. Consider narrowing audience targeting or implementing lookalike audiences.`);
    }
    // Drop-off specific recommendations
    const clickStage = stages.find(s => s.name === 'Clicks');
    if (clickStage && clickStage.dropOffRate > 98) {
        recommendations.push('Impressions to clicks drop-off is severe. Your ad creative may not be resonating with your audience.');
    }
    const conversionStage = stages.find(s => s.name === 'Conversions');
    if (conversionStage && conversionStage.dropOffRate > 95) {
        recommendations.push('High drop-off from clicks to conversions suggests a disconnect between ad promise and landing page experience.');
    }
    if (recommendations.length === 0) {
        recommendations.push('Your funnel is performing well. Focus on scaling spend while monitoring efficiency metrics.');
    }
 ...
 */
export function compareFunnels(summaries: AdMetricsSummary[]): {
    providerId: string;
