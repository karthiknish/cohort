...
export function resolveConvexQueryErrorMessage(data: unknown, options: Pick<UseConvexQueryErrorOptions, 'skipped' | 'loading' | 'fallbackMessage'> = {}): string | null {
...
export function useConvexQueryError(options: UseConvexQueryErrorOptions): string | null {
    ...
    return resolveConvexQueryErrorMessage(data, { skipped, loading, fallbackMessage });
}
/**
 * Merge shape-validation errors with Convex query transport errors.
 */
export function mergeQueryErrors(...errors: Array<string | null | undefined>): string | null {
    const first = errors.find((value) => typeof value === 'string' && value.trim().length > 0);
    return first ?? null;
}
export function queryErrorFromUnknown(error: unknown, fallback = 'Unable to load data. Please try again.'): string {
