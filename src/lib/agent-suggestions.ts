...
function capabilityForOperation(operation: string): DashboardCapability | null {
    if (operation === 'createProposalDraft')
        return 'proposals.manage';
    if (operation.includes('Ads') || operation === 'summarizeAdsPerformance')
        return 'agency.ads';
    if (operation === 'listWorkspaceClients' || operation === 'createClient')
...
function isSuggestionAllowed(suggestion: AgentSuggestion, role: string | null | undefined): boolean {
    if (suggestion.capability === 'navigate')
        return true;
...
export function isAgentSuggestionAllowed(suggestion: AgentSuggestion, role: string | null | undefined): boolean {
    return isSuggestionAllowed(suggestion, role);
}
export function filterAgentDashboardShortcuts<T extends {
    operation?: string;
}>(shortcuts: T[], role: string | null | undefined): T[] {
    return shortcuts.filter((shortcut) => {
        if (!shortcut.operation)
            return true;
...
export function getAgentSuggestions(pathname: string | null, options?: AgentSuggestionFilterOptions): AgentSuggestion[] {
    const path = pathname ?? '';
...
export function buildSuggestionContextHint(ids: AgentContextIds): string | null {
    const parts: string[] = [];
    if (ids.activeClientId)
        parts.push(`active client ${ids.activeClientId}`);
    if (ids.activeProjectId)
        parts.push(`active project ${ids.activeProjectId}`);
    if (ids.activeProposalId)
        parts.push(`active proposal ${ids.activeProposalId}`);
    if (parts.length === 0)
        return null;
    return parts.join(', ');
}
