...
import { refreshGoogleAccessToken } from './integration-token-refresh-google';
...
export { refreshGoogleAccessToken } from './integration-token-refresh-google';
...
export function isTokenExpiringSoon(expiresAt?: AnyTimestamp | null | string, bufferMs = 5 * 60 * 1000): boolean {
    ...
    if (expiresAt instanceof Date) {
    ...
    else if (typeof (expiresAt as {
        toMillis?: () => number;
    }).toMillis === 'function') {
        try {
            expiryMs = (expiresAt as {
                toMillis: () => number;
            }).toMillis();
        }
        catch {
        ...
        expiryMs = expiresAt < 1000000000000 ? expiresAt * 1000 : expiresAt;
    }
    ...
    for (let attempt = 0; attempt < TOKEN_REFRESH_CONFIG.maxRetries; attempt++) {
        try {
            ...
            if (!response.ok) {
                ...
                try {
                    parsedError = JSON.parse(errorPayload) as {
                        code?: number;
                        message?: string;
                    };
                }
                ...
                if (parsedError.code === 40001) {
                    throw new IntegrationTokenError('TikTok refresh token has been revoked or expired. Please reconnect your TikTok Ads account.', 'tiktok', userId, { isRetryable: false, httpStatus: response.status });
                }
                throw new IntegrationTokenError(`Failed to refresh TikTok access token (${response.status}): ${parsedError.message || errorPayload}`, 'tiktok', userId, { isRetryable: false, httpStatus: response.status });
            }
...
export async function refreshLinkedInAccessToken({ userId, clientId }: RefreshParams): Promise<string> {
    ...
    if (!integration?.refreshToken) {
        throw new IntegrationTokenError('No LinkedIn refresh token available. LinkedIn tokens expire after 60 days - please reconnect your account.', 'linkedin', userId, { isRetryable: false });
    }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (parsedError.error === 'invalid_grant') {
                    throw new IntegrationTokenError('LinkedIn refresh token has expired or been revoked. Please reconnect your LinkedIn Ads account.', 'linkedin', userId, { isRetryable: false, httpStatus: response.status });
                }
