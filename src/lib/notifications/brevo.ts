 ...
 */
async function isEmailNotificationEnabled(recipientEmail: string, prefKey: 'adAlerts' | 'performanceDigest' | 'taskActivity' | 'meetings'): Promise<boolean> {
    try {
...
export async function sendTransactionalEmail(options: BrevoSendOptions): Promise<BrevoEmailResult> {
    ...
    const attemptSend = async (attempt: number): Promise<{
        success: boolean;
        messageId?: string;
        error?: Error;
    }> => {
...
async function sendMeetingNotificationBatch(notificationType: 'scheduled' | 'rescheduled' | 'cancelled', recipientEmails: string[], sendEmail: (recipientEmail: string) => Promise<BrevoEmailResult>): Promise<MeetingNotificationSummary> {
