...
export type StoredNotificationPreferences = NotificationPreferencesV2 | LegacyNotificationPreferences | null | undefined;
...
export function createDefaultCategories(overrides?: Partial<Record<NotificationCategory, Partial<ChannelPrefs>>>): Record<NotificationCategory, ChannelPrefs> {
...
export function isNotificationPreferencesV2(prefs: StoredNotificationPreferences): prefs is NotificationPreferencesV2 {
    return (prefs !== null &&
        prefs !== undefined &&
        typeof prefs === 'object' &&
        'version' in prefs &&
...
export function migrateLegacyPreferences(legacy: LegacyNotificationPreferences | null | undefined): NotificationPreferencesV2 {
    return {
        version: 2,
        pauseAll: false,
        quietHours: { ...DEFAULT_QUIET_HOURS },
...
export function normalizePreferences(prefs: StoredNotificationPreferences): NotificationPreferencesV2 {
...
function mergeCategories(input: Partial<Record<NotificationCategory, Partial<ChannelPrefs>>> | undefined): Record<NotificationCategory, ChannelPrefs> {
...
export function kindToCategory(kind: WorkspaceNotificationKind | string): NotificationCategory {
    if (kind.startsWith('task.'))
        return 'tasks';
    if (kind.startsWith('collaboration.'))
        return 'collaboration';
    if (kind.startsWith('meeting.'))
        return 'meetings';
    if (kind === 'project.created' || kind === 'proposal.deck.ready')
        return 'projects';
...
export function isWithinQuietHours(quietHours: QuietHoursPrefs, now: Date = new Date()): boolean {
...
export function isChannelEnabled(prefs: NotificationPreferencesV2, category: NotificationCategory, channel: NotificationChannel, options?: {
    now?: Date;
}): boolean {
    if (prefs.pauseAll)
        return false;
...
export function isEmailPrefEnabled(prefs: StoredNotificationPreferences, prefKey: keyof typeof EMAIL_PREF_KEY_TO_CATEGORY, options?: {
    now?: Date;
}): boolean {
...
export function applyPreferencesPatch(current: StoredNotificationPreferences, patch: NotificationPreferencesPatch): NotificationPreferencesV2 {
