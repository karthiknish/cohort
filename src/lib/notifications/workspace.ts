...
export async function recordTaskMentionNotifications(options: {
    ...
    await Promise.all(options.mentions.map((mention) => createWorkspaceNotification({
        workspaceId,
        kind: 'task.mention',
...
export async function recordCollaborationNotification(options: {
    ...
    const clientId = (isClientChannel || isProjectChannel) &&
...
export async function recordMentionNotifications(options: {
    ...
    await Promise.all(message.mentions.map((mention) => {
...
export async function recordProjectCreatedNotification(options: {
    ...
    const clientId = typeof project.clientId === 'string' && project.clientId.trim().length > 0
