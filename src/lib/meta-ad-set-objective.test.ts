...
describe('meta-ad-set-objective', () => {
  ...
  it('builds promoted object for leads with page', () => {
    expect(
      buildMetaAdSetPromotedObject('OUTCOME_LEADS', { pageId: '123' }),
    ).toEqual({ page_id: '123' })
  })

  it('uses lead generation optimization for leads', () => {
    expect(
      resolveMetaAdSetObjectiveGoals('OUTCOME_LEADS', { pageId: '123' }),
    ).toEqual({
  ...
  it('uses engagement type for engagement campaigns', () => {
    expect(
      resolveMetaAdSetObjectiveGoals('OUTCOME_ENGAGEMENT', {
        pageId: '123',
  ...
  it('includes event_id when provided', () => {
    expect(
      buildMetaAdSetPromotedObject('OUTCOME_ENGAGEMENT', {
        pageId: '123',
  ...
  it('requires post for post engagement', () => {
    expect(
      validateMetaAdSetObjective('OUTCOME_ENGAGEMENT', {
        pageId: '123',
        engagementType: 'POST_ENGAGEMENT',
      }),
  ...
  it('builds sales promoted object with pixel and event', () => {
    expect(
      buildMetaAdSetPromotedObject('OUTCOME_SALES', {
        pixelId: 'px_1',
  ...
  it('builds catalog sales promoted object and goals', () => {
    ...
    expect(
      resolveMetaAdSetObjectiveGoals('OUTCOME_SALES', {
        salesOptimizationMode: 'catalog',
        productCatalogId: 'cat_1',
      }),
    ).toEqual({
  ...
  it('requires catalog when in catalog mode', () => {
    expect(
      validateMetaAdSetObjective('OUTCOME_SALES', { salesOptimizationMode: 'catalog' }),
    ).toContain('Select a product catalog for catalog sales.')
  })
})
