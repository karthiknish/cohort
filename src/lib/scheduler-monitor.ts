...
function determineSeverity(input: SchedulerEventInput): SchedulerEventSeverity {
    const providerThresholds = (Array.isArray(input.providerFailureThresholds) ? input.providerFailureThresholds : []).map((entry) => ({
        ...
        threshold: typeof entry.threshold === 'number' && Number.isFinite(entry.threshold) && entry.threshold >= 1
            ? entry.threshold
...
export async function recordSchedulerEvent(input: SchedulerEventInput): Promise<void> {
    ...
    const payload = {
        ...
        providerFailureThresholds: (input.providerFailureThresholds ?? []).map((entry) => ({
            providerId: entry.providerId,
            failedJobs: entry.failedJobs,
            threshold: typeof entry.threshold === 'number' && Number.isFinite(entry.threshold) && entry.threshold >= 0
                ? entry.threshold
                : null,
        })),
        severity,
