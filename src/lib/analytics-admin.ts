...
export async function getGoogleAnalyticsIntegration(options: {
    userId: string;
    clientId?: string | null;
...
export async function hasPendingGoogleAnalyticsSyncJob(options: {
    userId: string;
    clientId?: string | null;
}): Promise<boolean> {
    const workspaceId = await resolveWorkspaceIdForUser(options.userId);
...
export async function deleteGoogleAnalyticsIntegration(options: {
    userId: string;
    clientId?: string | null;
}): Promise<void> {
    const workspaceId = await resolveWorkspaceIdForUser(options.userId);
