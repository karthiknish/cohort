        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (parsedError.error === 'invalid_grant') {
                    throw new IntegrationTokenError(`${providerName} refresh token has been revoked or expired. Please reconnect your account.`, resolvedProviderId, userId, { isRetryable: false, httpStatus: response.status });
                }
                throw new IntegrationTokenError(`Failed to refresh ${providerName} token (${response.status}): ${errorMessage}`, resolvedProviderId, userId, { isRetryable: false, httpStatus: response.status });
            }
