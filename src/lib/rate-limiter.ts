 ...
 */
export function createRateLimitKey(endpoint: string, identifier: string | null): string {
...
export function getClientIdentifier(request: Request): string {
    ...
    const realIp = request.headers.get('x-real-ip') || (request as {
        ip?: string;
    }).ip;
...
/** Convex `@convex-dev/rate-limiter` presets — keep in sync with `RATE_LIMITS`. */
export const CONVEX_RATE_LIMITER_PRESETS = {
    standard: {
        kind: 'fixed window' as const,
        rate: RATE_LIMITS.standard.maxRequests,
        period: RATE_LIMITS.standard.windowMs,
    },
    sensitive: {
        kind: 'fixed window' as const,
        rate: RATE_LIMITS.sensitive.maxRequests,
        period: RATE_LIMITS.sensitive.windowMs,
    },
    critical: {
        kind: 'fixed window' as const,
        rate: RATE_LIMITS.critical.maxRequests,
        period: RATE_LIMITS.critical.windowMs,
    },
    bulk: {
        kind: 'fixed window' as const,
        rate: RATE_LIMITS.bulk.maxRequests,
        period: RATE_LIMITS.bulk.windowMs,
    },
} as const;
