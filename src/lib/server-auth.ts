...
function buildAuthResultFromBetterAuthSession(sessionPayload: BetterAuthSessionPayload): AuthResult | null {
    ...
    const role = (typeof user.role === 'string' && user.role.length > 0 ? user.role : undefined)
        ?? 'client';
    const status = (typeof user.status === 'string' && user.status.length > 0 ? user.status : undefined)
        ?? 'pending';
    const agencyId = (typeof user.agencyId === 'string' && user.agencyId.length > 0 ? user.agencyId : undefined)
