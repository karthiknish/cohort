...
function meaningfulSeriesPoints(spec: SpreadsheetChartSpec, series: SpreadsheetChartSeries): SpreadsheetChartPoint[] {
...
export function isChartSpecMeaningful(spec: SpreadsheetChartSpec): boolean {
    const activeSeries = spec.series.flatMap((series) => {
        ...
        if (points.length === 0 || sumPointValues(points) < MIN_TOTAL_VALUE)
            return [];
...
export function collapseSmallPieSlices(points: SpreadsheetChartPoint[]): SpreadsheetChartPoint[] {
    ...
    if (total <= 0 || positive.length <= MIN_CATEGORY_POINTS)
        return positive;
...
function aggregateByField<T>(rows: T[], labelKey: keyof T | string, valueKey: keyof T | string): SpreadsheetChartPoint[] {
    ...
    return limitPoints(Array.from(totals.entries()).map(([label, value]) => ({ label, value })), 10);
}
function aggregateByDate<T>(rows: T[], dateKey: keyof T | string, valueKey: keyof T | string): SpreadsheetChartPoint[] {
    const totals = new Map<string, number>();
    for (const row of rows) {
        const record = row as Record<string, unknown>;
    ...
    return sortByLabel(Array.from(totals.entries()).map(([label, value]) => ({ label, value })));
}
export function buildCategoryCountChart(rows: Record<string, unknown>[], field: string, title: string, kind: 'bar' | 'pie' = 'bar'): SpreadsheetChartSpec | null {
    const counts = rows.reduce<Map<string, number>>((map, row) => {
        const label = String(row[field] ?? 'Unknown').trim() || 'Unknown';
        map.set(label, (map.get(label) ?? 0) + 1);
        return map;
    }, new Map());
    const chartPoints = limitPoints(Array.from(counts.entries()).map(([label, value]) => ({ label, value })), 10);
    const points = positivePoints(chartPoints);
    if (points.length < MIN_CATEGORY_POINTS)
        return null;
...
export function buildMetricSnapshotChart(metrics: Record<string, number>, title: string): SpreadsheetChartSpec | null {
    const points = Object.entries(metrics).flatMap(([label, value]) => Number.isFinite(value) && value > 0 ? [{ label, value }] : []);
    const positive = positivePoints(limitPoints(points, 8));
    if (positive.length < MIN_CATEGORY_POINTS)
        return null;
    const spec: SpreadsheetChartSpec = {
        title,
        kind: 'bar',
...
export function buildTimeSeriesChart(rows: Record<string, unknown>[], dateField: string, valueField: string, title: string, kind: SpreadsheetChartKind = 'line', subtitle?: string): SpreadsheetChartSpec | null {
...
export function buildMultiMetricTimeSeriesChart(rows: Record<string, unknown>[], dateField: string, valueFields: string[], title: string, kind: SpreadsheetChartKind = 'line', subtitle?: string): SpreadsheetChartSpec | null {
...
export function buildCategoryBreakdownChart(rows: Record<string, unknown>[], labelField: string, valueField: string, title: string, kind: 'bar' | 'pie' = 'bar', subtitle?: string): SpreadsheetChartSpec | null {
...
export function buildMultiMetricTimeSeriesCharts(rows: Record<string, unknown>[], dateField: string, valueFields: string[], titlePrefix: string): SpreadsheetChartSpec[] {
    return valueFields
...
export function buildAdsMetricsCharts(rows: Array<{
    date: string;
    providerId: string;
    spend: number;
    impressions?: number;
...
export function buildCollaborationExportCharts(rows: Array<Record<string, unknown>>): SpreadsheetChartSpec[] {
    const charts: SpreadsheetChartSpec[] = [];
    ...
    const activityPoints = positivePoints(sortByLabel(Array.from(byDay.entries()).map(([label, value]) => ({ label, value }))));
...
export function buildSpreadsheetChartsFromTableData(rows: Array<Record<string, unknown>>, title = 'Export summary'): SpreadsheetChartSpec[] {
    if (rows.length === 0)
        return [];
    const charts: SpreadsheetChartSpec[] = [];
    const firstRow = rows[0];
    ...
    if (dateField) {
        ...
        if (primaryFields.length >= 2) {
            const combined = buildMultiMetricTimeSeriesChart(rows, dateField, primaryFields, `${title} trends`, 'line', 'Top numeric metrics over time');
            if (combined)
                charts.push(combined);
        }
