...
import { CHART_HEIGHT, CHART_PADDING, CHART_WIDTH, DEFAULT_COLORS, MIN_CATEGORY_POINTS, MIN_TIME_SERIES_POINTS, collapseSmallPieSlices, computeAxisMax, createAxisValueFormatter, filterMeaningfulCharts, positivePoints, sortByLabel, truncateLabel, type SpreadsheetChartImage, type SpreadsheetChartPoint, type SpreadsheetChartSeries, type SpreadsheetChartSpec, } from './cohorts-spreadsheet-chart-specs';
...
function drawSeriesLegend(ctx: CanvasRenderingContext2D, series: SpreadsheetChartSeries[], plot: {
    x: number;
    y: number;
    width: number;
}) {
...
function drawAxes(ctx: CanvasRenderingContext2D, labels: string[], maxValue: number, valueFormatter: (value: number) => string) {
...
function renderLineLikeChart(ctx: CanvasRenderingContext2D, spec: SpreadsheetChartSpec, filled: boolean, valueFormatter: (value: number) => string) {
...
function renderBarChart(ctx: CanvasRenderingContext2D, spec: SpreadsheetChartSpec, valueFormatter: (value: number) => string) {
...
function renderPieChart(ctx: CanvasRenderingContext2D, spec: SpreadsheetChartSpec, valueFormatter: (value: number) => string) {
    const series = spec.series[0];
    if (!series || series.points.length === 0)
        return;
...
export function renderSpreadsheetChartToBase64(spec: SpreadsheetChartSpec): string | null {
    if (typeof document === 'undefined')
        return null;
    if (!spec.series.some((series) => series.points.length > 0))
        return null;
