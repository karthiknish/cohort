...
export function mergeAgentMentions(fromText: AgentMentionEntity[], selected: AgentMentionEntity[]): AgentMentionEntity[] {
...
export function parseAgentMentionsFromStored(value: unknown): AgentMentionEntity[] | undefined {
    ...
    for (const entry of value) {
        ...
        if (!id || !name)
            continue;
        if (type !== 'client' && type !== 'project' && type !== 'team' && type !== 'user')
            continue;
        mentions.push({
            id,
            name,
...
export function serializeAgentMentionsForStorage(mentions: AgentMentionEntity[]): Array<{
    id: string;
    name: string;
    type: AgentMentionType;
    subtitle?: string;
