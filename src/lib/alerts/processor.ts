...
const fetchRecentMetrics = cache(async (convex: ConvexHttpClient, workspaceId: string, clientId: string | null, limit: number) => {
...
export async function processWorkspaceAlerts(options: {
    ...
    try {
        ...
        const maxBaseline = Math.max(...rules.map(r => {
            if (r.condition.type === 'anomaly')
                return r.condition.baselineDays;
            if (r.condition.type === 'trend')
                return r.condition.consecutivePeriods;
            return 0;
        }), 7);
