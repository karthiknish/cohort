...
describe('parseIntegrationError (meta)', () => {
    it('prefers error_user_msg over generic message', () => {
        ...
        const error = parseIntegrationError(response, {
            error: {
        ...
        expect(error.message).toBe('Ads creative post was created by an app that is in development mode. It must be in public to create this ad.');
    });
