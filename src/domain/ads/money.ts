...
export function resolveMetricCurrency(options: {
    ...
    if (metricC)
        return { currency: metricC, source: 'metric' };
    const integrationC = cleanCurrency(integrationCurrency);
    if (integrationC)
        return { currency: integrationC, source: 'integration' };
    const defaultC = cleanCurrency(providerDefaultCurrency);
    if (defaultC)
        return { currency: defaultC, source: 'integration' };
 ...
 */
export function assessComparability(currencies: Array<string | null | undefined>): FinancialComparability {
