 ...
 */
export function aggregateMetricFinancials(
  rows: MetricFinancialRow[],
): DashboardMetricsAggregate {
  const deliveryTotals: DeliveryTotals = { impressions: 0, clicks: 0, conversions: 0 }
  const byCurrency: Record<string, { spend: number; revenue: number }> = {}
  ...
  const visibleByCurrency = Object.fromEntries(
    Object.entries(byCurrency).filter(([key]) => key !== '__unknown__'),
  )
  ...
  const financialTotals: FinancialTotals = {
    ...
    primaryCurrency,
    spend:
      comparability === 'single_currency' && primaryCurrency
...
export function formatAggregatedMoney(
  ...
  financial: Pick<FinancialTotals, 'comparability' | 'primaryCurrency'>,
...
export function financialTotalsHelper(
  financial: Pick<FinancialTotals, 'comparability' | 'primaryCurrency' | 'totalsByCurrency'>,
  fallback: string,
): string {
  if (financial.comparability === 'single_currency' && financial.primaryCurrency) {
    return fallback
...
export function isFinancialComparable(
  comparability: FinancialComparability,
): comparability is 'single_currency' {
