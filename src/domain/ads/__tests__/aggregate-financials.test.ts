...
describe('formatAggregatedMoney', () => {
  ...
  it('formats with primary currency when comparable', () => {
    expect(
      formatAggregatedMoney(120, { comparability: 'single_currency', primaryCurrency: 'GBP' }, format),
    ).toBe('GBP:120')
  })

  it('returns multi-currency label when not comparable', () => {
    expect(
      formatAggregatedMoney(120, { comparability: 'mixed_currency', primaryCurrency: null }, format),
    ).toBe('Multi-currency')
  })
})
