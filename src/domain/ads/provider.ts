 ...
 */
export function normalizeAdsProviderId(raw: string | null | undefined): CanonicalAdsProviderId | null {
    const normalized = String(raw ?? '').trim().toLowerCase();
    if (!normalized || NON_ADS_ALIASES.has(normalized))
        return null;
    if (CANONICAL_IDS.has(normalized))
        return normalized as CanonicalAdsProviderId;
    return PROVIDER_ALIASES[normalized] ?? null;
}
/**
 * Returns true when raw resolves to a known ads provider.
 */
export function isCanonicalAdsProvider(raw: string | null | undefined): raw is CanonicalAdsProviderId {
 ...
 */
export function normalizeAdsAccountId(providerId: CanonicalAdsProviderId | string, rawAccountId: string | null | undefined): string | null {
 ...
 */
export function buildAccountKey(providerId: CanonicalAdsProviderId | string, accountId: string | null | undefined): string {
 ...
 * Returns null for empty values.
 */
export function normalizeSurfaceId(providerId: CanonicalAdsProviderId | string, rawSurface: string | null | undefined): string | null {
