...
const ACTION_PHRASE_PATTERN = new RegExp(ACTION_PHRASES.map(escapeRegExp).join('|'));
function containsTerm(text: string, term: string): boolean {
    return new RegExp(escapeRegExp(term)).test(text);
}
function scoreKeywordMatches(normalizedInput: string, wordSet: Set<string>, keywords: readonly string[]): number {
    let score = 0;
...
function scoreSubstringMatches(normalizedInput: string, terms: readonly string[], pointsPerMatch: number): number {
    let score = 0;
