...
function serializeCapiEvent(event: MetaCapiEventInput): Record<string, unknown> {
  ...
  if (event.customData) {
    ...
    if (event.customData.value != null) custom.value = event.customData.value
    ...
    if (event.customData.contentIds?.length) custom.content_ids = event.customData.contentIds
...
export async function sendMetaOfflineEvents(
  options: Omit<Parameters<typeof sendMetaCapiEvents>[0], 'events'> & {
    events: Array<Omit<MetaCapiEventInput, 'actionSource'> & { actionSource?: MetaCapiActionSource }>
  },
): Promise<MetaCapiSendResult> {
