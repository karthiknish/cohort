...
export async function executeMetaBatch(options: {
  ...
  maxRetries?: number
}): Promise<{ success: boolean; responses: MetaBatchResponseItem[]; error?: string }> {
  ...
  const batchPayload = requests.map((request) => {
    ...
    if (request.body) item.body = request.body
  ...
  try {
    const { payload } = await executeMetaApiRequest<
      Array<{ code?: number; body?: string; headers?: Array<{ name: string; value: string }> }> | { error?: { message?: string } }
    >({
      url,
      accessToken,
    ...
    if (!Array.isArray(payload)) {
      const message =
        payload && typeof payload === 'object' && 'error' in payload
          ? (payload.error as { message?: string })?.message ?? 'Batch failed'
