...
import type { CreateAdSetOptions, UpdateAdSetOptions, } from './types';
// =============================================================================
// LIST AD SETS
// =============================================================================
...
export async function createMetaAdSet(options: CreateAdSetOptions): Promise<{
    ...
    const { accessToken, adAccountId, campaignId, name, status = 'PAUSED', dailyBudget, lifetimeBudget, optimizationGoal, billingEvent, bidAmount, targeting, promotedObject, placementSoftOptOut, maxRetries = 3, } = options;
    const formattedAccountId = adAccountId.startsWith('act_') ? adAccountId : `act_${adAccountId}`;
    const params = new URLSearchParams();
...
export async function updateMetaAdSet(options: UpdateAdSetOptions): Promise<{
    ...
    const { accessToken, adSetId, name, status, dailyBudget, lifetimeBudget, bidAmount, targeting, maxRetries = 3, } = options;
    const params = new URLSearchParams();
    if (name !== undefined) {
        params.set('name', name);
    }
...
export async function updateMetaAdSetStatus(options: {
    ...
    const { accessToken, adSetId, status, maxRetries = 3, } = options;
    const params = new URLSearchParams();
    params.set('status', status);
    await appendMetaAuthParams({ params, accessToken, appSecret: process.env.META_APP_SECRET });
    const url = `${META_API_BASE}/${adSetId}?${params.toString()}`;
