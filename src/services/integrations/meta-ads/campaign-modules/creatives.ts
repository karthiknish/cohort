...
function collectAssetText(entries?: Array<{
    text?: string;
}>): string[] {
...
export function extractMetaCreativeContent(creative?: MetaAdCreative) {
    ...
    const callToAction = resolveMetaCallToActionType(storySpec?.link_data?.call_to_action
        ?? storySpec?.video_data?.call_to_action
        ?? storySpec?.template_data?.call_to_action
        ?? childAttachments.find((c) => c?.call_to_action)?.call_to_action, creative?.call_to_action_type ?? assetFeedCtaType);
    return {
        primaryTexts,
...
export function inferMetaDisplayAdType(options: {
    ...
    const hasPostReference = Boolean((typeof objectStoryId === 'string' && objectStoryId.trim().length > 0)
    ...
    const hasMeaningfulStorySpec = Boolean(storySpec?.link_data
    ...
    if (childCount >= 2)
        return 'carousel';
    const hasStoryVideo = Boolean(storySpec?.video_data?.video_id)
        || objectType === 'VIDEO'
    ...
    const isMultiAssetFeed = bodies > 1
        || titles > 1
        || images > 1
        || videos > 1
...
export async function fetchMetaCreatives(options: {
    ...
    const { accessToken, adAccountId, campaignId, adSetId, statusFilter = ['ACTIVE', 'PAUSED', 'DELETED', 'ARCHIVED'], maxRetries = 3, includeVideoMedia = false, videoLookupLimit = 20, maxPages = 25, } = options;
    ...
    for (let page = 0; page < maxPages; page += 1) {
        ...
        const rawNext = typeof payload?.paging?.next === 'string' && payload.paging.next.trim().length > 0
    ...
    const accountIds = Array.from(new Set(ads.flatMap((ad) => {
        const creative = ad.adcreatives?.data?.[0];
        const spec = creative?.object_story_spec;
        const pageFromBoost = metaPageIdFromObjectStoryId(creative?.object_story_id)
            || metaPageIdFromObjectStoryId(creative?.effective_object_story_id);
        const sponsorPageId = creative?.branded_content_sponsor_page_id
    ...
    let accountDetails: Record<string, {
        name: string;
        picture?: string;
    }> = {};
    if (accountIds.length > 0) {
        ...
        const { payload: accountPayload } = await metaAdsClient.executeRequest<Record<string, {
            name?: string;
            picture?: {
                data?: {
                    url?: string;
        ...
        if (accountPayload) {
            accountDetails = Object.fromEntries(Object.entries(accountPayload).map(([id, data]) => [
                id,
                {
    ...
    const baseCreatives = ads.flatMap((ad): MetaCreative[] => {
        ...
        const resolvedPageId = storySpec?.page_id
        ...
        return [{
                ...
                destinationSpec: sanitizeMetaDestinationSpec(creative?.destination_spec as Record<string, unknown> | undefined),
    ...
    const videoIds = Array.from(new Set(baseCreatives.flatMap((c) => typeof c.videoId === 'string' && c.videoId.length > 0 ? [c.videoId] : []))).slice(0, Math.max(0, videoLookupLimit));
    if (videoIds.length === 0) {
        return withPostMedia;
    }
    const videoInfoEntries = await Promise.all(videoIds.map(async (videoId) => {
        try {
    ...
    const videoInfoById = Object.fromEntries(videoInfoEntries) as Record<string, {
        source?: string;
        picture?: string;
    }>;
