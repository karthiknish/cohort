...
function pickPostAttachmentMedia(attachments?: Array<{
    media_type?: string;
    media?: {
        image?: {
            src?: string;
...
export function needsObjectStoryMediaEnrichment(creative: MetaCreative): boolean {
    const effectiveObjectStoryId = typeof creative.effectiveObjectStoryId === 'string' ? creative.effectiveObjectStoryId.trim() : '';
    const objectStoryId = typeof creative.objectStoryId === 'string' ? creative.objectStoryId.trim() : '';
 ...
 */
export async function enrichMetaCreativesWithObjectStoryMedia(accessToken: string, creatives: MetaCreative[], options?: {
    maxConcurrent?: number;
    maxRetries?: number;
}): Promise<MetaCreative[]> {
    const maxConcurrent = Math.max(1, Math.min(20, options?.maxConcurrent ?? 8));
    const maxRetries = options?.maxRetries ?? 2;
    const indices = creatives.flatMap((creative, index) => needsObjectStoryMediaEnrichment(creative) ? [{ creative, index }] : []);
    if (indices.length === 0)
        return creatives;
    const enriched = [...creatives];
    async function processBatch(batchIndex: number): Promise<void> {
        if (batchIndex * maxConcurrent >= indices.length)
            return;
        const batch = indices.slice(batchIndex * maxConcurrent, (batchIndex + 1) * maxConcurrent);
        const results = await Promise.all(batch.map(async ({ creative, index }) => {
            const storyId = (typeof creative.effectiveObjectStoryId === 'string' && creative.effectiveObjectStoryId.trim()) ||
            ...
            const message = creative.message ||
            ...
            return {
                ...
                creative: {
                    ...
                    message,
                    descriptions,
                    landingPageUrl: creative.landingPageUrl || media.permalinkUrl || creative.landingPageUrl,
                    instagramPermalinkUrl: creative.instagramPermalinkUrl || media.permalinkUrl || creative.instagramPermalinkUrl,
                },
            };
        }));
        for (const { index, creative } of results) {
            if (creative)
                enriched[index] = creative;
        }
