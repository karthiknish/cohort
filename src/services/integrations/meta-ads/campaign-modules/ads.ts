...
// =============================================================================
import { appendMetaAuthParams, buildTimeRange, coerceNumber, META_API_BASE, } from '../client';
...
export async function updateMetaAdStatus(options: {
    ...
    const { accessToken, adId, status, maxRetries = 3, } = options;
    const params = new URLSearchParams();
...
export async function fetchMetaAdMetrics(options: {
    ...
    const { accessToken, adAccountId, campaignId, adSetId, timeframeDays, maxRetries = 3, } = options;
