...
function parseAgeRange(range: string): {
    min?: number;
    max?: number;
} {
    ...
    if (plus)
        return { min: Number(plus[1]), max: 65 };
    return {};
}
export function buildMetaTargetingFromNormalized(source: MetaTargetingSource): Record<string, unknown> {
    ...
    const ageMin = source.demographics.ageMin ??
        (ages.length > 0 ? Math.min(...ages.map((a) => a.min ?? 18)) : undefined);
    const ageMax = source.demographics.ageMax ??
...
export function mergeMetaTargetingWithExisting(existing: Record<string, unknown> | null | undefined, patch: Record<string, unknown>): Record<string, unknown> {
