...
export async function fetchMetaAdAccounts(options: {
    ...
    const { accessToken, appSecret = process.env.META_APP_SECRET, limit = 100, // Increased from 25 to fetch more accounts per page
    maxPages = 10, // Limit total pages to prevent runaway pagination
    maxRetries = DEFAULT_RETRY_CONFIG.maxRetries, } = options;
    if (!accessToken) {
...
export function metaInsightRowsToNormalizedMetrics(adAccountId: string, rows: MetaInsightsRow[]): NormalizedMetric[] {
    ...
    rows.forEach((row) => {
        ...
        const publisherPlatform = typeof row?.publisher_platform === 'string' && row.publisher_platform.length > 0
            ? row.publisher_platform.toLowerCase()
            : undefined;
        const currency = typeof row?.account_currency === 'string' && row.account_currency.trim().length > 0
...
async function fetchMetaAdsMetricsInternal(options: MetaAdsOptions): Promise<NormalizedMetric[]> {
    ...
    const useAsyncInsights = options.useAsyncInsights === true ||
