...
export async function fetchLinkedInCreativeMetrics(options: {
    ...
    const { accessToken, accountId, campaignId, timeframeDays, maxRetries = 3, } = options;
...
export async function fetchLinkedInCreatives(options: {
    ...
    const { accessToken, accountId, campaignId, statusFilter = ['ACTIVE', 'PAUSED', 'ARCHIVED', 'CANCELED', 'DRAFT'], maxRetries = 3, } = options;
    const params = new URLSearchParams({
        q: 'search',
    ...
    elements.forEach(item => {
        ...
        if (textAdVars?.vectorImage)
            urnsToResolve.push(textAdVars.vectorImage);
        if (videoVars?.videoUrn)
            urnsToResolve.push(videoVars.videoUrn);
    });
