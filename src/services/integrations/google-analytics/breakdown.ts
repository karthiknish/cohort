...
const BREAKDOWN_CONFIG: Record<GoogleAnalyticsBreakdownDimension, {
    gaDimension: string;
    label: string;
}> = {
...
async function runBreakdownReport(options: {
    ...
    for (let page = 0; page < 20; page += 1) {
        ...
        if (rowCount !== null && offset >= rowCount)
            break;
        if (rowCount === null && pageRows.length < 10000)
            break;
    }
    return rows;
}
export async function fetchGoogleAnalyticsBreakdowns(options: {
    ...
    const results = await Promise.all(dimensions.map((dimension) => runBreakdownReport({
