...
import type { TikTokAdMetric, TikTokApiErrorResponse, TikTokAudienceTargeting, TikTokCampaign, TikTokCreative, TikTokReportResponse, } from './types';
// =============================================================================
// LIST CAMPAIGNS
// =============================================================================
export async function listTikTokCampaigns(options: {
    ...
    const { accessToken, advertiserId, statusFilter, maxRetries = 3, } = options;
...
export async function updateTikTokCampaignStatus(options: {
    ...
    const { accessToken, advertiserId, campaignId, status, maxRetries = 3, } = options;
...
export async function updateTikTokAdStatus(options: {
    ...
    const { accessToken, advertiserId, adId, status, maxRetries = 3, } = options;
    const { payload } = await tiktokAdsClient.executeRequest<TikTokApiErrorResponse>({
...
export async function updateTikTokCampaignBudget(options: {
    ...
    const { accessToken, advertiserId, campaignId, budget, budgetMode = 'BUDGET_MODE_DAY', maxRetries = 3, } = options;
    const { payload } = await tiktokAdsClient.executeRequest<TikTokApiErrorResponse>({
...
export async function fetchTikTokAdMetrics(options: {
    ...
    const { accessToken, advertiserId, campaignId, timeframeDays, maxRetries = 3, } = options;
...
export async function fetchTikTokCreatives(options: {
    ...
    const { accessToken, advertiserId, campaignId, adGroupId, statusFilter = ['ENABLE', 'DISABLE', 'DELETE'], maxRetries = 3, } = options;
    const requestPayload: Record<string, unknown> = {
        advertiser_id: advertiserId,
        page_size: 100,
    };
...
export async function fetchTikTokAudienceTargeting(options: {
    ...
    const { accessToken, advertiserId, campaignId, adGroupIds, maxRetries = 3, } = options;
    const requestPayload: Record<string, unknown> = {
        advertiser_id: advertiserId,
        page_size: 100,
    };
...
export async function createTikTokAudience(options: {
    ...
    const { accessToken, advertiserId, name, maxRetries = 3, } = options;
    const { payload } = await tiktokAdsClient.executeRequest<{
        code?: number;
        data?: {
...
export async function updateTikTokCampaignBidding(options: {
    ...
    return {
        success: false,
        error: 'TikTok campaign bidding updates are not supported in Cohort yet. Adjust bidding in TikTok Ads Manager at the ad group level.',
    };
}
