...
export interface AdPlatform {
    ...
    getMetrics: (campaignId: string, dateRange: {
        start: Date;
        end: Date;
    }, credentials?: PlatformCredentials) => Promise<CampaignMetrics[]>;
}
...
export class GoogleAdsService implements AdPlatform {
    ...
    async getMetrics(campaignId: string, dateRange: {
        start: Date;
        end: Date;
...
export class AdsManager {
    ...
    async getAllMetrics(connectedAccounts: Array<{
        platform: string;
        accountId: string;
        credentials?: PlatformCredentials;
    }>, dateRange: {
        ...
        const accountMetrics = await Promise.all(connectedAccounts.map(async (account) => {
            ...
            try {
                ...
                const metricsByCampaign = await Promise.all(campaigns.map((campaign) => platform.getMetrics(campaign.id, dateRange, account.credentials)));
