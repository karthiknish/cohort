...
export class CircuitBreaker {
    ...
    recordFailure(): void {
        ...
        if (this.state.failureCount >= this.config.failureThreshold) {
            this.state.state = 'open';
            this.halfOpenStartTime = null;
            console.error(`[CircuitBreaker] Circuit opened after ${this.state.failureCount} consecutive failures`);
        }
    }
    /**
    ...
    async execute<T>(fn: () => Promise<T>): Promise<T> {
        if (!this.canExecute()) {
            throw new Error(`Circuit breaker is ${this.state.state}. Too many recent failures. Please try again later.`);
        }
        try {
