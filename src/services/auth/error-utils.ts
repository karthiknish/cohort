import { InvalidCredentialsError, InvalidEmailError, WeakPasswordError, EmailAlreadyExistsError, AccountDisabledError, AccountSuspendedError, AccountPendingError, SessionExpiredError, NetworkError, NetworkTimeoutError, OAuthCancelledError, OAuthPopupBlockedError, OAuthError, RateLimitError, ServiceUnavailableError, UnauthorizedError, } from '@/lib/api-errors';
/**
 * Auth error codes from various sources (Firebase, Better Auth, generic)
 */
export type AuthErrorCode = 
// Firebase/Legacy codes
'auth/invalid-email' | 'auth/user-disabled' | 'auth/user-not-found' | 'auth/wrong-password' | 'auth/invalid-credential' | 'auth/too-many-requests' | 'auth/operation-not-allowed' | 'auth/email-already-in-use' | 'auth/weak-password' | 'auth/popup-closed-by-user' | 'auth/popup-blocked' | 'auth/unauthorized-domain' | 'auth/account-exists-with-different-credential' | 'auth/cancelled-popup-request' | 'auth/network-request-failed' | 'auth/timeout' | 'auth/requires-recent-login' | 'auth/internal-error' | 'auth/invalid-api-key' | 'auth/app-not-authorized' | 'auth/session-expired' | 'auth/token-expired'
// Better Auth codes
 | 'INVALID_CREDENTIALS' | 'USER_NOT_FOUND' | 'EMAIL_NOT_VERIFIED' | 'ACCOUNT_DISABLED' | 'ACCOUNT_SUSPENDED' | 'ACCOUNT_PENDING' | 'SESSION_EXPIRED' | 'INVALID_TOKEN' | 'WEAK_PASSWORD' | 'EMAIL_ALREADY_EXISTS' | 'INVALID_EMAIL' | 'TOO_MANY_REQUESTS' | 'NETWORK_ERROR' | 'OAUTH_CANCELLED' | 'OAUTH_POPUP_BLOCKED' | 'OAUTH_ERROR'
// Generic HTTP codes
...
export function isSessionExpiredError(error: unknown): boolean {
    ...
    const lowerMessage = errorMessage.toLowerCase();
    return (errorCode === 'auth/session-expired' ||
        errorCode === 'auth/token-expired' ||
        errorCode === 'SESSION_EXPIRED' ||
        errorCode === 'INVALID_TOKEN' ||
        lowerMessage.includes('session expired') ||
...
export function isAccountStatusError(error: unknown): boolean {
    ...
    return (errorCode === 'auth/user-disabled' ||
        errorCode === 'ACCOUNT_DISABLED' ||
...
export function isFirebaseError(error: unknown): error is {
    ...
    return typeof error === 'object' && error !== null && 'code' in error && typeof (error as {
        code?: unknown;
    }).code === 'string';
}
