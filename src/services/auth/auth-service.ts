...
import { bootstrapAndSyncSessionOnce, resetBootstrapSessionCache, startGoogleOAuthSignIn, type BootstrapProfile, } from '@/features/auth/auth-utils';
import { isValidRedirectUrl } from '@/lib/utils';
import { UnauthorizedError, ValidationError, BadRequestError, ServiceUnavailableError, InvalidCredentialsError, InvalidEmailError, WeakPasswordError, EmailAlreadyExistsError, AccountDisabledError, AccountSuspendedError, AccountPendingError, SessionExpiredError, NetworkError, NetworkTimeoutError, RateLimitError, } from '@/lib/api-errors';
...
export class AuthService {
    ...
    private extractSessionPayload(result: unknown): {
    ...
    private async resolveSessionUser(options?: {
    ...
    mapBetterAuthUser(user: Record<string, unknown>): AuthUser {
        ...
        return {
            ...
            role: normalizeRole((user as {
                role?: unknown;
            }).role),
            status: normalizeStatus((user as {
                status?: unknown;
            }).status),
            agencyId: typeof (user as {
                agencyId?: unknown;
            }).agencyId === 'string' ? String((user as {
                agencyId?: unknown;
            }).agencyId) : '',
    ...
    async startGoogleOauth(redirect?: string, clientId?: string | null): Promise<{
        url: string;
    }> {
        ...
        if (typeof legacy?.url === 'string' && legacy.url.length > 0)
            return { url: legacy.url };
        throw new ServiceUnavailableError('Google OAuth did not return a URL');
    }
    async startGoogleWorkspaceOauth(redirect?: string): Promise<{
        url: string;
    }> {
        if (redirect && !isValidRedirectUrl(redirect)) {
            throw new ValidationError('Invalid redirect URL');
    ...
    async startMetaOauth(redirect?: string, clientId?: string | null, surface?: 'facebook' | 'instagram', entryPoint?: 'socials' | 'ads'): Promise<{
        url: string;
    }> {
        if (redirect && !isValidRedirectUrl(redirect)) {
            throw new ValidationError('Invalid redirect URL');
    ...
    async startTikTokOauth(redirect?: string, clientId?: string | null): Promise<{
        url: string;
    }> {
        if (redirect && !isValidRedirectUrl(redirect)) {
            throw new ValidationError('Invalid redirect URL');
    ...
    async startLinkedInOauth(redirect?: string, clientId?: string | null): Promise<{
        url: string;
    }> {
        if (redirect && !isValidRedirectUrl(redirect)) {
            throw new ValidationError('Invalid redirect URL');
