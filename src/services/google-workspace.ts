...
function mapCalendarEventResponse(parsed: GoogleCalendarEventResponse): CalendarMeetEvent {
    ...
    return {
        ...
        meetLink: typeof parsed?.hangoutLink === 'string'
            ? parsed.hangoutLink
...
export async function exchangeGoogleWorkspaceCodeForTokens(options: ExchangeCodeOptions): Promise<GoogleWorkspaceTokenResponse> {
    if (!options.code) {
...
export async function refreshGoogleWorkspaceAccessToken(options: RefreshTokenOptions): Promise<GoogleWorkspaceTokenResponse> {
...
export async function configureGoogleMeetArtifacts(options: {
    ...
    const patchResponse = await fetch(`${GOOGLE_MEET_SPACES_ENDPOINT}/${encodeURIComponent(spaceName)}?updateMask=config.artifactConfig`, {
        method: 'PATCH',
        headers: {
            Authorization: `Bearer ${options.accessToken}`,
            'Content-Type': 'application/json',
