...
function getAppProxySecret(): string {
  ...
  if (!secret) {
    throw new ServiceUnavailableError(
      'Sign-in setup is incomplete on the server. Ensure COHORTS_API_IDEMPOTENCY_SECRET is set in .env.local and in the Convex dashboard (same value).',
    )
  }
  return secret
}
...
const handlers = adaptApiHandler(
  { auth: 'required', bodySchema, rateLimit: 'standard', skipIdempotency: true },
  async (_req, { auth }) => {
    if (!auth.uid) {
