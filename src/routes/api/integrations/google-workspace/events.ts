...
function normalizeTranscriptWebhookPayload(body: unknown, request: Request): NormalizedTranscriptWebhookPayload {
  ...
  const conferenceRecordName =
    ...
    (transcriptResourceName?.match(/conferenceRecords\/[^/]+\/?/)?.[0]?.replace(/\/$/, '') ?? null)
  ...
  return {
    workspaceId: firstString(
      eventPayload.workspaceId, eventPayload.workspace_id,
      context?.workspaceId, context?.workspace_id,
      metadata?.workspaceId, metadata?.workspace_id,
      meeting?.workspaceId,
      pubsub.attributes['workspaceId'], pubsub.attributes['workspace_id'], pubsub.attributes['workspaceid'], pubsub.attributes['x-workspace-id'],
    ),
    userId: firstString(
      eventPayload.userId, eventPayload.user_id,
      context?.userId, context?.user_id,
      metadata?.userId, metadata?.user_id,
      meeting?.userId,
      pubsub.attributes['userId'], pubsub.attributes['user_id'], pubsub.attributes['userid'], pubsub.attributes['x-user-id'],
    ),
    meetingLegacyId: firstString(
      eventPayload.meetingLegacyId, eventPayload.meeting_legacy_id,
      eventPayload.legacyId,
      meeting?.legacyId, meeting?.meetingLegacyId,
      metadata?.meetingLegacyId,
      pubsub.attributes['meetingLegacyId'], pubsub.attributes['meeting_legacy_id'], pubsub.attributes['meetinglegacyid'], pubsub.attributes['x-meeting-legacy-id'],
    ),
    calendarEventId: firstString(
      eventPayload.calendarEventId, eventPayload.calendar_event_id,
      calendarEvent?.eventId, calendarEvent?.id,
      meeting?.calendarEventId,
      metadata?.calendarEventId,
      pubsub.attributes['calendarEventId'], pubsub.attributes['calendar_event_id'], pubsub.attributes['calendareventid'], pubsub.attributes['x-calendar-event-id'],
    ),
    transcriptText,
    status,
    source,
...
const handlers = adaptApiHandler(
  { auth: 'none', bodySchema: webhookSchema, rateLimit: 'sensitive' },
  async (req, { body }) => {
    ...
    if (missingFields.length > 0) {
      const isGoogleWorkspaceEvent =
        normalized.eventType?.startsWith('google.workspace.') ||
        Boolean(normalized.pubsub.messageId) ||
        Boolean(normalized.pubsub.subscription)
    ...
    const updatedMeetingLegacyId = (await saveMeetingTranscript({
      ...
      rawPayload: {
        ...
        cloudEvent: { type: normalized.eventType, subject: normalized.cloudEventSubject, source: normalized.source },
        transcriptResourceName: normalized.transcriptResourceName,
        conferenceRecordName: normalized.conferenceRecordName,
      },
    })) as string
...
export const Route = createFileRoute('/api/integrations/google-workspace/events')({
  server: { handlers },
})
