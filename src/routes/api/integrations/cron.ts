...
const handlers = adaptApiHandler(
  { bodySchema: cronSchema, rateLimit: 'sensitive' },
  async (_req, { auth, body }) => {
    ...
    switch (operation) {
      ...
      case 'process_all_users': {
        const { scheduled, skipped } = await scheduleSyncsForAllUsers({ force, providerIds: resolvedProviderIds, maxUsers, timeframeDays })
        processedCount = scheduled.length
        enqueuedJobs = scheduled.reduce((total, entry) => total + entry.providerIds.length, 0)
        if (skipped.length > 0) {
          errors = skipped.map((entry) => `User ${entry.userId} skipped providers: ${entry.providerIds.join(', ')}`).slice(0, 10)
        }
        break
      }
      case 'cleanup_old_jobs': {
        ...
        try {
          const result = await convex.mutation(api.adsIntegrations.cleanupOldJobsServer, { cutoffMs, cronKey })
      ...
      case 'reset_stale_jobs': {
        ...
        const cronKey = process.env.INTEGRATIONS_CRON_SECRET
        try {
          const result = await convex.mutation(api.adsIntegrations.resetStaleJobsServer, { startedBeforeMs, cronKey })
      ...
      case 'schedule_user': {
        ...
        if (resolvedProviderIds && resolvedProviderIds.length === 1) {
          const scheduled = await scheduleIntegrationSync({ userId: resolvedUserId, providerId: resolvedProviderIds[0]!, force, timeframeDays })
          processedCount = scheduled ? 1 : 0
          enqueuedJobs = scheduled ? 1 : 0
        } else {
          const result = await scheduleSyncsForUser({ userId: resolvedUserId, providerIds: resolvedProviderIds, force, timeframeDays })
    ...
    // Run monitoring inline (was after() in Next.js - fires immediately)
    await recordSchedulerEvent({
      ...
      notes: operation === 'schedule_all_users' && processedCount === 0
...
export const Route = createFileRoute('/api/integrations/cron')({
  server: { handlers },
})
