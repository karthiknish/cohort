...
const handlers = adaptApiHandler(
  { workspace: 'required' as const, bodySchema: saveTranscriptSchema, rateLimit: 'sensitive' },
  async (_req, { auth, workspace, body }) => {
      ...
      if (effectiveTranscript.length >= 20) {
        try {
          await saveMeetingTranscript({
            userId: auth.uid, workspaceId: workspace.workspaceId, meetingLegacyId: body.legacyId,
            transcriptText: effectiveTranscript, source: body.source ?? currentMeeting.transcriptSource ?? 'in-site-voice',
            ...
            rawPayload: { mode, meetingLegacyId: body.legacyId },
          })
          ...
          await setMeetingProcessingState({
            ...
            transcriptProcessingState: 'failed', transcriptProcessingError: message,
        ...
        await updateMeetingRecord({
          ...
          legacyId: body.legacyId, status: 'completed',
        })
      }
    }
      ...
      try {
        ...
        if (notes) {
          await saveMeetingNotes({
            userId: auth.uid, workspaceId: workspace.workspaceId, legacyId: body.legacyId,
            summary: notes.summary, model: notes.model, eventType: 'notes.generated',
          })
          notesGenerated = true
          notesSaved = true
          ...
          await setMeetingProcessingState({
            userId: auth.uid, workspaceId: workspace.workspaceId, legacyId: body.legacyId, userEmail: auth.email,
            notesProcessingState: 'failed', notesProcessingError: 'AI notes generation is unavailable in this environment.',
          })
        }
      } catch (error) {
    ...
    if (mode === 'save-notes') {
      ...
      await saveMeetingNotes({
        userId: auth.uid, workspaceId: workspace.workspaceId, legacyId: body.legacyId,
        summary: incomingNotes, eventType: 'notes.manual_saved',
      })
      notesSaved = true
    ...
    return {
      meetingLegacyId: body.legacyId, meeting: updatedMeetingRecord.meeting,
      transcriptSaved, notesGenerated, notesSaved, summary, model, notesReason,
      transcriptTruncatedForNotes, mode,
    }
  },
)

export const Route = createFileRoute('/api/meetings/transcript')({
  server: { handlers },
})
