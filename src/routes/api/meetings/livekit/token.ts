...
const handlers = adaptApiHandler(
  { workspace: 'required' as const, bodySchema: createTokenSchema, rateLimit: 'sensitive' },
  async (req, { auth, workspace, body }) => {
    ...
    let resolvedMeeting: Awaited<ReturnType<typeof getMeetingRecord>>['meeting'] | null = null
    let migration: { created: boolean; calendarSyncWarning: string | null } | undefined

    if (body.legacyId) {
      const result = await getMeetingRecord({ userId: auth.uid, legacyId: body.legacyId, workspaceId: workspace.workspaceId, userEmail: auth.email })
      resolvedMeeting = result.meeting
    } else if (body.calendarEventId) {
      const result = await getMeetingRecordByCalendarEventId({ userId: auth.uid, calendarEventId: body.calendarEventId, workspaceId: workspace.workspaceId, userEmail: auth.email })
      resolvedMeeting = result.meeting
    } else if (body.roomName) {
      try {
        const result = await getMeetingRecordByRoomName({ userId: auth.uid, roomName: body.roomName, workspaceId: workspace.workspaceId, userEmail: auth.email })
        resolvedMeeting = result.meeting
      } catch { resolvedMeeting = null }
    }
    ...
    if (resolvedMeeting && !resolvedMeeting.roomName) {
      ...
      const roomUrl = buildCohortsMeetingUrl({ appUrl: new URL(req.url).origin, roomName })
      ...
      if (resolvedMeeting.calendarEventId) {
        try {
          ...
          if (tokens?.accessToken) {
            ...
            const tokenExpired = typeof tokens.accessTokenExpiresAtMs === 'number' && tokens.accessTokenExpiresAtMs <= Date.now() + 60000
            if (tokenExpired) {
              ...
              const refreshed = await refreshGoogleWorkspaceAccessToken({ clientId: googleClientId, clientSecret: googleClientSecret, refreshToken: tokens.refreshToken })
              accessToken = refreshed.access_token
              await upsertGoogleWorkspaceTokens({
                userId: integrationUserId, userEmail: tokens.userEmail, accessToken: refreshed.access_token,
                refreshToken: refreshed.refresh_token ?? tokens.refreshToken, idToken: refreshed.id_token ?? tokens.idToken,
                scopes: typeof refreshed.scope === 'string' ? refreshed.scope.split(' ').filter(Boolean) : tokens.scopes,
                accessTokenExpiresAtMs: typeof refreshed.expires_in === 'number' ? Date.now() + refreshed.expires_in * 1000 : tokens.accessTokenExpiresAtMs,
              })
            }
            await updateGoogleCalendarEvent({
              accessToken, eventId: resolvedMeeting.calendarEventId, title: resolvedMeeting.title,
              description: resolvedMeeting.description, startTimeMs: resolvedMeeting.startTimeMs,
              endTimeMs: resolvedMeeting.endTimeMs, timezone: resolvedMeeting.timezone,
              attendeeEmails: resolvedMeeting.attendeeEmails, meetingUrl: roomUrl,
            })
          } else {
      ...
      const ensured = await ensureMeetingNativeRoom({
        userId: auth.uid, userEmail: auth.email, workspaceId: workspace.workspaceId,
        legacyId: resolvedMeeting.legacyId, roomName, meetLink: roomUrl,
      })
      resolvedMeeting = ensured.meeting
      migration = { created: true, calendarSyncWarning }
    }
    ...
    const resolvedMeetLink = resolvedMeeting?.meetLink ?? buildCohortsMeetingUrl({ appUrl: new URL(req.url).origin, roomName: resolvedRoomName })
    ...
    return { token, serverUrl, roomName: resolvedRoomName, meetLink: resolvedMeetLink, meeting: resolvedMeeting ?? undefined, migration }
  },
)

export const Route = createFileRoute('/api/meetings/livekit/token')({
  server: { handlers },
})
