...
import { cancelGoogleCalendarEvent, refreshGoogleWorkspaceAccessToken, resolveGoogleWorkspaceOAuthCredentials } from '@/services/google-workspace'
...
const handlers = adaptApiHandler(
  { workspace: 'required' as const, bodySchema: cancelMeetingSchema, rateLimit: 'sensitive' },
  async (_req, { auth, workspace, body }) => {
    ...
    if (meeting.calendarEventId) {
      ...
      await cancelGoogleCalendarEvent({ accessToken, eventId: meeting.calendarEventId })
    }

    const { meeting: updatedMeeting } = await updateMeetingRecord({
      userId: auth.uid, userEmail: auth.email, workspaceId: workspace.workspaceId, legacyId: body.legacyId, status: 'cancelled',
    })
    ...
    const notificationSummary = await notifyMeetingCancelledEmails({
      recipientEmails: attendeeEmails, meetingTitle: meeting.title, meetingStartIso,
      meetingTimezone: meeting.timezone, cancelledBy: auth.name ?? auth.email ?? 'Cohorts',
...
export const Route = createFileRoute('/api/meetings/cancel')({
  server: { handlers },
})
