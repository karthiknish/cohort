...
import { getGoogleWorkspaceTokens, getMeetingRecord, updateMeetingRecord, upsertGoogleWorkspaceTokens } from '@/lib/meetings-admin'
...
const handlers = adaptApiHandler(
  { workspace: 'required' as const, bodySchema: rescheduleMeetingSchema, rateLimit: 'sensitive' },
  async (req, { auth, workspace, body }) => {
    ...
    const nextMeetLink = roomName
      ? buildCohortsMeetingUrl({ appUrl: new URL(req.url).origin, roomName })
      : meeting.meetLink
    ...
    if (meeting.calendarEventId) {
      ...
      await updateGoogleCalendarEvent({
        accessToken, eventId: meeting.calendarEventId, title: normalizedTitle,
        description: normalizedDescription, startTimeMs: body.startTimeMs, endTimeMs: body.endTimeMs,
        timezone: body.timezone, attendeeEmails, meetingUrl: nextMeetLink,
      })
    }

    const { meeting: updatedMeeting } = await updateMeetingRecord({
      userId: auth.uid, userEmail: auth.email, workspaceId: workspace.workspaceId, legacyId: body.legacyId,
      title: normalizedTitle, description: normalizedDescription, startTimeMs: body.startTimeMs,
      endTimeMs: body.endTimeMs, timezone: body.timezone, attendeeEmails,
      meetLink: nextMeetLink, roomName, status: 'scheduled',
    })
    ...
    const notificationSummary = await notifyMeetingRescheduledEmails({
      recipientEmails: attendeeEmails, meetingTitle: normalizedTitle, previousMeetingStartIso,
      newMeetingStartIso, newMeetingEndIso, meetingTimezone: body.timezone,
      organizerName: auth.name ?? auth.email ?? 'Cohorts', meetLink: nextMeetLink, inSiteJoinUrl: nextMeetLink,
    })
    return { meeting: updatedMeeting, notificationSummary }
  },
)

export const Route = createFileRoute('/api/meetings/reschedule')({
  server: { handlers },
})
