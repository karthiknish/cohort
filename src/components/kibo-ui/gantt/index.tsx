...
const getDateByMousePosition = (context: GanttContextProps, mouseX: number) => {
  const timelineStartDate = new Date(context.timelineData[0]?.year ?? 0, 0, 1);
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({
  ...
  return (
    <div
      className={cn(
        "relative flex items-center gap-2.5 p-2.5 text-xs hover:bg-secondary hover:text-secondary-foreground",
        className
      )}
      key={feature.id}
      onClick={handleClick}
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "gantt relative isolate grid h-full w-full min-w-0 select-none overflow-auto rounded-sm bg-secondary",
          range,
          className
        )}
        ref={scrollRef}
