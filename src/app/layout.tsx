...
export default async function RootLayout({ children, }: Readonly<{
    ...
    const needsConvexToken = pathname.startsWith('/dashboard')
        || pathname.startsWith('/for-you')
        || pathname.startsWith('/admin')
        || pathname.startsWith('/settings');
    const initialToken = needsConvexToken ? await getToken() : null;
    return (<html lang="en">
      <body className={cn(geistSans.variable, geistMono.variable, anybody.variable, 'min-h-screen bg-background font-sans antialiased text-foreground')}>
        <Suspense fallback={null}>
          <GoogleAnalyticsScript />
        </Suspense>
        <AppProviders initialToken={initialToken}>
          <a href="#main-content" className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-[1200] focus:rounded-md focus:bg-background focus:px-3 focus:py-2 focus:text-sm focus:font-medium focus:text-foreground focus:shadow-md">
