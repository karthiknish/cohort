...
type RedirectInProgressRef = { current: boolean }

function getRedirectInProgressRef(): RedirectInProgressRef {
  const key = "__cohortsRedirectInProgressRef"
  const globalStore = globalThis as Record<string, unknown>
  const existing = globalStore[key]

  if (typeof existing === 'object' && existing !== null && 'current' in existing) {
    return existing as RedirectInProgressRef
  }

  const created: RedirectInProgressRef = { current: false }
  globalStore[key] = created
  return created
}

const redirectInProgressRef = getRedirectInProgressRef()
...
export default function HomePage() {
  ...
  const readRedirectState = (): RedirectState | null => {
    ...
    let parsed: Partial<RedirectState> | null = null
    try {
      parsed = JSON.parse(raw) as Partial<RedirectState> | null
    } catch {
      return null
    }

    if (!parsed) return null
    if (typeof parsed.dest !== 'string') return null
