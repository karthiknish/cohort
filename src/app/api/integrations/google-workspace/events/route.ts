...
function normalizeTranscriptWebhookPayload(body: unknown, request: Request): NormalizedTranscriptWebhookPayload {
    ...
    const conferenceRecordName = firstString(conferenceRecord?.name, eventPayload.conferenceRecordName, eventPayload.conference_record_name, asRecord(eventPayload.conferenceRecord)?.name) ??
        (transcriptResourceName?.match(/conferenceRecords\/[^/]+/)?.[0] ?? null);
    const transcriptText = firstString(eventPayload.transcriptText, eventPayload.transcript_text, eventPayload.fullTranscript, eventPayload.transcript, transcript?.text, transcript?.content, transcript?.transcriptText, asRecord(eventPayload.resource)?.transcriptText, asRecord(eventPayload.resourceData)?.transcriptText, metadata?.transcriptText) ??
...
async function generateConciseMeetingNotes(transcriptText: string): Promise<{
    summary: string;
    model: string;
} | null> {
...
export const POST = createApiHandler({
    ...
    if (missingFields.length > 0) {
        const isGoogleWorkspaceEvent = normalized.eventType?.startsWith('google.workspace.') || Boolean(normalized.pubsub.messageId) || Boolean(normalized.pubsub.subscription);
