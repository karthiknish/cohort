...
import { CSRF_COOKIE, CSRF_HEADER, SESSION_COOKIE_MAX_AGE_SEC, appendSessionCookies, clearSessionCookies, generateCsrfToken, } from '@/lib/session-cookies';
...
export const POST = createApiHandler({
    ...
    const agencyId = typeof auth.claims?.agencyId === 'string' && auth.claims.agencyId.length > 0
        ? auth.claims.agencyId
        : auth.uid;
    if (!role || !status) {
        throw new UnauthorizedError('Profile not synced. Complete sign-in bootstrap before syncing the session.');
    }
