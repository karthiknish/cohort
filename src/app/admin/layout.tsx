...
import { Activity, ArrowLeft, ChevronRight, LayoutDashboard, Lightbulb, ShieldCheck, UserCheck, Users, AlertCircle, } from 'lucide-react';
...
function AdminNav() {
    ...
    return (<nav className="mx-auto flex w-full max-w-7xl flex-nowrap items-center gap-1 px-4 py-2 sm:px-6 lg:px-8">
      ...
      <div className="flex min-w-0 flex-1 items-center gap-0.5 overflow-x-auto pb-0.5 [-ms-overflow-style:none] [scrollbar-width:none] sm:gap-1 [&::-webkit-scrollbar]:hidden">
        {adminNavItems.map((item) => {
            ...
            return (<Button key={item.href} asChild variant={isActive ? 'secondary' : 'ghost'} size="sm" className={cn('shrink-0 gap-2 rounded-md', isActive && 'bg-accent/10 text-primary shadow-sm hover:bg-accent/15')}>
...
function AdminBreadcrumb() {
    ...
    if (breadcrumbs.length <= 1)
        return null;
    return (<Breadcrumb className="mx-auto max-w-7xl border-t border-border/40 px-4 py-2 text-sm sm:px-6 lg:px-8">
      <BreadcrumbList>
        {breadcrumbs.map((crumb, index) => (<React.Fragment key={crumb.href}>
            <BreadcrumbItem>
              {index < breadcrumbs.length - 1 ? (<BreadcrumbLink asChild>
...
export default function AdminLayout({ children }: {
    ...
    return (<ProtectedRoute requiredRole="admin" allowPreviewAccess={isScreenRecordingModeEnabled()}>
