...
export default function AdminUsersPage() {
  ...
  const handleRoleChange = async (userId: string, role: AdminUserRecord['role']) => {
    ...
    try {
      ...
      if (!response.ok) {
        const message = typeof payload?.error === 'string' ? payload.error : 'Failed to update user role'
        throw new Error(message)
      }
  ...
  const handleStatusAction = async (userRecord: AdminUserRecord) => {
    ...
    try {
      ...
      const payload = (await response.json().catch(() => null)) as { error?: string } | null
      if (!response.ok) {
        const message = typeof payload?.error === 'string' ? payload.error : 'Failed to update user status'
        throw new Error(message)
      }
      ...
      <div className="flex min-h-screen items-center justify-center bg-muted/40 px-4 py-16">
        <Card className="max-w-md border-muted/60">
          <CardHeader>
            ...
            <CardDescription>Log in to an admin account to manage users.</CardDescription>
          </CardHeader>
        </Card>
      </div>
    )
      ...
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col gap-6 px-4 py-10">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
          <div>
            ...
            <p className="text-muted-foreground">Review active accounts, adjust roles, and deactivate access as needed.</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <Button asChild variant="outline">
              <Link href="/admin">Contact queue</Link>
            </Button>
        ...
        <Card className="border-muted/60 bg-background">
          <CardHeader className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <CardDescription>Search and manage user permissions across Cohorts.</CardDescription>
            </div>
