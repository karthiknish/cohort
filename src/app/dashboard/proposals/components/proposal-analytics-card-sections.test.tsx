  ...
  it('renders loading/header/summary/success sections', () => {
    const markup = renderToStaticMarkup(
      <>
        ...
        <ProposalAnalyticsSummaryGrid summary={summary} formatDuration={(ms) => `${ms}ms`} />
        <ProposalAnalyticsSuccessRates summary={summary} formatDuration={(ms) => `${ms}ms`} formatPercentage={(value) => `${value}%`} />
      </>,
    )
  ...
  it('renders chart, by-client list, and empty state', () => {
    const markup = renderToStaticMarkup(
      <>
        ...
        />
        <ProposalAnalyticsByClientCard
          byClient={[{ clientId: '1', clientName: 'Acme', proposalCount: 4, submittedCount: 2, sentCount: 1, lastProposalAt: '2026-03-11T12:00:00.000Z' }]}
        />
        <ProposalAnalyticsEmptyState />
      </>,
    )
