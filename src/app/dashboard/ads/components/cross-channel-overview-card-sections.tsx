...
export function CrossChannelOverviewContent({ metrics, metricsLoading, summaryCards }: { metrics: MetricRecord[]; metricsLoading: boolean; summaryCards: SummaryCard[] }) {
  return <CardContent className="space-y-6"><FadeInStagger className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">{summaryCards.map((card) => <SummaryCardTile key={card.id} card={card} />)}</FadeInStagger><div className="h-[350px] w-full"><PerformanceChart metrics={metrics} loading={metricsLoading} /></div></CardContent>
}