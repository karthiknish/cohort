...
export function buildMetricsTableColumns(): ColumnDef<MetricRecord>[] {
  return [
    ...
    { accessorKey: 'spend', header: () => <HeaderWithTooltip title="Spend" tooltip="Total amount spent on ads" />, cell: ({ row }) => formatCurrency(row.getValue('spend')) },
    ...
    { accessorKey: 'revenue', header: () => <HeaderWithTooltip title="Revenue" tooltip="Total revenue generated from ads" />, cell: ({ row }) => { const revenue = row.getValue('revenue') as number | null | undefined; return revenue != null ? formatCurrency(revenue) : '—' } },
  ]
}
