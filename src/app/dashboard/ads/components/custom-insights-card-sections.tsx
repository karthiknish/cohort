...
function KpiTile({ benchmark, currency = 'USD', format, icon, invertTrend, label, trend, value }: KpiTileData & { currency?: string }) {
  ...
  return <div className={cn('group relative flex flex-col gap-3 overflow-hidden rounded-xl border border-muted/60 bg-card p-5 transition-[color,background-color,border-color,text-decoration-color,fill,stroke,opacity,box-shadow,transform,filter,backdrop-filter] duration-[var(--motion-duration-normal)] ease-[var(--motion-ease-out)] motion-reduce:transition-none hover:border-muted hover:shadow-md', hasAnomaly && 'ring-2 ring-amber-500/50 ring-offset-2 ring-offset-background')}>{hasAnomaly ? <TooltipProvider><Tooltip><TooltipTrigger asChild><div className="absolute right-3 top-3 z-10"><div className="flex h-7 w-7 items-center justify-center rounded-full bg-amber-100 dark:bg-amber-900/50"><AlertTriangle className="h-3.5 w-3.5 text-amber-600 dark:text-amber-400" /></div></div></TooltipTrigger><TooltipContent><p>This metric deviates significantly from benchmark</p></TooltipContent></Tooltip></TooltipProvider> : null}<div className="relative z-10 flex items-center gap-2.5"><div className="flex h-9 w-9 items-center justify-center rounded-xl border border-muted/60 bg-muted/30 text-muted-foreground">{icon}</div><span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground/80">{label}</span></div><div className="relative z-10 flex items-end justify-between gap-2"><div className="flex flex-col"><span className="text-3xl font-bold tracking-tight tabular-nums">{formatValue(value, format, currency)}</span><span className="mt-1 text-xs text-muted-foreground/60">{format === 'currency' && 'Cost per acquisition'}{format === 'percent' && 'Rate percentage'}{format === 'ratio' && 'Return multiplier'}</span></div>{trend !== null && trend !== undefined && Math.abs(trend) >= 0.01 ? <Badge variant="secondary" className={cn('flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold', trendStatus === 'up' && 'border border-emerald-200/50 bg-emerald-100 text-emerald-700 dark:border-emerald-800/50 dark:bg-emerald-950 dark:text-emerald-400', trendStatus === 'down' && 'border border-rose-200/50 bg-rose-100 text-rose-700 dark:border-rose-800/50 dark:bg-rose-950 dark:text-rose-400')}>{trendStatus === 'up' ? <TrendingUp className="h-3.5 w-3.5" /> : <TrendingDown className="h-3.5 w-3.5" />}{Math.abs(trend * 100).toFixed(1)}%</Badge> : null}</div></div>
}
