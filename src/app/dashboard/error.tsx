...
export default function DashboardError({ error, unstable_retry, reset }: DashboardErrorProps) {
    ...
    return (<div className="flex min-h-[60vh] items-center justify-center bg-muted/30 p-6">
      <RouteErrorLogger error={error} segment="dashboard" />
      <div className="w-full max-w-xl rounded-2xl border border-border/60 bg-background p-6 shadow-sm" role="alert" aria-live="assertive" aria-atomic="true">
        ...
        {isDevelopment ? (<div className="mt-4 rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-left">
            <p className="wrap-break-word text-xs font-mono text-destructive">{error.message}</p>
            {error.stack ? (<details className="mt-3">
        ...
        <div className="mt-6 flex flex-col gap-3">
          <button type="button" onClick={handleRetry} className="inline-flex w-full items-center justify-center rounded-md bg-foreground px-4 py-2 text-sm font-medium text-background transition-opacity hover:opacity-90">
            Try again
          </button>
          <Link href="/dashboard" className="inline-flex w-full items-center justify-center rounded-md border border-border px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-muted">
