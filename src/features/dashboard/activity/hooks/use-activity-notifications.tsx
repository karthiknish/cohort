...
export function useActivityNotifications(activities: Activity[]) {
    ...
    const showNotificationToast = useEffectEvent((activity: Activity, isFromOtherTab = false) => {
    ...
    useEffect(() => {
        if (!activities.length || !selectedClient?.id)
            return;
        const previousActivities = previousActivitiesRef.current;
        const newActivities = activities.filter(activity => !previousActivities.some(prev => prev.id === activity.id));
    ...
    }, [activities, selectedClient?.id, user?.id]);
    return {
        config,
