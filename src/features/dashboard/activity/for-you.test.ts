...
describe('buildActivityHubModel', () => {
    it('prioritizes overdue work and analytics setup issues', () => {
        ...
        expect(model.priorityItems.map((item) => item.title)).toEqual(expect.arrayContaining(['Overdue tasks need attention', 'Connect Google Analytics']));
    });
    ...
    it('creates spotlight tabs for worked on, deadlines, and performance', () => {
        ...
        expect(performanceItems.map((item) => item.title)).toEqual(expect.arrayContaining(['Google Analytics linked']));
