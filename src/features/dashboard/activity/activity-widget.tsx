...
function ActivityItem({ activity }: ActivityItemProps) {
    ...
    return (<div className="flex items-start gap-3 p-3 rounded-lg hover:bg-muted/50 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          {activity.navigationUrl && (<>
              ...
              <Link href={activity.navigationUrl} className="text-xs text-primary hover:underline">
                View
              </Link>
            </>)}
        </div>
...
export function ActivityWidget() {
    ...
    const readFilterFromUrl = (key: 'activityType' | 'dateRange') => {
    ...
    const updateFilters = (newActivityType: string, newDateRange: string) => {
    ...
    return (<Card className="shadow-sm h-full flex flex-col">
      <CardHeader className="flex flex-row items-center justify-between gap-y-0 pb-2">
        <div className="flex items-center gap-2">
          <CardTitle className="text-base">Recent Activity</CardTitle>
          {isRealTime && (<span className="relative flex size-2">
      ...
      <CardContent className="flex-1">
        {loading ? (<div className="space-y-4">
            {Array.from({ length: 5 }, (_, index) => `activity-${index + 1}`).map((slot) => (<div key={slot} className="flex gap-3 p-2">
          ...
          </div>) : error ? (<div className="flex h-full flex-col items-center justify-center py-8 text-center">
            <p className="text-sm text-destructive mb-2">{error}</p>
            <Button variant="outline" size="sm" onClick={handleRefresh}>
              Try again
            </Button>
          </div>) : filteredActivities.length === 0 ? (<div className="flex h-full flex-col items-center justify-center py-8 text-center">
            ...
            {(activityType !== 'all' || dateRange !== '7d') && (<Button variant="ghost" size="sm" className="mt-2 h-8 text-xs" onClick={handleClearFilters}>
                Clear filters
              </Button>)}
          </div>) : (<div className="space-y-0.5 -mx-2">
