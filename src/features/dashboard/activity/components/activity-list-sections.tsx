...
export function ActivityVirtualTranslateRow({
  start,
  className,
  dataIndex,
  measureRef,
...
export function ActivityListSelectAllHeader({
  sortedCount,
  selectedCount,
  onSelectAll,
}: {
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 border-b bg-muted/30">
      <Checkbox
        ...
        checked={selectedCount === sortedCount}
...
export function ActivityListErrorState({ error, onRetry }: { error: string; onRetry: () => void }) {
  return (
...
export function ActivityListVirtualizedBody({
  activityFlatRows,
  activityVirtualizer,
  virtualContainerStyle,
  handlers,
  ...
  virtualContainerStyle: { height: number }
          ...
          >
            <div className="cv-scroll-item-activity pt-2 sm:pt-3">
              <ActivityItem
                ...
                isSelected={handlers.selectedActivities.has(row.activity.id)}
                showReactions={handlers.showReactions}
                comments={handlers.comments[row.activity.id] || []}
                onSelectionChange={handlers.onSelectionChange}
                onTogglePin={handlers.onTogglePin}
                onMarkAsRead={handlers.onMarkAsRead}
                onAddReaction={handlers.onAddReaction}
                onAddComment={handlers.onAddComment}
                onShowReactionsChange={handlers.onShowReactionsChange}
                onViewDetails={handlers.onViewDetails}
                currentUserName={handlers.currentUserName}
              />
            </div>
          </ActivityVirtualTranslateRow>
        )
...
export function ActivityListGroupedBody({
  groupKeys,
  groupedActivities,
  handlers,
}: {
            ...
            <div className="ml-2 space-y-4 sm:space-y-6 border-l-2 border-muted pl-4 sm:pl-6 pb-2">
              {dateGroupActivities.map((activity) => (
                <ActivityItem
                  key={activity.id}
                  activity={activity}
                  isSelected={handlers.selectedActivities.has(activity.id)}
                  showReactions={handlers.showReactions}
                  comments={handlers.comments[activity.id] || []}
                  onSelectionChange={handlers.onSelectionChange}
                  onTogglePin={handlers.onTogglePin}
                  onMarkAsRead={handlers.onMarkAsRead}
                  onAddReaction={handlers.onAddReaction}
...
export function ActivityListLoadMoreFooter({
  hasMore,
  loading,
  sortedCount,
  observerTargetRef,
...
export function useActivityListInfiniteScroll({
  hasMore,
  loading,
  onLoadMore,
}: {
