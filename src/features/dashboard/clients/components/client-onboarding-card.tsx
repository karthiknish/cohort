...
export function ClientOnboardingChecklist({ items }: {
      ...
      <CardContent className="pt-6 space-y-6">
        ...
        <div className="space-y-2.5">
          {items.map((item) => {
            ...
            return (<div key={item.id} className={cn("group flex items-start gap-3 rounded-xl border border-muted/30 p-3 motion-chromatic", item.done ? "bg-accent/[0.03] border-accent/20" : "bg-muted/5")}>
                <div className={cn("mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full transition-colors", item.done ? "bg-accent/10 text-primary" : "bg-muted/20 text-muted-foreground/70")}>
                  <Icon className={cn("size-3", item.loading && "animate-spin")}/>
                </div>
                <div className="min-w-0 flex-1">
                  <p className={cn("text-xs font-bold tracking-tight transition-colors", item.done ? "text-primary" : "text-foreground")}>{item.label}</p>
                  {item.helper ? (<p className="mt-0.5 text-[10px] font-medium leading-relaxed text-muted-foreground/80">
                      {item.helper}
                    </p>) : null}
                </div>
              </div>);
