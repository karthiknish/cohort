...
function TeamMemberCard({ member }: {
    ...
    return (<div className="group flex items-center gap-4 rounded-xl border border-muted/30 bg-muted/5 p-4 motion-chromatic hover:bg-muted/10">
      <Avatar className="size-10 border border-muted/20">
        <AvatarFallback className={cn('text-xs font-bold uppercase tracking-wider', isManager ? 'bg-accent/10 text-primary' : 'bg-muted/30 text-muted-foreground')}>
          {getInitials(member.name)}
        </AvatarFallback>
      </Avatar>
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-bold text-foreground transition-colors group-hover:text-primary">{member.name}</p>
        <p className="truncate text-[10px] font-bold uppercase tracking-widest text-muted-foreground/70">{member.role || 'Contributor'}</p>
      </div>
      <Badge variant="secondary" className={cn('shrink-0 h-6 px-2 text-[10px] font-black uppercase tracking-widest rounded-full border-none', isManager ? 'bg-accent/10 text-primary' : 'bg-muted/20 text-muted-foreground')}>
...
function EmptyTeamState() {
    return (<div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-muted/30 bg-muted/5 py-12 px-6 text-center">
...
export function TeamMembersCard({ teamMembers, filteredTeamMembers, teamSearch, onTeamSearchChange, }: TeamMembersCardProps) {
    ...
    return (<Card className="overflow-hidden border-muted/40 bg-card shadow-sm motion-chromatic hover:shadow-md">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex-1 lg:flex-none">
              ...
              <Input placeholder="Search team…" value={teamSearch} onChange={handleTeamSearchChange} className="h-9 w-full rounded-xl border-muted/30 bg-card pl-10 pr-8 text-[11px] font-medium shadow-sm motion-chromatic focus:border-accent/40 focus:ring-4 focus:ring-primary/5 sm:w-48"/>
              {teamSearch && (<Button variant="ghost" size="sm" className="absolute right-1 top-1/2 size-7 -translate-y-1/2 rounded-lg p-0 text-muted-foreground/70 hover:text-foreground" onClick={handleClearTeamSearch}>
      ...
      <CardContent className="pt-6">
        {teamMembers.length === 0 ? (<EmptyTeamState />) : filteredTeamMembers.length === 0 ? (<div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-muted/30 bg-muted/5 py-12 text-center">
            ...
            {filteredTeamMembers.map((member) => (<TeamMemberCard key={`${member.name}-${member.role}`} member={member}/>))}
          </div>)}
      </CardContent>
    </Card>);
}
