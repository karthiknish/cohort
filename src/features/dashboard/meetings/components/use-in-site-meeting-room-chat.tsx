...
import { buildMeetingChatMessageContent, countUnreadMeetingChatMessages, DEFAULT_MEETING_CHAT_MENTION_STATE, detectMeetingChatMentionState, getMeetingChatAvatarUrlFromMetadata, insertMeetingChatMention, type MeetingChatAttachment, type MeetingChatMentionState, } from './in-site-meeting-room-chat.utils';
import { MeetingChatFloatingDock, type MeetingChatMentionCandidate } from './in-site-meeting-room-chat-sections';
import { createInitialMeetingChatState, meetingChatReducer, type InSiteMeetingRoomChatProps, } from './in-site-meeting-room-chat-state';
export function useInSiteMeetingRoomChat(props: InSiteMeetingRoomChatProps) {
    ...
    const syncMentionStateFromValue = (nextValue: string, caretPosition: number) => {
    ...
    const uploadPendingMeetingAttachments = async (attachments: PendingAttachment[]): Promise<MeetingChatAttachment[]> => {
        if (!user?.id) {
    ...
    const insertSelectedMention = (candidate: MeetingChatMentionCandidate) => {
    ...
    return (<MeetingChatFloatingDock compact={compact} isOpen={isOpen} onOpen={handleOpen} panelProps={panelProps} unreadCount={unreadCount}/>);
}
