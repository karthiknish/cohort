...
export function UpcomingMeetingsCard(props: UpcomingMeetingsCardProps) {
  ...
  return (
    <Card className={cn(DASHBOARD_THEME.cards.base, 'h-full')}>
      <CardHeader className="space-y-3 border-b border-muted/40 pb-4">
        <div className="flex items-start justify-between gap-3">
          ...
          {googleWorkspaceConnected ? (
            <Button type="button" variant="outline" className={cn(getButtonClasses('outline'), 'shrink-0')} asChild>
              <a href={GOOGLE_CALENDAR_URL} target="_blank" rel="noreferrer">
                View Calendar
              </a>
            </Button>
          ) : (
            <Button type="button" variant="outline" className={cn(getButtonClasses('outline'), 'shrink-0')} disabled>
              View Calendar
            </Button>
          )}
        </div>
