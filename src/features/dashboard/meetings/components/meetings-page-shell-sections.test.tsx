...
vi.mock('./meeting-room-loading-state', () => ({
    MeetingRoomLoadingState: ({ sharedRoomName }: {
        sharedRoomName: string;
    }) => <div>Loading {sharedRoomName}</div>,
}));
vi.mock('./in-site-meeting-card', () => ({
    ...
    }) => <div>Meeting Room {meeting.legacyId}</div>,
}));
const noop = vi.fn();
...
describe('meetings page shell sections', () => {
    it('renders the default meetings layout sections', () => {
        const markup = renderToStaticMarkup(<MeetingsDefaultView editingMeeting={false} meetingsHeaderProps={emptyProps} meetingCancelDialogProps={emptyProps} quickMeetDialogProps={emptyProps} createMeetingCardProps={emptyProps} rescheduleMeetingCardProps={rescheduleMeetingCardProps} showPreviewMode={true} showReadOnlyAccessAlert={true} upcomingMeetingsCardProps={emptyProps} timezone="Asia/Calcutta"/>);
