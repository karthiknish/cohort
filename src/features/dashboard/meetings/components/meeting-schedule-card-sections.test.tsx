...
describe('meeting schedule card sections', () => {
    it('renders the shared frame copy and workspace alert', () => {
        const markup = renderToStaticMarkup(<MeetingScheduleCardFrame {...baseProps} cardTitle="Schedule Meeting" cardDescription="Creates a Cohorts room." submitLabel="Schedule room" submittingLabel="Scheduling..."/>);
    ...
    it('renders footer actions when provided', () => {
        const markup = renderToStaticMarkup(<MeetingScheduleCardFrame {...baseProps} googleWorkspaceConnected={true} cardTitle="Reschedule Meeting" cardDescription="Update the room." submitLabel="Save Reschedule" submittingLabel="Saving…" footerAction={FOOTER_ACTION} onReset={console.log}/>);
