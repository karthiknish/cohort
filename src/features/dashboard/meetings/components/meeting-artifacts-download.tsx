...
export function MeetingArtifactsDownload({
  className,
  legacyId,
  meetingTitle,
  notesSummary,
  ...
  transcriptStorageId,
  compact = false,
}: MeetingArtifactsDownloadProps) {
  const { user } = useAuth()
  const workspaceId = getWorkspaceId(user)
  const [downloading, setDownloading] = useState<'notes-pdf' | 'notes-cloud' | 'transcript' | 'transcript-cloud' | null>(null)

  const artifactUrls = useQuery(
    meetingArchivesApi.getArtifactDownloadUrls,
    workspaceId ? { workspaceId, legacyId } : 'skip',
  )
  ...
  const handleDownloadNotesPdf = useCallback(() => {
  ...
  }, [meetingTitle, transcriptText])

  const handleDownloadNotesFromCloud = useCallback(async () => {
  ...
  }, [artifactUrls?.notesDownloadUrl, meetingTitle])

  const handleDownloadTranscriptFromCloud = useCallback(async () => {
