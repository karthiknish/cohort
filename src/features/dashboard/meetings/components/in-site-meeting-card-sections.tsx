...
export function MeetingRoomPageHeader({ joinConfigPresent, onBack, }: {
    joinConfigPresent: boolean;
    onBack: () => void;
}) {
    return (<div className="flex flex-wrap items-start justify-between gap-4 rounded-[28px] border border-border bg-card px-5 py-4 shadow-sm">
...
export function MeetingRoomHeroSection({ meetingStatus, meetingTitle, meetingDescription, meetingStartTimeMs, meetingEndTimeMs, meetingTimezone, joinConfigPresent, isMobileViewport, pipSupported, pipActive, canMinimizeRoom, isMinimized, meetingLink, onOpenSidebar, onTogglePictureInPicture, onToggleMinimize, onCopyLink, }: {
    ...
    return (<div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <div className="flex w-full flex-col gap-2 sm:w-auto sm:items-end">
        ...
        {joinConfigPresent ? (<Button type="button" variant="outline" onClick={onTogglePictureInPicture} disabled={!pipSupported} title={pipSupported ? undefined : 'Picture in Picture is unavailable in this browser'}>
            <PictureInPicture2 className="mr-2 size-4"/>
            {pipActive ? 'Exit PiP' : pipSupported ? 'Enter PiP' : 'PiP unavailable'}
          </Button>) : null}
        {joinConfigPresent && canMinimizeRoom ? (<Button type="button" variant="outline" className="md:hidden" onClick={onToggleMinimize}>
            {isMinimized ? <Maximize2 className="mr-2 size-4"/> : <Minimize2 className="mr-2 size-4"/>}
            {isMinimized ? 'Restore room' : 'Send to tray'}
          </Button>) : null}
        {meetingLink ? (<Button type="button" variant="outline" className="min-w-[152px]" onClick={onCopyLink}>
...
export function MeetingRoomEmptyState({ inlineJoinError, hasJoinReference, }: {
    inlineJoinError: string | null;
    hasJoinReference: boolean;
}) {
    return (<div className="flex min-h-[420px] items-center justify-center rounded-[32px] border border-border bg-card p-6 shadow-sm lg:min-h-[560px] lg:p-8">
      <div className="max-w-3xl space-y-6 text-center">
        ...
        {inlineJoinError ? (<Alert className="border-destructive/40 bg-destructive/5 text-left">
...
export function MeetingRoomCanvasSection({ autoCaptureEnabled, onEnableTranscriptRecording, transcriptRecordingEnabled, autoSyncing, canMinimize, finalizingSession, hasJoinReference, inlineJoinError, isMinimized, joinConfig, joinError, layoutContext, meetingTitle, notesProcessingError, notesProcessingState, onAppendTranscript, onCaptureStatusChange, onDisconnected, onError, onInterimTranscriptChange, onToggleMinimize, onTogglePictureInPicture, pipActive, pipSupported, roomPinnedToMobileTray, roomViewportRef, summaryPreview, transcriptProcessingError, transcriptProcessingState, }: MeetingRoomCanvasProps & {
    hasJoinReference: boolean;
    inlineJoinError: string | null;
    ...
    return (<div className={cn('relative', roomPinnedToMobileTray ? 'min-h-[168px]' : 'min-h-[420px]')}>
      {joinConfig ? (<>
          {roomPinnedToMobileTray ? (<div className="rounded-[28px] border border-dashed border-border bg-muted/20 p-5 shadow-sm md:hidden">
          ...
          <LiveKitRoom token={joinConfig.token} serverUrl={joinConfig.serverUrl} connect audio video onDisconnected={onDisconnected} onError={handleLiveKitError} className={cn(roomPinnedToMobileTray
                ? 'fixed inset-x-4 bottom-[max(1rem,env(safe-area-inset-bottom))] z-50 mx-auto w-auto max-w-[calc(100vw-2rem)] md:hidden'
                : 'h-full')}>
            <InSiteMeetingLiveRoomCanvas meetingTitle={meetingTitle} layoutContext={layoutContext} autoCaptureEnabled={autoCaptureEnabled} onEnableTranscriptRecording={onEnableTranscriptRecording} transcriptRecordingEnabled={transcriptRecordingEnabled} compact={roomPinnedToMobileTray} pipSupported={pipSupported} pipActive={pipActive} canMinimize={canMinimize} isMinimized={isMinimized} autoSyncing={autoSyncing} finalizingSession={finalizingSession} transcriptProcessingState={transcriptProcessingState} notesProcessingState={notesProcessingState} notesProcessingError={notesProcessingError} transcriptProcessingError={transcriptProcessingError} summaryPreview={summaryPreview} onTogglePictureInPicture={onTogglePictureInPicture} onToggleMinimize={onToggleMinimize} roomViewportRef={roomViewportRef} onAppendTranscript={onAppendTranscript} onInterimTranscriptChange={onInterimTranscriptChange} onCaptureStatusChange={onCaptureStatusChange}/>
          </LiveKitRoom>
        </>) : (<MeetingRoomEmptyState inlineJoinError={inlineJoinError} hasJoinReference={hasJoinReference}/>)}

      {joinConfig && joinError ? (<Alert className="border-destructive/40 bg-destructive/5">
          <AlertTriangle className="size-4"/>
...
export function MeetingRoomToolsSection({ captureErrorPresent, captureListening, finalizingSession, hasTranscriptSaved, roomAutomationMessage, roomAutomationBadge, lastAutoSyncAt, summaryReady, transcriptProcessingState, notesProcessingState, meetingTimezone, joinConfigPresent, viewport, join, roomActionLabel, onOpenSidebar, onToggleMinimize, onJoinRoom, }: {
    ...
    return (<div className="flex flex-wrap items-start justify-between gap-3 rounded-3xl border border-border bg-card p-4 shadow-sm">
      <div>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {transcriptProcessingState === 'processing' ? (<Badge variant="info">
              <LoaderCircle className="mr-1 size-3 animate-spin"/>
              Finalizing transcript
            </Badge>) : null}
          {notesProcessingState === 'processing' ? (<Badge variant="info">
              <LoaderCircle className="mr-1 size-3 animate-spin"/>
              Generating AI notes
            </Badge>) : null}
        </div>
        ...
        <MeetingAutomationPipeline className="mt-3" captureErrorPresent={captureErrorPresent} captureListening={captureListening} finalizingSession={finalizingSession} hasTranscriptSaved={hasTranscriptSaved} inRoom={joinConfigPresent} notesProcessingState={notesProcessingState} summaryReady={summaryReady} transcriptProcessingState={transcriptProcessingState}/>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {joinConfigPresent && isMobileViewport ? (<Button type="button" variant="outline" className="md:hidden" onClick={onToggleMinimize}>
            {isMinimized ? <Maximize2 className="mr-2 size-4"/> : <Minimize2 className="mr-2 size-4"/>}
            {isMinimized ? 'Restore room' : 'Send to tray'}
          </Button>) : null}
        {!joinConfigPresent ? (<Button type="button" className={cn(getButtonClasses('primary'), 'min-w-[168px]')} disabled={joiningRoom || isPreviewMeeting || !hasJoinReference} onClick={onJoinRoom}>
...
export function MeetingRoomLeaveDialog({ open, onOpenChange, onConfirm, }: {
    open: boolean;
