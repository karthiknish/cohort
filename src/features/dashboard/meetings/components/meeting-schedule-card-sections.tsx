...
import { MeetingAttendeesSection, MeetingDetailsSection, MeetingScheduleWhenSection, } from './meeting-form-sections';
...
function ScheduleFormSection({ title, description, children, className, }: {
    ...
    return (<section className={cn('rounded-lg border border-muted/50 bg-muted/15 p-4', className)}>
...
export function MeetingScheduleCardFrame({ attendeeEmails, attendeeInput, attendeeSuggestions, cardDescription, cardTitle, description, durationMinutes, footerAction, googleWorkspaceConnected, meetingDate, meetingTime, onAddSuggestedAttendee, onAttendeeInputChange, onAttendeeKeyDown, onCommitAttendeeInput, onDescriptionChange, onDurationMinutesChange, onMeetingDateChange, onMeetingTimeChange, onRemoveAttendee, onSubmit, onTimezoneChange, onTitleChange, scheduleDisabled, scheduleRequiresGoogleWorkspace, scheduling, submitDisabled, submitLabel, submittingLabel, timezone, title, }: MeetingScheduleCardFrameProps) {
    return (<Card className={cn(DASHBOARD_THEME.cards.base, 'overflow-hidden')}>
      ...
      <CardContent className="pt-5">
        {scheduleRequiresGoogleWorkspace && !googleWorkspaceConnected ? (<Alert className="mb-5">
        ...
        <form className="space-y-5" onSubmit={onSubmit}>
          <ScheduleFormSection title="When" description="Pick the date, start time, and length of the meeting.">
            <MeetingScheduleWhenSection disabled={scheduleDisabled} dateId="schedule-date" meetingDate={meetingDate} timeId="schedule-start-time" meetingTime={meetingTime} durationId="schedule-duration" durationMinutes={durationMinutes} timezone={timezone} timezoneId="schedule-timezone" onMeetingDateChange={onMeetingDateChange} onMeetingTimeChange={onMeetingTimeChange} onDurationMinutesChange={onDurationMinutesChange} onTimezoneChange={onTimezoneChange}/>
          </ScheduleFormSection>

          <ScheduleFormSection title="Details" description="Give attendees context before they join.">
            <div className="grid gap-4 md:grid-cols-1">
              <MeetingDetailsSection disabled={scheduleDisabled} title={title} titleId="schedule-title" titlePlaceholder="Weekly client strategy sync" description={description} descriptionId="schedule-description" descriptionPlaceholder="Agenda, links, and expected outcomes" onTitleChange={onTitleChange} onDescriptionChange={onDescriptionChange}/>
            </div>
          </ScheduleFormSection>

          <ScheduleFormSection title="Participants" description="Invite at least one person to create the room and calendar event.">
            <MeetingAttendeesSection label="Add attendees" inputId="schedule-attendees-input" inputValue={attendeeInput} selectedEmails={attendeeEmails} disabled={scheduleDisabled} emptyStateText="No attendees added yet." helperText="Press Enter or click Add after typing an email. Suggested teammates appear below." suggestionsLabel="Suggested participants" suggestions={attendeeSuggestions} onInputChange={onAttendeeInputChange} onInputKeyDown={onAttendeeKeyDown} onCommitInput={onCommitAttendeeInput} onRemoveEmail={onRemoveAttendee} onAddSuggestedEmail={onAddSuggestedAttendee}/>
          </ScheduleFormSection>
          ...
          <div className="flex flex-wrap items-center gap-3 border-t border-muted/40 pt-5">
            ...
            {footerAction}
            {submitDisabled && !scheduling ? (<p className="text-xs text-muted-foreground">Add a date, time, title, and at least one attendee to schedule.</p>) : null}
          </div>
        </form>
      </CardContent>
    </Card>);
