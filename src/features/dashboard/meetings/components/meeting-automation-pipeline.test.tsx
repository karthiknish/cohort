...
describe('meeting automation pipeline', () => {
    it('renders explicit in-progress transcript and notes steps', () => {
        const markup = renderToStaticMarkup(<MeetingAutomationPipeline captureListening={true} finalizingSession={true} hasTranscriptSaved={true} inRoom={true} notesProcessingState="processing" summaryReady={false} transcriptProcessingState="processing"/>);
    ...
    it('renders attention states when capture or post-call processing fails', () => {
        const markup = renderToStaticMarkup(<MeetingAutomationPipeline captureErrorPresent={true} captureListening={false} finalizingSession={false} hasTranscriptSaved={false} inRoom={false} notesProcessingState="failed" summaryReady={false} transcriptProcessingState="failed"/>);
