...
describe('meeting schedule card variants', () => {
    ...
    it('renders reschedule meeting copy explicitly', () => {
        const markup = renderToStaticMarkup(<MeetingScheduleCardFrame {...baseProps} cardTitle="Reschedule Meeting" cardDescription="Update the room." footerAction={createElement('button', { type: 'button' }, 'Cancel Edit')} submitLabel="Save Reschedule" submittingLabel="Saving…"/>);
    ...
    it('disables scheduling when no participant has been added', () => {
        const markup = renderToStaticMarkup(<CreateMeetingCard {...baseProps} attendeeEmails={[]} submitDisabled/>);
