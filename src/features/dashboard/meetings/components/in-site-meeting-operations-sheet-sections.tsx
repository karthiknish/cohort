...
export function MeetingOperationsSheetHeader({ joinConfig, meetingRoomName, }: {
    joinConfig: LiveKitJoinPayload | null;
    meetingRoomName: string;
}) {
    return (<SheetHeader className="border-b border-border">
...
export function MeetingOperationsCaptureCard({ canRecord, captureStatus, joinConfig, onEnableRecording, recordingEnabled, transcriptSource, }: {
    canRecord: boolean;
    ...
    return (<div className="space-y-3">
      <MeetingRecordingPromptCard canRecord={canRecord} captureError={captureStatus.error} captureSupported={captureStatus.supported} inRoom={Boolean(joinConfig)} recordingEnabled={recordingEnabled} onEnableRecording={onEnableRecording}/>
...
export function MeetingOperationsAttendeesCard({ meetingAttendeeEmails }: {
    ...
    return (<div className="rounded-2xl border border-border/80 bg-card p-4 shadow-sm">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {meetingAttendeeEmails.length > 0 ? (meetingAttendeeEmails.slice(0, 8).map((email) => (<Badge key={email} variant="outline" className="max-w-full break-all whitespace-normal text-left">
              {email}
            </Badge>))) : (<p className="text-sm text-muted-foreground">No attendees listed on this meeting yet.</p>)}
      </div>
    </div>);
}
export function MeetingOperationsAutomationCard({ autoSyncing, finalizingSession, joinConfig, markCompleted, notesProcessingState, retryingPostCallProcessing, transcriptProcessingState, }: {
    autoSyncing: boolean;
...
export function MeetingOperationsSyncCards({ notesModel, summaryStatus, transcriptLength, transcriptProcessingState, transcriptSource, transcriptStatus, transcriptTruncatedForNotes, }: {
...
export function MeetingOperationsAlerts({ captureError, notesProcessingError, notesReason, canGenerateNotes, canRetryPostCallProcessing, generatingNotes, retryingPostCallProcessing, onGenerateNotes, onRetryPostCallProcessing, transcriptProcessingError, }: {
    ...
    return (<>
      ...
      {transcriptProcessingError ? (<Alert className="border-destructive/40 bg-destructive/5">
          <AlertTitle>Transcript finalization failed</AlertTitle>
          <AlertDescription className="space-y-3">
            <p>{transcriptProcessingError}</p>
            {canRetryPostCallProcessing ? (<Button type="button" size="sm" variant="outline" onClick={onRetryPostCallProcessing} disabled={retryingPostCallProcessing}>
                {retryingPostCallProcessing ? 'Retrying post-call…' : 'Retry post-call processing'}
              </Button>) : null}
          </AlertDescription>
        </Alert>) : null}

      {notesReason === 'generation_failed' ? (<Alert className="border-destructive/40 bg-destructive/5">
          ...
          <AlertDescription className="space-y-3">
            ...
            {canGenerateNotes ? (<Button type="button" size="sm" variant="outline" onClick={onGenerateNotes} disabled={generatingNotes}>
                {generatingNotes ? 'Retrying AI notes…' : 'Retry AI notes'}
              </Button>) : null}
          </AlertDescription>
        </Alert>) : null}

      {notesProcessingError ? (<Alert className="border-destructive/40 bg-destructive/5">
...
export function MeetingOperationsSummaryCard({ canGenerateNotes, generatingNotes, legacyId, meetingTitle, notesProcessingState, notesStorageId, onGenerateNotes, postCallProcessingActive, summaryPreview, transcriptLength, transcriptStorageId, transcriptText, }: {
    canGenerateNotes: boolean;
    generatingNotes: boolean;
    legacyId: string;
    meetingTitle: string;
...
export function MeetingOperationsLiveCapturePreview({ interimTranscript }: {
