...
import { getPreviewGoogleWorkspaceStatus, getPreviewMeetingWorkspaceMembers, getPreviewMeetings, } from './preview-data';
describe('meetings preview data', () => {
    it('provides broad lifecycle and processing coverage for preview meetings', () => {
        ...
        expect(new Set(meetings.map((meeting) => meeting.status))).toEqual(new Set(['scheduled', 'in_progress', 'completed', 'cancelled']));
