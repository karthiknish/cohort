...
type UseInSiteMeetingRoomPostCallArgs = {
  ...
  lastAutoSyncedTranscriptRef: RefObject<string>
  finalizationInFlightRef: RefObject<boolean>
}

export function useInSiteMeetingRoomPostCall({
  meetingLegacyId,
  markCompleted,
  ...
  const submitTranscriptAction = useCallback(
    async (
      mode: TranscriptMode,
      overrides?: {
  ...
  const finalizeMeetingAfterRoomExit = useCallback(
    (closeAfterKickoff: boolean) => {
            ...
            onMeetingUpdated?.(
              buildMeetingSnapshot({
                ...
                notesProcessingError:
                  'AI notes could not be generated because post-call finalization failed.',
              }),
            )
          })
          .finally(() => {
    ...
    try {
      ...
      if (result.notesGenerated) {
        ...
        notifyFailure({
          ...
          message:
            'The request completed, but note generation failed. Try again after more transcript is captured.',
        })
      }
    } catch (error) {
      setNotesProcessingState('failed')
