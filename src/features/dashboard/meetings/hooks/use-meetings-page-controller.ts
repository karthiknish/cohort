...
export function useMeetingsPageController() {
    ...
    const meetings = useQuery(meetingsApi.list, workspaceId && !isPreviewMode
        ...
        : 'skip') as MeetingRecord[] | undefined;
    const googleWorkspaceStatus = useQuery(meetingIntegrationsApi.getGoogleWorkspaceStatus, workspaceId && !isPreviewMode
        ? { workspaceId }
        : 'skip') as {
        connected: boolean;
    ...
    const workspaceMembers = useQuery(usersApi.listWorkspaceMembers, workspaceId && !isPreviewMode
        ...
        : 'skip') as WorkspaceMember[] | undefined;
    const platformUsers = useQuery(usersApi.listAllUsers, workspaceId && !isPreviewMode
        ? { limit: 500 }
        : 'skip') as WorkspaceMember[] | undefined;
    ...
    const resolvedWorkspaceMembers = (isPreviewMode ? getPreviewMeetingWorkspaceMembers() : (workspaceMembers ?? []));
    const resolvedPlatformUsers = (isPreviewMode ? getPreviewMeetingWorkspaceMembers() : (platformUsers ?? []));
    ...
    const handleMeetingUpdated = (updatedMeeting: MeetingRecord) => {
    ...
    const resolveMeetingRecord = (meeting: MeetingRecord | null) => {
        if (!meeting)
            return null;
        ...
        const roomName = (typeof meeting.roomName === 'string' && meeting.roomName.trim().length > 0 ? meeting.roomName : null)
        ...
        if (normalizedTitle && startTimeMs && endTimeMs) {
            return pool.find((candidate) => (candidate.title === normalizedTitle && candidate.startTimeMs === startTimeMs && candidate.endTimeMs === endTimeMs)) ?? meeting;
        }
        return meeting;
    };
    ...
    const handleRescheduleMeeting = (meeting: MeetingRecord) => {
        ...
        setDurationMinutes(String(Math.max(15, Math.round((meeting.endTimeMs - meeting.startTimeMs) / 60000))));
    ...
    const handleScheduleMeeting = (event: {
        ...
        if (endTimeMs - startTimeMs < 10 * 60000 || endTimeMs - startTimeMs > 8 * 60 * 60 * 1000) {
