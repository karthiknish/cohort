...
export function useIntegrationStatusSummary(options: {
    clientIds: string[];
}) {
    ...
    const statusesByClient = useQuery(adsIntegrationsApi.listStatuses, isPreviewMode || !workspaceId || !user?.id || stableClientIds.length === 0
        ? 'skip'
        : {
            workspaceId,
            clientId: null,
