...
export function ComparisonInsights({ insights, loading }: ComparisonInsightsProps) {
    return (<div className="space-y-4">
      ...
      {!loading && insights.length === 0 && (<p className="text-sm text-muted-foreground">Select at least one workspace to see quick insights.</p>)}
      {!loading &&
            insights.map((insight) => {
                const Icon = insight.icon;
                return (<div key={insight.id} className={cn('rounded-lg border p-4 text-sm', insight.tone === 'positive' && 'border-success/20 bg-success/5', insight.tone === 'warning' && 'border-warning/20 bg-warning/10')}>
