...
export function MiniTaskKanban({ tasks, loading }: MiniTaskKanbanProps) {
    ...
    return (<Card className="shadow-sm">
      ...
      <CardContent>
        {loading ? (<div className="grid gap-3 md:grid-cols-2">
            <Skeleton className="h-28 w-full"/>
            <Skeleton className="h-28 w-full"/>
          </div>) : tasks.length === 0 ? (<div className="rounded-md border border-dashed border-muted/50 bg-muted/10 p-6 text-center text-sm text-muted-foreground">
            No tasks available. Create tasks to populate the board.
          </div>) : (<div className="grid gap-3 md:grid-cols-2">
            {columns.map(({ status, items }) => (<div key={status} className="flex flex-col gap-2 rounded-md border border-muted/50 bg-muted/10 p-3">
                <div className="flex items-center justify-between text-sm font-semibold text-foreground">
                  <span className="capitalize">{status.replace('-', ' ')}</span>
                  <Badge variant="outline" className="bg-background text-xs">{items.length}</Badge>
                </div>
                {items.length === 0 ? (<div className="rounded-md border border-dashed border-muted/40 bg-background px-3 py-4 text-center text-xs text-muted-foreground">
                    Empty lane
                  </div>) : (<div className="space-y-2">
                    {items.map((task) => (<Link key={task.id} href={`/dashboard/tasks?taskId=${task.id}`} className="block rounded-md border border-muted/40 bg-background p-2 text-sm shadow-sm transition hover:border-accent/40 hover:shadow">
