...
export function FeatureTips({ tips, storageKey, className }: FeatureTipsProps) {
    ...
    return (<Card className={cn('border-accent/20 bg-accent/5', className)}>
      <CardContent className="py-3 px-4">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                {tips.length > 1 && (<span className="text-xs text-muted-foreground">
                    {currentIndex + 1}/{tips.length}
                  </span>)}
              </div>
              <p className="text-sm text-muted-foreground">{currentTip.description}</p>
              
              {currentTip.action && (<Button variant="link" size="sm" className="h-auto p-0 text-primary" onClick={currentTip.action.onClick}>
          ...
          <div className="flex items-center gap-1">
            {tips.length > 1 && (<>
                <Button variant="ghost" size="icon" className="size-6" onClick={handlePrev} disabled={isFirst} aria-label="Previous tip">
                  <ChevronLeft className="size-4" aria-hidden/>
                </Button>
                <Button variant="ghost" size="icon" className="size-6" onClick={handleNext} disabled={isLast} aria-label="Next tip">
                  <ChevronRight className="size-4" aria-hidden/>
                </Button>
              </>)}
            <Button variant="ghost" size="icon" className="size-6 text-muted-foreground" onClick={handleDismiss} aria-label="Dismiss tips">
