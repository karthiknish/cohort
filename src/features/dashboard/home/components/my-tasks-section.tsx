...
const PRIORITY_STYLES: Record<string, string> = {
  urgent: 'border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-900/60 dark:bg-rose-950/40 dark:text-rose-300',
  high: 'border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-900/60 dark:bg-amber-950/40 dark:text-amber-300',
  medium: 'border-sky-200 bg-sky-50 text-sky-700 dark:border-sky-900/60 dark:bg-sky-950/40 dark:text-sky-300',
  low: 'border-slate-200 bg-slate-50 text-slate-600 dark:border-slate-800 dark:bg-slate-900/40 dark:text-slate-400',
}

const STATUS_STYLES: Record<string, string> = {
  'todo': 'border-slate-200 bg-slate-50 text-slate-600 dark:border-slate-800 dark:bg-slate-900/40 dark:text-slate-400',
    ...
    >
      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <Badge
            variant="outline"
            className={cn('rounded-full px-2 py-0 text-[10px] font-semibold uppercase tracking-wide', PRIORITY_STYLES[task.priority] ?? PRIORITY_STYLES.medium)}
          >
            {task.priority}
          </Badge>
          ...
          <Badge
            variant="outline"
            className={cn('rounded-full px-2 py-0 text-[10px] font-medium capitalize', STATUS_STYLES[task.status] ?? STATUS_STYLES.todo)}
          >
            {task.status.replace('-', ' ')}
          </Badge>
