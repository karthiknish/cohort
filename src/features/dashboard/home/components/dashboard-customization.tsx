...
export function DashboardCustomization({ availableWidgets = DEFAULT_WIDGETS, activeWidgets, onLayoutChange, onWidgetToggle, onWidgetCollapse, onReset, isEditing = false, onEditToggle, }: DashboardCustomizationProps) {
    ...
    const hiddenWidgets = availableWidgets.filter((w) => !activeWidgets.some((aw) => aw.id === w.id));
    ...
    const handleToggleVisibility = (widgetId: string, currentlyVisible: boolean) => {
        onWidgetToggle(widgetId, !currentlyVisible);
        const widget = availableWidgets.find((item) => item.id === widgetId);
        if (widget) {
            setLayoutAnnouncement(currentlyVisible
    ...
    const handleCollapse = (widgetId: string, currentlyCollapsed: boolean) => {
        onWidgetCollapse(widgetId, !currentlyCollapsed);
        const widget = activeWidgets.find((item) => item.id === widgetId);
        if (widget) {
            setLayoutAnnouncement(currentlyCollapsed
    ...
    return (<Card className={cn('border-dashed transition-colors', isEditing && 'border-primary')}>
      ...
      <CardHeader className="flex flex-row items-center justify-between py-3">
        ...
        <div className="flex items-center gap-1">
          <TooltipProvider delayDuration={200}>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button type="button" variant={isEditing ? 'default' : 'outline'} size="sm" onClick={onEditToggle}>
          ...
          {onReset && isEditing && (<Button type="button" variant="ghost" size="sm" onClick={handleReset}>
              Reset
            </Button>)}
        </div>
      </CardHeader>

      {isEditing && (<CardContent className="space-y-4">
          ...
          <ul className="list-none space-y-2" aria-describedby={instructionsId}>
            ...
            {visibleWidgets.map((widget, index) => (<DashboardWidgetRow key={widget.id} instructionsId={instructionsId} widget={widget} index={index} total={visibleWidgets.length} onMoveUp={handleMoveUp} onMoveDown={handleMoveDown} onCollapse={handleCollapse} onToggleVisibility={handleToggleVisibility}/>))}
          </ul>

          {hiddenWidgets.length > 0 && (<div className="space-y-2">
              <h3 className="text-xs font-medium text-muted-foreground uppercase">
                Hidden Widgets ({hiddenWidgets.length})
              </h3>
              {hiddenWidgets.map((widget) => (<HiddenDashboardWidgetRow key={widget.id} widget={widget} onToggleVisibility={handleToggleVisibility}/>))}
            </div>)}
        </CardContent>)}
    </Card>);
}
