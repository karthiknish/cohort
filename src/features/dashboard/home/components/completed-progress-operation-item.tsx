...
export function CompletedProgressOperationItem({ operation, onDismiss, }: {
    operation: Operation;
    onDismiss: () => void;
}) {
    useEffect(() => {
    ...
    return (<div className={cn('flex items-center gap-3 p-3 rounded-lg shadow-lg animate-in slide-in-from-top-2', config.className)}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {operation.description && (<p className="text-xs opacity-80 truncate">{operation.description}</p>)}
      </div>
      <Button variant="ghost" size="icon" className="size-7 shrink-0" onClick={onDismiss} aria-label={`Dismiss ${operation.name}`}>
