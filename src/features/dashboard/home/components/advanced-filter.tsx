...
type AdvancedFilterAction = {
    ...
    value: FilterMap | ((prev: FilterMap) => FilterMap);
} | {
    type: 'setFilterName';
    value: string;
} | {
...
function advancedFilterReducer(state: AdvancedFilterState, action: AdvancedFilterAction): AdvancedFilterState {
    switch (action.type) {
        ...
        case 'setCurrentFilters':
            return {
                ...state,
                currentFilters: typeof action.value === 'function' ? action.value(state.currentFilters) : action.value,
            };
        case 'setFilterName':
...
export function AdvancedFilter({ availableFilters = EMPTY_AVAILABLE_FILTERS, onFilterChange, onSortChange, activeFiltersCount = 0, savedFilters = EMPTY_SAVED_FILTERS, onSaveFilter, onDeleteFilter, className, }: AdvancedFilterProps) {
    ...
    const handleLoadSavedFilter = (config: FilterConfig) => {
        dispatch({
            type: 'loadSavedFilter',
    ...
    const filterFooter = (<AdvancedFilterFooter currentFilters={currentFilters} filterName={filterName} onSaveFilter={onSaveFilter} clearAllFilters={clearAllFilters} handleFilterNameChange={handleFilterNameChange} saveCurrentConfig={saveCurrentConfig} applyFilters={applyFilters}/>);
    return (<div className={cn('flex items-center gap-2', className)}>
      ...
      <ResponsiveOverlay open={open} onOpenChange={handleOpenChange} title="Advanced Filters" description="Configure multiple filters to find exactly what you need." dialogClassName="sm:max-w-md" mobileClassName="max-h-[88dvh]" footer={filterFooter}>
          <div className="space-y-4">
            {/* Saved filters */}
            {savedFilters.length > 0 && (<div className="space-y-2">
                <Label>Saved Filters</Label>
                <div className="flex flex-wrap gap-2">
                  {savedFilters.map((filter) => (<SavedFilterButton key={filter.id} filter={filter} onApply={handleLoadSavedFilter}>
                      {filter.name}
                    </SavedFilterButton>))}
                </div>
                {onDeleteFilter && (<div className="flex gap-2">
                    {savedFilters.map((filter) => (<SavedFilterDeleteButton key={filter.id} filterId={filter.id} onDelete={handleDeleteSavedFilter}/>))}
                  </div>)}
              </div>)}

            {/* Filter fields */}
            {availableFilters.map((filter) => (<AdvancedFilterField key={filter.key} filter={filter} value={currentFilters[filter.key]} onChange={applyFilter}/>))}

            {/* Sort options */}
            {onSortChange && (<div className="space-y-2">
                <Label>Sort By</Label>
 ...
 */
export function ActiveFiltersBar({ filters, onRemove, onClearAll, className, }: {
    filters: FilterMap;
    ...
    return (<div className={cn('flex items-center gap-2 flex-wrap', className)}>
      ...
      {entries.map(([key, value]) => (<ActiveFilterBadge key={key} filterKey={key} value={value} onRemove={onRemove}/>))}
...
function AdvancedFilterFooter({ currentFilters, filterName, onSaveFilter, clearAllFilters, handleFilterNameChange, saveCurrentConfig, applyFilters, }: AdvancedFilterFooterProps) {
    return (<div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <Button type="button" variant="ghost" size="sm" onClick={clearAllFilters} disabled={Object.keys(currentFilters).length === 0}>
        Clear All
      </Button>

      <div className="flex flex-wrap items-center justify-end gap-2">
        {onSaveFilter ? (<div className="flex flex-wrap gap-2">
            <Input placeholder="Filter name…" value={filterName} onChange={handleFilterNameChange} className="w-40"/>
...
function SavedFilterButton({ filter, onApply, children }: SavedFilterButtonProps) {
    const onApplySavedFilter = () => {
        onApply(filter);
    };
    return (<Button type="button" variant="outline" size="sm" className="h-6 rounded-full px-2 text-xs" onClick={onApplySavedFilter}>
      {children}
    </Button>);
}
...
function SavedFilterDeleteButton({ filterId, onDelete }: SavedFilterDeleteButtonProps) {
    const onDeleteSavedFilter = () => {
        onDelete(filterId);
    };
    return (<Button type="button" variant="ghost" size="sm" onClick={onDeleteSavedFilter}>
      <X className="size-3"/>
    </Button>);
}
...
function AdvancedFilterField({ filter, value, onChange }: AdvancedFilterFieldProps) {
    ...
    return (<div className="space-y-2">
      ...
      {filter.type === 'text' && (<Input value={getStringFilterValue(value)} onChange={handleTextChange} placeholder={`Search ${filter.label.toLowerCase()}...`}/>)}

      {filter.type === 'select' && (<Select value={getStringFilterValue(value)} onValueChange={handleSelectChange}>
          <SelectTrigger>
      ...
      {filter.type === 'multiselect' && (<div className="space-y-1">
          {filter.options?.map((option) => (<FilterMultiSelectOption key={option.value} filterKey={filter.key} option={option} checked={getStringArrayFilterValue(value).includes(option.value)} onChange={handleMultiSelectChange}/>))}
        </div>)}
    </div>);
}
...
function ActiveFilterBadge({ filterKey, value, onRemove }: ActiveFilterBadgeProps) {
    ...
    return (<Badge variant="secondary" className="gap-1 pr-1">
      ...
      <button type="button" onClick={handleRemove} className="rounded p-0.5 hover:text-destructive focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label={`Remove ${filterKey} filter`}>
        <X className="size-3"/>
      </button>
    </Badge>);
}
