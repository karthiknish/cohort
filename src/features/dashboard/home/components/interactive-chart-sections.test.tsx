...
import { ChartTooltipContent, InteractiveChartEmptyState, InteractiveChartHeader, PieTooltipContent, } from './interactive-chart-sections';
describe('interactive chart sections', () => {
    it('renders header controls and empty state', () => {
        const markup = renderToStaticMarkup(<>
        <InteractiveChartHeader chartType="bar" description="Revenue over time" handleExport={vi.fn()} isRefreshing={true} onRefresh={vi.fn()} setChartType={vi.fn()} setTimeRange={vi.fn()} showExport={true} showRefresh={true} timeRange="30d" title="Revenue"/>
    ...
    it('renders tooltip content blocks', () => {
        const chartMarkup = renderToStaticMarkup(<ChartTooltipContent active={true} payload={[{ payload: { date: '2026-03-11', value: 42, category: 'Paid' } }]} xAxisKey="date" dataKey="value" valueFormatter={formatDollarValue}/>);
        const pieMarkup = renderToStaticMarkup(<PieTooltipContent active={true} payload={[{ name: 'Paid', value: 42 }]} valueFormatter={formatDollarValue}/>);
