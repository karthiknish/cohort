...
const SEARCH_TYPE_ICONS: Record<SearchResultType, ComponentType<{
    className?: string;
}>> = {
    tasks: FileText,
    projects: FolderOpen,
...
function SearchResultRelativeTime({ createdAt }: {
    createdAt: string;
}) {
...
export function GlobalSearch({ trigger, searchFunctions = EMPTY_SEARCH_FUNCTIONS, onResultClick, shortcut = 'Cmd+K', }: GlobalSearchProps) {
    ...
    const runSearch = useEffectEvent(() => {
        ...
        const searchOneType = async ([type, fn]: [
            string,
            typeof searchFunctions[keyof typeof searchFunctions]
        ]) => {
    ...
    const handleSelectType = (type?: SearchResultType) => {
    ...
    const defaultTrigger = (<DialogTrigger asChild>
    ...
    return (<Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-2xl max-h-[80vh] flex flex-col p-0">
        ...
        <div className="flex items-center gap-2 p-4 border-b">
          ...
          <Input id="global-search-dialog-query" value={query} onChange={handleQueryChange} placeholder="Search across all items…" className="flex-1" onKeyDown={handleSearchInputKeyDown} aria-label="Search across workspace"/>
          {query && (<Button type="button" variant="ghost" size="icon" onClick={handleClearQuery} aria-label="Clear search">
              <X className="size-4" aria-hidden/>
            </Button>)}
        </div>

        {/* Type filters */}
        {resultTypes.length > 0 && (<div className="flex items-center gap-2 px-4 py-2 border-b">
            <SearchTypeButton label={`All (${results.length})`} selected={selectedType === 'all'} onSelect={handleSelectAllTypes}/>
            {resultTypes.map((type) => (<SearchTypeButton key={type} label={`${SEARCH_TYPE_LABELS[type]} (${groupedResults[type]?.length || 0})`} selected={selectedType === type} onSelect={handleSelectType} typeValue={type}/>))}
          </div>)}

        {/* Results */}
        <div className="flex-1 overflow-y-auto p-2">
          {isSearching && (<div className="flex items-center justify-center py-12 text-muted-foreground">
              <div className="size-5 animate-spin rounded-full border-2 border-primary border-t-transparent"/>
              <span className="ml-2">Searching…</span>
            </div>)}

          {!isSearching && query.trim().length < 2 && (<div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          {!isSearching && query.trim().length >= 2 && results.length === 0 && (<div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          {!isSearching && results.length > 0 && (<div className="space-y-4">
              {Object.entries(groupedResults).flatMap(([type, items]) => {
                ...
                return [(<div key={type} className="space-y-1">
                    ...
                    {items.map((result) => (<SearchResultItem key={result.id} result={result} onSelect={handleResultClick}/>))}
                  </div>)];
            })}
            </div>)}
        </div>
...
function SearchTypeButton({ label, selected, onSelect, typeValue }: SearchTypeButtonProps) {
    const onSelectSearchType = () => {
        onSelect(typeValue);
    };
    return (<button type="button" onClick={onSelectSearchType} className={cn('px-2 py-1 rounded-md text-xs font-medium transition-colors', selected ? 'bg-primary text-primary-foreground' : 'hover:bg-muted')}>
      {label}
    </button>);
}
function SearchResultItem({ result, onSelect }: SearchResultItemProps) {
    ...
    return (<button type="button" onClick={onSelectSearchResult} className="w-full text-left p-3 rounded-lg hover:bg-muted transition-colors">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {result.description && (<p className="text-xs text-muted-foreground line-clamp-1 mt-0.5">
              {result.description}
            </p>)}
          <div className="flex items-center gap-3 mt-1">
            {result.metadata?.status && (<Badge variant="outline" className="text-xs">
                {result.metadata.status}
              </Badge>)}
            {result.metadata?.priority && (<span className="text-xs text-muted-foreground">
                {result.metadata.priority}
              </span>)}
            {result.metadata?.createdAt ? <SearchResultRelativeTime createdAt={result.metadata.createdAt}/> : null}
            {result.metadata?.clientName && (<span className="text-xs text-muted-foreground">
 ...
 */
export function QuickSearchInput({ onSearch, placeholder = 'Search...', className, }: {
    onSearch: (query: string) => void;
    placeholder?: string;
    className?: string;
}) {
    const [value, setValue] = useState('');
    const onQuickSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    return (<form onSubmit={handleSubmit} className={cn('relative', className)}>
      ...
      <Input placeholder={placeholder} value={value} onChange={onQuickSearchChange} className="pl-9" aria-label={placeholder}/>
      {value && (<button type="button" onClick={handleClear} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground" aria-label="Clear search">
          <X className="size-4" aria-hidden/>
        </button>)}
    </form>);
}
