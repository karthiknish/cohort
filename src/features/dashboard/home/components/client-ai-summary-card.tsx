...
function ClientAiSummaryCardBody({ selectedClient, summarySnapshot, summarySnapshotHash, waitingForData, manualRefreshToken, }: {
    ...
    const hasCurrentSummary = Boolean(state.summary && summarySnapshotHash && state.persistedSnapshotHash === summarySnapshotHash);
    const effectiveStatus: SummaryStatus = state.summaryStatus === 'loading' || state.summaryStatus === 'error'
        ? state.summaryStatus
        : hasCurrentSummary
            ? 'ready'
            : 'idle';
    const generateSummary = useEffectEvent(() => {
    ...
    }, [effectiveStatus, summarySnapshot, waitingForData]);
    useEffect(() => {
    ...
    }, [manualRefreshToken, summarySnapshot, waitingForData]);
    return (waitingForData && !state.summary ? (<div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {summarySnapshot ? (<Badge variant="outline">
          ...
          <div className="rounded-xl border border-accent/10 bg-background/80 p-4">
            ...
            <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
              {state.summary.bullets.map((bullet) => (<li key={bullet} className="flex gap-2">
          ...
          {effectiveStatus === 'error' && state.summaryError ? (<p className="text-xs text-destructive">Refresh failed: {state.summaryError}</p>) : null}
        </>) : effectiveStatus === 'error' ? (<div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
          {state.summaryError || 'Unable to generate a summary right now.'}
        </div>) : (<div className="rounded-lg border border-dashed border-muted/60 bg-muted/10 p-4 text-sm text-muted-foreground">
          Summary will appear once the dashboard snapshot is ready.
        </div>));
}
export function ClientAiSummaryCard({ selectedClient, metrics, metricsLoading, taskSummary, tasksLoading, proposals, proposalsLoading, integrationSummary, integrationsLoading, lastRefreshed, }: ClientAiSummaryCardProps) {
    ...
    const summarySnapshotHash = (summarySnapshot ? buildClientSummarySnapshotHash(summarySnapshot) : null);
    const handleManualRefresh = () => {
        setManualRefreshToken((token) => token + 1);
    };
    return (<Card className="border-accent/15 bg-gradient-to-br from-background via-background to-primary/5 shadow-sm">
      <CardHeader className="flex flex-row items-start justify-between gap-3">
        ...
        <Button type="button" variant="outline" size="sm" onClick={handleManualRefresh} disabled={!summarySnapshot || waitingForData}>
      ...
      <CardContent className="space-y-3">
        {!selectedClient ? (<div className="rounded-lg border border-dashed border-muted/60 bg-muted/10 p-4 text-sm text-muted-foreground">
            Select a client to generate a concise AI summary from the current dashboard snapshot.
          </div>) : (<ClientAiSummaryCardBody key={selectedClient.id} selectedClient={selectedClient} summarySnapshot={summarySnapshot} summarySnapshotHash={summarySnapshotHash} waitingForData={waitingForData} manualRefreshToken={manualRefreshToken}/>)}
      </CardContent>
    </Card>);
}
