...
import { PROVIDER_LABELS } from '@/features/dashboard/analytics/hooks/types';
...
export function PlatformComparisonSummaryCard({ metrics, isLoading }: Props) {
    ...
    const totalSpend = summaries.reduce((acc, s) => acc + s.spend, 0);
    return (<Card className="shadow-sm">
      <CardHeader>
        <div className="flex items-start justify-between gap-4">
          ...
          {summaries.length > 0 && (<Badge variant="secondary" className="text-[10px] uppercase font-bold tracking-wide">
      ...
      <CardContent>
        {isLoading ? (<div className="space-y-2">
            ...
            <Skeleton className="h-6 w-full"/>
            <Skeleton className="h-6 w-full"/>
          </div>) : summaries.length === 0 ? (<p className="text-sm text-muted-foreground">
          ...
          </p>) : (<div className="overflow-hidden rounded-lg border">
            ...
            {summaries.slice(0, 5).map((s) => {
                ...
                return (<Link key={s.providerId} href={href} className="block focus:outline-none focus:ring-2 focus:ring-primary/30">
        ...
        {!isLoading && summaries.length > 0 && summaries.length < 2 && (<p className="mt-3 text-xs text-muted-foreground">Connect multiple platforms to see a comparison.</p>)}
      </CardContent>
    </Card>);
}
