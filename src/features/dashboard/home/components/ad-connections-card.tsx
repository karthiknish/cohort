...
import { ConnectionDialog, DisconnectDialog, type ConnectionStep } from '@/features/dashboard/ads/components/connection-dialog';
...
interface AdConnectionsCardProps {
    ...
    onDisconnect: (providerId: string, options?: {
...
function needsAccountSetup(statusInfo: IntegrationStatusInfo | undefined, isConnected: boolean): boolean {
...
function getStatusBadgeVariant(status: string | undefined, isConnected: boolean, stale?: boolean, setupRequired?: boolean): 'default' | 'secondary' | 'destructive' | 'outline' {
    if (!isConnected)
        return 'outline';
    if (setupRequired)
        return 'outline';
...
function getStatusLabel(status: string | undefined, isConnected: boolean, stale?: boolean, setupRequired?: boolean): string {
    if (!isConnected)
    ...
    if (status === 'error')
        return 'Sync failed';
    if (status === 'pending')
        return stale ? 'Sync stalled' : 'Syncing...';
    if (status === 'never')
        return 'Finish setup';
    return 'Connected';
}
...
function adConnectionsDialogReducer(state: AdConnectionsDialogState, action: AdConnectionsDialogAction): AdConnectionsDialogState {
    switch (action.type) {
...
// MAIN COMPONENT
// =============================================================================
export function AdConnectionsCard({ providers, connectedProviders, connectingProvider, connectionErrors, integrationStatuses = EMPTY_INTEGRATION_STATUSES, onConnect, onDisconnect, onOauthRedirect, onSyncNow, onRefresh, refreshing, syncingProviders = EMPTY_SYNCING_PROVIDERS, connectedCount, totalProviders, pendingSetupCount = 0, }: AdConnectionsCardProps) {
    ...
    const handleOpenConnectDialog = (provider: ProviderConfig) => {
        dispatch({ type: 'openConnectDialog', provider });
    };
    const handleDialogConnect = (): Promise<void> => {
        if (!selectedProvider)
    ...
    const handleOpenDisconnectDialog = (provider: ProviderConfig) => {
    ...
    const handleConnectDialogOpenChange = (value: boolean) => {
        dispatch({ type: 'setConnectDialogOpen', value });
    };
    const handleDisconnectDialogOpenChange = (value: boolean) => {
        dispatch({ type: 'setDisconnectDialogOpen', value });
    };
    // Handle quick reconnect (no dialog)
    const handleQuickReconnect = (provider: ProviderConfig) => {
    ...
    const handleSyncNow = (provider: ProviderConfig) => {
    ...
    return (<>
      <MotionCard className={ADS_PAGE_THEME.surfaceCard}>
        <CardHeader className="flex flex-col gap-4 border-b border-border/50 pb-5 md:flex-row md:items-start md:justify-between">
          ...
          <Button type="button" variant="outline" size="sm" onClick={onRefresh} disabled={refreshing} className="inline-flex h-10 items-center gap-2 rounded-xl border-border/70">
            <RefreshCw className={cn('size-4', refreshing && 'animate-spin')}/>
            Refresh
          </Button>
        </CardHeader>
        <CardContent className="space-y-6 pt-6">
          {typeof connectedCount === 'number' && typeof totalProviders === 'number' && totalProviders > 0 ? (<div className="rounded-2xl border border-primary/15 bg-primary/[0.04] p-4 ring-1 ring-primary/10">
              <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-2 text-sm font-medium text-foreground">
                  ...
                  {pendingSetupCount > 0 ? (<Badge variant="secondary" className="font-normal">
                      {pendingSetupCount} need setup
                    </Badge>) : connectedCount < totalProviders ? (<Badge variant="outline" className="font-normal">
              ...
              <Progress value={Math.round((connectedCount / totalProviders) * 100)} className="h-2" aria-label="Connected ad platforms"/>
            </div>) : null}

          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {providerStates.map((state) => (<ProviderCard key={state.provider.id} state={state} onConnect={handleOpenConnectDialog} onReconnect={handleQuickReconnect} onDisconnect={handleOpenDisconnectDialog} onSyncNow={handleSyncNow}/>))}
          </div>
      ...
      {/* Connection Dialog */}
      <ConnectionDialog open={connectDialogOpen} onOpenChange={handleConnectDialogOpenChange} providerId={selectedProvider?.id ?? null} providerIcon={selectedProvider?.icon} onConnect={handleDialogConnect} isConnecting={connectingProvider === selectedProvider?.id} connectionStep={connectionStep} error={selectedProvider ? (connectionErrors[selectedProvider.id] ?? null) : null} onRetry={handleRetry}/>

      {/* Disconnect Confirmation Dialog */}
      <DisconnectDialog open={disconnectDialogOpen} onOpenChange={handleDisconnectDialogOpenChange} providerName={selectedProvider?.name ?? ''} onConfirm={handleConfirmDisconnect} isDisconnecting={isDisconnecting}/>
    </>);
}
// =============================================================================
...
const ProviderCard = function ProviderCard({ state, onConnect, onReconnect, onDisconnect, onSyncNow, }: {
    state: ProviderState;
    ...
    const accountLabel = typeof statusInfo?.accountName === 'string' && statusInfo.accountName.length > 0
    ...
    return (<Card className={cn(ADS_PAGE_THEME.providerTile, 'motion-chromatic', isConnected && (theme?.border || 'border-primary/25 ring-1 ring-primary/10'), error && 'border-destructive/40', !isConnected && 'opacity-95 hover:border-border hover:opacity-100')}>
      {/* Status indicator bar */}
      {isConnected && (<div className={cn('absolute left-0 top-0 h-1 w-full', setupRequired && 'bg-primary', (statusInfo?.status === 'error' || stale) && 'bg-destructive', statusInfo?.status === 'pending' && !stale && !setupRequired && 'bg-warning', statusInfo?.status !== 'error' && statusInfo?.status !== 'pending' && !stale && !setupRequired && (theme?.indicator || 'bg-primary'))}/>)}

      <CardHeader className="space-y-3 pb-3">
        <div className="flex items-start justify-between">
          <span className={cn('flex size-12 items-center justify-center rounded-xl motion-chromatic shadow-sm', theme?.bg || (isConnected ? 'bg-accent/10' : 'bg-muted'), theme?.color || (isConnected ? 'text-primary' : 'text-muted-foreground'))}>
            <Icon className="size-6"/>
          </span>
          {isConnecting && (<Loader2 className="size-4 animate-spin text-muted-foreground"/>)}
        </div>
        <div>
      ...
      <CardContent className="space-y-3 pt-0">
        ...
        <div className="flex items-center justify-between">
          <Badge variant={statusVariant} className="rounded-full text-xs">
            {statusInfo?.status === 'pending' && !stale && !setupRequired && (<Loader2 className="mr-1 size-3 animate-spin"/>)}
            {(statusInfo?.status === 'error' || stale) && (<AlertTriangle className="mr-1 size-3"/>)}
            {isConnected && statusInfo?.status !== 'error' && statusInfo?.status !== 'pending' && !stale && !setupRequired && (<Check className="mr-1 size-3"/>)}
            {statusLabel}
          </Badge>
          {isConnected && (<TooltipProvider>
              <Tooltip>
        ...
        {isConnected && accountLabel && (<div className="text-xs text-muted-foreground">
            <span className="font-medium text-foreground/80">Account:</span>{' '}
            <span className="truncate">{accountLabel}</span>
          </div>)}

        {/* Sync error message from status */}
        {statusInfo?.status === 'error' && !error && (<Alert variant="destructive" className="py-2">
            <AlertTriangle className="size-3"/>
        ...
        {/* Stale sync alert */}
        {stale && (<Alert variant="destructive" className="py-2">
            <AlertTriangle className="size-3"/>
            <AlertDescription className="text-xs">
        ...
        <div className="flex gap-2">
          {!isConnected ? (<Button type="button" className="flex-1" size="sm" disabled={isConnecting} onClick={handleConnectClick}>
              {isConnecting ? (<>
                  <Loader2 className="mr-2 size-3 animate-spin"/>
                  Connecting…
                </>) : ('Connect')}
            </Button>) : (<>
              {(stale || statusInfo?.status === 'error') ? (<Button type="button" variant="secondary" size="sm" className="flex-1" disabled={isSyncingNow} onClick={handleSyncNowClick}>
                  {isSyncingNow ? (<>
                      <Loader2 className="mr-2 size-3 animate-spin"/>
                      Syncing…
                    </>) : ('Sync now')}
                </Button>) : (<Button type="button" variant="outline" size="sm" className="flex-1" disabled={isConnecting} onClick={handleReconnectClick}>
                  {isConnecting ? (<>
                      <Loader2 className="mr-2 size-3 animate-spin"/>
                      Connecting…
                    </>) : ('Reconnect')}
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button type="button" variant="ghost" size="icon" className="size-8 text-muted-foreground hover:text-destructive" disabled={isConnecting} onClick={handleDisconnectClick}>
