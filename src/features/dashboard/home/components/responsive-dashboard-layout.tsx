 ...
 */
export function ResponsiveGrid({ children, mobile = 1, tablet = 2, desktop = 3, className, gap = 'md', }: ResponsiveGridProps) {
    const gaps = {
        sm: 'gap-2 sm:gap-3',
        md: 'gap-4 sm:gap-5',
        lg: 'gap-6 sm:gap-8',
    ...
    const finalColClass = cn(mobile === 1 ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2', tablet === 1 ? 'sm:grid-cols-1' : tablet === 2 ? 'sm:grid-cols-2' : 'sm:grid-cols-3', desktop === 1 ? 'lg:grid-cols-1' : desktop === 2 ? 'lg:grid-cols-2' : desktop === 3 ? 'lg:grid-cols-3' : 'lg:grid-cols-4');
    return (<div className={cn('grid', gaps[gap], finalColClass, className)}>
      {children}
    </div>);
}
 ...
 */
export function ResponsiveSplit({ children, reverse = false, ratio = '2/1', className, }: ResponsiveSplitProps) {
    ...
    return (<div className={cn('grid gap-4', splitClasses[ratio], reverse && 'lg:flex lg:flex-row-reverse', className)}>
...
export function HideOnScroll({ children, className }: HideOnScrollProps) {
    return (<div className={cn('sticky top-0 z-40 transition-transform duration-[var(--motion-duration-normal)] ease-[var(--motion-ease-out)] motion-reduce:transition-none', className)}>
      {children}
    </div>);
}
 ...
 */
export function MobileFullWidth({ children, className, breakpoint = 'md', }: MobileFullWidthProps) {
    const breakpointClass = {
        sm: 'sm:mx-0 sm:max-w-none',
        md: 'md:mx-0 md:max-w-none',
        lg: 'lg:mx-0 lg:max-w-none',
    };
    return (<div className={cn('-mx-3 sm:-mx-4 w-[calc(100%+1.5rem)] sm:w-[calc(100%+2rem)]', breakpointClass[breakpoint], className)}>
      {children}
    </div>);
}
...
export function ResponsiveText({ children, className, size = 'base', }: ResponsiveTextProps) {
    ...
    return (<p className={cn(sizeClasses[size], className)}>
      {children}
    </p>);
}
...
export function MobileHidden({ children, className }: MobileHiddenProps) {
    return (<div className={cn('hidden md:block', className)}>
      {children}
    </div>);
}
...
export function DesktopHidden({ children, className }: DesktopHiddenProps) {
    return (<div className={cn('block md:hidden', className)}>
      {children}
    </div>);
}
 ...
 */
export function TouchOptimized({ children, className, minTouchSize = 44, }: TouchOptimizedProps) {
    const touchStyle = ({ minHeight: minTouchSize, minWidth: minTouchSize });
    return (<div className={cn('inline-block', className)} style={touchStyle}>
      {children}
    </div>);
}
 ...
 */
export function SafeAreaInset({ children, position = 'top', className, }: SafeAreaInsetProps) {
    const insets = {
    ...
    return (<div className={cn(insets[position], className)}>
      {children}
    </div>);
}
 ...
 */
export function MobileScrollContainer({ children, className, horizontal = false, hideScrollbar = true, }: MobileScrollContainerProps) {
    return (<div className={cn('overflow-auto', horizontal ? 'flex flex-row snap-x snap-mandatory' : 'flex flex-col', hideScrollbar && [
            'scrollbar-hide',
