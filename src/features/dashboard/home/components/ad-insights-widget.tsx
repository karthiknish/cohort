...
import { calculateAlgorithmicInsights, calculateEfficiencyScore, getGlobalBudgetSuggestions, enrichSummaryWithMetrics, type AdMetricsSummary, type AlgorithmicInsight, } from '@/lib/ad-algorithms';
// =============================================================================
// TYPES
// =============================================================================
...
// HELPER FUNCTIONS
// =============================================================================
function buildAdMetricsSummary(providerId: string, summary: {
    spend: number;
    impressions: number;
    clicks: number;
    conversions: number;
...
function CompactInsightItem({ insight }: {
    ...
    return (<div className="flex items-start gap-2 py-2">
      <div className={cn('mt-0.5 rounded-full p-1', levelStyles[insight.level])}>
...
function EfficiencyScoreMini({ score }: {
    score: number;
}) {
    const circumference = 2 * Math.PI * 16;
    const offset = circumference * (1 - score / 100);
    return (<div className="relative flex size-12 items-center justify-center">
      <svg className="size-full" viewBox="0 0 40 40">
        <circle className="stroke-muted/20" strokeWidth="3" fill="transparent" r="16" cx="20" cy="20"/>
        <circle className={cn('motion-chromatic-xslow', score > 70 ? 'stroke-success' : score > 40 ? 'stroke-warning' : 'stroke-destructive')} strokeWidth="3" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" fill="transparent" r="16" cx="20" cy="20" transform="rotate(-90 20 20)"/>
      </svg>
...
// MAIN COMPONENT
// =============================================================================
export function AdInsightsWidget({ providerSummaries, loading = false, showEmpty = false, }: AdInsightsWidgetProps) {
    // Calculate insights from provider summaries
    const { insights, globalEfficiencyScore, hasCriticalInsights, actionCount } = (() => {
        ...
        const weightedScore = totalSpend > 0
            ? Math.round(enrichedSummaries.reduce((sum, s, i) => sum + (providerScores[i] ?? 0) * s.totalSpend, 0) / totalSpend)
            : 0;
    ...
    return (<Card className="shadow-sm">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2 text-base">
            <Lightbulb className="size-4 text-warning"/>
            Ad Insights
            {actionCount > 0 && (<Badge variant="outline" className={cn('text-xs', hasCriticalInsights
      ...
      <CardContent className="space-y-1 pt-0">
        {insights.length === 0 ? (<div className="flex items-center gap-2 py-2 text-sm text-muted-foreground">
            <CircleCheck className="size-4 text-success"/>
            <span>All campaigns performing well</span>
          </div>) : (<>
            <div className="divide-y">
              {insights.map((insight) => (<CompactInsightItem key={`${insight.level}-${insight.title}-${insight.message}`} insight={insight}/>))}
            </div>
