...
const EMPTY_STATE_COPY: Record<PerformanceChartDataSource, {
    message: string;
    primaryHref: string;
    primaryLabel: string;
    detailHref: string;
...
export const PerformanceChart = function PerformanceChart({ metrics, loading, currency = 'USD', dataSource = 'analytics', hasAggregateData = false, adsAccountConnected = false, showDetailLink = true, hideHeader = false, metricsDisplayState, }: PerformanceChartProps) {
    const copy = dataSource === 'ads'
    ...
    const chartMargin = ({ top: 12, right: 12, left: 0, bottom: 28 });
    const chartTickStyle = ({ fontSize: 12 });
    const activeDot = ({ r: 5, strokeWidth: 0 });
    ...
    if (chartData.length === 0) {
        return (<div className="flex h-full flex-col gap-3">
        {!hideHeader ? (<div className="flex items-center justify-between gap-2">
    ...
    return (<div className="flex h-full flex-col gap-1">
      {!hideHeader ? (<div className="flex items-center justify-between px-1">
          ...
          {showDetailLink ? (detailLinkIsHash ? (<Button asChild variant="outline" size="sm">
