 ...
 */
export function TeamActivityWidget({ data, onMemberClick, onViewAllClick, className, compact = false, }: TeamActivityWidgetProps) {
    // Calculate stats
    const stats = (() => {
    ...
    const sortedMembers = data.members.toSorted((a, b) => {
    ...
    return (<Card className={cn('shadow-sm', className)}>
      <CardHeader className="flex flex-row items-center justify-between py-3">
        ...
        {onViewAllClick && (<Button variant="ghost" size="sm" onClick={onViewAllClick}>
            View All
          </Button>)}
      </CardHeader>
      ...
      <CardContent className="space-y-4 pt-0">
        ...
        {/* Top performer badge */}
        {!compact && stats.topPerformer && (<div className="flex items-center gap-2 rounded-lg bg-warning/10 p-2">
        ...
        <div className="space-y-2">
          {displayMembers.map((member) => (<TeamMemberItem key={member.id} member={member} onClick={onMemberClick}/>))}

          {compact && sortedMembers.length > 3 && (<Button variant="link" size="sm" className="text-xs p-0 h-auto" onClick={onViewAllClick}>
              +{sortedMembers.length - 3} more
            </Button>)}
        </div>

        {/* Activity summary */}
        {!compact && (<div className="pt-3 border-t">
...
function TeamMemberItem({ member, onClick }: TeamMemberItemProps) {
    ...
    return (<button type="button" onClick={onSelectTeamMember} className="w-full text-left flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors group">
      {/* Status indicator */}
      <div className={cn('size-2.5 rounded-full', 'flex-shrink-0', statusColors[member.status])}/>

      {/* Avatar */}
      <Avatar className="size-8">
        {member.avatarUrl ? (<AvatarImage src={member.avatarUrl} alt={member.name}/>) : (<AvatarFallback className="text-xs">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          {(member.tasksInProgress || 0) > 0 && (<span>{member.tasksInProgress} in progress</span>)}
          {member.hoursLogged && member.hoursLogged > 0 && (<span>{member.hoursLogged}h logged</span>)}
        </div>
      </div>

      {/* Tasks progress */}
      {(member.tasksCompleted || 0) > 0 && (<div className="text-xs text-muted-foreground">
 ...
 */
export function TeamPresenceStrip({ members, maxVisible = 6, onClick, className, }: {
    members: TeamMember[];
    maxVisible?: number;
    onClick?: () => void;
    className?: string;
    ...
    return (<Wrapper {...(onClick
        ...
        : {})} className={cn('flex items-center gap-1', onClick && 'cursor-pointer hover:opacity-80 transition-opacity focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2', className)}>
      {displayMembers.map((member) => (<Avatar key={member.id} className="size-7 border-2 border-background" title={`${member.name} - ${member.status}`}>
          {member.avatarUrl ? (<AvatarImage src={member.avatarUrl} alt={member.name}/>) : (<AvatarFallback className="text-[10px]">
              {member.name.charAt(0).toUpperCase()}
            </AvatarFallback>)}
        </Avatar>))}
      {remainingCount > 0 && (<div className="size-7 rounded-full border-2 border-background flex items-center justify-center bg-muted text-[10px] font-medium">
          +{remainingCount}
        </div>)}
    </Wrapper>);
}
 ...
 */
export function WorkloadDistribution({ members, className, }: {
    members: TeamMember[];
    className?: string;
}) {
