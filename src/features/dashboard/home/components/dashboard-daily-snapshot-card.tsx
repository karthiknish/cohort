...
export function DashboardDailySnapshotCard({ openTasks, pendingProposals, activeProjects, loading, }: DashboardDailySnapshotCardProps) {
    const values = {
        tasks: openTasks,
        projects: activeProjects,
        proposals: pendingProposals,
    };
    return (<Card id="tour-stats-cards" className={cn('overflow-hidden ring-1 ring-muted/20', DASHBOARD_THEME.cards.base)}>
      ...
      <CardContent className="pt-6">
        <FadeInStagger className="grid gap-4 sm:grid-cols-3">
          <SnapshotMetric link={SNAPSHOT_LINKS[0]} label="Open tasks" value={values.tasks} loading={loading} hint="Todo · in progress · review"/>
          <SnapshotMetric link={SNAPSHOT_LINKS[1]} label="Active projects" value={values.projects} loading={loading} hint="In motion"/>
          <SnapshotMetric link={SNAPSHOT_LINKS[2]} label="Live proposals" value={values.proposals} loading={loading} hint="Draft through sent"/>
        </FadeInStagger>
      </CardContent>
    </Card>);
}
function SnapshotMetric({ link, label, value, loading, hint, }: {
    ...
    return (<FadeInItem>
      <MotionCard>
        <Link href={link.href} className={cn('group flex h-full flex-col rounded-xl border border-muted/50 border-l-[3px] bg-linear-to-br p-4 shadow-sm transition-[border-color,box-shadow] hover:border-accent/30 hover:shadow-md', link.accent)}>
          ...
          {loading ? (<Skeleton className="mt-2 h-8 w-16 rounded-md"/>) : (<p className="mt-1 text-2xl font-bold tabular-nums tracking-tight text-foreground md:text-3xl">{value}</p>)}
          <div className="mt-2 flex items-center justify-between gap-2">
            <p className="text-xs text-muted-foreground">{hint}</p>
            <ArrowUpRight className="size-3.5 shrink-0 text-muted-foreground/60 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-primary" aria-hidden/>
          </div>
        </Link>
      </MotionCard>
    </FadeInItem>);
