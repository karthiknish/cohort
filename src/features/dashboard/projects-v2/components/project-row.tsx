...
import { Calendar, Eye, LoaderCircle, MessageSquare, Pencil, Tag } from 'lucide-react';
...
function ProjectRowComponent({
  ...
  const statusUpdateHandlers = Object.fromEntries(
    PROJECT_STATUSES.flatMap((status) =>
      status !== project.status ? ([[status, () => onUpdateStatus(project, status)] as const] as const) : [],
    ),
            ...
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-muted-foreground">
              ...
              <span className="inline-flex items-center gap-1.5">
                ...
                {project.recentActivityAt
                  ? `Active ${formatRelativeTime(project.recentActivityAt)}`
                  : 'No recent chat'}
              </span>
            </div>
