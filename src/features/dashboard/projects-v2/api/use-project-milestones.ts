...
import { mapMilestoneRecord } from '../utils/project-mappers';

export type UseProjectMilestonesArgs = {
...
export function useProjectMilestones({
  ...
  const milestonesRealtime = useQuery(
    ...
    milestonesQueryEnabled && projectIds.length > 0
      ? { workspaceId: workspaceId!, projectIds, refreshKey: milestonesRefreshKey }
      : 'skip',
  );
