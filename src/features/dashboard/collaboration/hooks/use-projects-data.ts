...
export function useProjectsData({ workspaceId, userId, selectedClientId, isPreviewMode, }: UseProjectsDataOptions) {
    const projectsRealtime = useQuery(projectsApi.list, isPreviewMode || !workspaceId || !userId
        ? 'skip'
        : {
            workspaceId,
