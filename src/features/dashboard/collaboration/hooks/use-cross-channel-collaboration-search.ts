...
export function useCrossChannelCollaborationSearch(
  ...
  return useCallback(
    async (query: CrossChannelSearchQuery): Promise<CrossChannelSearchResult[]> => {
      if (!workspaceId) {
        return []
      }
      ...
      await Promise.all(
        scopedChannels.map(async (channel) => {
          try {
            ...
            const resolvedPayload = (payload ?? {}) as { rows?: unknown[]; highlights?: string[] }
