...
import type { CollaborationAttachment, CollaborationChannelType, CollaborationMention, CollaborationMessage, CollaborationMessageFormat, CollaborationReaction, } from '@/types/collaboration';
...
export function useThreads({ workspaceId, currentUserId }: UseThreadsOptions) {
    ...
    const fetchThreadRepliesPage = async (threadRootId: string, cursor: string | null) => {
        if (isPreviewMode) {
            return {
        ...
        const nextCursor = hasMore && lastDisplayed && typeof lastDisplayed.createdAtMs === 'number'
    ...
    const clearThreadReplies = (threadRootId?: string) => {
        if (!threadRootId) {
    ...
    const addThreadReply = (rootId: string, message: CollaborationMessage) => {
    ...
    const addThreadReplyToState = (rootId: string, message: CollaborationMessage) => {
        const trimmedRootId = rootId.trim();
        if (!trimmedRootId || !message.id)
            return;
        setThreadMessagesByRootId((prev) => {
    ...
    const mutateThreadMessageById = (messageId: string, updater: (message: CollaborationMessage) => CollaborationMessage) => {
