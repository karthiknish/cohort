...
export function resolveSelectedChannelId(channels: Array<{
    id: string;
}>, pickedChannelId: string | null | undefined, visibleClientId: string | null = null): string | null {
    ...
    return ((preferredClientChannelId && channels.some((channel) => channel.id === preferredClientChannelId)
...
export function useChannelsData({ clients, projects, customChannels, fallbackDisplayName, fallbackRole, visibleClientId = null, channelAvatars, }: UseChannelsDataOptions) {
    const avatarForChannel = (channelId: string) => channelAvatars?.get(channelId) ?? null;
