...
import type { CollaborationMessage, CollaborationAttachment, CollaborationMention, CollaborationMessageFormat, } from '@/types/collaboration';
...
export function useFetchMessages({ workspaceId, channels, setMessagesByChannel, }: UseFetchMessagesOptions) {
    ...
    const fetchMessages = async (channelId: string, after?: string | null) => {
        ...
        try {
            ...
            const rowsRaw = Array.isArray(listResult)
                ...
                : Array.isArray((listResult as {
                    ...
                    : [];
            const rows = rowsRaw.filter((row): row is Record<string, unknown> => typeof row === 'object' && row !== null);
            const mapped: CollaborationMessage[] = rows
                .map((row) => {
                const rowChannelType = row.channelType;
                const normalizedChannelType = rowChannelType === 'client' || rowChannelType === 'team' || rowChannelType === 'project'
                    ? rowChannelType
                    : channel.type;
                return {
