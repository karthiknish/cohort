...
describe('MessageListRenderProvider', () => {
    it('lets MessageList read deleted rendering from context', () => {
        const markup = renderToStaticMarkup(<MessageListRenderProvider value={deletedRenderContextValue}>
        <MessageList messages={deletedMessages} currentUserId={null} isLoading={false} hasMore={false} onLoadMore={noop} onToggleReaction={asyncNoop}/>
    ...
    it('lets UnifiedThreadReplyCard read content rendering from context', () => {
        const markup = renderToStaticMarkup(<MessageListRenderProvider value={replyRenderContextValue}>
        <UnifiedThreadReplyCard reply={reply} currentUserId={null} editingMessageId={null} activeDeletingMessageId={null} messageUpdatingId={null} reactionPendingEmoji={null} onToggleReaction={noop}/>
      </MessageListRenderProvider>);
