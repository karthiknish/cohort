...
export function PollCardHeader({ isExpired, poll, totalVotes, }: {
    isExpired: boolean | null;
    poll: MessagePoll;
    totalVotes: number;
}) {
    return (<div className="border-b bg-muted/50 p-3">
...
export function PollOptionRow({ ui, handleToggleOption, option, percentage, sortedOptions, voteCount, }: {
    ...
    return (<button type="button" className={cn('relative rounded-lg border p-3 transition-colors', interactive && 'cursor-pointer hover:bg-background', isSelected && 'border-primary bg-accent/5', showOptionResults && hasLeadingWinner && percentage > 0 && 'border-accent/50')} onClick={handleRowClick} disabled={!interactive}>
      {showOptionResults && percentage > 0 ? (<div className="absolute inset-0 -z-10 rounded-lg bg-accent/5" style={barStyle}/>) : null}

      <div className="flex items-start gap-3">
        {!showOptionResults ? (<div className="pt-0.5">
            <Checkbox checked={isSelected} onChange={handleCheckboxChange}/>
          </div>) : null}

        {showOptionResults && hasLeadingWinner && voteCount > 0 && voteCount > (sortedOptions[1]?.voters.length ?? 0) ? (<Check className="size-4 text-primary"/>) : null}

        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium">{option.text}</p>
          <div className="mt-1 flex items-center gap-2">
            {showOptionResults ? (<div className="flex-1">
...
export function PollCardFooterActions({ ui, onEndPoll, onShowResults, onVote, selectedOptionsCount, }: {
    ...
    return (<div className="flex items-center justify-between border-t bg-muted/50 p-3">
      {canVote && !hasVoted && !isExpired ? (<Button type="button" size="sm" onClick={onVote} disabled={selectedOptionsCount === 0 || isVoting}>
          {isVoting ? 'Submitting...' : 'Submit Vote'}
        </Button>) : null}

      {canEnd && !isExpired ? (<Button type="button" size="sm" variant="outline" onClick={onEndPoll}>
          End Poll
        </Button>) : null}

      {hasVoted && !showResults ? (<Button type="button" size="sm" variant="ghost" onClick={onShowResults}>
          View Results
        </Button>) : null}
    </div>);
}
export function CreatePollDialogTrigger({ trigger }: {
    trigger?: ReactNode;
}) {
    if (trigger)
        return trigger;
...
export function CreatePollFormFields({ onAddOption, onOptionChange, onQuestionChange, onRemoveOption, options, question, }: {
    onAddOption: () => void;
    ...
    return (<div className="space-y-4 py-4">
      ...
      <div className="space-y-2">
        <Label>Options *</Label>
        <div className="space-y-2">
          {options.map((option, index) => (<CreatePollOptionRow key={option.id} index={index} option={option} optionsLength={options.length} onOptionChange={onOptionChange} onRemoveOption={onRemoveOption}/>))}
        </div>
...
function CreatePollOptionRow({ index, option, optionsLength, onOptionChange, onRemoveOption, }: CreatePollOptionRowProps) {
    ...
    return (<div className="flex items-center gap-2">
      ...
      {optionsLength > 2 ? (<Button type="button" variant="ghost" size="icon" className="size-8" onClick={handleRemoveOption} aria-label={`Remove option ${index + 1}`}>
          <Trash2 className="size-4"/>
          <span className="sr-only">Remove option {index + 1}</span>
        </Button>) : null}
    </div>);
}
export function CreatePollDialogFooter({ isCreating, onCancel, onCreate, question, }: {
    isCreating: boolean;
    onCancel: () => void;
    onCreate: () => void;
    question: string;
