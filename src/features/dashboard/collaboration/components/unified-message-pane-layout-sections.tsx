...
export function UnifiedMessagePaneConversationHeaderSection({ header, canSearchMessages, messageSearchOpen, onToggleMessageSearch, statusBanner, searchBar, }: UnifiedMessagePaneConversationHeaderSectionProps) {
    return (<>
      <UnifiedConversationHeader header={header} canSearchMessages={canSearchMessages} messageSearchOpen={messageSearchOpen} onToggleMessageSearch={onToggleMessageSearch}/>
      {statusBanner}
      {searchBar}
    </>);
}
...
export function UnifiedMessagePaneMessagesSection({ header, messageListRenderers, messages, currentUserId, currentUserRole, isLoading, hasMore, onLoadMore, onRefresh, onToggleReaction, reactionPendingByMessage, onReply, onDeleteMessage, activeDeletingMessageId, messageUpdatingId, emptyState, editingMessageId, effectiveFocusMessageId, effectiveFocusThreadId, typingIndicator, }: UnifiedMessagePaneMessagesSectionProps) {
    return (<div className="relative flex min-h-0 flex-1 overflow-hidden">
      <MessageListRenderProvider value={messageListRenderers}>
        <MessageList key={header.conversationKey ?? `${header.type}-${header.name}`} messages={messages} currentUserId={currentUserId} currentUserRole={currentUserRole} isLoading={isLoading} hasMore={hasMore} onLoadMore={onLoadMore} onRefresh={onRefresh} onToggleReaction={onToggleReaction} reactionPendingByMessage={reactionPendingByMessage} onReply={onReply} onDeleteMessage={onDeleteMessage} deletingMessageId={activeDeletingMessageId} updatingMessageId={messageUpdatingId} emptyState={emptyState} variant={header.type === 'channel' ? 'channel' : 'dm'} editingMessageId={editingMessageId} focusMessageId={effectiveFocusMessageId} focusThreadId={effectiveFocusThreadId} typingIndicatorText={typingIndicator}/>
...
export function UnifiedMessagePaneDeleteConfirm({ confirmingDeleteMessageId, activeDeletingMessageId, onOpenChange, onConfirm, onCancel, }: UnifiedMessagePaneDeleteConfirmProps) {
    return (<ConfirmDialog open={Boolean(confirmingDeleteMessageId)} onOpenChange={onOpenChange} title="Delete message" description="This removes the message content for everyone in the conversation and keeps a deleted placeholder in the timeline." confirmLabel="Delete" cancelLabel="Cancel" variant="destructive" isLoading={activeDeletingMessageId === confirmingDeleteMessageId} onConfirm={onConfirm} onCancel={onCancel}/>);
}
