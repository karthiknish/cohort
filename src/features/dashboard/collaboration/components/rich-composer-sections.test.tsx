...
describe('rich composer sections', () => {
    it('renders the toolbar actions', () => {
        const markup = renderToStaticMarkup(<RichComposerToolbar disabled={false} emojiPickerOpen={false} hasAttachments={true} onAction={vi.fn()} onAttachClick={vi.fn()} onEmojiClick={vi.fn()} onInsertMention={vi.fn()} onOpenEmojiChange={vi.fn()} onVoiceTranscript={vi.fn()}/>);
    ...
    it('renders the textarea shell and mention menu', () => {
        ...
        <RichComposerTextareaShell disabled={false} isDraggingOver={true} onBlur={vi.fn()} onChange={vi.fn()} onDragEnter={vi.fn()} onDragLeave={vi.fn()} onDragOver={vi.fn()} onDrop={vi.fn()} onFocus={vi.fn()} onKeyDown={vi.fn()} onPaste={vi.fn()} placeholder="Share an update…" textareaRef={EMPTY_TEXTAREA_REF} value="@a"/>
        <RichComposerMentionMenu highlightedMention={0} mentionQuery="a" mentionResults={participants} onMentionClick={vi.fn()} onMentionMouseDown={vi.fn()}/>
      </>);
    ...
    it('renders a stable no-results mention state', () => {
        const markup = renderToStaticMarkup(<RichComposerMentionMenu highlightedMention={0} mentionQuery="zoe" mentionResults={[]} onMentionClick={vi.fn()} onMentionMouseDown={vi.fn()}/>);
        expect(markup).toContain('Mention teammate');
