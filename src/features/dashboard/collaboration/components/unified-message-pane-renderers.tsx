...
import { renderMessageContentBlock, } from './unified-message-pane-render-utils';
...
function UnifiedThreadReplyRenderer({ reply }: {
    ...
    return (<UnifiedThreadReplyCard reply={reply} currentUserId={context.currentUserId} editingMessageId={context.editingMessageId} activeDeletingMessageId={context.activeDeletingMessageId} messageUpdatingId={context.messageUpdatingId} reactionPendingEmoji={context.reactionPendingByMessage[reply.id] ?? null} onToggleReaction={handleToggleReaction} onStartEdit={context.onEditMessage ? handleStartEdit : undefined} onRequestDelete={context.onDeleteMessage ? handleRequestDelete : undefined}/>);
}
...
export function UnifiedThreadSectionRenderer({ activeDeletingMessageId, expanded, handleLoadMoreThread, handleReaction, handleReply, handleRequestDelete, handleRetryThreadLoad, handleThreadToggle, handleStartEdit, headerType, message, onReply, resolveThreadRootId, threadErrorsByRootId, threadLoadingByRootId, threadMessagesByRootId, threadNextCursorByRootId, threadUnreadCountsByRootId, currentUserId, editingMessageId, messageUpdatingId, onDeleteMessage, onEditMessage, reactionPendingByMessage, }: {
    ...
    return (<UnifiedThreadReplyContext.Provider value={threadReplyContext}>
    <ThreadSection threadRootId={threadRootId} replyCount={replyCount} unreadCount={unreadCount} lastReplyIso={lastReplyIso} panel={threadPanel} error={threadError} replies={threadReplies} onToggle={handleToggle} onRetry={handleRetry} onLoadMore={handleLoadMore} onReply={handleReplyClick} canReply={Boolean(onReply)} ReplyRenderer={UnifiedThreadReplyRenderer}/>
    </UnifiedThreadReplyContext.Provider>);
}
export function SwipeableMessageRenderer({ children, currentUserId, handleReply, handleRequestDelete, message, onDeleteMessage, onReply, }: {
    children: ReactNode;
    currentUserId: string | null;
    handleReply: (message: UnifiedMessage) => void;
    handleRequestDelete: (messageId: string) => void;
    ...
    if (!enableSwipeGestures) {
        if (!message.deleted && onReply) {
            return (<LongPressMessage message={message} onLongPress={handleMessageReply}>
          {children}
        </LongPressMessage>);
        }
        return children;
    }
    return (<SwipeableMessage key={message.id} message={message} currentUserId={currentUserId} canDelete={!message.deleted && message.senderId === currentUserId && !!onDeleteMessage} onReply={!message.deleted && onReply ? handleMessageReply : undefined} onDelete={!message.deleted && onDeleteMessage ? handleMessageDelete : undefined}>
      {children}
    </SwipeableMessage>);
}
