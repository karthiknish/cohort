...
export type MessageListRendererProps = {
    ...
    renderMessageContent?: ComponentType<{
        message: UnifiedMessage;
    }> | ((message: UnifiedMessage) => ReactNode);
...
export function mergeMessageListRenderers({ renderers: renderersProp, renderMessageExtras, renderMessageActions, renderMessageContent, renderMessageAttachments, renderMessageFooter, renderThreadSection, renderEditForm, renderDeletedInfo, renderMessageWrapper, }: MessageListRendererProps): MessageListRenderers | undefined {
...
export function useMessageListRenderers({ renderers: renderersProp, renderMessageExtras, renderMessageActions, renderMessageContent, renderMessageAttachments, renderMessageFooter, renderThreadSection, renderEditForm, renderDeletedInfo, renderMessageWrapper, }: MessageListRendererProps): MessageListRenderers | undefined {
...
function isNormalizedMessageContentComponent(renderer: ComponentType<{
    message: UnifiedMessage;
}> | ((message: UnifiedMessage) => ReactNode)): renderer is ComponentType<{
    message: UnifiedMessage;
}> {
    return typeof renderer === 'function' && renderer.name === MESSAGE_CONTENT_COMPONENT_NAME;
}
export function toMessageContentComponent(renderer: ComponentType<{
    message: UnifiedMessage;
}> | ((message: UnifiedMessage) => ReactNode)): ComponentType<{
    message: UnifiedMessage;
}> {
    if (isNormalizedMessageContentComponent(renderer)) {
        return renderer;
    }
    const usesDirectMessageArg = typeof renderer === 'function' &&
        renderer.length === 1 &&
        renderer.name === '';
    return function MessageContentComponent({ message }: {
        ...
        return createElement(renderer as unknown as ComponentType<{
            message: UnifiedMessage;
        }>, { message });
    };
}
