...
export type CollaborationDashboardContextValue = {
  ...
  handleOpenChannelMessage: (messageId: string, options?: {
...
/**
 * Hook that gathers all collaboration dashboard state.
 * No provider needed — call once at the top of the dashboard component
 * and pass the result down as a prop.
 */
export function useCollaborationDashboard(): CollaborationDashboardContextValue {
  ...
  useEffect(() => {
    clearThreadReplies();
    // clearThreadReplies is not memoized in useThreads (new ref each render).
    // Including it here causes an infinite re-render loop because
    // clearThreadReplies() calls setActiveThreadIds(new Set()) which always
    // creates a new reference. Only depend on the channel id.
  ...
  return useMemo(
    ...
    [collab, dm, currentUserId, currentUserRole, isAdmin, dialogs, urlState, selectedCustomChannel, workspaceId, workspaceMembers, actions],
  );
}
