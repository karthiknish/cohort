...
import { useCollaborationDashboard, type CollaborationDashboardContextValue } from './collaboration-dashboard-provider';
...
function createUnifiedInboxSidebarProps(context: Context) {
...
function createUnifiedInboxManageChannelProps(context: Context) {
  const { isAdmin, selectedCustomChannel, openManageMembersDialog } = context;
  if (!isAdmin || !selectedCustomChannel) return undefined;
  return {
...
export function CollaborationDashboard() {
  const context = useCollaborationDashboard();
...
function CollaborationUrlWarnings({ context }: { context: Context }) {
  const { dismissUnresolvedChannelUrl, dismissUnresolvedConversationUrl, unresolvedChannelUrl, unresolvedConversationUrl } = context;
...
function CollaborationHeaderSection({ context }: { context: Context }) {
  const { collab, currentUserId, currentUserRole, handleCreateChannel, handleOpenChannelMessage, handleSelectChannel, isAdmin, workspaceId, workspaceMembers } = context;
    ...
    <DashboardPageHero innerClassName="relative flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      ...
      {currentUserRole !== 'client' ? (
        <div className="flex w-full shrink-0 flex-wrap items-center gap-3 sm:w-auto sm:justify-end">
          ...
          {isAdmin ? (
            <CreateChannelDialog workspaceId={workspaceId} userId={currentUserId} teamMembers={workspaceMembers} onCreate={handleCreateChannel} />
          ) : null}
        </div>
      ) : null}
    </DashboardPageHero>
...
function CollaborationProjectBanner({ context }: { context: Context }) {
  const { clearProjectFilter, requestedProjectId, requestedProjectName } = context;
...
function CollaborationInboxSection({ context }: { context: Context }) {
  ...
  const mentionParticipants = useMemo(() => {
    ...
    workspaceMembers.forEach((member) => {
      ...
      if (!key || members.has(key)) return;
      members.set(key, { id: member.id, name, role: member.role?.trim() || 'Contributor' });
    });
...
function ChannelMembersDialogSection({ context }: { context: Context }) {
  const { handleDeleteChannel, handleSaveChannelMembers, isManageMembersDialogOpen, selectedCustomChannel, setIsManageMembersDialogOpen, workspaceMembers } = context;
  return (
