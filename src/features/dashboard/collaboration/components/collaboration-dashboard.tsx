...
function createUnifiedInboxChannelPaneProps(context: CollaborationDashboardContext, mentionParticipants: ClientTeamMember[], channelExtras: ReturnType<typeof useCollaborationChannelExtras>, onShareToPlatform?: (message: UnifiedMessage, platform: 'email') => Promise<void>) {
...
function createUnifiedInboxDirectMessagePaneProps(context: CollaborationDashboardContext, channelExtras: ReturnType<typeof useCollaborationChannelExtras>) {
...
export function CollaborationDashboard() {
    return (<CollaborationDashboardProvider>
...
function CollaborationDashboardContent() {
    ...
    return (<PageSkeletonBoundary loading={collab.isBootstrapping} loadingContent={<CollaborationSkeleton />}>
...
function CollaborationUrlWarnings() {
    ...
    return (<div className="mx-4 mb-3 space-y-2">
      {unresolvedChannelUrl ? (<Alert variant="destructive">
      ...
      {unresolvedConversationUrl ? (<Alert variant="destructive">
...
function CollaborationHeaderSection() {
    ...
    const handleSearchResultClick = (messageId: string, channelId: string, threadRootId?: string | null) => {
        handleSelectChannel(channelId);
        handleOpenChannelMessage(messageId, { threadId: threadRootId ?? null });
    };
    return (<DashboardPageHero innerClassName="relative flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      ...
      {currentUserRole !== 'client' ? (<div className="flex w-full shrink-0 flex-wrap items-center gap-3 sm:w-auto sm:justify-end">
          <CrossChannelSearch onSearch={searchAcrossChannels} onResultClick={handleSearchResultClick}/>
          {isAdmin ? (<CreateChannelDialog workspaceId={workspaceId} userId={currentUserId} teamMembers={workspaceMembers} onCreate={handleCreateChannel}/>) : null}
        </div>) : null}
    </DashboardPageHero>);
}
function CollaborationProjectBanner() {
    ...
    return (<div className="mx-4 mb-3 flex items-center justify-between rounded-md border border-muted/40 bg-muted/10 px-3 py-2 text-xs text-muted-foreground">
      ...
      <div className="flex items-center gap-2">
        ...
        <Button type="button" variant="ghost" size="icon" className="size-6 hover:text-foreground" onClick={clearProjectFilter} aria-label="Clear project filter from collaboration">
...
function ChannelMembersDialogSection() {
    ...
    return (<ChannelMembersDialog open={isManageMembersDialogOpen} onOpenChange={setIsManageMembersDialogOpen} channel={selectedCustomChannel} teamMembers={workspaceMembers} onSave={handleSaveChannelMembers} onDelete={handleDeleteChannel}/>);
}
