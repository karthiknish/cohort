 ...
 */
export function UserPresenceIndicator({ status, showLabel = false, className, size = 'md', }: UserPresenceIndicatorProps) {
    const sizeClasses = {
        sm: 'size-2',
        md: 'size-3',
        lg: 'size-4',
    ...
    const indicator = (<Circle className={cn(sizeClasses[size], statusClasses[status], className)}/>);
    if (showLabel) {
 ...
 */
export function UserAvatarWithPresence({ src, alt, fallback, status = 'offline', size = 'md', className, onClick, }: UserAvatarWithPresenceProps) {
    const sizeClasses = {
        sm: 'size-8',
        md: 'size-10',
        lg: 'size-12',
    ...
    const wrapperClassName = cn('relative inline-flex flex-shrink-0', clickable && 'cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:ring-offset-2', className);
    const content = (<>
      {/* Avatar */}
      <Avatar className={cn(sizeClasses[size], 'rounded-full bg-accent/10 flex items-center justify-center text-xs font-medium text-primary ring-2 ring-background')}>
        {src ? <AvatarImage src={src} alt={alt} className="object-cover"/> : null}
        <AvatarFallback>{fallback}</AvatarFallback>
      </Avatar>

      {/* Presence indicator */}
      {status !== 'offline' && (<span className={cn('absolute rounded-full ring-2 ring-background', indicatorSizes[size], indicatorPositions[size], status === 'online' && 'bg-success', status === 'away' && 'bg-warning', status === 'busy' && 'bg-destructive')}/>)}
    </>);
    if (clickable) {
        return (<button type="button" className={wrapperClassName} onClick={onClick} aria-label={`Open profile for ${alt}`}>
        {content}
      </button>);
    }
 ...
 */
export function ChannelPresenceList({ members, title = 'Online', maxVisible = 5, className, }: ChannelPresenceListProps) {
    // Filter to only show online or away users
    const onlineMembers = members.filter((m) => m.status === 'online' || m.status === 'away' || m.status === 'busy');
    ...
    return (<div className={cn('flex items-center gap-1', className)}>
      ...
      <div className="flex">
        {displayMembers.map((member, memberIndex) => (<TooltipProvider key={member.id} delayDuration={300}>
            <Tooltip>
              <TooltipTrigger asChild>
                <div className={cn('relative', memberIndex > 0 && '-ml-1')}>
                  ...
                  <span className={cn('absolute bottom-0 right-0 size-2 rounded-full ring-1 ring-background', member.status === 'online' && 'bg-success', member.status === 'away' && 'bg-warning', member.status === 'busy' && 'bg-destructive')}/>
                </div>
              </TooltipTrigger>
