    ...
    return (<Dialog open={isOpen} onOpenChange={handleOnOpenChange}>
      ...
      <div className="absolute left-0 right-0 top-0 z-10 flex items-center justify-between bg-gradient-to-b from-black/60 to-transparent p-4">
        <div className="flex items-center gap-3">
          ...
          {currentImage.size && (<span className="text-xs text-viewer-chrome/60">{currentImage.size}</span>)}
          {hasMultipleImages && (<span className="rounded-full bg-viewer-chrome/10 px-2 py-0.5 text-xs text-viewer-chrome/80">
              {normalizedIndex + 1} / {images.length}
            </span>)}
        </div>
        <div className="flex items-center gap-1">
          <Button type="button" variant="ghost" size="icon" className="size-9 text-viewer-chrome/80 hover:text-viewer-chrome hover:bg-viewer-chrome/10" onClick={handleZoomOutClick} disabled={zoom <= 1} aria-label="Zoom out">
            <ZoomOut className="size-5" aria-hidden/>
          </Button>
          <span className="min-w-[50px] text-center text-xs text-viewer-chrome/70" aria-live="polite">
            {Math.round(zoom * 100)}%
          </span>
          <Button type="button" variant="ghost" size="icon" className="size-9 text-viewer-chrome/80 hover:text-viewer-chrome hover:bg-viewer-chrome/10" onClick={handleZoomInClick} disabled={zoom >= 4} aria-label="Zoom in">
          ...
          <div className="mx-2 h-6 w-px bg-viewer-chrome/20"/>
          <Button type="button" variant="ghost" size="icon" className="size-9 text-viewer-chrome/80 hover:text-viewer-chrome hover:bg-viewer-chrome/10" onClick={handleCloseClick} aria-label="Close preview">
      ...
      {/* Navigation Arrows */}
      {hasMultipleImages && (<>
          <Button type="button" variant="ghost" size="icon" className="absolute left-4 top-1/2 z-10 size-12 -translate-y-1/2 rounded-full bg-black/40 text-viewer-chrome hover:bg-black/60" onClick={handlePreviousClick} aria-label="Previous image">
            <ChevronLeft className="size-8" aria-hidden/>
          </Button>
          <Button type="button" variant="ghost" size="icon" className="absolute right-4 top-1/2 z-10 size-12 -translate-y-1/2 rounded-full bg-black/40 text-viewer-chrome hover:bg-black/60" onClick={handleNextClick} aria-label="Next image">
      ...
      {/* Thumbnails */}
      {hasMultipleImages && (<div className="absolute bottom-0 left-0 right-0 z-10 flex items-center justify-center gap-2 bg-gradient-to-t from-black/60 to-transparent p-4">
