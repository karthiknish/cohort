 ...
 */
export function VirtualizedMessageList({ messages, renderItem, estimateSize = () => 100, overscan = 5, className, loadMore, hasMore = false, isLoadingMore = false, }: VirtualizedMessageListProps) {
    'use no memo';
    ...
    const containerStyle = ({ contain: 'strict' as const });
    const contentStyle = ({
        display: 'flex',
    ...
    return (<div ref={containerRef} className={cn('overflow-y-auto', className)} onScroll={handleScroll} style={containerStyle}>
      <div style={contentStyle}>
        {messages.map((message, index) => {
            if (!message)
                return null;
            return (<div key={message.id} data-index={index}>
