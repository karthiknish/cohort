...
export function SwipeableMessage({ canDelete = false, onReply, onDelete, children, className, }: SwipeableMessageProps) {
    ...
    return (<div className="relative overflow-hidden">
      {showActions && (<div className={cn('absolute inset-y-0 z-10 flex items-center gap-2 px-4', pendingAction === 'reply' ? 'left-0 bg-info/10' : 'right-0 bg-destructive/10')}>
          {pendingAction === 'reply' && (<>
      ...
      <div ref={containerRef} className={cn('transition-transform duration-[var(--motion-duration-fast)] ease-[var(--motion-ease-out)] motion-reduce:transition-none touch-pan-y', state.isSwiping && 'transition-none', className)} style={transformStyle} {...handlers}>
        {children}
      </div>
    </div>);
}
...
export function LongPressMessage({ onLongPress, children, className, }: LongPressMessageProps) {
    ...
    return (<div className={cn(isPressed && 'bg-muted/50 scale-[0.98]', 'motion-chromatic', className)} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove}>
      {children}
    </div>);
}
