...
export function CodeBlock({ code, language = 'javascript', filename, className, maxHeight = '400px', }: CodeBlockProps) {
    ...
    const highlightedCode = createTrustedHtml(highlightCode(visibleCode, language), 'collaboration-code-block:highlightCode:html-escaped');
    ...
    const contentStyle = ({ maxHeight: isExpanded ? undefined : maxHeight });
    return (<div className={cn('rounded-lg border bg-muted/50 overflow-hidden', className)}>
      {/* Header bar */}
      <div className="flex items-center justify-between px-4 py-2 border-b bg-muted">
        <div className="flex items-center gap-2">
          <Code className="size-4 text-muted-foreground"/>
          {filename ? (<span className="text-sm font-medium">{filename}</span>) : (<DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="max-h-60 overflow-y-auto">
                {LANGUAGES.map((lang) => (<DropdownMenuItem key={lang.value} asChild>
        ...
        <div className="flex items-center gap-1">
          ...
          {lines.length > 10 && (<Button type="button" variant="ghost" size="icon" className="size-7" onClick={handleToggleExpanded} aria-label={isExpanded ? 'Collapse code block' : 'Expand code block'}>
          ...
          {/* Copy button */}
          <Button type="button" variant="ghost" size="icon" className="size-7" onClick={handleCopy} aria-label={copied ? 'Code copied' : 'Copy code'}>
      ...
      {/* Code content */}
      <div className={cn('overflow-auto', !isExpanded && 'max-h-[400px]')} style={contentStyle}>
        <pre className="p-4 text-sm">
          <TrustedHtml as="code" html={highlightedCode} className={cn('font-mono', getLanguageColor(language))}/>
        </pre>
      </div>

      {/* Truncated indicator */}
      {shouldTruncate && (<div className="px-4 py-2 border-t bg-muted/50 text-center">
          <Button type="button" variant="ghost" size="sm" onClick={handleShowMore} className="text-xs">
...
export function InlineCode({ children, className }: {
    ...
    return (<code className={cn('px-1.5 py-0.5 rounded text-sm font-mono bg-muted text-foreground', className)}>
      {children}
    </code>);
}
/**
