...
export function CollaborationChannelList({ channels, filteredChannels, selectedChannel, onSelectChannel, searchQuery, onSearchQueryChange, channelSummaries, }: CollaborationChannelListProps) {
    const handleSearchChange = (event: ChangeEvent<HTMLInputElement>) => {
        onSearchQueryChange(event.target.value);
    };
    const selectChannelHandlers = Object.fromEntries(filteredChannels.map((channel) => [channel.id, () => onSelectChannel(channel.id)])) as Record<string, () => void>;
      ...
      <div className="flex-1 overflow-y-auto">
        {channels.length === 0 ? (<p className="px-4 py-6 text-sm text-muted-foreground">Add a client to create your first collaboration channel.</p>) : filteredChannels.length === 0 ? (<p className="px-4 py-6 text-sm text-muted-foreground">No channels match your search.</p>) : (<div className="space-y-2 p-3">
            {filteredChannels.map((channel) => {
                ...
                return (<MotionPressable key={channel.id} onClick={selectChannelHandlers[channel.id]} className={cn(listItemEnterClass, 'flex w-full max-w-full min-w-0 flex-col gap-1.5 overflow-hidden rounded-lg border p-3 text-left motion-chromatic outline-none focus-visible:ring-2 focus-visible:ring-primary/20', isSelected
                  ...
                  <div className="flex w-full items-start justify-between gap-2">
                    ...
                    {summary?.lastTimestamp && (<span className="shrink-0 text-[10px] text-muted-foreground">
                  ...
                  <p className="mt-0.5 line-clamp-2 break-all text-xs text-muted-foreground/80" title={summary?.lastMessage ?? undefined}>
                    {summary?.lastMessage
