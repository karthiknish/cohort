...
function useCrossChannelSearchController({ onSearch, onResultClick, }: Pick<CrossChannelSearchProps, 'onSearch' | 'onResultClick'>) {
    ...
    const handleResultClick = (result: SearchResult) => {
        const threadRootId = result.message.threadRootId?.trim() ||
...
function CrossChannelSearchBar({ query, isSearching, onQueryChange, onKeyDown, onSearch, }: {
    ...
    return (<div className="flex gap-2">
      <div className="relative flex-1">
        ...
        <Input placeholder="Search messages…" value={query} onChange={onQueryChange} onKeyDown={onKeyDown} className="pl-9"/>
      </div>
...
function CrossChannelSearchFilters({ filterState, hasActiveFilters, onChannelTypeChange, onToggleAttachment, onToggleLink, onClear, }: {
    ...
    return (<div className="flex flex-wrap gap-2">
      <div className="flex gap-1">
        {(['all', 'team', 'client', 'project'] as const).map((type) => (<CrossChannelSearchFilterButton key={type} variant={filterState.selectedChannelType === type ? 'default' : 'outline'} size="sm" onSelect={onChannelTypeChange} value={type} className="capitalize">
            {type}
          </CrossChannelSearchFilterButton>))}
      </div>

      <Button type="button" variant={filterState.hasAttachment ? 'default' : 'outline'} size="sm" onClick={onToggleAttachment} className="gap-1">
        <Paperclip className="size-3"/>
        Has attachment
      </Button>

      <Button type="button" variant={filterState.hasLink ? 'default' : 'outline'} size="sm" onClick={onToggleLink} className="gap-1">
        🔗 Has link
      </Button>

      {hasActiveFilters ? (<Button type="button" variant="ghost" size="sm" onClick={onClear} className="gap-1 text-muted-foreground">
          <X className="size-3"/>
          Clear
        </Button>) : null}
    </div>);
...
function CrossChannelSearchFilterButton({ variant, size, onSelect, value, className, children, }: CrossChannelSearchFilterButtonProps) {
    const onSelectFilter = () => {
        onSelect(value);
    };
    return (<Button type="button" variant={variant} size={size} onClick={onSelectFilter} className={className}>
      {children}
    </Button>);
}
function CrossChannelSearchResults({ query, isSearching, results, onResultClick, }: {
    ...
    return (<div className="flex-1 overflow-y-auto -mx-6 px-6">
      {results.length === 0 && query && !isSearching ? (<div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      {results.length === 0 && !query ? (<div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <div className="space-y-1">
        {results.map((result) => (<SearchResultItem key={`${result.channel.id}-${result.message.id}`} result={result} onSelect={onResultClick}/>))}
      </div>
    </div>);
}
function CrossChannelSearchResultsFooter({ count }: {
    ...
    if (count === 0)
        return null;
    return (<div className="border-t pt-3 text-sm text-muted-foreground">
 ...
 */
export function CrossChannelSearch({ onSearch, onResultClick, trigger, }: CrossChannelSearchProps) {
    const { open, setOpen, query, setQuery, isSearching, results, filterState, hasActiveFilters, handleSearch, handleKeyDown, handleResultClick, clearSearch, setSelectedChannelType, handleToggleAttachment, handleToggleLink, } = useCrossChannelSearchController({ onSearch, onResultClick });
    ...
    const dialogTrigger = trigger ?? (<DialogTrigger asChild>
    ...
    return (<Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-2xl max-h-[80vh] flex flex-col">
        ...
        <CrossChannelSearchBar query={query} isSearching={isSearching} onQueryChange={handleQueryChange} onKeyDown={handleKeyDown} onSearch={handleSearchClick}/>

        <CrossChannelSearchFilters filterState={filterState} hasActiveFilters={hasActiveFilters} onChannelTypeChange={setSelectedChannelType} onToggleAttachment={handleToggleAttachment} onToggleLink={handleToggleLink} onClear={clearSearch}/>

        <CrossChannelSearchResults query={query} isSearching={isSearching} results={results} onResultClick={handleResultClick}/>
...
function SearchResultItem({ result, onSelect }: SearchResultItemProps) {
    ...
    return (<button type="button" onClick={onSelectSearchResult} className="w-full text-left p-3 hover:bg-muted/50 rounded-lg transition-colors group">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {message.createdAt ? (<ClientRelativeTime value={message.createdAt} className="text-xs text-muted-foreground"/>) : null}
          </div>
          ...
          <div className="flex items-center gap-3 mt-1.5">
            {message.attachments && message.attachments.length > 0 && (<span className="flex items-center gap-1 text-xs text-muted-foreground">
                <Paperclip className="size-3"/>
                {message.attachments.length}
              </span>)}
            {message.mentions && message.mentions.length > 0 && (<span className="flex items-center gap-1 text-xs text-muted-foreground">
                <AtSign className="size-3"/>
                {message.mentions.length}
              </span>)}
            {message.reactions && message.reactions.length > 0 && (<span className="flex items-center gap-1 text-xs text-muted-foreground">
                {message.reactions.map((r) => r.emoji).join(' ')}
              </span>)}
          </div>

          {/* Highlights */}
          {highlights.length > 0 && (<div className="flex flex-wrap gap-1 mt-2">
              {highlights.slice(0, 3).map((highlight) => (<span key={`${result.message.id}-${highlight}`} className="px-1.5 py-0.5 bg-accent/10 text-primary text-xs rounded">
                  {highlight}
                </span>))}
            </div>)}
        </div>
 ...
 */
export function QuickSearchInput({ onSearch, placeholder = 'Search messages...', className, }: {
    onSearch: (query: string) => void;
    placeholder?: string;
    className?: string;
}) {
    const [value, setValue] = useState('');
    const onQuickSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
