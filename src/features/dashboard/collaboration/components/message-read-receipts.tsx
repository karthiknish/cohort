 ...
 */
export function MessageReadReceipts({ message, currentUserId, channelMemberCount = 0, readByNames = EMPTY_READ_BY_NAMES, className, }: MessageReadReceiptsProps) {
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <span className={cn('flex items-center gap-0.5 text-muted-foreground', className)}>
            ...
            {status === 'read_by_all' && (<CheckCheck className="size-3.5 text-primary fill-primary"/>)}
            {readCount > 0 && (<span className="text-xs text-muted-foreground">{readCount}</span>)}
          </span>
        </TooltipTrigger>
 ...
 */
export function CompactReadReceipt({ message, currentUserId, className, }: {
    message: CollaborationMessage;
    currentUserId: string | null;
    className?: string;
}) {
    ...
    if (readBy.length === 0)
        return null;
    return (<span className={cn('flex items-center gap-0.5 text-xs text-muted-foreground', 'bg-muted/50 px-1.5 py-0.5 rounded-full', className)}>
 ...
 */
export function ReadReceiptDetail({ message, channelMembers, className, }: {
    message: CollaborationMessage;
    ...
    return (<div className={cn('space-y-2', className)}>
      ...
      {deliveredTo.length > readBy.length && (<div>
 ...
 */
export function MessageDeliveryStatus({ message, currentUserId, memberNames, className, }: {
    message: CollaborationMessage;
    currentUserId: string | null;
    channelMemberCount?: number;
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <span className={cn('flex items-center gap-0.5 text-muted-foreground', className)}>
            ...
            {status === 'delivered' && (<CheckCheck className="size-3.5 text-muted-foreground"/>)}
            {status === 'read' && (<CheckCheck className="size-3.5 text-primary fill-primary"/>)}
            {(readCount > 0 || deliveredCount > 0) && (<span className="text-xs text-muted-foreground">
