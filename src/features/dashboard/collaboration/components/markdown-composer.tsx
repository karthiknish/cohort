...
function MarkdownPreviewLink(props: AnchorHTMLAttributes<HTMLAnchorElement>) {
    return (<a {...props} target="_blank" rel="noopener noreferrer" className="text-primary underline underline-offset-2 hover:no-underline">
      {props.children}
    </a>);
}
...
function MarkdownToolbarActionButton({ insert, title, icon, onInsert, }: {
    ...
    const onInsertMarkdown = () => {
        onInsert(insert);
    };
    return (<Button type="button" variant="ghost" size="sm" className="h-7 px-2" onClick={onInsertMarkdown} title={title}>
      {icon}
    </Button>);
}
 ...
 */
export function MarkdownPreview({ value, onChange, placeholder = 'Type your message…', maxLength = 2000, className, minRows = 3, }: MarkdownPreviewProps) {
    ...
    const onMarkdownChange = (event: ChangeEvent<HTMLTextAreaElement>) => onChange(event.target.value);
    const charCount = value.length;
    const remaining = maxLength - charCount;
    const isNearLimit = remaining < 100;
    const isAtLimit = remaining <= 0;
    return (<div className={cn('border rounded-lg overflow-hidden', isFullscreen && 'fixed inset-4 z-50 rounded-lg shadow-2xl', className)}>
      {/* Header bar */}
      <div className="flex items-center justify-between px-3 py-2 border-b bg-muted">
        ...
        <div className="flex items-center gap-1">
          {/* Preview toggle */}
          <Button type="button" variant="ghost" size="sm" onClick={handleTogglePreview} className="h-7 px-2">
            {showPreview ? (<>
          ...
          {/* Fullscreen toggle */}
          <Button type="button" variant="ghost" size="icon" className="size-7" onClick={handleToggleFullscreen} aria-label={isFullscreen ? 'Exit fullscreen' : 'Enter fullscreen'}>
      ...
      {showPreview ? (
        ...
        <div className="p-4 min-h-[120px] max-h-[400px] overflow-y-auto">
          {value ? (<MarkdownPreviewContent value={value}/>) : (<p className="text-sm text-muted-foreground italic">
              Nothing to preview…
            </p>)}
        </div>) : (
        ...
        <Tabs defaultValue="write" className="w-full">
          ...
          <TabsContent value="write" className="mt-0">
            <Textarea value={value} onChange={onMarkdownChange} placeholder={placeholder} maxLength={maxLength} rows={minRows} className="min-h-[120px] max-h-[400px] resize-none border-0 rounded-none focus-visible:ring-0"/>
          </TabsContent>
      ...
      <div className="flex items-center justify-between px-3 py-2 border-t bg-muted/50">
        ...
        {/* Character count */}
        <div className={cn('text-xs', isNearLimit && !isAtLimit && 'text-warning', isAtLimit && 'text-destructive')}>
          {charCount}/{maxLength}
        </div>
      </div>
    </div>);
}
function MarkdownPreviewContent({ value }: {
    value: string;
}) {
    return (<div className="prose prose-sm max-w-none break-words">
 ...
 */
export function MarkdownToolbar({ onInsert, className, }: {
    onInsert: (markdown: string) => void;
    className?: string;
}) {
    const buttons = [
    ...
    return (<div className={cn('flex items-center gap-1 p-1 border-b bg-muted/50', className)}>
      {buttons.map((btn) => (<MarkdownToolbarActionButton key={btn.label} insert={btn.insert} title={btn.title} icon={btn.icon} onInsert={onInsert}/>))}
    </div>);
}
