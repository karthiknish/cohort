...
export function SharedPlatformBadges({ platforms }: {
    platforms?: Array<'email'>;
}) {
    if (!platforms || platforms.length === 0)
        return null;
    return (<div className="mt-1 flex items-center gap-1">
      <span className="text-[10px] text-muted-foreground">Sent to:</span>
      {platforms.map((platform) => (<TooltipProvider key={platform}>
          <Tooltip>
            <TooltipTrigger>
