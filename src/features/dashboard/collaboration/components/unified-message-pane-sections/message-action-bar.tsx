...
export function UnifiedMessageActionBar({ headerType, message, currentUserId, currentUserRole, activeDeletingMessageId, messageUpdatingId, sharingTo, onReply, onStartEdit, onRequestDelete, onShare, onCreateTask, onForward, pinWorkspaceId, pinMessage, }: UnifiedMessageActionBarProps) {
    const canManageMessage = Boolean(currentUserId &&
        !message.deleted &&
    ...
    return (<div className="flex items-center gap-1">
      {onCreateTask && !message.deleted ? (<TooltipProvider delayDuration={150}>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" className="size-6 transition-transform hover:scale-105" disabled={isBusy} onClick={handleCreateTaskClick}>
      ...
      {headerType === 'channel' && onForward && !message.deleted ? (<TooltipProvider delayDuration={150}>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" className="size-6 transition-transform hover:scale-105" disabled={isBusy} onClick={handleForwardClick}>
      ...
      {headerType === 'channel' && pinWorkspaceId && pinMessage && !message.deleted ? (<PinMessageButton message={pinMessage} workspaceId={pinWorkspaceId} userId={currentUserId} variant="icon"/>) : null}

      {headerType === 'channel' && onReply ? (<TooltipProvider delayDuration={150}>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" className="size-6 transition-transform hover:scale-105" disabled={isBusy} onClick={handleReplyClick}>
      ...
      {onStartEdit && canManageMessage ? (<TooltipProvider delayDuration={150}>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" className="size-6 transition-transform hover:scale-105" disabled={isBusy} onClick={handleEditClick}>
      ...
      {onRequestDelete && canManageMessage ? (<TooltipProvider delayDuration={150}>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" className="size-6 text-destructive transition-transform hover:scale-105 hover:text-destructive" disabled={isBusy} onClick={handleDeleteClick}>
      ...
      {onShare ? (<DropdownMenu>
            <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" className="size-6 transition-transform hover:scale-105" disabled={isBusy} aria-label={`Share message from ${message.senderName ?? 'sender'}`}>
