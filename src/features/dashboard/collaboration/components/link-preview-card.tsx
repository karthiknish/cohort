...
export function LinkPreviewCard({ url }: LinkPreviewCardProps) {
    ...
    return (<Card className="max-w-xl border-muted/60 bg-muted/10">
      <CardContent className="flex gap-3 p-3">
        <div className={cn("relative hidden h-24 w-32 flex-shrink-0 overflow-hidden rounded-md border border-muted/40 bg-background sm:block", imageUrl ? undefined : "flex items-center justify-center p-3")} aria-hidden={!imageUrl}>
          {isLoading ? (<div className="flex size-full items-center justify-center">
              <LoaderCircle className="size-5 animate-spin text-muted-foreground"/>
            </div>) : imageUrl ? (<LazyImage src={imageUrl} alt={title} className="size-full object-cover" onError={handleImageError}/>) : (<div className="flex size-full items-center justify-center text-muted-foreground">
              <ImageIcon className="size-5"/>
            </div>)}
        </div>
        <div className="min-w-0 flex-1 space-y-2">
          <div>
            <a href={url} target="_blank" rel="noreferrer noopener" className="line-clamp-2 text-sm font-semibold text-foreground hover:underline">
              {title}
            </a>
