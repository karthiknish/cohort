...
function PendingAttachmentRow({ attachment, disabled, onRemove, }: {
    ...
    return (<div className={cn('flex items-center justify-between gap-3 rounded-md border border-muted/50 bg-background p-2 text-sm', listItemEnterClass)} key={attachment.id}>
      <div className="flex min-w-0 items-center gap-2">
        {isImageType ? (<ImageIcon className="size-4 text-muted-foreground"/>) : (<FileText className="size-4 text-muted-foreground"/>)}
        <span className="truncate" title={attachment.name}>
          {attachment.name}
        </span>
        <span className="text-xs text-muted-foreground">{attachment.sizeLabel}</span>
      </div>
      <Button type="button" variant="ghost" size="icon" onClick={handleRemove} disabled={disabled} className="size-7">
        <X className="size-4"/>
...
export function PendingAttachmentsList({ attachments, uploading, disabled = false, onRemove, }: PendingAttachmentsListProps) {
    if (attachments.length === 0)
        return null;
    return (<div className="space-y-2 rounded-md border border-dashed border-muted/60 bg-muted/20 p-3 listItemEnterClass">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{attachments.length} attachment{attachments.length === 1 ? '' : 's'} ready to send</span>
        {uploading && (<span className="inline-flex items-center gap-1">
            <LoaderCircle className="size-3.5 animate-spin"/> Uploading…
          </span>)}
      </div>
      <div className="space-y-2">
        {attachments.map((attachment) => (<PendingAttachmentRow key={attachment.id} attachment={attachment} disabled={disabled} onRemove={onRemove}/>))}
      </div>
    </div>);
}
...
export function MessageComposer({ channel, messageInput, onMessageInputChange, onSend, sending, isSendDisabled, pendingAttachments, uploading, onRemoveAttachment, channelParticipants, replyingToMessage, onCancelReply, onComposerFocus, onComposerBlur, typingIndicator, fileInputRef, onAttachmentInputChange, onComposerDrop, onComposerDragOver, onComposerPaste, }: MessageComposerProps) {
    const handleAttachClick = () => {
        fileInputRef.current?.click();
    };
    return (<div className="border-t border-muted/40 bg-background p-4">
      <div className="space-y-3">
        <PendingAttachmentsList attachments={pendingAttachments} uploading={uploading} disabled={sending} onRemove={onRemoveAttachment}/>

        <div className="space-y-3">
          <div className="w-full rounded-lg border border-muted/40 bg-background shadow-sm motion-chromatic focus-within:border-accent/40 focus-within:ring-1 focus-within:ring-primary/20">
            {replyingToMessage && (<ReplyIndicator message={replyingToMessage} onCancel={onCancelReply}/>)}
