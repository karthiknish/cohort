...
import { COLLABORATION_MESSAGE_GROUP_CONTINUATION_DISPLAY, COLLABORATION_MESSAGE_GROUP_HEADER_DISPLAY, } from '../message-pane-display-state';
...
export function CollaborationMessageViewport({ canLoadMore, channelMessages, currentUserId, currentUserRole, editingMessageId, editingPreview, editingValue, expandedThreadIds, flattenedMessages, isLoading, isSearchActive, loadingMore, messageDeletingId, messageUpdatingId, messagesEndRef, messagesError, onRetryMessages, messagesRetrying, onCancelEdit, onConfirmDelete, onConfirmEdit, onCreateTask, onEditingValueChange, onLoadMore, onLoadMoreThread, onReply, onRetryThreadLoad, onStartEdit, onThreadToggle, onToggleReaction, reactionPendingByMessage, threadErrorsByRootId, threadLoadingByRootId, threadMessagesByRootId, threadNextCursorByRootId, visibleMessages, typingIndicator, }: CollaborationMessageViewportProps) {
    return (<div className="flex-1 overflow-y-auto">
      <div className="space-y-4 p-4">
        {isLoading ? (<div className="flex justify-center py-6 text-muted-foreground">
        ...
        {messagesError ? (<MessagesErrorState error={messagesError} onRetry={onRetryMessages} isRetrying={messagesRetrying}/>) : null}

        {!isLoading && !messagesError && !isSearchActive && canLoadMore && onLoadMore ? (<div className="flex justify-center">
        ...
        {!isSearchActive ? (<div className="space-y-1">
            {flattenedMessages.map((item) => {
                if (item.type === 'separator') {
                    return (<div key={item.id} data-index={item.id}>
                    <DateSeparator label={item.label}/>
                  </div>);
                }
                return (<div key={item.id} data-index={item.id}>
                  <CollaborationMessageItem currentUserId={currentUserId} currentUserRole={currentUserRole} editingMessageId={editingMessageId} editingPreview={editingPreview} editingValue={editingValue} expandedThreadIds={expandedThreadIds} message={item.message} messageDeletingId={messageDeletingId} messageUpdatingId={messageUpdatingId} onCancelEdit={onCancelEdit} onConfirmDelete={onConfirmDelete} onConfirmEdit={onConfirmEdit} onCreateTask={onCreateTask} onEditingValueChange={onEditingValueChange} onLoadMoreThread={onLoadMoreThread} onReply={onReply} onRetryThreadLoad={onRetryThreadLoad} onStartEdit={onStartEdit} onThreadToggle={onThreadToggle} onToggleReaction={onToggleReaction} reactionPendingByMessage={reactionPendingByMessage} display={item.isFirstInGroup
                        ? COLLABORATION_MESSAGE_GROUP_HEADER_DISPLAY
                        : COLLABORATION_MESSAGE_GROUP_CONTINUATION_DISPLAY} threadErrorsByRootId={threadErrorsByRootId} threadLoadingByRootId={threadLoadingByRootId} threadMessagesByRootId={threadMessagesByRootId} threadNextCursorByRootId={threadNextCursorByRootId}/>
                </div>);
            })}
          </div>) : (<CollaborationSearchMessageList currentUserId={currentUserId} currentUserRole={currentUserRole} editingMessageId={editingMessageId} editingPreview={editingPreview} editingValue={editingValue} expandedThreadIds={expandedThreadIds} messageDeletingId={messageDeletingId} messageUpdatingId={messageUpdatingId} onCancelEdit={onCancelEdit} onConfirmDelete={onConfirmDelete} onConfirmEdit={onConfirmEdit} onCreateTask={onCreateTask} onEditingValueChange={onEditingValueChange} onLoadMoreThread={onLoadMoreThread} onReply={onReply} onRetryThreadLoad={onRetryThreadLoad} onStartEdit={onStartEdit} onThreadToggle={onThreadToggle} onToggleReaction={onToggleReaction} reactionPendingByMessage={reactionPendingByMessage} threadErrorsByRootId={threadErrorsByRootId} threadLoadingByRootId={threadLoadingByRootId} threadMessagesByRootId={threadMessagesByRootId} threadNextCursorByRootId={threadNextCursorByRootId} visibleMessages={visibleMessages}/>)}

        {typingIndicator && !isSearchActive ? (<ChatTypingIndicator label={typingIndicator} variant="bubble" className="mt-2"/>) : null}
