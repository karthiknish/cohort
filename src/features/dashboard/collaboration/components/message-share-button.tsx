    ...
    return (<TooltipProvider delayDuration={200}>
      <DropdownMenu>
        <Tooltip>
          <TooltipTrigger asChild>
            <DropdownMenuTrigger asChild>
              <Button type="button" variant="ghost" size="icon" className="size-7" disabled={isSharing} aria-label={`Share message from ${message.senderName ?? 'sender'} via email`}>
        ...
        <DropdownMenuContent align="end" className="w-48">
          <DropdownMenuItem onClick={handleEmailShareClick} disabled={isSharing || isSharedToEmail} className="gap-2">
