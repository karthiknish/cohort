...
describe('CreateProjectFormFields', () => {
    it('renders the create-project fields and tag count', () => {
        const markup = renderToStaticMarkup(<CreateProjectFormFields loading={false} clients={clients} state={state} onNameChange={vi.fn()} onDescriptionChange={vi.fn()} onStatusChange={vi.fn()} onClientChange={vi.fn()} onStartDateChange={vi.fn()} onEndDateChange={vi.fn()} onTagInputChange={vi.fn()} onTagKeyDown={vi.fn()} onAddTag={vi.fn()} onRemoveTag={vi.fn()} formatStatusLabel={formatStatusLabel}/>);
