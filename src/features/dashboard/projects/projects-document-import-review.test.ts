...
import { getProjectImportReviewBlocker, projectNeedsClientReview, } from './projects-document-import-review';
...
describe('projectNeedsClientReview', () => {
    it('flags ambiguous client matches', () => {
        expect(projectNeedsClientReview(buildProject({ clientStatus: 'ambiguous', clientId: '', suggestions: ['Acme Corp', 'Acme LLC'] }))).toBe(true);
    });
});
describe('getProjectImportReviewBlocker', () => {
    it('requires at least one selected project', () => {
        expect(getProjectImportReviewBlocker([buildProject({ include: false })])).toBe('Select at least one project to create.');
    });
});
