...
export function ProjectsListState({ error, hasActiveFilters, hasVisibleProjects, initialLoading, loading, onClearAllFilters, onDelete, onEdit, onRefresh, onSearchClear, onUpdateStatus, pendingStatusUpdates, projects, searchInput, sortedProjects, viewMode, onClearFocusAndFilters, hasMoreProjects, loadingMore, onLoadMore, }: ProjectsListStateProps) {
    ...
    if (error) {
        return (<div className="rounded-xl border border-destructive/40 bg-destructive/10 p-8 text-center">
    ...
    if (projects.length === 0 && !hasActiveFilters) {
        return (<div className={PROJECTS_THEME.emptyPanel}>
    ...
    if (!hasVisibleProjects && hasActiveFilters) {
        ...
        <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
          {searchInput ? (<Button type="button" variant="outline" size="sm" onClick={onSearchClear}>
              Clear search
            </Button>) : null}
    ...
    const loadMoreFooter = hasMoreProjects ? (<div className="flex justify-center border-t border-border/50 py-4">
    ...
    if (viewMode === 'list') {
        return (<div className="space-y-3 py-2">
        {sortedProjects.map((project) => (<ProjectRow key={project.id} project={project} onDelete={onDelete} onEdit={onEdit} onUpdateStatus={onUpdateStatus} isPendingUpdate={pendingStatusUpdates.has(project.id)}/>))}
        {loadMoreFooter}
      </div>);
    }
    if (viewMode === 'grid') {
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {sortedProjects.map((project) => (<ProjectCard key={project.id} project={project} onDelete={onDelete} onEdit={onEdit} onUpdateStatus={onUpdateStatus} isPendingUpdate={pendingStatusUpdates.has(project.id)}/>))}
        </div>
        {loadMoreFooter}
      </div>);
    }
    return (<div className="space-y-3">
      <ProjectKanban projects={sortedProjects} pendingStatusUpdates={pendingStatusUpdates} onUpdateStatus={onUpdateStatus} onEdit={onEdit} onDelete={onDelete}/>
      {loadMoreFooter}
    </div>);
}
