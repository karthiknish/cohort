...
export function ProjectTaskProgress({ project, className }: ProjectTaskProgressProps) {
    ...
    return (<div className={cn('space-y-1.5', className)}>
      <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
        ...
        {total > 0 ? (<span className="tabular-nums text-[10px] font-semibold text-muted-foreground/70">{progress}% done</span>) : null}
      </div>
      <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted/50 ring-1 ring-border/30">
        <div className="h-full rounded-full bg-linear-to-r from-primary/70 to-primary motion-chromatic" style={progressStyle}/>
      </div>
    </div>);
}
