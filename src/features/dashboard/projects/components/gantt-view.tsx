...
import { STATUS_CLASSES, STATUS_ICONS, formatStatusLabel, computeTimelineRange, parseDate, addDays, formatShortDate, milestoneStatusColor } from './utils';
...
export function GanttView({ projects, milestones, loading, error, onRefresh, onMilestoneCreated }: GanttViewProps) {
    ...
    if (loading) {
        ...
        {loadingSlots.map((slot) => (<div key={slot} className="flex items-center gap-3">
    ...
    if (projects.length === 0) {
        return (<div className="rounded-md border border-dashed border-muted/60 bg-muted/10 p-8 text-center">
    ...
    return (<div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-muted-foreground">
        ...
        <div className="flex items-center gap-2 text-xs">
          {MILESTONE_STATUSES.map((status) => (<div key={status} className="inline-flex items-center gap-1 rounded-full border px-2 py-1">
      ...
      <div className="grid grid-cols-[240px_1fr] items-start gap-2">
        <div className="space-y-2">
          ...
          {projects.map((project) => (<ProjectTimelineRow key={project.id} project={project} onMilestoneCreated={onMilestoneCreated}/>))}
        </div>
            ...
            <div className="relative">
              <div className="relative bg-background" style={timelineStyle}>
                ...
                <div className="divide-y">
                  {projects.map((project) => (<div key={project.id} className="relative h-16">
                      <ProjectMilestones milestoneList={milestones[project.id] ?? []} start={start} dayWidth={dayWidth} totalDays={totalDays}/>
                    </div>))}
                </div>
              </div>
            </div>
...
function ProjectMilestones({ milestoneList, start, dayWidth, totalDays, }: {
    ...
    return milestoneList.map((milestone) => (<MilestoneBar key={milestone.id} milestone={milestone} start={start} dayWidth={dayWidth} totalDays={totalDays}/>));
}
...
function MilestoneBar({ milestone, start, dayWidth, totalDays, }: {
    ...
    return (<div className="absolute top-2 flex h-10 items-center rounded-md px-3 text-xs text-primary-foreground shadow-sm" style={barStyle} title={`${milestone.title} • ${startLabel} → ${endLabel}`}>
...
function TimelineGrid({ start, totalDays, dayWidth }: {
    ...
    return (<div className="relative">
      ...
      <div className="absolute inset-0 pointer-events-none">
        {Array.from({ length: totalDays }, (_, index) => `day-${index + 1}`).map((slot, index) => (<TimelineDayCell key={slot} left={index * dayWidth} width={dayWidth}/>))}
      </div>
    </div>);
}
function TimelineWeekCell({ weekNumber, label, width }: {
    ...
    return (<div className="border-r px-2 py-1" style={weekStyle}>
...
function TodayMarker({ start, dayWidth }: {
    ...
    if (offsetDays < 0)
        return null;
    return (<div className="pointer-events-none absolute inset-y-0" style={markerStyle}>
...
function StatusPill({ status }: {
    ...
    return (<span className={cn('inline-flex items-center gap-1 rounded-full px-2 py-1 text-[11px] font-medium', STATUS_CLASSES[status])}>
