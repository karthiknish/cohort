...
export function ProjectsPageSkeleton() {
    ...
    return (<div className={PROJECTS_THEME.page}>
      <DashboardPageHero>
      ...
      <div className={PROJECTS_THEME.summaryStrip}>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {summarySlots.map((slot) => (<Skeleton key={slot} className="h-[5.5rem] rounded-xl"/>))}
        </div>
      </div>

      <div className="flex flex-wrap gap-2">
        {pillSlots.map((slot) => (<Skeleton key={slot} className="h-8 w-24 rounded-full"/>))}
      </div>
      ...
      <div className={PROJECTS_THEME.workspace}>
        ...
        <div className="space-y-3 p-4">
          {rowSlots.map((slot) => (<Skeleton key={slot} className="h-28 w-full rounded-xl"/>))}
        </div>
      </div>
    </div>);
}
