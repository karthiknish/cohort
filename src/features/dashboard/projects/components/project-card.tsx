...
function ProjectCardComponent({ project, onDelete, onEdit, onUpdateStatus, isPendingUpdate, compact = false, kanban = false, }: ProjectCardProps) {
    ...
    return (<ViewTransition>
      <article className={cn('group relative flex flex-col justify-between overflow-hidden rounded-xl border border-border/60 bg-card shadow-sm ring-1 ring-border/30', 'transition-[border-color,box-shadow,transform] hover:border-primary/20 hover:shadow-md', listItemEnterClass, clickableCardClass, compact ? 'p-3.5' : 'p-5', isPendingUpdate && 'pointer-events-none opacity-75')}>
        ...
        <div className="space-y-3">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0 flex-1 space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                <h3 className={cn('font-semibold leading-tight text-foreground line-clamp-1 group-hover:text-primary transition-colors', compact ? 'text-sm' : 'text-lg')}>
                  {project.name}
                </h3>
              ...
              {project.clientName ? (<p className="text-xs text-muted-foreground">
                  {project.clientId ? (<Link href={`/dashboard/clients?clientId=${project.clientId}`} className="font-medium hover:text-primary hover:underline">
                      {project.clientName}
            ...
            <div className="flex shrink-0 items-center gap-1">
              <Tooltip>
                <TooltipTrigger asChild>
                  <Badge variant="outline" className={cn(STATUS_CLASSES[project.status], 'h-6 gap-1 border px-2 py-0')}>
            ...
            : kanban
              ...
              <ProjectActionsMenu project={project} onEdit={onEdit} onDelete={onDelete} onUpdateStatus={onUpdateStatus}/>
            </div>
          </div>

          {!compact && project.description ? (<p className="text-sm text-muted-foreground/80 line-clamp-2 leading-relaxed min-h-[2.5rem]">
              {project.description}
            </p>) : null}
          ...
          {project.tags.length > 0 && !compact ? (<div className="flex flex-wrap gap-1.5">
              {project.tags.slice(0, 3).map((tag) => (<Badge key={tag} variant="secondary" className="h-5 px-2 text-[10px] font-medium">
                  #{tag}
                </Badge>))}
              {project.tags.length > 3 ? (<span className="text-[10px] text-muted-foreground">+{project.tags.length - 3}</span>) : null}
            </div>) : null}
        </div>

        <div className={cn('flex items-center gap-2 border-t border-muted/30', compact ? 'mt-3 pt-3' : 'mt-4 pt-4')}>
