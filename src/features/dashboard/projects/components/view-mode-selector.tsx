...
export function ViewModeSelector({ viewMode, onChange }: ViewModeSelectorProps) {
    return (<fieldset className={cn(PROJECTS_THEME.segmented, 'm-0 min-w-0 border-0 p-0')} aria-label="Project view mode">
      {VIEW_OPTIONS.map(({ mode, label, icon: Icon }) => (<ViewModeButton key={mode} mode={mode} active={viewMode === mode} label={label} icon={Icon} onChange={onChange}/>))}
    </fieldset>);
}
function ViewModeButton({ mode, active, label, icon: Icon, onChange, }: {
    ...
    return (<Tooltip>
      <TooltipTrigger asChild>
        <button type="button" className={PROJECTS_THEME.segmentedItem(active)} onClick={onSelectViewMode} aria-label={label} aria-pressed={active}>
