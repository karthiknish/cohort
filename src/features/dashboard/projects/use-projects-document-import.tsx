...
} from '../tasks/lib/extract-document-text'
import { uploadTaskImportDocument } from '../tasks/lib/upload-import-document'
import type {
  ProposedImportProject,
  ProposedImportProjectFromServer,
  ProjectDocumentImportPhase,
} from './projects-document-import-types'
import {
  getProjectImportReviewBlocker,
  needsProjectImportReview,
  projectNeedsClientReview,
  projectNeedsEndDateReview,
  projectNeedsStartDateReview,
...
export function useProjectsDocumentImport({
  workspaceId,
  ownerId,
  clients,
  preferredClientId,
  ...
  const createProjectsFromProposals = useCallback(
    async (projects: ProposedImportProject[]) => {
  ...
  const runDocumentImport = useCallback(
    async (files: File[]) => {
      ...
      try {
        ...
        if (needsProjectImportReview(mapped)) {
          ...
          const dateIssues = mapped.filter(
            (project) => projectNeedsStartDateReview(project) || projectNeedsEndDateReview(project),
          ).length
    ...
    [
      createProjectsFromProposals,
      disabledReason,
      extractPdfOnServer,
      extractProjectsFromDocument,
      isPreviewMode,
      preferredClientId,
      preferredClientName,
      uploadForVision,
  ...
  const updateProposedProject = useCallback((localId: string, patch: Partial<ProposedImportProject>) => {
    setProposedProjects((current) =>
      current.map((project) => (project.localId === localId ? { ...project, ...patch } : project)),
    )
  }, [])
