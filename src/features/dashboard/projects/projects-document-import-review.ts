...
export function projectNeedsImportReview(project: ProposedImportProject): boolean {
  return (
    projectNeedsClientReview(project) ||
...
export function getProjectImportReviewBlocker(projects: ProposedImportProject[]): string | null {
  ...
  const unclearStartDates = selected.filter(
    (project) => projectNeedsStartDateReview(project) && !project.startDate.trim(),
  )
  ...
  const unclearEndDates = selected.filter(
    (project) => projectNeedsEndDateReview(project) && !project.endDate.trim(),
  )
...
export function buildProjectImportReviewDescription(
  documentSummary: string | null,
  projects: ProposedImportProject[],
  ...
  const needsDates = projects.some(
