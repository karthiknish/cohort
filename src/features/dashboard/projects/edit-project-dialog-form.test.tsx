...
describe('EditProjectFormFields', () => {
    it('renders the main fields and tag summary', () => {
        const markup = renderToStaticMarkup(<EditProjectFormFields loading={false} name="Website refresh" description="Refresh the public site." status="active" clientId="client-1" startDate={PROJECT_START_DATE} endDate={PROJECT_END_DATE} tags={['design', 'marketing']} tagInput="seo" validationErrors={validationErrors} clients={clients} onDispatch={vi.fn()} onAddTag={vi.fn()} onRemoveTag={vi.fn()} onTagKeyDown={vi.fn()} formatStatusLabel={formatStatusLabel}/>);
