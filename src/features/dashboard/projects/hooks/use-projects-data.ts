...
export function useProjectsData({ workspaceId, userId, isPreviewMode, selectedClientId, statusFilter, debouncedSearchQuery, }: UseProjectsDataArgs) {
    ...
    const { mergedItems: paginatedProjects, nextCursor: projectsNextCursor, isInitialLoading: projectsInitialLoading, isLoadingMore: projectsLoadingMore, loadMore: loadMoreProjects, reset: resetProjectPagination, } = useAccumulatedCursorPages<ProjectRecord, ProjectPageCursor>({
    ...
    const projectsWithTaskCounts = useMemo(() => {
        if (isPreviewMode) {
            ...
            if (searchQuery) {
                ...
                previewProjects = previewProjects.filter((project) => project.name.toLowerCase().includes(query) ||
                    project.description?.toLowerCase().includes(query) ||
                    project.clientName?.toLowerCase().includes(query));
            }
            return previewProjects;
        }
        const counts = taskCountsRealtime && typeof taskCountsRealtime === 'object'
            ? (taskCountsRealtime as Record<string, {
                taskCount: number;
