...
export function useProjectsMutations({
  ...
  }, [isPreviewMode, projectToDelete, setProjects, softDeleteProject, toast, userId, workspaceId])

  const handleUpdateStatus = useCallback(
    async (project: ProjectRecord, newStatus: ProjectStatus) => {
      if (isPreviewMode) {
        setProjects((previous) =>
          previous.map((current) => (current.id === project.id ? { ...current, status: newStatus } : current)),
        )
        toast({
          title: 'Preview mode',
    ...
    [isPreviewMode, pendingStatusUpdates, setProjects, toast, updateProject, userId, workspaceId],
  )
