...
export function ImportReviewProjectRow({
  project,
  index,
  clients,
  preferredClientName,
  onUpdateProject,
}: ImportReviewProjectRowProps) {
  ...
  const handleStartDateChange = useCallback(
    ...
    [onUpdateProject, project.localId, project.startDateStatus],
  )
  ...
  const handleEndDateChange = useCallback(
    ...
    [onUpdateProject, project.localId, project.endDateStatus],
  )
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {project.clientStatus === 'preferred' && resolvedClientName ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            {needsStartDateReview ? (
              <div className="space-y-2">
                <Label htmlFor={`import-project-start-${project.localId}`}>Start date</Label>
                {project.startDateHint ? (
                  <p className="text-xs text-muted-foreground">Document hint: {project.startDateHint}</p>
                ) : null}
                <Input
            ...
            {needsEndDateReview ? (
              <div className="space-y-2">
                ...
                {project.endDateHint ? (
                  <p className="text-xs text-muted-foreground">Document hint: {project.endDateHint}</p>
                ) : null}
                <Input
          ...
          {project.sourceExcerpt ? (
            <p className="text-xs italic text-muted-foreground">“{project.sourceExcerpt}”</p>
          ) : null}
        </div>
      </div>
    </article>
