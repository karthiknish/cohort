...
export function TaskBulkToolbar({ selectedCount, totalVisible, hasSelection, bulkActive, bulkLabel, bulkProgress, onSelectAll, onClearSelection, onSelectHighPriority, onSelectDueSoon, highPriorityCount, dueSoonCount, onBulkStatusChange, onBulkAssign, onBulkDueDate, onBulkDelete, }: TaskBulkToolbarProps) {
    ...
    const handleAssignInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    return (<div className="space-y-3 border-b border-border/50 bg-primary/[0.03] px-4 py-3">
      <div className="flex flex-wrap items-center gap-3 text-sm">
        <div className="inline-flex items-center gap-2 rounded-xl border border-border/60 bg-background px-3 py-2 shadow-sm">
          <Checkbox ref={masterRef} checked={masterChecked} onChange={handleMasterChange} aria-label={selectAllLabel}/>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          <Select onValueChange={handleStatusChange} disabled={!hasSelection || bulkActive}>
            ...
            <SelectContent>
              {TASK_STATUSES.map((status) => (<SelectItem key={status} value={status}>
                  {status === 'todo' && 'To do'}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Input id="bulk-assign" value={assignInput} onChange={handleAssignInputChange} placeholder="Comma-separated names" className="h-9 w-[220px]" disabled={!hasSelection || bulkActive}/>
          <Button size="sm" variant="outline" onClick={handleAssignClick} disabled={!hasSelection || bulkActive}>
            Apply
          </Button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" size="sm" className={cn('w-[160px] h-9 justify-start text-left font-normal', !dueDate && 'text-muted-foreground')} disabled={!hasSelection || bulkActive}>
                <CalendarIcon className="mr-2 size-4"/>
                {dueDate ? format(dueDate, 'PP') : <span>Pick a date</span>}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar mode="single" selected={dueDate} disabled={isTaskDueDateDisabled} onSelect={setDueDate} initialFocus/>
            </PopoverContent>
          </Popover>
          <Button size="sm" variant="outline" onClick={handleUpdateDueDate} disabled={!hasSelection || bulkActive || !dueDate}>
            Update
          </Button>
          <Button size="sm" variant="ghost" onClick={handleClearDate} disabled={!hasSelection || bulkActive}>
            Clear date
          </Button>
        </div>

        <div className="flex items-center gap-2">
          <Button size="sm" variant="destructive" onClick={onBulkDelete} disabled={!hasSelection || bulkActive}>
            <Trash2 className="mr-2 size-4"/> Delete
          </Button>
        </div>
      </div>

      {bulkActive && (<div className="flex flex-col gap-2 rounded-md border border-muted/50 bg-background px-3 py-2">
