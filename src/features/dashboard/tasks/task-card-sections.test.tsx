...
describe('task card sections', () => {
    it('renders the header context and indicators', () => {
        const markup = renderToStaticMarkup(<TaskCardHeaderSection task={task} isPendingUpdate={false} onOpen={vi.fn()} searchQuery="" highlightMatch={highlightMatch} onEdit={vi.fn()} onDelete={vi.fn()} onQuickStatusChange={vi.fn()} onClone={vi.fn()} onShare={vi.fn()}/>);
