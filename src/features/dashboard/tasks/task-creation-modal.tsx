...
export function TaskCreationModal({ isOpen, onClose, initialData, onTaskCreated, }: TaskCreationModalProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        startTransition(async () => {
            ...
            const attachmentsPromise = pendingAttachments.length > 0
                ? Promise.all(pendingAttachments.map((attachment) => uploadTaskAttachment({
                    userId: user.id,
    ...
    return (<ResponsiveFormSheet open={isOpen} onOpenChange={handleOpenChange} contentClassName={TASKS_THEME.sheet.content}>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                {contextInfo.isAutoPopulated ? (<span className="text-success">Pre-filled from your current workspace context.</span>) : ('Add a task without leaving this screen.')}
              </p>
            </div>
          </div>
        </div>
