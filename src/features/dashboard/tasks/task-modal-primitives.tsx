...
export function TaskSheetHeader({ icon: Icon, title, description, }: {
    icon: LucideIcon;
    title: string;
    description?: string;
}) {
    return (<div className={TASKS_THEME.sheet.header}>
...
export function TaskFormSection({ title, children, className, }: {
    title: string;
    children: ReactNode;
    className?: string;
}) {
    return (<section className={cn(TASKS_THEME.formSection, className)} aria-labelledby={undefined}>
...
export function TaskContextChip({ children }: {
    children: ReactNode;
}) {
    return <div className={TASKS_THEME.contextChip}>{children}</div>;
}
export function TaskModalError({ message }: {
    message: string;
}) {
    return (<div className={TASKS_THEME.error} role="alert">
      {message}
    </div>);
}
export function TaskModalActions({ onCancel, cancelLabel = 'Cancel', submitLabel, loadingLabel, isLoading, submitDisabled, }: {
    onCancel: () => void;
    cancelLabel?: string;
    submitLabel: string;
    loadingLabel?: string;
