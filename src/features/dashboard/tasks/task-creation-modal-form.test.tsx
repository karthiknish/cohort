...
describe('TaskCreationModalFormFields', () => {
    it('renders modal form sections and summaries', () => {
        const markup = renderToStaticMarkup(<TaskCreationModalFormFields title="Plan launch retro" description="Prepare questions and stakeholders." priority="urgent" dueDate="2026-03-20" projectName="Spring Launch" clientName="Acme Corp" assigneeCount={2} error={null} isLoading={false} pendingAttachments={[]} fileInputRef={EMPTY_FILE_INPUT_REF} onTitleChange={vi.fn()} onDescriptionChange={vi.fn()} onPriorityChange={vi.fn()} onDateSelect={vi.fn()} onAddAttachments={vi.fn()} onRemoveAttachment={vi.fn()} onCancel={vi.fn()}/>);
