...
export function TasksHeader({ loading, retryCount, onRefresh, onNewTaskClick, scopeLabel, scopeHelper, newTaskDisabledReason = null, }: TasksHeaderProps) {
    ...
    return (<DashboardPageHero>
      <div className="min-w-0 space-y-2">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
              ...
              {scopeLabel ? (<span className="truncate text-sm font-medium text-muted-foreground">/ {scopeLabel}</span>) : null}
            </div>
        ...
        {retryCount > 0 ? (<p className="text-xs font-medium text-warning">
            Retrying… {retryCount}/{RETRY_CONFIG.maxRetries}
          </p>) : null}
        {newTaskDisabledReason ? (<p className="max-w-2xl text-xs text-warning">
            {newTaskDisabledReason}
          </p>) : null}
      </div>

      <div className="flex shrink-0 items-center gap-2">
        <Button type="button" variant="outline" size="sm" className={cn(getButtonClasses('outline'), 'h-9 gap-1.5')} onClick={onRefresh} disabled={loading} aria-busy={loading}>
          {loading ? (<LoaderCircle className="size-4 animate-spin" aria-hidden/>) : (<RefreshCw className="size-4" aria-hidden/>)}
          <span className="hidden sm:inline">Refresh</span>
        </Button>
        <Tooltip>
          <TooltipTrigger asChild>
            <Button type="button" size="sm" className={cn(getButtonClasses('primary'), 'h-9 gap-1.5 shadow-none')} onClick={onNewTaskClick} disabled={Boolean(newTaskDisabledReason)}>
              <Plus className="size-4" aria-hidden/>
              New task
            </Button>
          </TooltipTrigger>
          <TooltipContent className="max-w-xs text-balance">
            {newTaskDisabledReason ? (newTaskDisabledReason) : (<span className="flex flex-wrap items-center gap-2">
                Create a task
