...
const stripStyles: Record<TaskStatus, {
    ring: string;
    accent: string;
    text: string;
    hover: string;
...
function StatusChip({ status, count, isSelected, onSelect, }: {
    ...
    return (<button type="button" className={className} onClick={onSelectStatusSummary} onKeyDown={handleKeyDown} aria-pressed={isSelected} aria-label={`${formatStatusLabel(status)}: ${count} tasks`}>
      {content}
    </button>);
}
