...
function TaskKeyCell({ row }: CellContext<TaskRecord, unknown>) {
  return (
    <span className="truncate font-mono text-xs text-muted-foreground">{formatTaskKey(row.original.id)}</span>
  )
}
...
export function TaskDataTable({
  ...
  const columns = useMemo(() => createTaskColumns(showSelection), [showSelection])

  const actions = useMemo<TaskDataTableActions>(
    () => ({
      pendingStatusUpdates,
      selectedTaskIds,
      onOpen,
  ...
  const rowClassName = useCallback(
    (task: TaskRecord) =>
      cn(
        pendingStatusUpdates.has(task.id) && 'pointer-events-none opacity-60',
        selectedTaskIds?.has(task.id) && 'bg-primary/4',
        task.status === 'completed' && 'opacity-90',
      ),
    [pendingStatusUpdates, selectedTaskIds],
  )

  const getRowId = useCallback((task: TaskRecord) => task.id, [])
