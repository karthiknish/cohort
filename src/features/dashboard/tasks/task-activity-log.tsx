...
export function TaskActivityLog({ activities, loading, className, maxItems = 20 }: TaskActivityLogProps) {
    ...
    if (loading) {
        ...
        {loadingSlots.map((slot) => (<div key={slot} className="flex gap-3">
    ...
    return (<div className={cn('space-y-0', className)}>
      <ScrollArea className="h-[300px] pr-4">
        <div className="space-y-4">
          {displayedActivities.map((activity, index) => {
            ...
            return (<div key={activity.id} className="flex gap-3">
                {/* Timeline connector line */}
                <div className="flex flex-col items-center">
                  <div className={cn('flex size-8 shrink-0 items-center justify-center rounded-full border', ACTIVITY_COLORS[activity.action])}>
                    <Icon className="size-4"/>
                  </div>
                  {!isLast && (<div className="w-px flex-1 bg-muted min-h-[2rem] mt-1"/>)}
                </div>
      ...
      {activities.length > maxItems && (<div className="pt-2 text-center">
