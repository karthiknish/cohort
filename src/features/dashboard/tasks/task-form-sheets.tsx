      ...
      <form className="flex h-full min-h-0 flex-col" onSubmit={onSubmit}>
        ...
        <div className={TASKS_THEME.sheet.body}>
          ...
          {taskId ? (<section className={TASKS_THEME.formSection}>
              ...
              <TaskCommentsPanel taskId={taskId} workspaceId={currentWorkspaceId} userId={currentUserId} userName={currentUserName} userRole={currentUserRole} participants={participants}/>
            </section>) : null}
        </div>
        <div className={TASKS_THEME.sheet.footer}>
          <Button type="submit" disabled={updating} className={TASKS_THEME.footerPrimary}>
            {updating ? (<span className="inline-flex items-center gap-2">
