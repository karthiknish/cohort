...
import { buildTaskImportFileName, combineExtractedDocumentText, filterTasksDocumentFiles, isTasksDocumentFile, isTasksVisualDocumentFile, } from './extract-document-text';
describe('isTasksDocumentFile', () => {
    it('accepts pdf, word, and image files by extension and mime type', () => {
        ...
        expect(isTasksDocumentFile(new File([''], 'brief.docx', {
...
describe('buildTaskImportFileName', () => {
    it('uses a single file name or a count summary', () => {
        ...
        expect(buildTaskImportFileName([
            new File([''], 'a.pdf'),
            new File([''], 'b.png'),
        ])).toBe('2 documents');
    });
