...
import { TaskCommentsComposerSection, TaskCommentsDeleteDialog, TaskCommentsSummaryHeader, TaskCommentsThreadList, type TaskCommentComposerAttachment, } from './task-comments-sections';
...
export function taskCommentsPanelReducer(state: TaskCommentsPanelState, action: TaskCommentsPanelAction): TaskCommentsPanelState {
    switch (action.type) {
...
export function useTaskCommentsPanel(props: TaskCommentsPanelProps) {
    ...
    const handleStartReply = (comment: TaskComment) => {
    ...
    const composerTitle = draftMode === 'edit'
        ? 'Editing comment'
        : draftMode === 'reply'
            ? `Replying to ${activeReplyTo?.authorName ?? 'thread'}`
            : 'New comment';
    const composerDescription = draftMode === 'edit'
    ...
    const composerPlaceholder = draftMode === 'edit'
        ? 'Refine your comment...'
        : draftMode === 'reply'
    ...
    const handleRequestDelete = (comment: TaskComment) => {
