...
function highlightMatch(text: string, query: string): React.ReactNode {
    ...
    return parts.map((part) => {
        ...
        cursor += part.length;
        if (isMatch) {
            return (<mark key={key} className="rounded bg-accent px-0.5 text-accent-foreground">
          {part}
        </mark>);
        }
        return part;
...
function TaskCardComponent({ task, variant = 'grid', isPendingUpdate, onOpen, onEdit, onDelete, onQuickStatusChange, onClone, onShare, selected = false, searchQuery = '', }: TaskCardProps) {
        ...
        <div className={cn('flex flex-1 flex-col', isBoard ? 'gap-2.5' : 'gap-3')}>
          <div className="flex items-start justify-between gap-2">
            <div className="flex min-w-0 flex-wrap items-center gap-1.5">
              {isBoard ? (<TaskCardPriorityBadge priority={task.priority}/>) : (<>
                  <TaskCardStatusBadge status={task.status}/>
                  <TaskCardPriorityBadge priority={task.priority}/>
                </>)}
            </div>
            <TaskCardHeaderSection {...headerProps} visibility={TASK_CARD_MENU_VISIBILITY}/>
          </div>

          <TaskCardHeaderSection {...headerProps} visibility={titleVisibility} titleClassName={isBoard ? 'text-sm' : undefined}/>

          {!isBoard && task.description ? (<p className="line-clamp-2 text-sm leading-6 text-muted-foreground">
              {highlightMatch(task.description, searchQuery)}
            </p>) : null}

          {isBoard ? (<TaskCardHeaderSection {...headerProps} visibility={TASK_CARD_BOARD_META_VISIBILITY}/>) : null}
...
export const TaskCard = Object.assign(TaskCardComponent, { displayName: 'TaskCard' });
