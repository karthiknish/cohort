...
describe('task comment sections', () => {
    it('renders the summary header state', () => {
        const markup = renderToStaticMarkup(<TaskCommentsSummaryHeader commentsCount={2} replyCount={1} replyTo={rootComment} editingCommentId={null}/>);
    ...
    it('renders threaded comment cards', () => {
        const markup = renderToStaticMarkup(<TaskCommentsThreadList loading={false} roots={[rootComment]} repliesByParent={new Map([['comment-1', [replyComment]]])} replyToId={null} editingCommentId={null} deletingCommentId={null} canManageComment={canManageComment} onStartReply={vi.fn()} onStartEdit={vi.fn()} onRequestDelete={vi.fn()}/>);
