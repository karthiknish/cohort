...
export function useTaskForm({ selectedClient, selectedClientId, projectContext, userId, participants = [], initialCreateOpen = false, onCreateOpenChange, onCreateTask, onUpdateTask, }: UseTaskFormOptions): UseTaskFormReturn {
    ...
    if (formState.clientId !== nextClientId ||
    ...
    const handleCreateAttachmentsAdd = (files: FileList | null) => {
        if (!files || files.length === 0)
            return;
        const next = buildPendingTaskAttachments(files);
        setCreateAttachments((prev) => [...prev, ...next].slice(0, 10));
    };
    const handleCreateAttachmentRemove = (attachmentId: string) => {
    ...
    const handleCreateSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            if (createAttachments.length > 0) {
                const uploadedAttachments = await Promise.all(createAttachments.map((attachment) => uploadTaskAttachment({
                    userId,
