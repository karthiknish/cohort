...
export const buildInitialFormState = (client?: {
    id: string | null;
    name: string | null;
}, project?: {
    id: string | null;
...
export function clientRosterAssigneeNames(client: {
...
export function teamMembersToMentionable(members: TaskParticipant[]): Array<{
    id: string;
    name: string;
    role?: string;
}> {
...
export function formatAssigneeDraftFromUserIds(userIds: string[], participants: TaskParticipant[]): string {
    return userIds
        .flatMap((userId) => {
...
export function resolveAssigneeUserIds(value: string, participants: TaskParticipant[]): string[] {
...
export function getAssigneeDraftIssue(value: string, participants: TaskParticipant[]): string | null {
    const names = parseMentionNames(value);
    if (names.length === 0)
        return null;
...
export function resolveAssigneeLabel(value: string, participants: TaskParticipant[]): string {
    const byId = participants.find((participant) => participant.id === value);
    if (byId?.name)
        return byId.name;
    const byName = participants.find((participant) => participant.name.trim().toLowerCase() === value.trim().toLowerCase());
    return byName?.name ?? value;
}
export function formatAssigneeList(assignees: string[], participants: TaskParticipant[]): string {
