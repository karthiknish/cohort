...
describe('task list sections', () => {
    ...
    it('renders task items in grid and list modes', () => {
        const markup = renderToStaticMarkup(<>
        <TaskListItems onDelete={vi.fn()} onEdit={vi.fn()} onOpen={vi.fn()} onQuickStatusChange={vi.fn()} pendingStatusUpdates={new Set()} tasks={[task]} viewMode="grid"/>
        <TaskListItems onDelete={vi.fn()} onEdit={vi.fn()} onOpen={vi.fn()} onQuickStatusChange={vi.fn()} pendingStatusUpdates={new Set()} tasks={[task]} viewMode="list"/>
      </>);
