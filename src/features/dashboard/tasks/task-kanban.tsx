...
export function TaskKanban({ tasks, loading, initialLoading, error, pendingStatusUpdates, onEdit, onDelete, onQuickStatusChange, onRefresh, loadingMore, hasMore, onLoadMore, emptyStateMessage, showEmptyStateFiltered, onEmptyClearFilters, onEmptyCreateTask, onClone, onShare, searchQuery = '', selectedTaskIds = EMPTY_SELECTED_TASK_IDS, onToggleTaskSelection, bulkActive = false, workspaceId = null, userId = null, userName = null, userRole = null, participants = EMPTY_TASK_PARTICIPANTS, }: TaskKanbanProps) {
    ...
    const handleDragStart = (e: React.DragEvent, task: TaskRecord) => {
        if (bulkActive)
            return; // Disable drag during bulk operations
        dispatch({
            type: 'startDrag',
    ...
    const handleKeyboardMoveTask = (task: TaskRecord, direction: 'previous' | 'next') => {
    ...
    const handleViewingTaskDialogOpenChange = (open: boolean) => {
        if (!open) {
    ...
    if (initialLoading) {
        ...
        return (<div className="flex h-full min-h-[24rem] gap-3 px-4 pb-4">
    ...
    if (error) {
        return (<div className="flex h-full min-h-[24rem] items-center justify-center rounded-xl border border-destructive/20 bg-destructive/5 p-8 text-center">
        <div>
    ...
    if (tasks.length === 0) {
        return (<div className="flex h-full min-h-[24rem] items-center justify-center rounded-xl border border-dashed border-border/70 bg-muted/20 p-10 text-center sm:p-12">
        <div>
    ...
    return (<div className="flex h-full min-h-[24rem] flex-col gap-3 px-4 pb-4 pt-2">
      ...
      <TaskViewDialog task={viewingTask} open={!!viewingTask} workspaceId={workspaceId} userId={userId} userName={userName} userRole={userRole} participants={participants} onEdit={onEdit} onDelete={onDelete} onQuickStatusChange={onQuickStatusChange} onOpenChange={handleViewingTaskDialogOpenChange}/>
    </div>);
}
function KanbanColumn({ bulkActive, column, dragOverStatus, draggedTask, handleDragEnd, handleDragLeave, handleDragOver, handleDrop, handleDragStart, keyboardInstructionsId, onKeyboardMoveTask, handleViewTask, onClone, onDelete, onEdit, onQuickStatusChange, onShare, onToggleTaskSelection, pendingStatusUpdates, searchQuery, selectedTaskIds, }: {
    ...
    return (<section aria-label={`${column.label} task lane`} className={cn('flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-xl border border-border/70 bg-muted/15 shadow-sm transition-colors', isDragTarget && 'border-primary/30 bg-primary/5 ring-1 ring-primary/15', isDraggingFrom && !isDragTarget && 'opacity-60')} onDragOver={handleColumnDragOver} onDragLeave={handleDragLeave} onDrop={handleColumnDrop}>
      ...
      <ScrollArea className="min-h-0 flex-1 w-full">
        <ul className="list-none space-y-3 p-3 min-h-full">
