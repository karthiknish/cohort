...
export function TaskViewDialogHeader({ summary, title }: { summary: string; title: string }) {
  return (
    <DialogHeader className="border-b border-border/60 px-6 py-5">
      <DialogTitle className="text-2xl leading-tight text-foreground">{title}</DialogTitle>
...
export function TaskViewDialogTabsList({ commentCount }: { commentCount: number }) {
  return (
    <TabsList className="rounded-2xl border border-border/60 bg-muted/30 p-1">
      ...
      <TabsTrigger value="comments" className="rounded-xl px-4">
        Comments
        <span className="ml-2 inline-flex min-w-6 items-center justify-center rounded-full bg-background px-1.5 py-0.5 text-[11px] font-semibold text-muted-foreground shadow-sm">
          {commentCount}
        </span>
      </TabsTrigger>
    </TabsList>
  ...
  return (
    <TabsContent value="details" className="mt-0 space-y-6">
      <section className="space-y-2">
        ...
        <div className="rounded-2xl border border-border/60 bg-muted/30 p-4 text-sm leading-6 text-foreground shadow-sm">
          ...
          <div className="space-y-2">
            {(task.attachments ?? []).map((attachment) => (
              <div
                ...
                className="flex items-center justify-between gap-3 rounded-2xl border border-border/60 bg-background px-4 py-3 text-sm text-foreground shadow-sm"
              >
                <a
                  href={attachment.url}
                  target="_blank"
                ...
                <div className="flex items-center gap-2">
                  <span className="shrink-0 text-xs text-muted-foreground">{attachment.size ?? attachment.type ?? 'Open'}</span>
                  <Button asChild type="button" variant="outline" size="sm" className="h-8 rounded-xl">
  ...
  return (
    <TabsContent value="comments" className="mt-0 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h3 className="text-sm font-semibold uppercase tracking-[0.2em] text-muted-foreground">Comments</h3>
          <p className="mt-1 text-sm text-muted-foreground">Write updates and review the full task conversation here.</p>
        </div>
        <span className="inline-flex items-center gap-2 rounded-full border border-border/60 bg-muted/30 px-3 py-1 text-xs font-semibold text-muted-foreground">
