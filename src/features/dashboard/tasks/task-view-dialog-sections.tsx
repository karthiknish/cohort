...
function TaskStatusBadge({ status }: {
    ...
    return (<Badge variant="outline" className={cn('gap-1 rounded-md px-2 py-0.5 text-[11px] font-semibold', taskViewStatusPill[status])}>
...
function TaskPriorityBadge({ priority }: {
    ...
    return (<Badge variant="outline" className={cn('gap-1 rounded-md px-2 py-0.5 text-[11px] font-semibold', taskViewPriorityPill[priority])}>
      <ChevronUp className="size-3 shrink-0" aria-hidden/>
      {formatPriorityLabel(priority)}
    </Badge>);
}
export function TaskViewDialogHeader({ title, status, priority, client, assignedTo, dueDate, timeSpentMinutes, onEdit, onDelete, onQuickStatusChange, }: {
    title: string;
    status: TaskStatus;
    priority: TaskPriority;
    client?: string | null;
    ...
    return (<DialogHeader className={TASKS_THEME.viewDialog.header}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-wrap items-center gap-1.5">
          <TaskStatusBadge status={status}/>
          <TaskPriorityBadge priority={priority}/>
          {client ? (<Badge variant="outline" className={cn('rounded-md px-2 py-0.5 text-[11px] font-medium', taskPillColors.client)}>
              {client}
            </Badge>) : null}
        </div>
        {showMenu ? (<DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button type="button" variant="ghost" size="icon" className="size-8 shrink-0 text-muted-foreground" aria-label="Task options">
                <MoreHorizontal className="size-4"/>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-48">
              {onQuickStatusChange
                ? TASK_STATUSES.flatMap((s) => s !== status && s !== 'archived'
                    ? [(<TaskQuickStatusMenuItem key={s} nextStatus={s} onQuickStatusChange={onQuickStatusChange}/>)]
                    : [])
                : null}
...
export function TaskViewDialogTabsList({ commentCount }: {
    commentCount: number;
}) {
    return (<TabsList className={cn(TASKS_THEME.tabList, TASKS_THEME.viewDialog.tabList)}>
...
function DetailBlock({ label, children }: {
    ...
    return (<section className={cn(taskInfoPanelClasses.base, 'space-y-2')}>
      <h3 className={taskInfoPanelClasses.label}>{label}</h3>
      {children}
    </section>);
}
export function TaskViewDetailsTab({ task }: {
    task: TaskRecord;
}) {
    return (<TabsContent value="details" className="mt-0 flex h-full min-h-0 flex-col overflow-hidden focus-visible:outline-none data-[state=inactive]:hidden">
      <ScrollArea className="h-full max-h-[min(52vh,32rem)]">
        ...
        <p className={cn(taskInfoPanelClasses.value, 'font-normal leading-relaxed')}>
          {task.description?.trim() ? task.description : (<span className="text-muted-foreground">No description provided.</span>)}
        </p>
      </DetailBlock>

      {task.projectId || task.projectName ? (<DetailBlock label="Project">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
            {task.projectName ? (<span className="inline-flex items-center gap-1.5 font-medium text-foreground">
                <FolderKanban className="size-3.5 text-muted-foreground" aria-hidden/>
                {task.projectName}
              </span>) : null}
            {task.projectId ? (<>
                <Link href={buildProjectRoute(task.projectId, task.projectName)} className="text-primary underline-offset-4 hover:underline">
                  Open project
                </Link>
                <Link href={buildProjectTasksRoute({
      ...
      <DetailBlock label="Attachments">
        {(task.attachments ?? []).length > 0 ? (<ul className="divide-y divide-border text-sm">
            {(task.attachments ?? []).map((attachment) => (<li key={`${attachment.url}-${attachment.name}`} className="flex flex-wrap items-center justify-between gap-2 py-2 first:pt-0 last:pb-0">
                <a href={attachment.url} target="_blank" rel="noreferrer" className="inline-flex min-w-0 items-center gap-2 font-medium text-foreground hover:text-primary">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <a href={attachment.url} download={attachment.name} className="inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline">
                    <Download className="size-3"/>
                    Download
                  </a>
                </div>
              </li>))}
          </ul>) : (<p className="text-sm text-muted-foreground">No attachments.</p>)}
      </DetailBlock>
...
export function TaskViewDialogFooter({ onClose, onEdit, onMarkComplete, }: {
    onClose: () => void;
    onEdit?: () => void;
    onMarkComplete?: () => void;
}) {
    return (<DialogFooter className={TASKS_THEME.viewDialog.footer}>
      ...
      <div className="flex w-full flex-col-reverse gap-2 sm:w-auto sm:flex-row">
        ...
        {onMarkComplete ? (<Button type="button" variant="outline" onClick={onMarkComplete}>
