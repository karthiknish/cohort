...
describe('proposal template selector sections', () => {
    it('renders loading before empty and workspace guidance when templates cannot be managed', () => {
        const loadingMarkup = renderToStaticMarkup(<ProposalTemplateDropdownContent templates={[]} loading={true} deletingTemplateId={null} canManageTemplates={false} onApplyTemplate={vi.fn()} onDeleteTemplate={vi.fn()} onOpenSaveDialog={vi.fn()}/>);
    ...
    it('renders template rows with saved count', () => {
        const markup = renderToStaticMarkup(<ProposalTemplateDropdownContent templates={[{ id: 'tpl-1', name: 'SaaS Growth', description: 'For product-led SaaS', formData: {} as never, industry: 'SaaS', tags: ['growth'], isDefault: true, createdAt: null, updatedAt: null }]} loading={false} deletingTemplateId={null} canManageTemplates={true} onApplyTemplate={vi.fn()} onDeleteTemplate={vi.fn()} onOpenSaveDialog={vi.fn()}/>);
    ...
    it('renders save dialog guidance and fields', () => {
        const markup = renderToStaticMarkup(<ProposalTemplateSaveDialog open={true} saving={false} templateName="Starter" templateDescription="Good default" templateIndustry="E-commerce" isDefault={true} onOpenChange={vi.fn()} onTemplateNameChange={vi.fn()} onTemplateDescriptionChange={vi.fn()} onTemplateIndustryChange={vi.fn()} onDefaultChange={vi.fn()} onSave={vi.fn()}/>);
