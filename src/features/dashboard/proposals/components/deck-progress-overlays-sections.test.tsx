...
describe('deck progress overlay sections', () => {
    it('renders the proposal generation overlay content', () => {
        const markup = renderToStaticMarkup(<DeckProgressOverlayShell className="overlay-shell">
        <ProposalGenerationOverlayContent currentStageHelper="Writing tailored recommendations" currentStageLabel="Drafting strategy..." isComplete={false} progressPercent={60} stageIndex={2} stageLabels={['Analyzing', 'Gathering', 'Drafting', 'Formatting', 'Generating']}/>
    ...
    it('renders the simple deck progress states', () => {
        const markup = renderToStaticMarkup(<DeckProgressOverlayShell className="deck-shell">
        <DeckProgressOverlayContent stage="error" copy={errorCopy}/>
      </DeckProgressOverlayShell>);
