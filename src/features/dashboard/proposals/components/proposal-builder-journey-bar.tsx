...
export function ProposalBuilderJourneyBar({ isSubmitting, isRecheckingDeck, submitted, isPresentationReady, activeProposalIdForDeck, deckDownloadUrl, autosaveStatus, totalSteps, }: ProposalBuilderJourneyBarProps) {
    ...
    if (isSubmitting || isRecheckingDeck) {
        return (<output className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-primary/20 bg-primary/5 px-4 py-3" aria-live="polite">
    ...
    if (submitted && isPresentationReady) {
        return (<div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-success/25 bg-success/5 px-4 py-3">
        <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          {deckHref ? (<Button size="sm" asChild>
              <Link href={deckHref}>
    ...
    if (submitted) {
        return (<output className="flex items-center gap-3 rounded-xl border border-muted/50 bg-muted/20 px-4 py-3" aria-live="polite">
    ...
    const autosaveTone = autosaveStatus === 'error'
        ? 'text-destructive'
        : autosaveStatus === 'saving'
            ? 'text-primary'
            : 'text-muted-foreground';
    return (<div className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-border/50 bg-muted/15 px-3 py-2 text-xs">
