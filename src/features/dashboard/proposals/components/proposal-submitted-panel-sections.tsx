...
import { blobVariants, blobVariantsSlow, fadeInUpVariants, slideInLeftVariants, slideInRightVariants, subtlePulseVariants, transitions, } from '@/lib/motion';
...
export function ProposalSubmittedHero({ activeProposalIdForDeck, canResumeSubmission, deckDownloadUrl, isSubmitting, onResumeSubmission, }: {
    ...
    const presentationHref = activeProposalIdForDeck
        ...
        : null;
    return (<m.div initial="hidden" animate="visible" variants={fadeInUpVariants} className="relative overflow-hidden rounded-[2rem] border border-accent/20 bg-background p-10 shadow-2xl shadow-primary/5">
      ...
      <div className="relative flex flex-col items-center gap-10 md:flex-row">
        ...
        <div className="flex-1 space-y-4 text-center md:text-left">
          ...
          <div className="flex flex-wrap justify-center gap-4 pt-4 md:justify-start">
            {deckDownloadUrl && presentationHref ? (<Button size="lg" className="h-14 rounded-2xl bg-primary px-8 text-base font-bold shadow-xl shadow-primary/25 motion-chromatic hover:scale-105 hover:bg-accent/90 active:scale-95" asChild>
                <Link href={presentationHref} transitionTypes={['nav-forward']}>
                  <Presentation className="mr-3 size-6"/>
                  View Presentation
                </Link>
              </Button>) : null}
            {canResumeSubmission ? (<Button size="lg" variant="outline" className="h-14 rounded-2xl border-muted/60 px-8 text-base font-bold backdrop-blur-sm motion-chromatic hover:bg-muted/10" onClick={onResumeSubmission} disabled={isSubmitting}>
...
export function ProposalStrategyBriefCard({ onCopySummary, summary }: {
    onCopySummary: () => void;
    summary: ProposalFormData;
}) {
    return (<m.div initial="hidden" animate="visible" variants={slideInLeftVariants} transition={proposalSubmittedHeroTransition}>
          ...
          <div className="grid gap-5">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {summary.goals.objectives.length ? summary.goals.objectives.map((objective) => (<Badge key={objective} variant="secondary" className="border-accent/10 bg-accent/5 px-2 py-0.5 text-[10px] font-semibold text-primary">
                    {objective}
                  </Badge>)) : <span className="text-xs italic text-muted-foreground">None yet</span>}
              </div>
            </div>
...
function ProposalDeckReadyState({ activeProposalIdForDeck, deckDownloadUrl, isRecheckingDeck, onCopyShareLink, onRecheckDeck, presentationDeck, viewerHref, }: {
    ...
    return (<div className="flex h-full flex-col gap-6">
      <div className="grid gap-6 sm:grid-cols-2">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="space-y-3">
            ...
            {viewerHref ? (<Button variant="outline" className="group h-14 w-full justify-start rounded-2xl border-muted/60 motion-chromatic hover:border-accent/30 hover:bg-accent/[0.03]" asChild>
      ...
      <div className="mt-auto flex items-center justify-between border-t border-muted/40 pt-6">
        ...
        {onRecheckDeck && (presentationDeck.status === 'pending' || presentationDeck.status === 'processing') ? (<Button variant="ghost" size="sm" onClick={onRecheckDeck} disabled={isRecheckingDeck} className="h-8 rounded-xl border border-accent/10 px-4 text-[10px] font-black uppercase tracking-widest text-primary hover:bg-accent/5">
...
function ProposalDeckGeneratingState() {
    return (<div className="flex h-full flex-col items-center justify-center py-16 text-center">
      <div className="relative mb-6">
        <m.div animate={proposalPulseAnimate} transition={proposalPulseTransition} className="absolute inset-0 rounded-full bg-accent/20 blur-2xl"/>
...
export function ProposalAssetDeliveryCard({ activeProposalIdForDeck, deckDownloadUrl, isRecheckingDeck, onCopyShareLink, onRecheckDeck, presentationDeck, viewerHref, }: {
    activeProposalIdForDeck: string | null;
    deckDownloadUrl: string | null;
    isRecheckingDeck: boolean;
    onCopyShareLink: () => void;
    ...
    return (<m.div initial="hidden" animate="visible" variants={slideInRightVariants} transition={proposalAssetDeliveryTransition} className="lg:col-span-2">
      <Card className="flex h-full flex-col overflow-hidden border-muted/60 bg-background/50 shadow-sm backdrop-blur-sm">
        <CardHeader className="border-b border-muted/40 bg-muted/30 pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
            ...
            {presentationDeck ? (<Badge variant="outline" className={cn('h-6 rounded-lg px-2.5 text-[10px] font-bold uppercase tracking-[0.1em]', presentationDeck.status === 'ready' ? 'border-success/20 bg-success/10 text-success' : 'border-accent/20 bg-accent/5 text-primary')}>
                {presentationDeck.status}
              </Badge>) : null}
          </div>
        </CardHeader>
        <CardContent className="flex flex-1 flex-col pt-6">
          {presentationDeck ? (<ProposalDeckReadyState activeProposalIdForDeck={activeProposalIdForDeck} deckDownloadUrl={deckDownloadUrl} isRecheckingDeck={isRecheckingDeck} onCopyShareLink={onCopyShareLink} onRecheckDeck={onRecheckDeck} presentationDeck={presentationDeck} viewerHref={viewerHref}/>) : (<ProposalDeckGeneratingState />)}
        </CardContent>
      </Card>
    </m.div>);
}
export function ProposalSubmittedPanelLayout({ activeProposalIdForDeck, canResumeSubmission, deckDownloadUrl, isRecheckingDeck, isSubmitting, onCopyShareLink, onCopySummary, onRecheckDeck, onResumeSubmission, presentationDeck, summary, viewerHref, }: {
    activeProposalIdForDeck: string | null;
    canResumeSubmission: boolean;
    deckDownloadUrl: string | null;
    isRecheckingDeck: boolean;
    ...
    return (<LazyMotion features={domAnimation}>
      <div className="space-y-8">
        <ProposalSubmittedHero activeProposalIdForDeck={activeProposalIdForDeck} canResumeSubmission={canResumeSubmission} deckDownloadUrl={deckDownloadUrl} isSubmitting={isSubmitting} onResumeSubmission={onResumeSubmission}/>

        <div className="grid gap-6 lg:grid-cols-3">
          <ProposalStrategyBriefCard onCopySummary={onCopySummary} summary={summary}/>
          <ProposalAssetDeliveryCard activeProposalIdForDeck={activeProposalIdForDeck} deckDownloadUrl={deckDownloadUrl} isRecheckingDeck={isRecheckingDeck} onCopyShareLink={onCopyShareLink} onRecheckDeck={onRecheckDeck} presentationDeck={presentationDeck} viewerHref={viewerHref}/>
        </div>
      </div>
    </LazyMotion>);
}
