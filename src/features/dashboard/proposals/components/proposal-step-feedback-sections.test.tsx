...
import { ProposalStepFeedbackHeader, ProposalStepFeedbackRequiredBadges, ProposalStepFeedbackValidationBody, } from './proposal-step-feedback-sections';
describe('proposal step feedback sections', () => {
    it('renders the header status summary', () => {
        const markup = renderToStaticMarkup(<ProposalStepFeedbackHeader completedRequiredFields={1} currentStep={1} hasErrors={true} requiredFieldCount={2} stepDescription="Tell us who you are" stepTitle="Company Information" totalSteps={6} validationMessageCount={1}/>);
    ...
    it('renders required badges and the ready/error body variants', () => {
        ...
        const readyMarkup = renderToStaticMarkup(<ProposalStepFeedbackValidationBody hasErrors={false} validationMessages={[]}/>);
