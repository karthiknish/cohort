...
export function ProposalMetricCard({ stat }: {
    stat: ProposalMetricStat;
}) {
    const Icon = stat.icon ?? METRIC_ICONS[stat.label] ?? FileText;
    return (<MotionCard className={cn(DASHBOARD_THEME.stats.card, 'group relative overflow-hidden')}>
      <CardContent className="relative p-5 sm:p-6">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className={cn('flex size-10 shrink-0 items-center justify-center rounded-xl border border-border/50', stat.bg, stat.color)}>
            <Icon className="size-5" aria-hidden/>
          </div>
        </div>
        <div className={cn('pointer-events-none absolute inset-x-0 bottom-0 h-1 opacity-60', stat.bg.replace('/10', '/40'))} aria-hidden/>
      </CardContent>
    </MotionCard>);
}
