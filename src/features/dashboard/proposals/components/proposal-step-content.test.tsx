...
describe('ProposalStepContent', () => {
    it('renders the company step through the thin dispatcher', () => {
        const markup = renderToStaticMarkup(<ProposalStepContent stepId="company" formState={baseFormState as never} summary={baseFormState as never} validationErrors={companyValidationErrors} onUpdateField={vi.fn()} onToggleArrayValue={vi.fn()} onChangeSocialHandle={vi.fn()}/>);
    ...
    it('renders the value step summary through the thin dispatcher', () => {
        const markup = renderToStaticMarkup(<ProposalStepContent stepId="value" formState={baseFormState as never} summary={baseFormState as never} validationErrors={emptyValidationErrors} onUpdateField={vi.fn()} onToggleArrayValue={vi.fn()} onChangeSocialHandle={vi.fn()}/>);
