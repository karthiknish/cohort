'use client';
import { BarChart3, CircleCheck, CircleX, Clock, FileText, LoaderCircle, RefreshCw, Send, TrendingUp, TriangleAlert, } from 'lucide-react';
...
function SummaryStatCard({ iconKey, label, toneClassName, value }: {
    iconKey: SummaryStatIconKey;
    label: string;
    toneClassName: string;
    value: string | number;
}) {
    const icon = iconKey === 'drafts' ? <FileText className="size-5 text-primary"/> :
...
function ProposalAnalyticsChartBar({ point, maxGenerations, }: {
    ...
    return (<div className="flex flex-1 flex-col justify-end" title={`${point.date}: ${totalGenerations} generations, ${totalFailures} failures`}>
    ...
    return (<Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          {failedCount > 0 ? (<Badge variant="destructive" className="gap-1">
...
export function ProposalAnalyticsHeader({ loading, onRefresh, onTimeRangeChange, timeRange, }: {
    loading: boolean;
    onRefresh: () => void;
...
export function ProposalAnalyticsSummaryGrid({ summary, formatDuration }: {
...
export function ProposalAnalyticsSuccessRates({ formatDuration, formatPercentage, summary, }: {
    formatDuration: (ms: number | null) => string;
    formatPercentage: (value: number) => string;
    summary: ProposalAnalyticsSummary;
}) {
    return (<div className="grid gap-4 lg:grid-cols-2">
      <SuccessRateCard averageDuration={formatDuration(summary.averageAiGenerationTime)} description={`${summary.aiGenerationsSucceeded} of ${summary.aiGenerationsAttempted} generations successful`} failedCount={summary.aiGenerationsFailed} succeededCount={summary.aiGenerationsSucceeded} title="AI Generation Success Rate" value={formatPercentage(summary.successRate)}/>
      <SuccessRateCard averageDuration={formatDuration(summary.averageDeckGenerationTime)} description={`${summary.deckGenerationsSucceeded} of ${summary.deckGenerationsAttempted} deck generations successful`} failedCount={summary.deckGenerationsFailed} succeededCount={summary.deckGenerationsSucceeded} title="Deck Generation Success Rate" value={formatPercentage(summary.deckSuccessRate)}/>
    </div>);
}
export function ProposalAnalyticsActivityChart({ chartData }: {
    chartData: ChartData;
}) {
    return (<Card>
      <CardHeader className="pb-3">
      ...
      <CardContent>
        <div className="flex h-32 items-end gap-1">
          {chartData.points.map((point) => (<ProposalAnalyticsChartBar key={point.date} point={point} maxGenerations={chartData.maxGenerations}/>))}
        </div>
        ...
        <div className="mt-3 flex items-center justify-center gap-4 text-xs">
          ...
          {chartData.totalFailures > 0 ? (<span className="flex items-center gap-1.5">
...
export function ProposalAnalyticsByClientCard({ byClient }: {
    ...
    return (<Card>
      ...
      <CardContent>
        <div className="space-y-3">
          {byClient.slice(0, 10).map((client) => (<div key={client.clientId} className="flex items-center justify-between rounded-md border border-muted/40 px-3 py-2">
              <div>
                <p className="text-sm font-medium">{client.clientName}</p>
                <p className="text-xs text-muted-foreground">
                  Last proposal:{' '}
                  {client.lastProposalAt ? (<ClientFormattedDate value={client.lastProposalAt} formatStr="PPP" dateTime={client.lastProposalAt}/>) : 'N/A'}
                </p>
              </div>
              <div className="flex items-center gap-2">
