...
export function useProposalArtifactUrls(workspaceId: string | null, legacyId: string | null) {
  return useQuery(
    proposalArchivesApi.getArtifactDownloadUrls,
    workspaceId && legacyId ? { workspaceId, legacyId } : 'skip',
  )
}
...
export function resolveProposalDeckUrls(args: {
  ...
  const pdfUrl =
    args.artifactUrls?.pdfDownloadUrl ??
    args.pdfUrl ??
    deck?.pdfUrl ??
    deck?.pdfStorageUrl ??
    null
  const pptUrl =
    args.artifactUrls?.pptDownloadUrl ??
    args.pptUrl ??
    deck?.storageUrl ??
    deck?.pptxUrl ??
    null
