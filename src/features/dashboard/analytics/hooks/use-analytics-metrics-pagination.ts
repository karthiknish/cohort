...
export function mergeAnalyticsMetricPages(firstPage: MetricRecord[], olderPages: MetricRecord[]): MetricRecord[] {
...
export function parsePaginatedAnalyticsMetrics(value: unknown): PaginatedAnalyticsMetricsResult | null {
    ...
    const breakdowns = (value as {
        breakdowns?: unknown;
    }).breakdowns;
    if (!Array.isArray(metrics))
        return null;
    const nextCursor = (value as {
        nextCursor?: unknown;
    }).nextCursor;
    const parsedCursor = nextCursor &&
