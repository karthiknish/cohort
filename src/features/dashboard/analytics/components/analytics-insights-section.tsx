...
function formatInsightProviderLabel(providerId: string) {
    if (providerId === 'global')
        return 'Overall property';
    if (providerId === 'google_vs_facebook')
        return 'Cross-platform comparison';
...
function InsightMarkdown({ content }: {
    ...
    return (<ReactMarkdown remarkPlugins={[remarkGfm]} className="space-y-3 text-sm text-muted-foreground" components={INSIGHT_MARKDOWN_COMPONENTS} skipHtml>
      {normalized}
    </ReactMarkdown>);
}
...
export function AnalyticsInsightsSection({ insights, algorithmic, insightsError, insightsLoading, insightsRefreshing, initialInsightsLoading, onRefreshInsights, }: AnalyticsInsightsSectionProps) {
    return (<div className="space-y-6">
      <Card className={DASHBOARD_THEME.cards.base}>
        ...
        <CardContent>
          {initialInsightsLoading ? (<div className="grid gap-4 sm:grid-cols-2">
              {['algo-skeleton-1', 'algo-skeleton-2'].map((slotKey) => (<Skeleton key={slotKey} className="h-32 w-full rounded-lg"/>))}
            </div>) : algorithmic.length === 0 ? (<NoInsightsData className="rounded-lg border border-dashed border-border/60"/>) : (<div className="grid gap-4 sm:grid-cols-2">
              {algorithmic.flatMap((group) => group.suggestions.map((suggestion) => (<div key={`${group.providerId}-${suggestion.level}-${suggestion.title}`} className={cn('rounded-lg border p-4 shadow-sm', suggestionLevelClasses(suggestion.level))}>
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          <div className={cn('flex size-7 items-center justify-center rounded-md', suggestionIconClasses(suggestion.level))}>
                      ...
                      {suggestion.score ? (<div className="flex size-10 shrink-0 flex-col items-center justify-center rounded-lg border border-border/60 bg-card">
      ...
      <Card className={DASHBOARD_THEME.cards.base}>
        <CardHeader className={DASHBOARD_THEME.cards.header}>
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <Button type="button" variant="outline" size="sm" onClick={onRefreshInsights} disabled={insightsLoading || insightsRefreshing} className="gap-2">
        ...
        <CardContent className="space-y-4">
          {insightsError ? (<Alert variant="destructive">
              <TriangleAlert className="size-4"/>
              <AlertTitle>Insight generation failed</AlertTitle>
              <AlertDescription>{insightsError.message}</AlertDescription>
            </Alert>) : null}
          {initialInsightsLoading ? (<div className="space-y-4">
              {['insight-skeleton-1', 'insight-skeleton-2', 'insight-skeleton-3'].map((slotKey) => (<div key={slotKey} className="rounded-lg border border-border/60 bg-muted/30 p-4">
                  <Skeleton className="h-3 w-24 rounded-full"/>
                  <Skeleton className="mt-4 h-16 w-full rounded-lg"/>
                </div>))}
            </div>) : insights.length === 0 ? (<NoIntegrationConnected platform="analytics property" className="rounded-lg border border-dashed border-border/60"/>) : (insights.map((insight) => (<div key={insight.providerId} className="rounded-lg border border-border/60 bg-muted/30 p-5">
