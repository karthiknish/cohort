...
import { AnalyticsConversionRateTooltip, AnalyticsConversionsTooltip, AnalyticsRevenueTooltip, AnalyticsUsersSessionsTooltip, } from './analytics-chart-tooltips';
import { ANALYTICS_CHART_CONTAINER_CLASS, ANALYTICS_CHART_TOOLTIP_PROPS, } from './analytics-chart-tooltips-constants';
import type { AnalyticsChartPoint } from './analytics-chart-tooltips-types';
import { Area, AreaChart, Bar, BarChart, CartesianGrid, ChartContainer, ChartLegend, ChartLegendContent, RechartsTooltip, XAxis, YAxis, } from './chart-imports';
import { conversionRateChartConfig, conversionsChartConfig, revenueChartConfig, usersSessionsChartConfig, } from './chart-configs';
...
function ChartCardHeader({ title, description, tip, }: {
    ...
    return (<CardHeader className={CHART_HEADER_CLASS}>
    ...
    return (<div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <Card className={CHART_CARD_CLASS}>
        ...
        <CardContent className="pt-6">
          {showChartSkeleton ? (<Skeleton className="h-[280px] w-full rounded-lg"/>) : chartData.length === 0 ? (<AnalyticsEmptyState variant="no-filters" title="No performance data" description="There is no performance data for the selected date range." className="h-[280px] py-6"/>) : (<ChartContainer config={usersSessionsChartConfig} className={ANALYTICS_CHART_CONTAINER_CLASS}>
      ...
      <Card className={CHART_CARD_CLASS}>
        ...
        <CardContent className="pt-6">
          {showChartSkeleton ? (<Skeleton className="h-[280px] w-full rounded-lg"/>) : chartData.length === 0 ? (<AnalyticsEmptyState variant="no-filters" title="No performance data" description="There is no performance data for the selected date range." className="h-[280px] py-6"/>) : (<ChartContainer config={conversionsChartConfig} className={ANALYTICS_CHART_CONTAINER_CLASS}>
      ...
      <Card className={CHART_CARD_CLASS}>
        <ChartCardHeader title="Conversion rate" description="Conversions divided by sessions, per day" tip="Hover to inspect rate changes — low session days can exaggerate swings."/>
        <CardContent className="pt-6">
          {showChartSkeleton ? (<Skeleton className="h-[280px] w-full rounded-lg"/>) : chartData.length === 0 ? (<AnalyticsEmptyState variant="no-filters" title="No performance data" description="There is no performance data for the selected date range." className="h-[280px] py-6"/>) : (<ChartContainer config={conversionRateChartConfig} className={ANALYTICS_CHART_CONTAINER_CLASS}>
              <AreaChart data={chartData} margin={CHART_MARGIN}>
                <defs>
