...
const DIMENSION_META: Record<BreakdownDimension, {
    title: string;
    description: string;
    icon: typeof Share2;
}> = {
...
function BreakdownShareBar({ share }: {
    ...
    const widthStyle = ({ width: `${Math.max(share, 2)}%` });
    return (<div className="h-1.5 overflow-hidden rounded-full bg-muted">
...
function BreakdownCard({ dimension, rows, formatRevenue, }: {
    ...
    return (<Card className={DASHBOARD_THEME.cards.base}>
      ...
      <CardContent className="space-y-3 pt-2">
        {entries.length === 0 ? (<p className="text-sm text-muted-foreground">No breakdown data for this range yet.</p>) : (entries.map((entry) => (<div key={entry.label} className="space-y-1">
