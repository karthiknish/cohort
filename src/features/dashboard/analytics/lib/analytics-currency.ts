import { aggregateMetricFinancials, formatAggregatedMoney, isFinancialComparable, type MetricFinancialRow, } from '@/domain/ads/aggregate-financials';
...
function toFinancialRows(metrics: AnalyticsMoneyRow[], integrationCurrency?: string | null): MetricFinancialRow[] {
    const fallback = typeof integrationCurrency === 'string' && integrationCurrency.trim().length > 0
        ? integrationCurrency.trim().toUpperCase()
        : null;
    return metrics.map((metric) => ({
        spend: Number(metric.spend ?? 0),
        revenue: metric.revenue ?? 0,
        currency: typeof metric.currency === 'string' && metric.currency.trim().length > 0
 ...
 */
export function buildAnalyticsMoneyDisplay(metrics: AnalyticsMoneyRow[], options?: {
    ...
    const integrationCurrency = typeof options?.integrationCurrency === 'string' && options.integrationCurrency.trim().length > 0
        ? options.integrationCurrency.trim().toUpperCase()
        : null;
    if (!isFinancialComparable(financialTotals.comparability) &&
    ...
    const formatRevenue = (amount: number | null | undefined) => formatAggregatedMoney(amount ?? null, financialTotals, formatCurrency);
    return {
        financialTotals,
