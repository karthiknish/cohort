...
import { AudienceControlHeaderActionsSlot, AudienceControlInterestSectionSlot, } from './audience-control-section-slots';
...
export function AudienceControlHeaderActions({ loading, onOpenBuilder, onRefresh, }: {
    loading: boolean;
    onOpenBuilder: () => void;
    onRefresh: () => void;
}) {
    return (<>
      ...
      <Button variant="ghost" size="icon" className="size-9" onClick={onRefresh} disabled={loading} aria-label="Refresh audience targeting">
...
export function AudienceControlLoadingCard() {
    return (<MotionCard className={ADS_PAGE_THEME.surfaceCard}>
      <CampaignControlHeader icon={Target} title="Audience targeting" description="Loading targeting from the ad platform…"/>
...
export function AudienceControlEmptyCard({ builderOpen, headerActionsProps, onBuilderOpenChange, providerId, }: {
    builderOpen: boolean;
    ...
    return (<MotionCard className={ADS_PAGE_THEME.surfaceCard}>
      <CampaignControlHeader icon={Target} title="Audience targeting" description="No targeting data returned for this campaign yet." actions={createElement(AudienceControlHeaderActionsSlot, headerActionsProps)}/>
      <CardContent className="pt-2">
        <div className={cn(ADS_PAGE_THEME.emptyState, 'py-14')}>
          <div className={ADS_PAGE_THEME.controlHeaderIcon}>
...
export function AudienceControlExcludedLocations({ excludedLocations, }: {
    excludedLocations: AggregatedTargetingData['locations']['excluded'];
}) {
    if (excludedLocations.length === 0) {
        return null;
    }
    return (<div className="rounded-2xl border border-destructive/20 bg-destructive/[0.04] px-5 py-4">
...
export function AudienceControlMainCard({ targeting, insights, aggregatedData, locationMarkers, selectedTargetingId, expandedSections, geocodeFailedNames, audienceStats, headerActionsProps, interestSectionProps, customAudiencesSectionProps, builderOpen, providerId, workspaceId, clientId, canEditMetaTargeting, editingSection, onTargetingChange, onToggleEditing, onToggleSection, onBuilderOpenChange, onAddLocation, onRemoveLocation, onSaveLocations, onSaveDemographics, onDraftDemographicsChange, draftDemographics, draftPlacements, draftPlacementDetail, onSavePlacements, onTogglePlatform, onTogglePlacementPosition, savingTargeting, campaignObjective, }: {
    ...
    onDraftDemographicsChange?: (updater: (prev: {
        ageMin: number;
        ageMax: number;
        genders: string[];
    }) => {
    ...
    return (<MotionCard className={ADS_PAGE_THEME.surfaceCard}>
      <CampaignControlHeader icon={Target} title="Audience targeting" description={<>
            {entityCount} configuration
            {entityCount === 1 ? '' : 's'} across ad sets - map, demographics, and segments in one view.
          </>} actions={createElement(AudienceControlHeaderActionsSlot, headerActionsProps)} stats={audienceStats}/>

      <CardContent className="space-y-6 pt-6">
        {geocodeFailedNames.length > 0 ? (<Alert variant="default" className="border-warning/40 bg-warning/10">
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)]">
          <div className={cn(ADS_PAGE_THEME.controlFormPanel, 'p-5')}>
            <LocationTargetingSection targeting={targeting} aggregatedData={aggregatedData} locationMarkers={locationMarkers} selectedTargetingId={selectedTargetingId} onTargetingChange={onTargetingChange} editingSection={editingSection} onToggleEditing={onToggleEditing} workspaceId={workspaceId} clientId={clientId} canSearchGeo={canEditMetaTargeting} onAddLocation={onAddLocation} onRemoveLocation={onRemoveLocation} onSaveLocations={canEditMetaTargeting && editingSection === 'locations' ? onSaveLocations : undefined} savingTargeting={savingTargeting}/>
          </div>

          <div className="space-y-2.5">
            <p className={cn(ADS_PAGE_THEME.controlSectionLabel, 'px-0.5')}>Segments & criteria</p>
            <DemographicSection aggregatedData={aggregatedData} expandedSections={expandedSections} toggleSection={onToggleSection} editingSection={editingSection} onToggleEditing={onToggleEditing} canEdit={canEditMetaTargeting} draftDemographics={draftDemographics} onDraftChange={canEditMetaTargeting ? onDraftDemographicsChange : undefined} onSaveDemographics={canEditMetaTargeting && editingSection === 'demographics' ? onSaveDemographics : undefined} savingTargeting={savingTargeting}/>
            {showPlacementSection ? (<PlacementTargetingSection aggregatedData={aggregatedData} expandedSections={expandedSections} toggleSection={onToggleSection} editingSection={editingSection} onToggleEditing={onToggleEditing} canEdit={canEditMetaTargeting} draftPublisherPlatforms={draftPlacements} draftPlacementDetail={draftPlacementDetail} onTogglePlatform={onTogglePlatform} onTogglePlacementPosition={onTogglePlacementPosition} onSavePlacements={editingSection === 'placements' ? onSavePlacements : undefined} savingTargeting={savingTargeting}/>) : null}
            <AudienceDisplaySection aggregatedData={aggregatedData} expandedSections={expandedSections} toggleSection={onToggleSection} hidePlacements={showPlacementSection} customAudiencesSection={showCustomAudiences ? (<CustomAudiencesTargetingSection {...customAudiencesSectionProps}/>) : undefined} interestSection={createElement(AudienceControlInterestSectionSlot, interestSectionProps)}/>
          </div>
        </div>
...
export function AudienceControlInterestEditorSection({ aggregatedData, expandedSections, toggleSection, editingSection, onToggleEditing, canEditMetaTargeting, workspaceId, clientId, onAddInterest, onRemoveInterest, onSaveTargeting, savingTargeting, }: AudienceControlInterestEditorSectionProps) {
    return (<AudienceEditorSection aggregatedData={aggregatedData} expandedSections={expandedSections} toggleSection={toggleSection} editingSection={editingSection} onToggleEditing={onToggleEditing} canEditTargeting={canEditMetaTargeting} workspaceId={workspaceId} clientId={clientId} onAddInterest={onAddInterest} onRemoveInterest={onRemoveInterest} onSaveTargeting={onSaveTargeting} savingTargeting={savingTargeting}/>);
}
