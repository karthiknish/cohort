...
function AudienceRemoveButton({ audienceId, audienceName, onRemove, }: {
    ...
    return (<button type="button" className="ml-0.5 rounded-sm hover:text-destructive" aria-label={`Remove ${audienceName}`} onClick={handleRemove}>
      ×
    </button>);
}
export function CustomAudiencesTargetingSection({ aggregatedData, expandedSections, toggleSection, editingSection, onToggleEditing, canEdit, workspaceId, clientId, onAddAudience, onRemoveAudience, onSaveTargeting, savingTargeting, }: Props) {
    const [catalog, dispatchCatalog] = useReducer((state: CatalogState, action: {
        type: 'set';
        value: CatalogState;
    }) => action.value, { catalog: [], loading: false });
    ...
    useEffect(() => {
        if (!canEdit || !workspaceId || !isEditing)
            return;
        let cancelled = false;
    ...
    const handleUploadEmailsChange = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
        setUploadEmailsRaw(event.target.value);
    };
    const handleUploadUsers = () => {
        if (!workspaceId || !uploadAudienceId)
            return;
    ...
    const handleAddAudienceSelect = (id: string) => {
    ...
    const editButton = canEdit && onAddAudience ? (<TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <Button variant="ghost" size="icon" className="size-7" onClick={handleEdit} aria-label="Edit custom audiences">
    ...
    return (<TargetingCollapsiblePanel sectionId="audiences" icon={UserCheck} title="Custom audiences" count={included.length} expanded={expandedSections.has('audiences')} onToggle={handleToggle} headerActions={editButton}>
      <div className="space-y-3">
        {isEditing && canEdit && workspaceId ? (catalog.loading ? (<div className="flex items-center gap-2 text-sm text-muted-foreground">
              <Loader2 className="size-4 animate-spin" aria-hidden/>
              Loading audiences…
            </div>) : availableToAdd.length > 0 ? (<Select onValueChange={handleAddAudienceSelect} disabled={savingTargeting}>
              <SelectTrigger className="h-9">
                <SelectValue placeholder="Add custom audience…"/>
              </SelectTrigger>
              <SelectContent>
                {availableToAdd.map((row) => (<SelectItem key={row.id} value={row.id}>
                    {row.name}
                  </SelectItem>))}
              </SelectContent>
        ...
        {included.length > 0 ? (<div className="flex flex-wrap gap-1.5">
            {included.map((audience) => (<Badge key={audience.id} variant="secondary" className="gap-1 text-xs">
                {audience.name}
                {isEditing && onRemoveAudience ? (<AudienceRemoveButton audienceId={audience.id} audienceName={audience.name} onRemove={onRemoveAudience}/>) : null}
              </Badge>))}
          </div>) : (<p className="text-xs text-muted-foreground">
            {isEditing ? 'Add custom audiences above, then save to Meta.' : 'No custom audiences on this ad set.'}
          </p>)}

        {isEditing && canEdit && workspaceId && catalog.catalog.length > 0 ? (<div className="space-y-2 rounded-lg border border-dashed border-border/60 p-3">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={uploadAudienceId} onValueChange={setUploadAudienceId} disabled={uploadingUsers}>
                ...
                <SelectContent>
                  {catalog.catalog.map((row) => (<SelectItem key={row.id} value={row.id}>
                      {row.name}
                    </SelectItem>))}
                </SelectContent>
              </Select>
            </div>
            <Textarea value={uploadEmailsRaw} onChange={handleUploadEmailsChange} placeholder={'user@example.com\nanother@example.com'} rows={4} className="text-sm" disabled={uploadingUsers}/>
            <Button type="button" size="sm" variant="secondary" disabled={uploadingUsers || !uploadAudienceId || !uploadEmailsRaw.trim()} onClick={handleUploadUsers}>
              {uploadingUsers ? (<>
        ...
        {isEditing && onSaveTargeting ? (<div className="flex justify-end border-t border-border/60 pt-3">
