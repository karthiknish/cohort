export const LOCATION_COORDINATES: Record<string, {
    lat: number;
    lng: number;
}> = {
...
const LOCATION_KEYS_BY_LENGTH = Object.keys(LOCATION_COORDINATES).sort((a, b) => b.length - a.length);
...
function findFuzzyLocationCoordinates(normalizedName: string): {
    lat: number;
    lng: number;
} | null {
    for (const key of LOCATION_KEYS_BY_LENGTH) {
        if (!containsLocationTerm(normalizedName, key))
            continue;
    ...
    for (const key of LOCATION_KEYS_BY_LENGTH) {
        if (!containsLocationTerm(key, normalizedName))
            continue;
        const coords = LOCATION_COORDINATES[key];
        if (coords)
            return coords;
...
export function findLocationCoordinates(name: string): {
    lat: number;
    lng: number;
} | null {
