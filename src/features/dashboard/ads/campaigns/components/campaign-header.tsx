...
function formatCampaignDateRange(startTime?: string, stopTime?: string): string {
    ...
    if (hasStart && !hasStop)
        return `Started ${formatDate(start, DATE_FORMATS.SHORT)}`;
    if (!hasStart && hasStop)
        return `Ends ${formatDate(stop, DATE_FORMATS.SHORT)}`;
...
function getCampaignLifetimeRange(startTime?: string, stopTime?: string): DateRange | null {
    ...
    const rangeStart: Date = hasStart && start !== null ? start : new Date(new Date(end).setDate(end.getDate() - 30));
    return {
        start: rangeStart,
        end,
    };
}
export function CampaignHeader({ campaign, loading, dateRange, onDateRangeChange, onRefresh, refreshing, onCampaignUpdated, }: CampaignHeaderProps) {
    ...
    const getProviderIcon = () => {
        if (!campaign?.providerId)
            return null;
        const svglBrand: SvglBrandSlug | null = campaign.providerId === 'facebook' || campaign.providerId === 'meta'
            ? 'meta'
    ...
    return (<header className={ADS_PAGE_THEME.innerHero}>
      ...
      <div className="relative space-y-5">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0 flex-1 space-y-3">
            {loading ? (<div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {campaign?.accountLogoUrl ? (<Avatar className="size-8 ring-1 ring-border/60">
                      <AvatarImage src={campaign.accountLogoUrl} alt={campaign.accountName}/>
                      <AvatarFallback className="text-[10px]">
                        {campaign.accountName?.[0] || '?'}
                      </AvatarFallback>
                    </Avatar>) : null}
                  {campaign?.accountName ? (<span className="text-xs font-medium text-muted-foreground">{campaign.accountName}</span>) : null}
                  <Badge variant="outline" className="flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[10px] font-medium capitalize">
                    {getProviderIcon()}
                    {providerDisplay}
                  </Badge>
                  {objectiveLabel ? (<Badge variant="secondary" className="rounded-full text-[10px] font-medium capitalize">
                      {objectiveLabel}
                    </Badge>) : null}
                  {sharedCampaignKey ? (<ViewTransition name={`campaign-status-${sharedCampaignKey}`} share="morph" default="none">
                      <Badge variant="outline" className={cn('rounded-full text-[10px] font-semibold uppercase tracking-wide', isActive
                ...
                {sharedCampaignKey ? (<ViewTransition name={`campaign-title-${sharedCampaignKey}`} share="text-morph" default="none">
                    <h1 className={cn(DASHBOARD_THEME.layout.title, 'text-balance')}>{campaignName}</h1>
                  </ViewTransition>) : (<h1 className={cn(DASHBOARD_THEME.layout.title, 'text-balance')}>{campaign?.name}</h1>)}
          ...
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:justify-end">
            {!loading && campaign && (campaign.providerId === 'facebook' || campaign.providerId === 'meta') ? (<EditMetaCampaignDialog campaignId={campaign.id} initialName={campaign.name} initialStartTime={campaign.startTime} initialStopTime={campaign.stopTime} onUpdated={onCampaignUpdated}/>) : null}
            <DateRangePicker value={dateRange} onChange={onDateRangeChange} lifetimeRange={campaignLifetimeRange} className="w-full sm:w-auto [&_button]:h-10 [&_button]:rounded-xl [&_button]:border-border/70 [&_button]:bg-background/90"/>
            <Button type="button" variant="outline" size="sm" className="h-10 gap-1.5 rounded-xl border-border/70" onClick={onRefresh} disabled={refreshing} aria-busy={refreshing}>
