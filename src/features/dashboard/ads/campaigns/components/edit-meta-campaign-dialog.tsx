...
function EditMetaCampaignFormBody({ campaignId, initialName, initialStartTime, initialStopTime, workspaceId, selectedClientId, onClose, onUpdated, }: EditMetaCampaignFormBodyProps) {
    ...
    return (<>
      <div className="space-y-4 py-2">
        ...
        <FormField id="campaign-edit-start" label="Start (optional)">
          <Input id="campaign-edit-start" type="datetime-local" value={startTime} onChange={handleStartTimeChange}/>
        </FormField>
        <FormField id="campaign-edit-stop" label="End (optional)">
          <Input id="campaign-edit-stop" type="datetime-local" value={stopTime} onChange={handleStopTimeChange}/>
        </FormField>
      </div>
      <DialogFooter>
...
export function EditMetaCampaignDialog({ campaignId, initialName, initialStartTime, initialStopTime, onUpdated, }: Props) {
    const { user } = useAuth();
    ...
    return (<Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {open ? (<EditMetaCampaignFormBody key={formResetKey} campaignId={campaignId} initialName={initialName} initialStartTime={initialStartTime} initialStopTime={initialStopTime} workspaceId={workspaceId} selectedClientId={selectedClientId} onClose={handleClose} onUpdated={onUpdated}/>) : null}
      </DialogContent>
    </Dialog>);
}
