...
export function CampaignSection({ title, description, eyebrow, children, className, }: {
    title: string;
    description?: string;
    eyebrow?: string;
    children: ReactNode;
    ...
    return (<section className={cn(ADS_PAGE_THEME.sectionBlock, className)}>
      <div className={ADS_PAGE_THEME.sectionHeader}>
        ...
        {description ? (<p className={cn(ADS_PAGE_THEME.sectionDescription, 'text-pretty')}>{description}</p>) : null}
      </div>
      {children}
    </section>);
}
...
export function CampaignPageLayout({ performance, controls, creatives, advanced, creativesCount, }: CampaignPageLayoutProps) {
    const [tab, setTab] = useState('performance');
    const performanceBlock = (<div className="space-y-8">
      {performance}
      ...
      <div className={ADS_PAGE_THEME.stickyTabBar}>
        <TabsList className={ADS_PAGE_THEME.mobileTabs}>
          ...
          <TabsTrigger value="creatives" className={ADS_PAGE_THEME.mobileTabTrigger}>
            ...
            Creatives
            {typeof creativesCount === 'number' && creativesCount > 0 ? (<span className="rounded-full bg-background px-1.5 py-0 text-[10px] font-semibold tabular-nums text-foreground shadow-sm">
                {creativesCount}
              </span>) : null}
          </TabsTrigger>
        </TabsList>
