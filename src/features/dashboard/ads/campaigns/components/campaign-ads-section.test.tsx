...
describe('CampaignAdsSection composition', () => {
    it('renders the preview-mode state', () => {
        const markup = renderToStaticMarkup(<CampaignAdsSection providerId="meta" campaignId="campaign-1" clientId="client-1" isPreviewMode/>);
    ...
    it('renders the live loading shell before creatives load', () => {
        const markup = renderToStaticMarkup(<CampaignAdsSection providerId="google" campaignId="campaign-2" clientId="client-2"/>);
        expect(markup).toContain('Create creative');
