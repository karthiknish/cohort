...
import { getMetaCreativeObjectTypeOptions, type MetaCreativeObjectTypeOption, } from '@/lib/meta-campaign-ui';
...
function createOptionalSelectChangeHandler(onChange: (value: string) => void, emptyValue: string) {
    return (value: string) => {
        onChange(value === emptyValue ? '' : value);
    };
}
export function CreateCreativeDialogHeader({ providerId }: {
    providerId: string;
}) {
    return (<DialogHeader>
...
export function CreateCreativeDialogForm({ availableAdSets, body, callToActionType, description, imageHash, imagePreviewSrc, imageUrl, onClearImage, instagramActorId, instagramActorOptions, isMeta, linkUrl, loading, loadingPageActors, metaPageActors, name, objectType, onBodyChange, onCallToActionTypeChange, onClose, onDescriptionChange, onImageUpload, onVideoUpload, onImageUrlChange, onClearVideo, videoPreviewSrc, videoId, uploadingVideo, onInstagramActorIdChange, onLinkUrlChange, onNameChange, onObjectTypeChange, onPageIdChange, onSelectedAdSetIdChange, onStatusChange, onSubmit, onTitleChange, onVideoIdChange, pageId, selectedAdSetId, selectedPage, status, title, uploadingImage, workspaceId, clientId, campaignObjective, leadFormId, onLeadFormIdChange, }: CreateCreativeDialogFormProps) {
    ...
    const handleInstagramActorIdChange = (value: string) => {
    ...
    return (<form onSubmit={onSubmit} className="space-y-4">
      ...
      <FormField id="objectType" label="Ad Format">
        <Select value={objectType} onValueChange={createSelectChangeHandler<CreativeObjectType>(onObjectTypeChange)} disabled={loading}>
          ...
          <SelectContent>
            {allowedObjectTypes.map((format) => (<SelectItem key={format} value={format}>
                {objectTypeLabels[format]}
              </SelectItem>))}
          </SelectContent>
        </Select>
        {objectType === 'DYNAMIC' ? (<p className="text-xs text-muted-foreground">
      ...
      {availableAdSets?.length ? (<FormField id="adSet" label="Ad Set" description="The ad will be created in the selected ad set.">
          <Select value={selectedAdSetId} onValueChange={onSelectedAdSetIdChange} disabled={loading}>
            ...
            <SelectContent>
              {availableAdSets.map((adSet) => (<SelectItem key={adSet.id} value={adSet.id}>{adSet.name || adSet.id}</SelectItem>))}
            </SelectContent>
          </Select>
        </FormField>) : null}

      {isMeta && !selectedAdSetId && !availableAdSets?.length ? (<div className="flex items-start gap-2 rounded-md border border-warning/20 bg-warning/10 p-3">
      ...
      <FormField id="cta" label="Call to Action">
        <Select value={callToActionType} onValueChange={onCallToActionTypeChange} disabled={loading}>
          ...
          <SelectContent>
            {CTA_OPTIONS.map((option) => (<SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>))}
          </SelectContent>
        </Select>
      </FormField>
      ...
      {objectType === 'IMAGE' ? (<FormField id="image" label="Creative Image" description={imageHash ? 'Uploaded to Meta — safe to create your ad.' : 'Upload the image Meta will serve in the feed.'}>
          <CreativeMediaField previewSrc={imagePreviewSrc} imageUrl={imageUrl} imageHash={imageHash} uploading={uploadingImage} disabled={loading} onImageUrlChange={onImageUrlChange} onFileSelect={onImageUpload} onClear={onClearImage}/>
        </FormField>) : null}
      {objectType === 'VIDEO' ? (<FormField id="video" label="Creative Video" description={videoId ? 'Uploaded to Meta — safe to create your ad.' : 'Upload the video Meta will serve in the feed.'}>
          <CreativeMediaField mode="video" previewSrc={videoPreviewSrc} imageUrl="" videoId={videoId} uploading={uploadingVideo} disabled={loading} onImageUrlChange={noopImageUrlChange} onFileSelect={onVideoUpload} onClear={onClearVideo}/>
          <div className="mt-2 space-y-1.5">
            <Label htmlFor="videoId" className="text-xs text-muted-foreground">
              Or paste Meta video ID
            </Label>
            <Input id="videoId" placeholder="Meta video ID" value={videoId} onChange={createTextChangeHandler(onVideoIdChange)} disabled={loading || uploadingVideo}/>
          </div>
        </FormField>) : null}

      <FormField id="pageId" label="Facebook Page *" description={!loadingPageActors && metaPageActors.length === 0
        ...
        <Select value={pageId || undefined} onValueChange={onPageIdChange} disabled={loading || loadingPageActors}>
          ...
          <SelectContent>
            {metaPageActors.map((actor) => (<SelectItem key={actor.id} value={actor.id}>{actor.name}</SelectItem>))}
          </SelectContent>
        </Select>
      </FormField>

      <CreateCreativeLeadFormSection workspaceId={workspaceId} clientId={clientId} campaignObjective={campaignObjective} pageId={pageId} leadFormId={leadFormId} disabled={loading} onLeadFormIdChange={onLeadFormIdChange}/>

      <FormField id="instagramActorId" label="Instagram Business Account" description={selectedPage?.instagramBusinessAccountId
        ...
        <Select value={instagramActorId || '__none__'} onValueChange={handleInstagramActorIdChange} disabled={loading || loadingPageActors || instagramActorOptions.length === 0}>
          ...
          <SelectContent>
            ...
            {instagramActorOptions.map((actor) => (<SelectItem key={actor.id} value={actor.id}>{actor.label}</SelectItem>))}
          </SelectContent>
        </Select>
      </FormField>
