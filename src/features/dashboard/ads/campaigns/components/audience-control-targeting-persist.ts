...
export function buildBaseMetaTargetingPayload(
  aggregatedData: AggregatedAudienceData,
  audiences: AggregatedAudienceData['audiences'],
) {
  return {
...
export async function persistAdSetTargeting({
  ...
  fetchTargeting,
  dispatch,
  selectAdSetDescription,
  successDescription,
  logContext,
  targeting,
  clearDrafts,
}: PersistAdSetTargetingArgs): Promise<boolean> {
