...
export type CreateCreativeDialogAction = {
    ...
    type: 'setMetaPageActors';
    value: MetaPageActorOption[];
} | {
    ...
    actors: MetaPageActorOption[];
} | {
    type: 'clearMetaPageActorsOnError';
};
export function createInitialCreateCreativeDialogState(selectedAdSetId: string | undefined): CreateCreativeDialogState {
    return {
        open: false,
        loading: false,
...
export function createCreativeDialogReducer(state: CreateCreativeDialogState, action: CreateCreativeDialogAction): CreateCreativeDialogState {
    switch (action.type) {
        case 'setOpen':
        ...
        case 'applyMetaPageActors': {
            ...
            const nextPageId = currentPageId && normalizedActors.some((actor) => actor.id === currentPageId)
                ? currentPageId
