...
function parseBudgetMode(providerId: string, budgetType?: string | null): BudgetUiMode | null {
    ...
    if (providerId === 'tiktok') {
        if (raw === 'budget_mode_day')
            return 'daily';
        if (raw === 'budget_mode_total')
            return 'total';
    }
    if (raw === 'daily')
        return 'daily';
...
function getBudgetModeOptions(providerId: string): Array<{
    value: BudgetUiMode;
    label: string;
    hint: string;
}> {
...
function BudgetModeOption({ opt, selected, disabled, onSelect, }: {
    ...
    return (<button type="button" role="radio" aria-checked={selected} disabled={disabled} onClick={selectBudgetModeValue} className={cn('flex flex-col items-start gap-0.5 rounded-xl border px-4 py-3 text-left transition-[border-color,box-shadow,background-color]', selected
            ? 'border-primary/40 bg-primary/[0.06] shadow-sm ring-1 ring-primary/20'
            : 'border-border/60 bg-card/50 hover:border-primary/25 hover:bg-card', disabled && 'pointer-events-none opacity-60', 'motion-reduce:transition-none')}>
...
export function BudgetControlSection({ providerId, campaignId, clientId, isPreviewMode, currency, budget, budgetType, onReloadCampaign, }: Props) {
    const canEdit = !isPreviewMode;
    const currencyCode = normalizeCurrencyCode(currency ?? undefined);
    ...
    const [amount, setAmount] = useState<string>(() => typeof budget === 'number' && Number.isFinite(budget) ? String(budget) : '');
    const { user } = useAuth();
    ...
    const hasUnsavedChanges = (() => {
        const budgetChanged = hasValidDraft && (typeof budget !== 'number' || Math.abs(parsedDraft - budget) > 0.001);
    ...
    const handleModeSelect = (value: BudgetUiMode) => {
        setMode(value);
    };
    ...
    const headerActions = (<>
      <Button variant="ghost" size="icon" className="size-9" onClick={handleRefresh} disabled={saving} aria-label="Refresh campaign budget">
    ...
    return (<MotionCard className={ADS_PAGE_THEME.surfaceCard}>
      <CampaignControlHeader icon={DollarSign} title="Budget control" description={<>
      ...
      <CardContent className="space-y-5 pt-6">
        {!canEdit ? (<output className={cn(ADS_PAGE_THEME.controlPreviewBanner, 'block')}>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
          <div className={ADS_PAGE_THEME.controlHighlightTile}>
            ...
            {hasUnsavedChanges && draftBudgetLabel ? (<p className="mt-3 rounded-lg border border-primary/20 bg-primary/5 px-3 py-2 text-xs text-muted-foreground">
                Draft after save:{' '}
          ...
          <div className={cn(ADS_PAGE_THEME.controlFormPanel, 'space-y-5')}>
            {modeOptions.length > 0 ? (<div className="space-y-2.5">
                <Label className={ADS_PAGE_THEME.controlSectionLabel}>Budget type</Label>
                <div className="grid gap-2 sm:grid-cols-2" role="radiogroup" aria-label="Budget type">
                  {modeOptions.map((opt) => (<BudgetModeOption key={opt.value} opt={opt} selected={mode === opt.value} disabled={!canEdit || saving} onSelect={handleModeSelect}/>))}
                </div>
              </div>) : null}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm font-medium text-muted-foreground" aria-hidden>
                  $
                </span>
                <Input id="budget-amount" inputMode="decimal" placeholder="50.00" value={amount} onChange={handleAmountChange} disabled={!canEdit || saving} className="h-11 pl-7 text-base tabular-nums"/>
              </div>
