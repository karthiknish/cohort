...
export function useAudienceControlSection({
  ...
  const locationMarkers: LocationMarker[] = useMemo(
    () => buildAudienceLocationMarkers(visibleTargeting, resolvedCoordinates),
    [resolvedCoordinates, visibleTargeting],
  )

  const aggregatedData = useMemo(
    () => aggregateAudienceTargetingData(visibleTargeting),
    [visibleTargeting],
  )
  ...
  const editorAggregatedData = useMemo(
    ...
    [aggregatedData, displayAudiences, displayInterests, displayLocations],
  )
  ...
  const customAudiencesSectionProps = useMemo(
    () => ({
      ...
      onSaveTargeting:
        canEditMetaTargeting && editingSection === 'audiences' ? handlePersistAudiences : undefined,
      savingTargeting,
    }),
    [
      canEditMetaTargeting,
