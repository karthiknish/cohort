...
export function buildPreviewCreative(providerId: ProviderId, campaignId: string, creativeId: string, campaignName: string): Creative {
    const baseCreative = {
        providerId,
        creativeId,
        campaignId,
...
export function buildPreviewCreativeMetrics(providerId: ProviderId, creativeId: string, campaignId: string, days: string): NormalizedAdMetric[] {
    const dayCount = Math.max(1, Number.parseInt(days, 10) || 7);
    const baseByProvider: Record<ProviderId, {
        impressions: number;
        clicks: number;
        spend: number;
        conversions: number;
...
export function buildPreviewCopySuggestions(kind: 'headlines' | 'captions', creative: Creative, campaignName: string): string[] {
...
export function buildCreativePreviewCreative(creative: Creative, editedHeadlines: string[], editedDescriptions: string[], editedCta: string, editedLandingPage: string, previewHeadlineIndex: number, previewDescriptionIndex: number): Creative {
    ...
    const orderedHeadlines = headlines.length > 0
        ? [headlines[safeHeadlineIndex]!, ...headlines.filter((_, i) => i !== safeHeadlineIndex)]
        : creative.headlines;
    const orderedDescriptions = descriptions.length > 0
...
export function buildCreativePerformanceSummary(creativeMetrics: NormalizedAdMetric[] | null, convexProviderId: ProviderId | null, days: string, currency?: string | null) {
    if (!creativeMetrics || !convexProviderId)
        return null;
