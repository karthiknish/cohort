...
export default async function CreativeDetailPage({ searchParams }: CreativeDetailPageProps) {
    ...
    return (<Suspense fallback={CREATIVE_DETAIL_PAGE_FALLBACK}>
      <CreativeDetailPageClient campaignName={getFirstSearchParam(resolvedSearchParams?.campaignName)} currency={getFirstSearchParam(resolvedSearchParams?.currency)} searchParamsString={toSearchParamsString(resolvedSearchParams)}/>
    </Suspense>);
}
