...
function PreviewAvatar(props: {
    ...
    return (<div className={cn('shrink-0 overflow-hidden rounded-full', ringClassName, className)}>
      {profileImageUrl && !profileImageError ? (<NextImage src={profileImageUrl} alt="" width={40} height={40} unoptimized className="size-full object-cover" onError={onProfileImageError} style={crispEdgesStyle}/>) : (<div className="flex size-full items-center justify-center text-sm font-semibold" style={s.avatarPlaceholder}>
          {initial}
        </div>)}
    </div>);
}
export function CreativePlatformMock({ activePlatform, campaignName, creative, displayName, pageDisplayName, primaryText, headlineText, landingHostname, profileImageError, onProfileImageError, previewMediaProps, }: {
    ...
    return (<AnimatePresence mode="wait">
      <m.div key={activePlatform} initial="hidden" animate="visible" exit={exitSlideLeft} variants={slideInLeftVariants} transition={transitions.normal} className="w-full">
        {activePlatform === 'instagram' && (<div className="overflow-hidden" style={s.igSurface}>
            <div className="flex items-center justify-between px-3 py-2.5">
              <div className="flex min-w-0 items-center gap-2.5">
                <div className="rounded-full p-[2px]" style={s.igRing}>
                  <PreviewAvatar pageName={pageDisplayName} profileImageUrl={creative.pageProfileImageUrl} profileImageError={profileImageError} onProfileImageError={onProfileImageError} className="size-8"/>
                </div>
            ...
            {primaryText ? (<div className="px-3 pb-3 pt-0.5">
            ...
            <div className="space-y-1 px-3 pb-2">
              ...
              {!primaryText ? (<p className="text-[14px] leading-[1.4]">
                  <span className="font-semibold">{pageDisplayName}</span>{' '}
                  <span className="font-normal" style={s.textPrimary}>See more</span>
                </p>) : null}
              {headlineText ? (<p className="text-[13px] font-medium" style={s.link}>{headlineText}</p>) : null}
            </div>

            {creative.callToAction ? (<div className="border-t px-3 py-2.5" style={s.borderLightTop}>
                <button type="button" className="w-full rounded-lg py-2 text-center text-[14px] font-semibold text-white" style={s.ctaInstagram}>
                  {formatCTALabel(creative.callToAction)}
                </button>
              </div>) : null}
          </div>)}

        {activePlatform === 'linkedin' && (<div className="bg-background rounded-tr-[2.5rem] rounded-tl-[2.5rem] overflow-hidden text-left">
            <div className="p-4 flex items-center gap-3">
              <div className="size-10 rounded-lg bg-info flex items-center justify-center text-info-foreground font-black text-lg overflow-hidden shrink-0">
                {creative.pageProfileImageUrl && !profileImageError ? (<NextImage src={creative.pageProfileImageUrl} alt="" width={40} height={40} unoptimized className="size-full object-cover" onError={onProfileImageError} style={crispEdgesStyle}/>) : ((creative.pageName || creative.campaignName || campaignName || 'A').slice(0, 1).toUpperCase())}
              </div>
            ...
            <div className="p-4 bg-muted/5 flex items-center justify-between border-y border-muted-foreground/10">
              ...
              {creative.callToAction && (<Button variant="outline" className="h-9 px-4 text-[11px] font-black uppercase tracking-widest border-info text-info hover:bg-info/10 shrink-0 rounded-full">
                  {formatCTALabel(creative.callToAction)}
                </Button>)}
            </div>
        ...
        {activePlatform === 'tiktok' && (<div className="bg-background rounded-tr-[2.5rem] rounded-tl-[2.5rem] overflow-hidden text-left">
            ...
            <div className="p-4 space-y-4">
              ...
              {creative.callToAction && (<Button className="w-full h-10 rounded-xl bg-foreground text-background hover:bg-foreground/90 text-[11px] font-black uppercase tracking-widest">
                  {formatCTALabel(creative.callToAction)}
                </Button>)}
            </div>
          </div>)}

        {activePlatform === 'facebook' && (<div className="relative flex flex-col font-[system-ui,-apple-system,BlinkMacSystemFont,'Segoe_UI',Roboto,Helvetica,Arial,sans-serif]" style={s.fbOuter}>
            <div className="flex flex-col" style={s.fbInner}>
              <div className="flex items-center gap-2 px-3 pb-1 pt-3">
                <PreviewAvatar pageName={pageDisplayName} profileImageUrl={creative.pageProfileImageUrl} profileImageError={profileImageError} onProfileImageError={onProfileImageError} className="size-10"/>
              ...
              <div className="flex items-stretch gap-3 border-b px-3 py-2.5" style={s.fbLinkRow}>
                ...
                {creative.callToAction ? (<button type="button" className="shrink-0 self-center rounded-md px-4 py-2 text-[15px] font-semibold" style={s.fbCta}>
                    {formatCTALabel(creative.callToAction)}
                  </button>) : null}
              </div>

              <div className="flex items-center justify-around border-b p-1" style={s.fbActionsBar}>
                <button type="button" className="flex flex-1 items-center justify-center gap-1.5 rounded-md py-2 text-[15px] font-semibold hover:bg-muted/40" style={s.textSecondary}>
