...
import {
  createCopyHandler,
  createIndexClickHandler,
  createTextChangeHandler,
  getWidthStyle,
  toStableStringItems,
  type CreativeEditorTabSharedProps,
} from './creative-editor-tabs-utils'
...
export function CopyValueButton(props: {
  ...
  return (
    <Button variant="ghost" size="icon" className={className} onClick={handleCopy} aria-label={ariaLabel}>
      {copiedField === field ? <Check className={iconClassName} /> : <Copy className={iconClassName} />}
    </Button>
  )
}

export function InsightScoreBar(props: { score: number }) {
  const { score } = props
  const style = useMemo(() => ({ width: `${score}%` }), [score])
...
export function CreativeEditorInsightsTab({
  performanceSummary = null,
  onRefreshPerformance,
  algorithmicInsights,
}: Pick<CreativeEditorTabSharedProps, 'performanceSummary' | 'onRefreshPerformance' | 'algorithmicInsights'>) {
  return (
...
export function CreativeEditorDetailsTab({
  creative,
  copiedField,
  onCopy,
}: Pick<CreativeEditorTabSharedProps, 'creative' | 'copiedField' | 'onCopy'>) {
  return (
