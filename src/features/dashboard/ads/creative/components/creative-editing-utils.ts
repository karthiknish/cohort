...
export function creativeCopyIsDirty(creative: Creative, edited: {
    headlines: string[];
    descriptions: string[];
    cta: string;
    landingPage: string;
}): boolean {
    if (!listsEqual(edited.headlines, creative.headlines ?? []))
        return true;
    if (!listsEqual(edited.descriptions, creative.descriptions ?? []))
        return true;
    if (normalizeCreativeCtaValue(edited.cta)
        !== normalizeCreativeCtaValue(creative.callToAction)) {
        return true;
    }
    if (edited.landingPage.trim() !== (creative.landingPageUrl ?? '').trim())
        return true;
    return false;
}
/** Merge edited copy into an existing Meta asset_feed_spec JSON string. */
export function mergeMetaAssetFeedSpecForSave(existingSpec: string | undefined, headlines: string[], descriptions: string[], landingPageUrl: string): string | undefined {
    ...
    if (url) {
        ...
        if (linkUrls.length > 0) {
            spec.link_urls = linkUrls.map((entry, index) => index === 0 ? { ...entry, website_url: url } : entry);
        }
        else {
