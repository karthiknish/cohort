...
export function CreativeHeader(props: {
    ...
    const socialPermalink = creative.providerId === 'facebook'
    ...
    return (<LazyMotion features={domAnimation}>
      <m.header initial="hidden" animate="visible" variants={fadeInDownVariants} transition={transitions.slow} className={cn(ADS_PAGE_THEME.innerHero, 'mb-6')}>
        ...
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex min-w-0 items-start gap-4">
            <Link href={backUrl} className="group shrink-0">
              <div className="flex size-10 items-center justify-center rounded-xl border border-border/60 bg-background/80 transition-colors group-hover:border-primary/25 group-hover:bg-primary/10">
                <ArrowLeft className="size-5 text-muted-foreground transition-colors group-hover:text-primary" aria-hidden/>
              </div>
            </Link>
            ...
            <div className="min-w-0 space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className={cn('rounded-full border px-2 py-0.5 text-[9px] font-bold uppercase tracking-widest', getStatusVariant(creative.status))}>
                  {creative.status}
                </div>
                {isDirty ? (<Badge variant="secondary" className="h-5 gap-1 rounded-full px-2 text-[10px] font-semibold">
              ...
              <h1 className="flex flex-wrap items-center gap-2.5 text-xl font-semibold tracking-tight sm:text-2xl">
                ...
                {socialPermalink ? (<a href={socialPermalink} target="_blank" rel="noopener noreferrer" className="inline-flex size-9 shrink-0 items-center justify-center rounded-xl border border-border/60 text-muted-foreground hover:border-primary/30 hover:text-primary" title="Open post on Instagram or Facebook" aria-label="Open social permalink">
                    <ExternalLink className="size-4" aria-hidden/>
                  </a>) : null}
              </h1>
              {creative.campaignName ? (<p className="text-sm text-muted-foreground">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:justify-end">
            {isDirty ? (<>
                <Button type="button" variant="outline" size="sm" onClick={onCancelEditing} disabled={isSaving} className="h-10 rounded-xl border-border/70">
                  <X className="mr-1.5 size-3.5" aria-hidden/>
                  Discard
                </Button>
                <Button type="button" size="sm" onClick={onSave} disabled={isSaving} className="h-10 rounded-full px-5 text-sm font-semibold shadow-sm">
                  {isSaving ? (<RefreshCw className="mr-1.5 size-3.5 animate-spin" aria-hidden/>) : (<Save className="mr-1.5 size-3.5" aria-hidden/>)}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button type="button" variant="outline" size="icon" className="size-10 rounded-xl border-border/70" aria-label="Creative actions">
