...
import { META_CAPI_ACTION_SOURCES, META_CAPI_STANDARD_EVENTS, META_OFFLINE_ACTION_SOURCE, } from '@/lib/meta-capi-events';
import { hasMetaEventsTools, resolveMetaCampaignUiVisibility, } from '@/lib/meta-campaign-ui';
...
function defaultEventsTab(tabs: Array<'capi' | 'offline' | 'batch'>): string {
    if (tabs.includes('capi'))
        return 'capi';
    if (tabs.includes('offline'))
        return 'offline';
    return tabs[0] ?? 'batch';
}
export function MetaEventsToolsPanel({ workspaceId, clientId, adAccountId, campaignObjective, scope, }: MetaEventsToolsPanelProps) {
    const visibility = resolveMetaCampaignUiVisibility({ campaignObjective, scope });
    const eventTabs = (() => {
        const tabs: Array<'capi' | 'offline' | 'batch'> = [];
        if (visibility.showCapi)
            tabs.push('capi');
        if (visibility.showOfflineEvents)
            tabs.push('offline');
        if (visibility.showBatchApi)
            tabs.push('batch');
        return tabs;
    })();
    ...
    const handleBatchJsonChange = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
        setBatchJson(event.target.value);
    };
    if (!hasMetaEventsTools(visibility) || eventTabs.length === 0) {
        return null;
    }
    const shellDescription = visibility.showCapi && visibility.showOfflineEvents
    ...
    return (<MetaToolsPanelShell icon={Server} title="Conversions API & events" description={shellDescription}>
      <Tabs value={syncedActiveTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="h-9 w-full flex-wrap justify-start gap-1 bg-muted/40 p-1">
          ...
          {visibility.showOfflineEvents ? (<TabsTrigger value="offline" className="gap-1.5 text-xs sm:text-sm">
              <Store className="size-3.5 shrink-0" aria-hidden/>
              Offline
            </TabsTrigger>) : null}
          {visibility.showBatchApi ? (<TabsTrigger value="batch" className="gap-1.5 text-xs sm:text-sm">
              <Zap className="size-3.5 shrink-0" aria-hidden/>
              Batch API
            </TabsTrigger>) : null}
        </TabsList>

        {visibility.showCapi ? (<MotionTabsContent activeTab={activeTab} tabValue="capi" className="mt-4 space-y-5">
            <MetaToolsFormSection title="Pixel" description="Events are attributed to this pixel in Events Manager.">
            ...
            <MetaToolsFormSection title="Event" description="Standard event name and where the conversion happened.">
              <EventFields eventName={eventName} actionSource={actionSource} email={email} value={value} currency={currency} orderId={orderId} testEventCode={testEventCode} onEventNameChange={setEventName} onActionSourceChange={setActionSource} onEmailChange={setEmail} onValueChange={setValue} onCurrencyChange={setCurrency} onOrderIdChange={setOrderId} onTestEventCodeChange={setTestEventCode} showActionSource/>
            </MetaToolsFormSection>
        ...
        {visibility.showOfflineEvents ? (<MotionTabsContent activeTab={activeTab} tabValue="offline" className="mt-4 space-y-5">
            <MetaToolsFormSection title="Pixel" description="Offline uploads use the same pixel dataset as your web events.">
              <MetaPixelPicker pixelId={pixelId} pixels={pixels} onPixelIdChange={setPixelId}/>
            </MetaToolsFormSection>

            <MetaToolsFormSection title="Store event" description="Sent with action_source physical_store for in-store or CRM conversions.">
              <EventFields eventName={eventName} actionSource={META_OFFLINE_ACTION_SOURCE} email={email} value={value} currency={currency} orderId={orderId} testEventCode={testEventCode} onEventNameChange={setEventName} onActionSourceChange={setActionSource} onEmailChange={setEmail} onValueChange={setValue} onCurrencyChange={setCurrency} onOrderIdChange={setOrderId} onTestEventCodeChange={setTestEventCode} showActionSource={false}/>
            </MetaToolsFormSection>

            <MetaToolsActionBar>
              <Button type="button" disabled={sendingOffline || !pixelId.trim()} onClick={handleSendOffline}>
                {sendingOffline ? (<Loader2 className="mr-2 size-4 animate-spin" aria-hidden/>) : (<Store className="mr-2 size-4" aria-hidden/>)}
                Send offline event
              </Button>
            </MetaToolsActionBar>
          </MotionTabsContent>) : null}

        {visibility.showBatchApi ? (<MotionTabsContent activeTab={activeTab} tabValue="batch" className="mt-4 space-y-5">
            <MetaToolsFormSection title="Batch payload" description="Array of { method, relativeUrl, body?, name? } objects. Useful for quick Graph API probes.">
              ...
              <Textarea value={batchJson} onChange={handleBatchJsonChange} rows={8} className="font-mono text-xs leading-relaxed" placeholder={'[\n  { "method": "GET", "relativeUrl": "act_123/campaigns?fields=id,name" }\n]'} spellCheck={false}/>
            </MetaToolsFormSection>

            <MetaToolsActionBar>
              <Button type="button" disabled={runningBatch || !batchJson.trim()} onClick={handleRunBatch}>
                {runningBatch ? <Loader2 className="mr-2 size-4 animate-spin" aria-hidden/> : null}
                Run batch (max 50)
              </Button>
            </MetaToolsActionBar>

            {batchResult ? (<MetaJsonResultBlock title="Response" content={batchResult}/>) : null}
          </MotionTabsContent>) : null}
      </Tabs>
    </MetaToolsPanelShell>);
}
function EventFields({ eventName, actionSource, email, value, currency, orderId, testEventCode, onEventNameChange, onActionSourceChange, onEmailChange, onValueChange, onCurrencyChange, onOrderIdChange, onTestEventCodeChange, showActionSource, }: {
    ...
    const handleEmailChange = (event: React.ChangeEvent<HTMLInputElement>) => onEmailChange(event.target.value);
    const handleOrderIdChange = (event: React.ChangeEvent<HTMLInputElement>) => onOrderIdChange(event.target.value);
    const handleValueChange = (event: React.ChangeEvent<HTMLInputElement>) => onValueChange(event.target.value);
    const handleCurrencyChange = (event: React.ChangeEvent<HTMLInputElement>) => onCurrencyChange(event.target.value);
    const handleTestEventCodeChange = (event: React.ChangeEvent<HTMLInputElement>) => onTestEventCodeChange(event.target.value);
    return (<div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <Select value={eventName} onValueChange={onEventNameChange}>
            ...
            <SelectContent>
              {META_CAPI_STANDARD_EVENTS.map((event) => (<SelectItem key={event.value} value={event.value}>
                  {event.label}
                </SelectItem>))}
            </SelectContent>
          </Select>
        </div>
        {showActionSource ? (<div className="space-y-1.5">
            ...
            <Select value={actionSource} onValueChange={onActionSourceChange}>
              ...
              <SelectContent>
                {META_CAPI_ACTION_SOURCES.map((source) => (<SelectItem key={source.value} value={source.value}>
                    {source.label}
                  </SelectItem>))}
              </SelectContent>
            </Select>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <Input value={email} onChange={handleEmailChange} placeholder="customer@example.com" className="h-10" autoComplete="off"/>
        ...
        <div className="space-y-1.5">
          ...
          <Input value={orderId} onChange={handleOrderIdChange} placeholder="Optional deduplication key" className="h-10"/>
        </div>
        <div className="space-y-1.5">
          <Label className="text-xs font-medium">Value</Label>
          <Input value={value} onChange={handleValueChange} placeholder="0.00" inputMode="decimal" className="h-10 tabular-nums"/>
        </div>
        <div className="space-y-1.5">
          <Label className="text-xs font-medium">Currency</Label>
          <Input value={currency} onChange={handleCurrencyChange} placeholder="USD" className="h-10 uppercase" maxLength={3}/>
        </div>
      </div>
      ...
      <Alert className="border-info/20 bg-info/5">
        <AlertDescription className="space-y-2 text-xs leading-relaxed">
          ...
          <div className="space-y-1.5 pt-1">
            ...
            <Input id="meta-test-event-code" value={testEventCode} onChange={handleTestEventCodeChange} placeholder="TEST12345" className="h-10 font-mono text-sm"/>
          </div>
        </AlertDescription>
      </Alert>
    </div>);
