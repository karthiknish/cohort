export type AdsMetricsDisplayState = 'loading' | 'needs_connection' | 'needs_sync' | 'synced_no_delivery' | 'has_delivery';
...
export function resolveAdsMetricsDisplayState({ metricsLoading, connectedAccountCount, hasSuccessfulSync, hasMetricData, }: ResolveAdsMetricsDisplayStateArgs): AdsMetricsDisplayState {
    if (metricsLoading) {
        return 'loading';
    }
