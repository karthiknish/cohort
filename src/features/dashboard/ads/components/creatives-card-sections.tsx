...
function CreativeImage({ src, alt, fallbackSrc }: { src?: string; alt?: string; fallbackSrc?: string }) {
  ...
  return <div className="relative h-full w-full"><NextImage src={currentSrc} alt={alt || ''} fill unoptimized sizes="(max-width: 768px) 100vw, 320px" className="object-cover" style={{ imageRendering: 'crisp-edges' }} onError={() => { if (fallbackSrc && !useFallback) setUseFallback(true); else setError(true) }} /></div>
}
