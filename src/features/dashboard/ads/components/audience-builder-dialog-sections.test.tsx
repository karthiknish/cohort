...
vi.mock('@/shared/ui/location-map', () => ({
    ...
    }) => <div>Locations: {selectedLocations.length}</div>,
}));
...
describe('audience builder dialog sections', () => {
    it('renders the dialog header and progress steps', () => {
        ...
        const markup = renderToStaticMarkup(<AudienceBuilderDialogHeader activeTab="basics" completionSteps={completionSteps} onSelectStep={onSelectStep} providerId="facebook"/>);
    ...
    it('renders the targeting tab and footer summary', () => {
        ...
        const tabsMarkup = renderToStaticMarkup(<AudienceBuilderDialogTabs activeTab="targeting" formData={sharedFormData} newInterest="" newSegment="" onAddInterest={onAddInterest} onAddSegment={onAddSegment} onAgePreset={onAgePreset} onDescriptionChange={onDescriptionChange} onGenderToggle={onGenderToggle} onInterestInputChange={onInterestInputChange} onLocationRemove={onLocationRemove} onLocationSelect={onLocationSelect} onNameChange={onNameChange} onRemoveInterest={onRemoveInterest} onRemoveSegment={onRemoveSegment} onResetGenders={onResetGenders} onSegmentInputChange={onSegmentInputChange} providerId="linkedin" setActiveTab={setActiveTab}/>);
        const onCancel = vi.fn();
        const onCreate = vi.fn();
        const footerMarkup = renderToStaticMarkup(<AudienceBuilderDialogFooter completedCount={2} loading={false} onCancel={onCancel} onCreate={onCreate} totalSteps={4}/>);
