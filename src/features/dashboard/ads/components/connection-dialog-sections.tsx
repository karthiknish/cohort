...
const STEP_CONFIG: Record<ConnectionStep, {
    label: string;
    order: number;
}> = {
...
export function ConnectionDialogHeader({ Icon, providerInfo }: {
...
export function ConnectionProgress({ step, providerName }: {
    step: ConnectionStep;
    providerName: string;
}) {
      ...
      {showPreConnect ? (<>
          <div>
            ...
            <ul className="space-y-1.5">
              {providerInfo.benefits.map((benefit) => (<li key={benefit} className="flex items-start gap-2 text-sm text-muted-foreground">
          ...
          <div>
            ...
            <ul className="space-y-1.5">
              {providerInfo.requirements.map((requirement) => (<li key={requirement} className="flex items-start gap-2 text-sm text-muted-foreground">
                  <ChevronRight className="mt-0.5 size-4 flex-shrink-0 text-muted-foreground/60"/>
                  {requirement}
                </li>))}
            </ul>
          </div>
          {isMetaAds ? (<Alert className="border-warning/20 bg-warning/5">
          ...
          {providerInfo.loginMethod === 'redirect' ? (<Alert className="border-info/20 bg-info/5">
          ...
          {providerInfo.loginMethod === 'popup' ? (<Alert className="border-info/20 bg-info/5">
...
export function ConnectionDialogFooterActions({ connectionStep, handleClose, handleConnect, isConnecting, isInProgress, onRetry, providerInfo, showPreConnect, error }: {
    connectionStep: ConnectionStep;
    handleClose: () => void;
    handleConnect: () => void;
