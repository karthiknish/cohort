...
export function CreativeMediaField({ mode = 'image', previewSrc, imageUrl, imageHash, videoId, uploading = false, disabled = false, onImageUrlChange, onFileSelect, onClear, }: CreativeMediaFieldProps) {
    ...
    return (<div className="space-y-3">
      <input ref={inputRef} type="file" accept={accept} aria-label="Upload creative media" className="sr-only" disabled={disabled || uploading} onChange={onFileSelect}/>

      {hasPreview ? (<div className="relative overflow-hidden rounded-2xl border border-border/60 bg-muted/20 ring-1 ring-border/40">
          <div className="relative aspect-[1.91/1] max-h-[220px] w-full bg-muted/40 sm:aspect-video">
            <NextImage src={previewSrc!} alt="Uploaded creative preview" fill unoptimized sizes="(max-width: 640px) 100vw, 480px" className="object-cover" onError={handlePreviewError}/>
            {uploading ? (<div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-background/75 backdrop-blur-sm">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-border/50 bg-card/80 px-3 py-2.5">
            ...
            <div className="flex gap-1.5">
              ...
              {onClear ? (<Button type="button" variant="ghost" size="icon" className="size-8 text-muted-foreground hover:text-destructive" onClick={onClear} disabled={disabled || uploading} aria-label="Remove image">
                  <Trash2 className="size-4" aria-hidden/>
                </Button>) : null}
            </div>
          </div>
        </div>) : (<button type="button" onClick={openFilePicker} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} disabled={disabled || uploading} className={cn(ADS_PAGE_THEME.controlMapFrame, 'flex w-full flex-col items-center justify-center gap-3 px-6 py-10 text-center transition-colors', 'border-2 border-dashed', dragging ? 'border-primary bg-primary/5' : 'border-border/70 bg-muted/15 hover:border-primary/35 hover:bg-muted/25', (disabled || uploading) && 'cursor-not-allowed opacity-60')}>
          {uploading ? (<Loader2 className="size-9 animate-spin text-primary" aria-hidden/>) : (<div className="flex size-12 items-center justify-center rounded-2xl bg-primary/10 ring-1 ring-primary/20">
      ...
      {!isVideo ? (<div className="flex items-center justify-between gap-2">
        ...
        {imageHash ? (<span className="font-mono text-[10px] text-muted-foreground truncate max-w-[180px]" title={imageHash}>
            hash …{imageHash.slice(-8)}
          </span>) : null}
        {videoId ? (<span className="font-mono text-[10px] text-muted-foreground truncate max-w-[180px]" title={videoId}>
            video …{videoId.slice(-8)}
          </span>) : null}
      </div>) : null}

      {showUrlField && !isVideo ? (<div className="space-y-1.5 rounded-xl border border-border/50 bg-muted/10 p-3">
          <Label htmlFor="creative-image-url" className="text-xs text-muted-foreground">
            Image URL (optional if you upload a file)
          </Label>
          <Input id="creative-image-url" type="url" placeholder="https://cdn.example.com/hero.jpg" value={imageUrl} onChange={handleImageUrlChange} disabled={disabled || uploading}/>
