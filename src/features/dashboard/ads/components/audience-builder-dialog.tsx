...
import { hasMetaAdvancedTools, hasMetaEventsTools, resolveMetaCampaignUiVisibility, } from '@/lib/meta-campaign-ui';
import { MetaAdvancedToolsPanel } from '@/features/dashboard/ads/components/meta-advanced-tools-panel';
import { AudienceBuilderDialogFooter, AudienceBuilderDialogHeader, AudienceBuilderDialogTabs, type AudienceFormData, } from './audience-builder-dialog-sections';
type Props = {
    isOpen: boolean;
...
type AudienceBuilderAction = {
    ...
    type: 'setFormData';
    value: AudienceFormData | ((prev: AudienceFormData) => AudienceFormData);
} | {
    type: 'setNewSegment';
    value: string;
} | {
...
export function AudienceBuilderDialog({ isOpen, onOpenChange, providerId }: Props) {
    ...
    const handleLocationSelect = (location: LocationMarker) => {
        dispatch({
            type: 'setFormData',
            value: (prev) => {
                if (prev.locations.some((l) => l.id === location.id))
                    return prev;
    ...
    return (<Dialog open={isOpen} onOpenChange={handleDialogOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] p-0 gap-0 overflow-hidden">
        ...
        {isMetaAudience && user?.agencyId ? (<div className="space-y-3 px-6 pb-2">
            <Alert>
            ...
            {showAccountEventsTools ? (<MetaEventsToolsPanel workspaceId={String(user.agencyId)} clientId={selectedClientId} scope="account"/>) : null}
            {showAccountAdvancedTools ? (<MetaAdvancedToolsPanel workspaceId={String(user.agencyId)} clientId={selectedClientId} scope="account"/>) : null}
          </div>) : null}
        <AudienceBuilderDialogTabs activeTab={activeTab} formData={formData} newInterest={newInterest} newSegment={newSegment} onAddInterest={handleAddInterest} onAddSegment={handleAddSegment} onAgePreset={handleAgePreset} onDescriptionChange={handleDescriptionChange} onGenderToggle={handleGenderToggle} onInterestInputChange={handleInterestInputChange} onLocationRemove={handleLocationRemove} onLocationSelect={handleLocationSelect} onNameChange={handleNameChange} onRemoveInterest={handleRemoveInterest} onRemoveSegment={handleRemoveSegment} onResetGenders={handleResetGenders} onSegmentInputChange={handleSegmentInputChange} providerId={providerId} setActiveTab={handleSelectStep}/>
