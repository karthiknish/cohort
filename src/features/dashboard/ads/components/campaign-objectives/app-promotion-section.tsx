...
export function AppPromotionSection({ formData, onChange, disabled }: ObjectiveComponentProps) {
    ...
    const handleAppStoreUrlChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        onChange({ appStoreUrl: event.target.value });
    };
    const handleDestinationUrlChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        onChange({ destinationUrl: event.target.value });
    };
    const handleAppEventTypeChange = (value: string) => {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input id="app-id" placeholder="e.g., 1234567890" value={formData.appId || ''} onChange={handleAppIdChange} disabled={disabled}/>
          ...
          <div className="space-y-2">
            ...
            <Input id="app-url" type="url" placeholder="https://apps.apple.com/app/your-app" value={formData.appStoreUrl || ''} onChange={handleAppStoreUrlChange} disabled={disabled}/>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Select value={formData.appEventType} onValueChange={handleAppEventTypeChange} disabled={disabled}>
              <SelectTrigger id="app-event">
                <SelectValue placeholder="Select app event"/>
              </SelectTrigger>
              <SelectContent>
                {APP_EVENT_TYPES.map((event) => (<SelectItem key={event.value} value={event.value}>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            <Input id="deep-link" placeholder="e.g., myapp://product/123" value={formData.destinationUrl || ''} onChange={handleDestinationUrlChange} disabled={disabled}/>
            <p className="text-xs text-muted-foreground">
