...
import type { CampaignObjective, ObjectiveComponentProps } from './types';
interface ObjectiveRendererProps extends ObjectiveComponentProps {
    objective?: CampaignObjective;
    metaContext?: ObjectiveComponentProps['metaContext'];
}
export function ObjectiveRenderer({ objective, formData, onChange, disabled, metaContext, }: ObjectiveRendererProps) {
    switch (objective) {
        case 'OUTCOME_SALES':
            return (<SalesObjectiveSection formData={formData} onChange={onChange} disabled={disabled} providerId={formData.providerId || 'facebook'} metaContext={metaContext}/>);
        case 'OUTCOME_LEADS':
            return (<LeadsObjectiveSection formData={formData} onChange={onChange} disabled={disabled} providerId={formData.providerId || 'facebook'} metaContext={metaContext}/>);
        ...
        case 'OUTCOME_ENGAGEMENT':
            return (<EngagementObjectiveSection formData={formData} onChange={onChange} disabled={disabled} providerId={formData.providerId || 'facebook'} metaContext={metaContext}/>);
        ...
        default:
            return (<div className="p-8 text-center border-2 border-dashed rounded-lg">
