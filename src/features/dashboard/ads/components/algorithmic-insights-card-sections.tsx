...
function InsightItem({ compact, insight }: { compact?: boolean; insight: AlgorithmicInsight }) {
  ...
  return <div className={cn('rounded-lg border border-l-4 bg-background p-4 transition-colors hover:bg-muted/50', levelStyles[insight.level])}><div className="flex items-start gap-3"><div className={cn('mt-0.5 rounded-full p-1', insight.level === 'success' && 'bg-success/10 text-success', insight.level === 'info' && 'bg-info/10 text-info', insight.level === 'warning' && 'bg-warning/10 text-warning', insight.level === 'critical' && 'bg-destructive/10 text-destructive')}><InsightLevelIcon level={insight.level} /></div><div className="flex-1 space-y-1"><div className="flex items-center gap-2"><h4 className="text-sm font-semibold">{insight.title}</h4>{!compact ? <InsightLevelBadge level={insight.level} /> : null}</div><p className="text-sm text-muted-foreground">{insight.message}</p>{!compact && insight.suggestion ? <div className="mt-2 flex items-start gap-2 rounded-md bg-muted/50 p-2 text-xs text-muted-foreground"><Lightbulb className="mt-0.5 h-3.5 w-3.5 shrink-0 text-warning" /><span>{insight.suggestion}</span></div> : null}</div>{insight.score !== undefined ? <div className="text-right"><span className="text-lg font-bold">{insight.score}</span><span className="text-xs text-muted-foreground">/100</span></div> : null}</div></div>
}

function getInsightKey(insight: AlgorithmicInsight, index: number) {
  return insight.id || `${insight.type}-${insight.title}-${index}`
}
...
function EfficiencyScoreRing({ score, size = 'md' }: { score: number; size?: 'sm' | 'md' | 'lg' }) {
  ...
  return <div className={cn('relative flex items-center justify-center', d.container)} role="img" aria-label={`Efficiency Score: ${score} out of 100, performance is ${performanceLevel}`}><svg className="h-full w-full" viewBox="0 0 100 100" aria-hidden="true"><circle className="stroke-muted/20" strokeWidth={d.stroke} fill="transparent" r={d.radius} cx="50" cy="50" /><circle className={cn('transition-[color,background-color,border-color,text-decoration-color,fill,stroke,opacity,box-shadow,transform,filter,backdrop-filter] duration-[var(--motion-duration-xslow)] ease-[var(--motion-ease-out)] motion-reduce:transition-none', score > 70 ? 'stroke-success' : score > 40 ? 'stroke-warning' : 'stroke-destructive')} strokeWidth={d.stroke} strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" fill="transparent" r={d.radius} cx="50" cy="50" transform="rotate(-90 50 50)" /></svg><div className="absolute inset-0 flex flex-col items-center justify-center" aria-hidden="true"><span className={cn('font-black tracking-tighter', d.text)}>{score}</span><span className={cn('font-bold uppercase tracking-widest text-muted-foreground/60', d.label)}>Score</span></div></div>
}
...
export function AlgorithmicInsightsEmptyCard({ description, emptyMessage, title }: { description: string; emptyMessage: string; title: string }) {
  return <Card className="shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2 text-lg"><Lightbulb className="h-5 w-5 text-warning" />{title}</CardTitle><CardDescription>{description}</CardDescription></CardHeader><CardContent><div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-muted/60 p-10 text-center text-sm text-muted-foreground"><Lightbulb className="h-8 w-8 text-muted-foreground/50" /><p>{emptyMessage}</p></div></CardContent></Card>
}
...
export function AlgorithmicInsightsCompactCard({ criticalCount, displayedInsights, globalEfficiencyScore, hasMoreInsights, insightsCount, onViewAll, title, warningCount }: { criticalCount: number; displayedInsights: AlgorithmicInsight[]; globalEfficiencyScore: number; hasMoreInsights: boolean; insightsCount: number; onViewAll?: () => void; title: string; warningCount: number }) {
  return <Card className="shadow-sm"><CardHeader className="pb-3"><div className="flex items-center justify-between"><div><CardTitle className="flex items-center gap-2 text-base"><Lightbulb className="h-4 w-4 text-warning" />{title}{criticalCount > 0 || warningCount > 0 ? <Badge variant="outline" className="border-warning/50 text-xs text-warning">{criticalCount + warningCount} action{criticalCount + warningCount !== 1 ? 's' : ''} needed</Badge> : null}</CardTitle></div><div className="flex items-center gap-2"><EfficiencyScoreRing score={globalEfficiencyScore} size="sm" /></div></div></CardHeader><CardContent className="space-y-2">{displayedInsights.map((insight, index) => <InsightItem key={getInsightKey(insight, index)} insight={insight} compact />)}{hasMoreInsights && onViewAll ? <Button variant="ghost" size="sm" className="w-full" onClick={onViewAll}>View all {insightsCount} insights<ArrowRight className="ml-1 h-3 w-3" /></Button> : null}</CardContent></Card>
}
