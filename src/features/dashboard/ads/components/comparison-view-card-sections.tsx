...
function TrendIndicator({ delta, invertTrend }: { delta: number | null; invertTrend?: boolean }) {
  ...
  return isPositive ? <TrendingUp className={cn('h-4 w-4', isGood ? 'text-success' : 'text-destructive')} aria-label={label} /> : <TrendingDown className={cn('h-4 w-4', isGood ? 'text-success' : 'text-destructive')} aria-label={label} />
}
