...
describe('comparison view card sections', () => {
    ...
    it('renders empty comparison states', () => {
        const markup = renderToStaticMarkup(<ComparisonViewCardShell onTabChange={vi.fn()} periodComparison={null} providerComparison={[]} selectedTab="period"/>);
    ...
    it('renders populated period and provider comparisons', () => {
        const markup = renderToStaticMarkup(<ComparisonViewCardShell currency="GBP" onTabChange={vi.fn()} periodComparison={periodComparison} providerComparison={[{ providerId: 'google_ads', metrics: { spend: 650, ctr: 0.031, roas: 3.4, cpa: 18.2, conversions: 42 } }]} providerCurrencies={{ google_ads: 'GBP' }} selectedTab="platform"/>);
    ...
    it('does not fall back to USD when no comparable currency exists', () => {
        const markup = renderToStaticMarkup(<ComparisonViewCardShell onTabChange={vi.fn()} periodComparison={periodComparison} providerComparison={[]} selectedTab="period"/>);
        expect(markup).toContain('1,000');
