...
function formatMetricMoney(value: number, currency?: string | null) {
  if (!currency) {
...
export function buildMetricsTableColumns(currency?: string): ColumnDef<MetricRecord>[] {
  return [
    ...
    { accessorKey: 'spend', header: () => <HeaderWithTooltip title="Spend" tooltip="Total amount spent on ads" />, cell: ({ row }) => formatMetricMoney(row.getValue('spend'), row.original.currency ?? currency) },
    ...
    { accessorKey: 'revenue', header: () => <HeaderWithTooltip title="Revenue" tooltip="Total revenue generated from ads" />, cell: ({ row }) => { const revenue = row.getValue('revenue') as number | null | undefined; return revenue != null ? formatMetricMoney(revenue, row.original.currency ?? currency) : '—' } },
  ]
}
