...
describe('formula builder card sections', () => {
    it('renders the header and create dialog', () => {
        ...
        const dialogMarkup = renderToStaticMarkup(<FormulaBuilderDialog availableMetrics={[{ name: 'spend', label: 'Spend', type: 'currency' }]} dialogOpen={true} exampleFormulas={[{ name: 'Cost per Click', formula: 'spend / clicks', output: 'cpc' }]} formulaExpression="spend / clicks" formulaName="Cost per Click" onDialogOpenChange={vi.fn()} onExpressionChange={vi.fn()} onFormulaNameChange={vi.fn()} onInsertMetric={vi.fn()} onOutputMetricChange={vi.fn()} onSave={vi.fn()} onUseExample={vi.fn()} outputMetric="cpc" previewResult={2.34} validation={VALIDATION}/>);
    ...
    it('renders saved formulas', () => {
        const markup = renderToStaticMarkup(<FormulaBuilderList items={[{ formula: { workspaceId: 'w1', formulaId: 'f1', name: 'ROAS', formula: 'revenue / spend', inputs: ['revenue', 'spend'], outputMetric: 'roas', isActive: true, createdBy: 'u1' }, result: 4.5 }]} onDelete={vi.fn()}/>);
