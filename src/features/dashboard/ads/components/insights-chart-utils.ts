...
export function resolveInsightsChartCurrency(selectedProvider: string, fallbackCurrency: string | undefined, providerCurrencyMap: Record<string, string>): string {
...
export function buildProviderCurrencyMapFromMetrics(metrics: MetricRecord[]): Record<string, string> {
    ...
    for (const [providerId, rows] of rowsByProvider) {
        ...
        if (financialTotals.comparability === 'single_currency' &&
...
export function resolveCurrencyFromProcessedMetrics(metrics: MetricRecord[]): string | null {
 ...
 */
export function resolveAdsDisplayCurrency(fallbackCurrency: string | null | undefined, metrics: MetricRecord[], providerCurrencyMap: Record<string, string> = {}): string | undefined {
...
export function resolveChartProviderKey(selected: string, chartKeys: string[]): string {
    if (selected === 'all') {
        if (chartKeys.includes('all'))
            return 'all';
        return chartKeys[0] ?? 'all';
    }
    const normalized = normalizeProviderId(selected);
    if (chartKeys.includes(normalized))
        return normalized;
...
export function providerSummariesToSyntheticMetrics(summaries: Record<string, ProviderSummary>): MetricRecord[] {
...
export function pickDefaultInsightsTab(chartData: InsightsChartDataSlice | undefined): InsightsTabId {
    if (!chartData)
        return 'comparison';
    const hasComparison = chartData.providerComparison.some((p) => p.metrics.spend > 0 || p.metrics.revenue > 0);
    if (hasComparison)
        return 'comparison';
    const hasFunnel = Object.values(chartData.funnelCharts).some((stages) => stages.some((s) => s.value > 0));
    if (hasFunnel)
        return 'funnel';
...
export function tabHasChartData(tab: InsightsTabId, chartData: InsightsChartDataSlice, providerKey: string): boolean {
    switch (tab) {
        case 'comparison':
            return chartData.providerComparison.some((p) => p.metrics.spend > 0 || p.metrics.revenue > 0);
        case 'funnel': {
