...
import { RAW_ADS_PROVIDER_IDS, type ConvexIntegrationStatusRow, type IntegrationStatusInfo, } from './ads-connections-types';
        ...
        return [
            {
                ...
                lastSyncRequestedAt: typeof row.lastSyncRequestedAtMs === 'number'
...
export function buildIntegrationStatusMap(statuses: IntegrationStatus[]): Record<string, IntegrationStatusInfo> {
...
export function deriveConnectedProviders(statuses: IntegrationStatus[] | undefined): Record<string, boolean> {
    if (!statuses) {
        return {};
    }
