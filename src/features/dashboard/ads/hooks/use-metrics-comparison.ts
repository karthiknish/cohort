...
function filterMetricsByDateRange(metrics: MetricRecord[], range: DateRange): MetricRecord[] {
...
function calculateMetricsDelta(current: ComparisonMetrics, previous: ComparisonMetrics): MetricsDelta {
    return {
...
function calculateMetricsDeltaPercent(current: ComparisonMetrics, previous: ComparisonMetrics): MetricsDelta {
    return {
 ...
 */
export function useMetricsComparison(options: UseMetricsComparisonOptions): UseMetricsComparisonReturn {
    const { metrics, dateRange } = options;
    // Period comparison (current vs previous period)
    const periodComparison = ((): PeriodComparison | null => {
    ...
    const getProviderMetrics = (providerId: string): ComparisonMetrics | null => {
        const provider = providerComparison.find((p) => p.providerId === providerId);
        return provider?.metrics ?? null;
    };
    // Compare two custom date ranges
    const compareDateRanges = (range1: DateRange, range2: DateRange) => {
    ...
    // Compare two providers
    const compareProviders = (provider1: string, provider2: string) => {
