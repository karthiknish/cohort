...
import { buildProviderSummariesFromServer, dedupeAndFilterMetrics, formatMetricQueryDate, hasAdsMetricActivity, isAdsProviderId, isMetricDateInRange, mapRealtimeMetricRow, metricsForTableDisplay, parseMetricDate, } from './use-ads-metrics.helpers';
...
describe('useAdsMetrics helpers', () => {
    ...
    it('detects activity from server summary when row metrics are empty', () => {
        expect(hasAdsMetricActivity([], { totals: { spend: 282, impressions: 19417, clicks: 145, conversions: 0, revenue: 0 }, providers: {}, count: 1 }, null)).toBe(true);
    });
