...
export function useAdsConnectionActions({
  ...
  const handleConnect = useCallback(
    ...
    [setConnectedProviderOverrides, setConnectionErrors, setConnectingProvider, triggerRefresh],
  )
      ...
      try {
        ...
        if (providerId === PROVIDER_IDS.FACEBOOK) {
          ...
          if (typeof url !== 'string' || url.length === 0) {
            throw new Error(
              'Meta OAuth did not return a URL. Check server logs and environment variables.',
            )
          }
        ...
        const message =
          isMetaConfigError || isTikTokConfigError ? rawMessage : asErrorMessage(error)
        ...
        if (
          ...
          setMetaSetupMessage(
            'Meta business login is not configured. Add META_APP_ID, META_BUSINESS_CONFIG_ID, and META_OAUTH_REDIRECT_URI environment variables.',
          )
        }
        if (
          ...
          setTiktokSetupMessage(
            'TikTok OAuth is not configured. Add TIKTOK_CLIENT_KEY, TIKTOK_CLIENT_SECRET, and TIKTOK_OAUTH_REDIRECT_URI environment variables.',
          )
        }
        reportConvexFailure({
          error,
  ...
  const handleDisconnect = useCallback(
    async (providerId: string, options?: DisconnectOptions) => {
  ...
  const handleSyncNow = useCallback(
    ...
    [runManualSyncAction, selectedClientId, setSyncingProviders, toast, triggerRefresh, workspaceId],
  )
