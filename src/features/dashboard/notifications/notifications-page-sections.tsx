...
export function NotificationVirtualRow({ start, dataIndex, measureRef, children, }: {
    start: number;
    dataIndex: number;
    ...
    return (<div data-index={dataIndex} ref={measureRef} className="absolute left-0 top-0 w-full pb-2" style={style}>
      {children}
    </div>);
}
export function NotificationsLoadingSkeleton() {
    return (<output className="block space-y-3 py-2" aria-live="polite" aria-busy="true" aria-label="Loading notifications">
      {['n-sk-1', 'n-sk-2', 'n-sk-3', 'n-sk-4', 'n-sk-5'].map((key) => (<div key={key} className="flex items-start gap-4 rounded-lg border p-4">
...
export function NotificationsPageHeader({ onRefresh, onMarkAllRead, onClearAll, refreshing, unreadCount, notificationsCount, ackInFlight, }: {
    onRefresh: () => void;
    onMarkAllRead: () => void;
    onClearAll: () => void;
    refreshing: boolean;
    ...
    return (<DashboardPageHero>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button variant="outline" size="sm" onClick={onRefresh} disabled={refreshing} className={getButtonClasses('outline')}>
          {refreshing ? <LoaderCircle className={cn('mr-2 size-4', DASHBOARD_THEME.animations.spin)}/> : null}
          Refresh
        </Button>
        <Button variant="outline" size="sm" onClick={onMarkAllRead} disabled={unreadCount === 0 || ackInFlight} className={getButtonClasses('outline')}>
          <CheckCheck className="mr-2 size-4"/>
          Mark all read
        </Button>
        <Button variant="outline" size="sm" onClick={onClearAll} disabled={notificationsCount === 0 || ackInFlight} className={getButtonClasses('outline')}>
...
export function NotificationsErrorAlert({ error, isPreviewMode, retrying, onRetry, }: {
    error: string;
    ...
    return (<Alert variant="destructive">
      ...
      <AlertDescription className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        ...
        {!isPreviewMode ? (<Button type="button" variant="outline" size="sm" className="shrink-0 border-destructive/40" onClick={onRetry} disabled={retrying}>
...
export function NotificationsFilterTabsList({ activeFilter, notificationsCount, unreadCount, }: {
    activeFilter: FilterType;
    notificationsCount: number;
    unreadCount: number;
}) {
    return (<TabsList className="grid h-auto w-full grid-cols-2 gap-1 sm:grid-cols-3 lg:grid-cols-6">
      <TabsTrigger value="all" className="relative">
        All
        {activeFilter === 'all' && notificationsCount > 0 ? (<Badge variant="secondary" className="ml-2">
            {notificationsCount}
          </Badge>) : null}
      </TabsTrigger>
      <TabsTrigger value="unread" className="relative">
        Unread
        {unreadCount > 0 ? (<Badge variant="default" className="ml-2">
            {unreadCount}
          </Badge>) : null}
      </TabsTrigger>
...
export function NotificationsBulkSelectionBar({ selectedCount, ackInFlight, onBulkMarkRead, onBulkDismiss, onClearSelection, }: {
    ...
    return (<div className="mb-4 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-border/60 bg-muted/20 px-3 py-2">
...
export function NotificationsFeedList({ page, }: {
        ...
        <div ref={notificationScrollRef} className="h-[calc(100vh-24rem)] overflow-y-auto">
          <div className="relative w-full" style={virtualContainerStyle}>
            {notificationVirtualizer.getVirtualItems().map((vi) => {
                ...
                return (<NotificationVirtualRow key={notification.id} start={vi.start} dataIndex={vi.index} measureRef={notificationVirtualizer.measureElement}>
                  <NotificationItem notification={notification} ackInFlight={ackInFlight} selected={selectedIds.has(notification.id)} onOpen={handleOpenNotification} onDismiss={handleDismiss} onMarkRead={handleMarkAsRead} onSelectToggle={handleSelectToggle}/>
                </NotificationVirtualRow>);
            })}
          </div>
        </div>
        <NotificationsFeedFooter loading={loading} loadingMore={loadingMore} nextCursor={Boolean(nextCursor)} notificationsCount={notifications.length} onLoadMore={handleLoadMore}/>
      </>);
    }
    return (<>
      <ScrollArea className="h-[calc(100vh-24rem)]">
        <div className="space-y-2">
          {notifications.map((notification) => (<NotificationItem key={notification.id} notification={notification} ackInFlight={ackInFlight} selected={selectedIds.has(notification.id)} onOpen={handleOpenNotification} onDismiss={handleDismiss} onMarkRead={handleMarkAsRead} onSelectToggle={handleSelectToggle}/>))}
        </div>
      </ScrollArea>
      <NotificationsFeedFooter loading={loading} loadingMore={loadingMore} nextCursor={Boolean(nextCursor)} notificationsCount={notifications.length} onLoadMore={handleLoadMore}/>
    </>);
}
export function NotificationsFeedFooter({ loading, loadingMore, nextCursor, notificationsCount, onLoadMore, }: {
    loading: boolean;
    loadingMore: boolean;
    nextCursor: boolean;
    notificationsCount: number;
    ...
    return (<>
      {!loading && notificationsCount > 0 && nextCursor ? (<div className="mt-4 flex justify-center">
      ...
      {!loading && notificationsCount > 0 ? (<div className="mt-4 text-center text-xs text-muted-foreground">
    ...
    return (<Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          {unreadCount > 0 && activeFilter !== 'unread' ? (<Badge variant="destructive">{unreadCount} unread</Badge>) : null}
        </div>
      </CardHeader>
      <CardContent>
