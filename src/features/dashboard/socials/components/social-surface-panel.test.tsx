...
describe('SocialSurfacePanel', () => {
    it('renders loading skeleton when overviewLoading is true', () => {
        const markup = renderToStaticMarkup(<SocialSurfacePanel surface="facebook" kpis={[]} overview={null} overviewLoading={true} connected={true} setupComplete={true} hasData={false} onRequestSync={noop}/>);
        expect(markup).not.toContain('Facebook not connected');
    });
    it('renders organic KPI grid when connected and data loaded', () => {
        const markup = renderToStaticMarkup(<SocialSurfacePanel surface="facebook" kpis={organicKpis} overview={overview} overviewLoading={false} connected={true} setupComplete={true} hasData={true} onRequestSync={noop}/>);
    ...
    it('renders empty state when not connected', () => {
        const markup = renderToStaticMarkup(<SocialSurfacePanel surface="instagram" kpis={[]} overview={null} overviewLoading={false} connected={false} setupComplete={false} hasData={false} onRequestSync={noop}/>);
