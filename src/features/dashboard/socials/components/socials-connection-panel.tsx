...
function SetupStepper({ connected, setupComplete, hasSynced, }: {
    ...
    return (<ol className="flex flex-wrap items-center gap-2 sm:gap-3" aria-label="Organic social setup steps">
      {SETUP_STEPS.map((step, index) => {
            ...
            <div className="flex items-center gap-2">
              <span className={cn('flex size-7 items-center justify-center rounded-full border text-xs font-semibold transition-colors', complete && 'border-success/30 bg-success/10 text-success', current && 'border-primary/30 bg-primary/10 text-primary', !complete && !current && 'border-muted/60 bg-muted/30 text-muted-foreground')} aria-hidden>
                {complete ? <CheckCircle2 className="size-3.5"/> : <Circle className="size-2 fill-current"/>}
              </span>
              <span className={cn('text-xs font-medium sm:text-sm', complete || current ? 'text-foreground' : 'text-muted-foreground')}>
                {step.label}
              </span>
            </div>
            {index < SETUP_STEPS.length - 1 ? (<span className="hidden h-px w-6 bg-border sm:block" aria-hidden/>) : null}
          </li>);
        })}
    </ol>);
}
export function SocialsConnectionPanel({ panelId, selectedClientName, connected, setupComplete, accountName, lastSyncedAtMs, lastSyncStatus, oauthPending, syncPending, connectionError, statusQueryError, onConnectMeta, onDisconnect, onRequestSync, }: SocialsConnectionPanelProps) {
    ...
    return (<div id={panelId} className="space-y-3">
      <Card className={cn('overflow-hidden border-muted/50 shadow-sm ring-1 ring-muted/20', DASHBOARD_THEME.cards.base)}>
        <CardContent className="space-y-5 p-5 sm:p-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex min-w-0 gap-4">
              ...
              <div className="min-w-0 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <ConnectionStatusBadge connected={connected} setupComplete={setupComplete} syncPending={syncPending} lastSyncStatus={lastSyncStatus}/>
                </div>
            ...
            <Button type="button" onClick={handleConnectMeta} disabled={oauthPending} className={cn(getButtonClasses('primary'), 'w-full shrink-0 gap-2 shadow-sm lg:w-auto lg:min-w-[220px]')}>
              {oauthPending ? (<LoaderCircle className="size-4 animate-spin" aria-hidden/>) : (<Shield className="size-4" aria-hidden/>)}
          ...
          <div className="flex flex-wrap gap-2">
            {selectedClientName ? (<Badge variant="outline" className={cn(DASHBOARD_THEME.badges.base, 'font-normal')}>
                Workspace · {selectedClientName}
              </Badge>) : null}
            {accountName ? (<Badge variant="outline" className={cn(DASHBOARD_THEME.badges.base, 'font-normal')}>
                Authorized · {accountName}
              </Badge>) : null}
            {connected ? (<Badge variant="outline" className={cn(DASHBOARD_THEME.badges.base, 'font-normal tabular-nums')}>
                {syncLabel}
            </Badge>) : null}
          </div>

          {connectionError ? (<div role="alert" className="flex items-start gap-2 rounded-xl border border-destructive/20 bg-destructive/5 px-3 py-2.5 text-sm text-destructive">
          ...
          {connected ? (<div className="flex flex-wrap items-center gap-2 border-t border-muted/40 pt-4">
            <Button type="button" variant="outline" size="sm" onClick={onRequestSync} disabled={!canSync} className={getButtonClasses('outline')}>
            ...
            <Button type="button" variant="outline" size="sm" onClick={handleDisconnectClick} disabled={oauthPending} className={cn(getButtonClasses('outline'), 'text-destructive hover:bg-destructive/5 hover:text-destructive')}>
              ...
              {!setupComplete ? (<p className="w-full text-xs text-muted-foreground sm:w-auto sm:ml-auto">
                  Confirm a Page before syncing.
                </p>) : null}
          </div>) : (<p className="border-t border-muted/40 pt-4 text-xs text-muted-foreground">
