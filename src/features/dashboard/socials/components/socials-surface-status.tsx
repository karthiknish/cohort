...
export function SurfaceTabStatusBadge({ status }: {
    status: SocialsSurfaceStatus;
}) {
    const { label, className, icon: Icon } = getSurfaceTabBadge(status);
    const spinning = status === 'loading';
    return (<Badge variant="outline" className={cn('ml-2 gap-1 border-0 px-2 py-0 text-[10px] font-semibold normal-case tracking-normal', className)}>
