...
import { SocialsMetaSetupCard, SocialsMetaSourceSwitcherCard, SocialsSurfaceInventoryCard, } from './socials-connection-panel-sections';
const setupState = {
...
describe('SocialsMetaSetupCard', () => {
    it('renders switch-source guidance for partial recovery states', () => {
        const markup = renderToStaticMarkup(<SocialsMetaSetupCard setupState={setupState} selectedSourceLabel="Acme Ads" sourceSelectionRequired={false} loadingSources={false} facebookStatus="ready" instagramStatus="empty" facebookCount={2} instagramCount={0} onReloadSources={vi.fn()} onRetryDiscovery={vi.fn()}/>);
...
describe('SocialsMetaSourceSwitcherCard', () => {
    it('renders switch-source controls with current source context', () => {
        const markup = renderToStaticMarkup(<SocialsMetaSourceSwitcherCard title="Switch Meta source for this workspace" description="Try another source if the current one only loads one surface." selectedMetaAccountId="acct-1" selectedSourceLabel="Acme Ads" metaAccountOptions={[
                { id: 'acct-1', name: 'Acme Ads' },
                { id: 'acct-2', name: 'Acme Commerce' },
            ]} loadingMetaAccountOptions={false} initializingMeta={false} onReloadAccounts={vi.fn()} onSelectAccount={vi.fn()} onConfirmSource={vi.fn()} confirmationLabel="Switch source"/>);
...
describe('SocialsSurfaceInventoryCard', () => {
    ...
    it('renders surfaced items when data is available', () => {
        const markup = renderToStaticMarkup(<SocialsSurfaceInventoryCard {...baseProps} count={1} status="ready" items={[{ id: 'page-1', name: 'Acme Page', subtitle: 'Publishing and insight access enabled' }]}/>);
