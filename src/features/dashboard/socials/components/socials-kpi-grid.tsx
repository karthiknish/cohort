...
export function SocialsKpiGrid({ items }: SocialsKpiGridProps) {
  return (
    <FadeInStagger as="div" className={DASHBOARD_THEME.stats.container}>
      {items.map((item) => {
          ...
          <FadeInItem key={item.id}>
            <Card
              className={cn(
                ...
                'overflow-hidden border-muted/50 shadow-sm transition-[box-shadow,border-color,transform] hover:-translate-y-0.5 hover:border-accent/20 hover:shadow-md motion-reduce:hover:translate-y-0',
                accent,
              )}
            >
