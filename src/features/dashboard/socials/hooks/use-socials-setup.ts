...
export function useSocialsSetup(status: SocialsConnectionStatus | null): UseSocialsSetupReturn {
    ...
    useEffect(() => {
        ...
        void loadPages();
    }, [isConnected, selectedClientId, status?.facebookPageId, workspaceId]);
    const confirmSelectedPage = async () => {
        if (!canAct || !workspaceId || !selectedPageId)
            return;
    ...
    const setupState: SocialsMetaSetupState = (() => {
        if (!status?.connected) {
            return {
                stage: 'disconnected',
    ...
    const selectedPageLabel = pages.find((p) => p.id === selectedPageId)?.name ?? status?.facebookPageName ?? null;
    return {
        setupState,
        pages,
        pagesLoading,
