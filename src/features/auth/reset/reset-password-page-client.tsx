...
import { ResetPasswordErrorState, ResetPasswordLoadingState, ResetPasswordReadyForm, ResetPasswordSuccessState, } from './reset-password-sections';
...
function ResetPasswordContent({ oobCode }: ResetPasswordPageClientProps) {
    ...
    return (<PageSkeletonBoundary loading={status === 'loading'} loadingContent={<AuthPageSkeleton />}>
      <AuthShell>
        <AuthPanel title="Set new password" description="Choose a strong password you have not used on this account before." icon={authLockIconLg}>
          {status === 'loading' && <ResetPasswordLoadingState />}

          {status === 'error' && <ResetPasswordErrorState verificationError={verificationError}/>}

          {status === 'ready' && (<ResetPasswordReadyForm email={email} newPassword={newPassword} confirmPassword={confirmPassword} showPassword={showPassword} showConfirmPassword={showConfirmPassword} submitting={submitting} formError={formError} passwordStrength={passwordStrength} passwordsMatch={passwordsMatch} onSubmit={handleSubmit} onNewPasswordChange={handleNewPasswordChange} onConfirmPasswordChange={handleConfirmPasswordChange} onToggleShowPassword={handleToggleShowPassword} onToggleShowConfirmPassword={handleToggleShowConfirmPassword}/>)}
