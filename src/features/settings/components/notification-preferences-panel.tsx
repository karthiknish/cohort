...
import { applyPreferencesPatch, normalizePreferences, type NotificationCategory, type NotificationChannel, type NotificationPreferencesV2, } from '@/lib/notifications/preferences';
...
type NotificationPreferencesPanelAction = {
    type: 'setLocalPrefs';
    value: NotificationPreferencesV2 | null;
} | {
    type: 'setLoading';
    value: boolean;
} | {
    ...
    type: 'syncPreview';
    value: NotificationPreferencesV2;
} | {
    type: 'syncServer';
    value: NotificationPreferencesV2;
} | {
    ...
    type: 'beginPersist';
    previous: NotificationPreferencesV2;
    next: NotificationPreferencesV2;
} | {
    type: 'persistSuccess';
    value: NotificationPreferencesV2;
} | {
...
function notificationPreferencesPanelReducer(state: NotificationPreferencesPanelState, action: NotificationPreferencesPanelAction): NotificationPreferencesPanelState {
    switch (action.type) {
        case 'setLocalPrefs':
...
export function NotificationPreferencesPanel() {
    ...
    const persist = async (patch: Parameters<typeof applyPreferencesPatch>[1], options?: {
        silent?: boolean;
    }) => {
        if (!localPrefs)
            return null;
    ...
    const handleQuietHoursEnabledChange = (checked: boolean) => {
        if (!localPrefs)
            return;
    ...
    const handleChannelChange = (category: NotificationCategory, channel: NotificationChannel, enabled: boolean) => {
        void persist({
            categories: {
    ...
    const handleQuietHoursStartChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        handleQuietHoursTimeChange('start', event.target.value);
    };
    const handleQuietHoursEndChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    return (<div className="space-y-6">
      <Card>
        <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <Link href="/dashboard/notifications" className="inline-flex shrink-0 items-center gap-1.5 text-sm font-medium text-primary hover:underline">
            Open notification center
            <ExternalLink className="size-3.5" aria-hidden/>
          </Link>
        </CardHeader>
        <CardContent className="space-y-6">
          {saving ? (<output className="flex items-center gap-2 text-sm text-muted-foreground" aria-live="polite">
              <LoaderCircle className="size-4 animate-spin" aria-hidden/>
              Saving…
            </output>) : null}

          <FormField id="pause-all-notifications" label="Pause all notifications" description="Temporarily stop in-app and email alerts. Your preferences are kept." orientation="horizontal" className="items-center justify-between rounded-lg border border-border/60 bg-muted/20 px-4 py-3">
            <Switch id="pause-all-notifications" checked={localPrefs.pauseAll} disabled={saving} onCheckedChange={handlePauseAllChange}/>
          </FormField>

          <div className="space-y-4 rounded-lg border border-border/60 p-4">
            <FormField id="quiet-hours-enabled" label="Quiet hours" description="Pause email notifications during a daily window (in-app still delivers)." labelPrefix={QUIET_HOURS_LABEL_PREFIX} orientation="horizontal" className="items-start justify-between">
              <Switch id="quiet-hours-enabled" checked={localPrefs.quietHours.enabled} disabled={saving || localPrefs.pauseAll} onCheckedChange={handleQuietHoursEnabledChange}/>
            </FormField>

            {localPrefs.quietHours.enabled ? (<div className="grid gap-4 sm:grid-cols-2">
                <FormField id="quiet-hours-start" label="From">
                  <Input id="quiet-hours-start" type="time" value={localPrefs.quietHours.start} disabled={saving || localPrefs.pauseAll} onChange={handleQuietHoursStartChange}/>
                </FormField>
                <FormField id="quiet-hours-end" label="Until">
                  <Input id="quiet-hours-end" type="time" value={localPrefs.quietHours.end} disabled={saving || localPrefs.pauseAll} onChange={handleQuietHoursEndChange}/>
                </FormField>
              </div>) : null}
          </div>

          <NotificationCategoryMatrix preferences={localPrefs} disabled={saving} onChannelChange={handleChannelChange}/>
