...
export function DeleteAccountCard({ embedded = false }: DeleteAccountCardProps) {
    ...
    const body = (<>
      ...
      <CardContent className={embedded ? 'px-0' : undefined}>
        ...
        {isPreviewMode ? (<p className="mt-3 text-sm text-muted-foreground">
    ...
    return (<>
      {embedded ? (<div className="space-y-0">{body}</div>) : (<Card className="border-destructive/40">{body}</Card>)}
...
export function DeleteAccountDialog({ open, onOpenChange }: DeleteAccountDialogProps) {
    ...
    const handleAccountDeletion = () => {
        ...
        const keysToRemove = Object.keys(localStorage).filter((key) => key.startsWith('cohorts_') ||
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="space-y-3">
          <Input id="delete-account-confirm" value={deleteConfirm} onChange={handleDeleteConfirmChange} placeholder="Type DELETE to confirm" autoComplete="off" autoCorrect="off" spellCheck={false} aria-invalid={deleteAccountError ? true : undefined} aria-describedby={deleteAccountError ? 'delete-account-error' : 'delete-account-hint'}/>
          {deleteAccountError ? (<p id="delete-account-error" className="text-sm text-destructive" role="alert">
              {deleteAccountError}
            </p>) : (<p id="delete-account-hint" className="sr-only">
        ...
        <DialogFooter>
          ...
          <Button type="button" variant="destructive" onClick={handleAccountDeletion} disabled={deleteConfirm.trim().toLowerCase() !== 'delete' || deleteAccountLoading}>
