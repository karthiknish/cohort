...
import {
  adminTeamPageReducer,
  createInitialAdminTeamPageState,
  deriveNextStatus,
  isAdminUserRole,
  isAdminUserStatus,
  looksLikeEmail,
...
export function useAdminTeamPage() {
  ...
  const handleRoleChange = useCallback((userId: string, role: AdminUserRecord['role']) => {
    if (isPreviewMode) {
      dispatch({
  ...
  }, [clearActionError, isPreviewMode, reportActionFailure, toast, updateUserRoleStatus, users])

  const handleAdminToggle = useCallback((record: AdminUserRecord, makeAdmin: boolean) => {
  ...
  const handleStatusAction = useCallback((userRecord: AdminUserRecord) => {
  ...
  const createRoleChangeHandler = useCallback(
  ...
  const createAdminToggleHandler = useCallback(
    (record: AdminUserRecord) => (event: ChangeEvent<HTMLInputElement>) => {
      handleAdminToggle(record, event.target.checked)
    },
    [handleAdminToggle]
  )

  const createStatusActionHandler = useCallback(
