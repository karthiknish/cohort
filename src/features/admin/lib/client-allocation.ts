...
function pushUnmatched(unmatched: UnmatchedClientAllocation[], seenUnmatched: Set<string>, item: UnmatchedClientAllocation) {
    const normalizedPerson = normalizeAllocationValue(item.person);
    if (!normalizedPerson)
        return;
    const dedupeKey = `${item.clientId}:${normalizedPerson}`;
    if (seenUnmatched.has(dedupeKey))
        return;
...
export function dedupeClientTeamMembers(accountManager: string, teamMembers: Array<{
    name: string;
    role?: string;
}>) {
