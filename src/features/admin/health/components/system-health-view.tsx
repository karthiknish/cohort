...
import { Activity, AlertCircle, CheckCircle2, ChevronDown, ChevronUp, Clock, Database, Globe, Mail, Package, PieChart, RefreshCw, Server, Settings, XCircle, Zap, } from 'lucide-react';
...
function ServiceHealthCard({ name, check, isExpanded, onToggleExpand }: ServiceHealthCardProps) {
    ...
    return (<Card className={cn('overflow-hidden border-border/80 bg-card shadow-sm transition-[border-color,box-shadow]', check.status === 'error' && 'border-destructive/40 bg-destructive/3', isWarning && 'border-warning/35 bg-warning/6')}>
      <CardHeader className="flex flex-row items-start justify-between gap-3 pb-3">
        <div className="flex min-w-0 flex-1 items-start gap-3">
          <div className={cn('flex size-10 shrink-0 items-center justify-center rounded-md', check.status === 'ok'
        ...
        <div className="flex shrink-0 flex-col items-end gap-1.5 sm:flex-row sm:items-center sm:gap-2">
          {check.responseTime !== undefined ? (<Badge variant="outline" className="font-mono text-[10px] tabular-nums">
      ...
      <CardContent className="space-y-3 pt-0">
        {check.message ? (<p className={cn('text-xs font-medium leading-relaxed', check.status === 'error'
                ? 'text-destructive'
        ...
        {check.metadata && Object.keys(check.metadata).length > 0 ? (<div className="space-y-2 border-t border-border/50 pt-3">
            <Button type="button" variant="ghost" size="sm" className="h-8 w-full justify-between px-2 text-xs text-muted-foreground hover:text-foreground" onClick={handleToggleExpand} aria-expanded={isExpanded}>
              <span>{isExpanded ? 'Hide details' : 'Show details'}</span>
              {isExpanded ? (<ChevronUp className="size-4 shrink-0" aria-hidden/>) : (<ChevronDown className="size-4 shrink-0" aria-hidden/>)}
            </Button>

            {isExpanded ? (<div className="max-h-48 overflow-auto rounded-md border border-border/60 bg-muted/30 p-3 text-[11px] leading-relaxed">
...
function SectionLabel({ children }: {
    ...
    return (<p className="text-[11px] font-medium uppercase tracking-[0.2em] text-muted-foreground">{children}</p>);
}
      ...
      <div className="space-y-4 border-b border-border/60 pb-8">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-start gap-3 sm:items-center">
            <div className={cn('flex size-12 shrink-0 items-center justify-center rounded-xl', resolvedData?.status === 'healthy'
            ? 'bg-success/10'
          ...
          <Button type="button" variant="outline" size="sm" className="shrink-0 gap-2 self-start sm:self-center" onClick={handleRefetch} disabled={refreshing || isPreviewMode} title={isPreviewMode ? 'Disabled in preview mode' : 'Runs a fresh health request'}>
