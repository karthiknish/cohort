...
export function UserSearchPicker({ id, value, onChange, options, placeholder, searchPlaceholder, emptyText, disabled = false, excludeNames = EMPTY_EXCLUDE_NAMES }: UserSearchPickerProps) {
    ...
    const selectHandlers = Object.fromEntries(availableOptions.map((option) => [option.id, () => handleSelectOption(option.name)])) as Record<string, () => void>;
    return (<Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button id={triggerId} type="button" variant="outline" role="combobox" aria-expanded={open} aria-controls={listboxId} disabled={disabled} className={cn('w-full justify-between font-normal', !selectedLabel && 'text-muted-foreground')}>
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0" align="start">
        <Command>
          ...
          <CommandList id={listboxId} role="listbox" aria-labelledby={triggerId}>
            ...
            return (<CommandItem key={option.id} value={`${option.name} ${option.email ?? ''}`} onSelect={selectHandlers[option.id]}>
