...
export function AdminClientsStatsGrid({ clientsLoading, clientsCount, nextCursor, existingTeamMembers, assignableUsersCount, unmatchedCount, }: AdminClientsStatsGridProps) {
...
export function AdminClientsNewClientForm({ assignableUsers, clientName, clientAccountManager, teamMemberFields, clientSaving, onFormSubmit, onClientNameChange, onAccountManagerChange, onAccountManagerInputChange, onAddTeamMemberField, onUpdateTeamMemberName, onUpdateTeamMemberRole, onRemoveTeamMemberField, onResetClientForm, }: AdminClientsNewClientFormProps) {
    return (<form className="space-y-4" onSubmit={onFormSubmit}>
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="admin-client-name">Client name</Label>
          <Input id="admin-client-name" placeholder="e.g. Horizon Ventures" value={clientName} onChange={onClientNameChange} required disabled={clientSaving}/>
        </div>
        <div className="space-y-2">
          <Label htmlFor="admin-client-owner">Account manager</Label>
          {assignableUsers.length > 0 ? (<UserSearchPicker id="admin-client-owner" value={clientAccountManager} onChange={onAccountManagerChange} options={assignableUsers} placeholder="Select a workspace owner" searchPlaceholder="Search internal teammates" emptyText="No matching teammate found." disabled={clientSaving}/>) : (<Input id="admin-client-owner" placeholder="Primary owner" value={clientAccountManager} onChange={onAccountManagerInputChange} required disabled={clientSaving}/>)}
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {teamMemberFields.map((member, index) => (<AdminClientsTeamMemberFieldRow key={member.key} member={member} index={index} assignableUsers={assignableUsers} clientSaving={clientSaving} teamMembersLength={teamMemberFields.length} onUpdateName={onUpdateTeamMemberName} onUpdateRole={onUpdateTeamMemberRole} onRemove={onRemoveTeamMemberField} excludeNames={teamMemberFields.flatMap((candidate) => candidate.key !== member.key ? [candidate.name] : [])}/>))}
        </div>
        <p className="text-xs text-muted-foreground">
...
export function AdminClientsWorkspaceList({ clientsLoading, clientsCount, clientsError, workspaceQueryError, clientSearch, filteredClients, unmatchedByClientId, nextCursor, loadingMore, addingMember, clientPendingMembersId, deletingClientId, removingTeamMemberKey, onClientSearchChange, onRequestAddTeamMember, onRequestDeleteClient, onRemoveTeamMember, onEditTeamMemberRole, updatingMemberRoleKey, onLoadMore, }: AdminClientsWorkspaceListProps) {
    return (<div className="space-y-3">
      ...
      {clientsLoading ? (<div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          {nextCursor && (<div className="flex justify-center pt-4">
...
export function AdminClientsDeleteDialog({ open, clientName, deletingClientId, onOpenChange, onCancel, onConfirm, }: AdminClientsDeleteDialogProps) {
...
export function AdminClientsAddTeamMemberDialog({ open, clientName, assignableUsers, memberName, memberRole, addingMember, existingMemberNames, onOpenChange, onMemberNameChange, onMemberNameInputChange, onMemberRoleChange, onCancel, onConfirm, }: AdminClientsAddTeamMemberDialogProps) {
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
        ...
        <div className="space-y-4 py-2">
          <div className="space-y-2">
            ...
            {assignableUsers.length > 0 ? (<UserSearchPicker id="team-member-name-input" value={memberName} onChange={onMemberNameChange} options={assignableUsers} placeholder="Select teammate" searchPlaceholder="Search teammates" emptyText="No matching teammate found." disabled={addingMember} excludeNames={existingMemberNames}/>) : (<Input id="team-member-name-input" placeholder="e.g. Priya Patel" value={memberName} onChange={onMemberNameInputChange} disabled={addingMember}/>)}
          </div>
          <div className="space-y-2">
            <Label htmlFor="team-member-role-input">Role (optional)</Label>
            <Input id="team-member-role-input" placeholder="e.g. Paid Media Lead" value={memberRole} onChange={onMemberRoleChange} disabled={addingMember}/>
          </div>
        </div>
        <DialogFooter>
...
export function AdminClientsEditTeamMemberRoleDialog({ open, clientName, memberName, memberRole, updatingRole, onOpenChange, onMemberRoleChange, onCancel, onConfirm, }: AdminClientsEditTeamMemberRoleDialogProps) {
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <Input id="edit-team-member-role-input" placeholder="e.g. Paid Media Lead" value={memberRole} onChange={onMemberRoleChange} disabled={updatingRole}/>
          </div>
        </div>
        <DialogFooter>
