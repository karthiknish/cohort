...
export function NotificationItem({ notification, compact = false, selected = false, ackInFlight = false, onOpen, onDismiss, onMarkRead, onSelectToggle, }: NotificationItemProps) {
    ...
    return (<div role="button" tabIndex={0} onClick={onOpenNotification} onKeyDown={handleKeyDown} className={cn('group relative flex w-full cursor-pointer gap-3 border-b border-border/50 border-l-[3px] text-left transition-[background-color,box-shadow]', listItemEnterClass, accentClass, compact ? 'px-3.5 py-3' : 'rounded-lg border p-4', !notification.read && 'shadow-sm', selected && 'ring-2 ring-primary/25 ring-inset', 'hover:bg-muted/35 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/25 focus-visible:ring-inset')}>
      ...
      <div className="min-w-0 flex-1 space-y-1">
        <div className="flex flex-wrap items-start gap-1.5 pr-14">
          ...
          <Badge variant="outline" className={cn('h-5 shrink-0 px-1.5 text-[10px] font-semibold uppercase tracking-wide', badgeClass)}>
            {chipLabel}
          </Badge>
        </div>
      ...
      <div className="absolute right-2 top-2 flex shrink-0 gap-0.5 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
        {!notification.read && onMarkRead ? (<Button type="button" variant="ghost" size="icon" className="size-7 rounded-md bg-background/80 shadow-sm" disabled={ackInFlight} onClick={handleMarkRead} aria-label="Mark as read">
            <Check className="size-3.5"/>
          </Button>) : null}
        {onDismiss ? (<Button type="button" variant="ghost" size="icon" className="size-7 rounded-md bg-background/80 text-muted-foreground shadow-sm hover:text-destructive" disabled={ackInFlight} onClick={handleDismiss} aria-label="Dismiss notification">
