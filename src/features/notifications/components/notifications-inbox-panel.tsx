...
export function NotificationsInboxPanel({ unreadCount, ackInFlight, isLoadingInitial, notifications, groupedNotifications, hasNextPage, isFetchingNextPage, onMarkAllRead, onLoadMore, onOpen, onDismiss, variant = 'dropdown', }: NotificationsInboxPanelProps) {
    const scrollMaxHeight = variant === 'drawer' ? 'min(28rem,calc(85dvh - 11rem))' : 'min(22rem,68vh)';
    const scrollAreaStyle = ({ maxHeight: scrollMaxHeight });
    return (<>
      <div className={HEADER_DROPDOWN_THEME.header}>
        <div className="flex items-start justify-between gap-3">
          ...
          <Button size="sm" variant="outline" className="h-8 shrink-0 gap-1 border-border/60 text-xs shadow-sm" onClick={onMarkAllRead} disabled={unreadCount === 0 || ackInFlight}>
      ...
      <ScrollArea className="min-h-0 flex-1" style={scrollAreaStyle}>
        <div className={HEADER_DROPDOWN_THEME.inboxBody}>
          {isLoadingInitial ? (<div className="flex items-center justify-center gap-2 px-3 py-12 text-sm text-muted-foreground">
              <LoaderCircle className="size-4 animate-spin" aria-hidden/>
              Loading…
            </div>) : notifications.length === 0 ? (<NotificationEmptyState />) : (<NotificationGroupList groups={groupedNotifications} compact ackInFlight={ackInFlight} onOpen={onOpen} onDismiss={onDismiss}/>)}
        </div>
      </ScrollArea>

      <div className={HEADER_DROPDOWN_THEME.footer}>
        <Button variant="ghost" size="sm" className="h-8 text-xs text-muted-foreground hover:text-foreground" onClick={onLoadMore} disabled={isFetchingNextPage || !hasNextPage}>
          {isFetchingNextPage ? (<LoaderCircle className="mr-1.5 size-3.5 animate-spin" aria-hidden/>) : null}
