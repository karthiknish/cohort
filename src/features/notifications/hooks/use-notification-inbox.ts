...
export function useNotificationInbox() {
    ...
    const unreadCountQuery = useConvexQuery(notificationsApi.getUnreadCount, workspaceId
        ? {
            workspaceId,
    ...
    const notificationsListQueryError = (() => {
        if (!open || !workspaceId) {
            return null;
        }
    ...
    const updateNotificationStatus = (ids: string[], action: AckAction, options: AckOptions = {}) => {
