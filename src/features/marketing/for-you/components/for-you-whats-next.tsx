...
function taskDueMeta(dueDate: string | null | undefined, nowMs: number): { label: string; pill: string; variant: WhatsNextItem['pillVariant']; sortMs: number } {
  if (!dueDate) {
...
function tasksToItems(tasks: TaskRecord[], nowMs: number): WhatsNextItem[] {
  return tasks.map((task) => {
    ...
    if (task.client) sourceParts.push(task.client)
    else if (task.projectName) sourceParts.push(task.projectName)
...
function WhatsNextRow({ item }: { item: WhatsNextItem }) {
...
export function ForYouWhatsNext() {
  ...
  }, [isPreviewMode, liveNotifications?.notifications])
  ...
  return (
    <section aria-labelledby="for-you-whats-next-heading">
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-border/60 bg-card px-4 py-10 text-center text-sm text-muted-foreground">
