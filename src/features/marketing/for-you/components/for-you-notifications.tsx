...
function NotificationRow({ notification }: {
    ...
    const createdLabel = notification.createdAt
        ...
        : '';
    return (<li>
      <Link href={href} className="group flex gap-3 rounded-lg p-2.5 transition-colors hover:bg-muted/50">
        <div className="relative mt-1 flex size-2 shrink-0 items-center justify-center">
          {!notification.read ? (<Circle className="size-2 fill-primary text-primary" aria-label="Unread"/>) : (<span className="size-1.5 rounded-full bg-border" aria-hidden/>)}
        </div>
        <div className="min-w-0 flex-1">
          <p className={cn('text-sm leading-snug', !notification.read ? 'font-semibold text-foreground' : 'text-foreground')}>
            {notification.title}
          </p>
          {notification.body ? (<p className="mt-0.5 line-clamp-2 text-xs leading-relaxed text-muted-foreground">{notification.body}</p>) : null}
...
function NotificationsSkeleton() {
    return (<div className="space-y-2">
      {['n1', 'n2', 'n3', 'n4', 'n5'].map((key) => (<div key={key} className="flex gap-3 p-2">
...
export function ForYouNotifications() {
    ...
    return (<section aria-labelledby="for-you-notifications-heading" className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            {unreadCount > 0 ? (<Badge variant="default" className="h-5 rounded-full px-2 text-[10px]">
                {unreadCount} new
              </Badge>) : null}
          </div>
      ...
      {notifications.length === 0 ? (<div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border/60 px-4 py-10 text-center">
        ...
        </div>) : (<ul className="divide-y divide-border/50 rounded-lg border border-border/60 bg-card">
