...
export function ForYouTasks() {
  ...
  }, [isPreviewMode, rawTasks])

  const { pending, upcoming } = useMemo(() => partitionTasks(tasks, nowMs), [tasks, nowMs])
  ...
  return (
    <section aria-labelledby="for-you-tasks-heading" className="space-y-6">
      ...
      <TaskGroup
        ...
        tasks={pending}
        nowMs={nowMs}
        emptyMessage="Nothing due this week. You're caught up on near-term work."
      />
      <TaskGroup
        title="Upcoming"
        tasks={upcoming}
        nowMs={nowMs}
        emptyMessage="No work scheduled beyond this week."
      />
    </section>
  )
}
