...
const dotPatternStyle = {
    backgroundImage: 'radial-gradient(rgb(from var(--primary) r g b / 0.11) 1px, transparent 1px)',
...
const diagonalShimmerStyle = {
    backgroundImage: 'repeating-linear-gradient(118deg, rgb(from var(--primary) r g b / 0.4) 0px, rgb(from var(--primary) r g b / 0.4) 1px, transparent 1px, transparent 40px)',
} as const;
 ...
 */
export function SectionPatternBackground({ className, variant = 'default', }: SectionPatternBackgroundProps) {
    ...
    return (<div className={cn('pointer-events-none absolute inset-0 overflow-hidden', className)} aria-hidden>
      ...
      <div className="absolute inset-0 opacity-[0.45] [mask-image:radial-gradient(ellipse_90%_80%_at_50%_0%,black,transparent)] motion-reduce:opacity-30" style={gridPatternStyle}/>
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-primary/25 to-transparent"/>
      <div className="absolute inset-0 opacity-[0.03] motion-reduce:hidden" style={diagonalShimmerStyle}/>
    </div>);
}
