...
export function ProposalsPanel() {
    return (<div className="space-y-3">
      <div className="grid grid-cols-4 gap-2">
        {PROPOSAL_STAGES.map((stage) => (<div key={stage.id} className={cn('rounded-xl border px-2.5 py-2 text-center', TONE_BADGE[stage.tone])}>
      ...
      <div className="space-y-2">
        {PROPOSALS.map((p) => (<div key={p.id} className="flex items-center justify-between rounded-xl border border-border/40 bg-muted/20 px-3 py-2.5">
      ...
      <div className="flex items-center gap-2 rounded-xl border border-accent/20 bg-accent/5 px-3 py-2.5">
        ...
        <div className="flex items-center gap-[3px]">
          {BOUNCE_DOT_STYLES.map((dot) => (<BouncingStatusDot key={dot.id} style={dot.style}/>))}
        </div>
      </div>
    </div>);
}
