        ...
        {OVERVIEW_METRICS.map((m) => {
          ...
          return (
            <button
              ...
              className={cn(
                'rounded-xl border px-3 py-2.5 text-left outline-none transition-[colors,transform,box-shadow] duration-200 focus-visible:ring-2 focus-visible:ring-ring/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background motion-reduce:transition-none',
                isActive
                  ? cn('border-transparent bg-muted/90 shadow-sm ring-1 ring-primary/15', TONE_BADGE[m.tone])
                  : 'border-border/40 bg-muted/20 hover:scale-[1.01] hover:bg-muted/40 hover:shadow-sm motion-reduce:hover:scale-100',
              )}
            ...
            {metric.bars.map((bar) => (
              <div
                ...
                className={cn(
                  ...
                  bar.h,
                  bar.accent ? 'bg-accent' : 'bg-accent/10',
                )}
              />
            ))}
          </div>
