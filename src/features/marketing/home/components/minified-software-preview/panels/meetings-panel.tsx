...
export function MeetingsPanel() {
    return (<div className="space-y-2.5">
      ...
      {MEETING_ITEMS.map((meeting) => (<div key={meeting.id} className="rounded-xl border border-border/40 bg-muted/20 px-3 py-2.5">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className={cn('shrink-0 rounded-full border px-2 py-0.5 text-[9px] font-semibold', TONE_BADGE[meeting.tone])}>
                  {meeting.type}
                </span>
              </div>
              <div className="mt-1.5 flex items-center gap-2">
                <div className="flex">
                  {meeting.attendees.slice(0, 3).map((att, attendeeIndex) => (<div key={att} className={cn('flex size-4 items-center justify-center rounded-full border border-background text-[7px] font-bold text-viewer-chrome', attendeeIndex > 0 && '-ml-1.5', ATTENDEE_COLORS[att] ?? 'bg-muted-foreground')}>
                      {att[0]}
                    </div>))}
                  {meeting.attendees.length > 3 ? (<div className="-ml-1.5 flex size-4 items-center justify-center rounded-full border border-background bg-muted text-[7px] font-bold text-muted-foreground">
                      +{meeting.attendees.length - 3}
                    </div>) : null}
                </div>
                <div className={cn('flex items-center gap-1 text-[9px] font-medium', meeting.noteStatus === 'ready'
                ? 'text-success'
