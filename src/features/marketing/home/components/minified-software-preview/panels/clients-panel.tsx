...
export function ClientsPanel() {
    return (<div className="space-y-2.5">
      ...
      {CLIENT_ACCOUNTS.map((client) => (<div key={client.id} className="rounded-xl border border-border/40 bg-muted/20 px-3 py-2.5">
          <div className="flex items-center gap-2.5">
            <div className={cn('flex size-8 shrink-0 items-center justify-center rounded-full text-[11px] font-bold text-viewer-chrome', client.color)}>
              {client.initials}
            </div>
