...
export function PreviewWindow({ activeTabId, activeMetricId, surfaceRef, tab, activeTabIndex, handleTabClick, handleTabKeyDown, handleMetricClick, stepTab, }: PreviewWindowProps) {
    const handlePreviousTab = () => stepTab(-1);
    const handleNextTab = () => stepTab(1);
    return (<div ref={surfaceRef} className="relative overflow-hidden rounded-2xl border border-border/40 bg-background/95 shadow-2xl backdrop-blur-xl transition-transform duration-300 ease-out motion-reduce:transition-none">
      <div className="flex items-center justify-between border-b border-border/40 bg-muted/60 px-4 py-2.5">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-[7px]" aria-hidden="true">
            {WINDOW_STATUS_DOTS.map((dot) => (<span key={dot.id} className={cn('block size-[11px] rounded-full', dot.className)}/>))}
          </div>
      ...
      <div className="border-b border-border/30 bg-muted/30 px-4 py-2 sm:px-5">
        <div className="flex items-center gap-2">
          <button type="button" className="flex size-7 shrink-0 items-center justify-center rounded-md border border-border/50 bg-background/80 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/60" aria-label="Previous preview section" onClick={handlePreviousTab} disabled={activeTabIndex <= 0}>
            <ChevronLeft className="size-3.5"/>
          </button>
          <fieldset className="m-0 flex min-w-0 flex-1 gap-1 border-0 p-0" aria-label="Preview sections">
            {PREVIEW_TAB_ORDER.map((id, index) => (<button key={id} type="button" data-tab-id={id} onClick={handleTabClick} className={cn('h-1 min-w-[12px] flex-1 rounded-full transition-colors duration-300 motion-reduce:transition-none', index === activeTabIndex ? 'bg-primary' : 'bg-border/70 hover:bg-border')} aria-label={`Show ${PREVIEW_TABS.find((t) => t.id === id)?.label ?? id} preview`} aria-current={index === activeTabIndex ? 'step' : undefined}/>))}
          </fieldset>
          <button type="button" className="flex size-7 shrink-0 items-center justify-center rounded-md border border-border/50 bg-background/80 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/60" aria-label="Next preview section" onClick={handleNextTab} disabled={activeTabIndex >= PREVIEW_TAB_ORDER.length - 1}>
      ...
      <div className="relative grid grid-cols-1 sm:grid-cols-[52px_1fr] lg:grid-cols-[52px_1fr_260px]">
        <div aria-hidden className="pointer-events-none absolute inset-0 opacity-[0.35] motion-reduce:opacity-20" style={PREVIEW_DOT_PATTERN}/>
        ...
        <div className="hidden border-r border-border/30 bg-muted/30 py-3 sm:block">
          <div className="flex flex-col items-center gap-1" role="tablist" aria-label="Preview sections, sidebar">
            {PREVIEW_TABS.map((t) => (<button key={t.id} type="button" id={`preview-tab-d-${t.id}`} data-tab-id={t.id} role="tab" aria-selected={t.id === activeTabId} aria-controls="preview-panel-main" tabIndex={t.id === activeTabId ? 0 : -1} onClick={handleTabClick} onKeyDown={handleTabKeyDown} title={t.label} aria-label={`${t.label} preview`} className={cn('flex size-9 items-center justify-center rounded-lg outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background', t.id === activeTabId
        ...
        <div className="min-h-[380px] p-4 sm:p-5" role="tabpanel" id="preview-panel-main" aria-label={`${tab.label} workspace preview (sample)`}>
          ...
          <div className="mb-4 flex snap-x snap-mandatory gap-1.5 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] sm:hidden [&::-webkit-scrollbar]:hidden" role="tablist" aria-label="Preview sections, mobile">
            {PREVIEW_TABS.map((t) => (<button key={t.id} type="button" id={`preview-tab-m-${t.id}`} data-tab-id={t.id} role="tab" aria-selected={t.id === activeTabId} aria-controls="preview-panel-main" tabIndex={t.id === activeTabId ? 0 : -1} onClick={handleTabClick} onKeyDown={handleTabKeyDown} aria-label={`${t.label} preview`} className={cn('flex snap-start shrink-0 items-center gap-1 rounded-lg border px-2.5 py-1.5 text-[10px] font-semibold tracking-wide uppercase outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background', t.id === activeTabId
          ...
          <div key={activeTabId} className="animate-in fade-in-0 slide-in-from-right-2 duration-300 motion-reduce:animate-none">
            {activeTabId === 'overview' ? (<OverviewPanel activeMetricId={activeMetricId} onMetricClick={handleMetricClick}/>) : null}
        ...
        <div className="hidden border-l border-border/30 bg-accent/[0.03] lg:block">
          ...
          <div className="p-3.5">
            <div className="space-y-2.5">
              {tab.agentItems.map((item) => (<div key={item.id} className="rounded-lg border border-border/30 bg-background/60 px-3 py-2.5">
