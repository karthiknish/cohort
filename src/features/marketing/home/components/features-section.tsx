...
function ScrollGroup() {
    return (<div className="flex flex-col gap-2 py-1">
      {PORTAL_CLIENTS.map((client) => (<div key={client.id} className="flex items-center gap-3 rounded-[1.1rem] border border-border/50 bg-background/95 px-3 py-2.5 shadow-sm">
...
function DotConnector({ direction }: {
    direction: 'ltr' | 'rtl';
}) {
    ...
    return (<LazyMotion features={domAnimation}>
      ...
      <div className="hidden h-10 w-full items-center justify-evenly lg:flex" aria-hidden="true">
        {HUB_CONNECTOR_DOTS.map((dot, index) => (<m.span key={dot.id} className="block size-[5px] rounded-full bg-accent/50" initial={prefersReducedMotion ? false : CONNECTOR_DOT_INITIAL} animate={prefersReducedMotion ? CONNECTOR_DOT_REDUCED : CONNECTOR_DOT_ANIMATE} transition={getConnectorTransition(direction, index)}/>))}
      </div>
    </LazyMotion>);
}
function IntegrationChip({ chip }: {
    chip: Integration;
}) {
    return (<TooltipProvider delayDuration={120}>
      <Tooltip>
        ...
        <TooltipContent side="bottom" sideOffset={8} className="w-52 border border-border/60 bg-popover p-0 shadow-xl">
          <div className="px-3 pt-3 pb-2">
            ...
            <div className="space-y-2">
              {chip.insights.map((insight) => (<div key={`${chip.id}-${insight.label}`} className="flex items-center justify-between">
                  <div>
...
function HubCore() {
    const prefersReducedMotion = useReducedMotion();
    return (<LazyMotion features={domAnimation}>
      <m.div className="relative hidden items-center rounded-full bg-foreground px-5 py-2.5 shadow-2xl lg:flex" initial={prefersReducedMotion ? false : HUB_CORE_INITIAL} animate={prefersReducedMotion ? HUB_CORE_REDUCED : HUB_CORE_ANIMATE} transition={HUB_CORE_TRANSITION}>
        <m.span className="absolute inset-[1px] rounded-full border border-viewer-chrome/20" aria-hidden="true" initial={prefersReducedMotion ? false : HUB_RING_INITIAL} animate={prefersReducedMotion ? HUB_RING_REDUCED : HUB_RING_ANIMATE} transition={HUB_CORE_TRANSITION}/>
        <m.span className="absolute -inset-3 rounded-full border border-white/12" aria-hidden="true" initial={prefersReducedMotion ? false : HUB_GLOW_INITIAL} animate={prefersReducedMotion ? HUB_GLOW_REDUCED : HUB_GLOW_ANIMATE} transition={HUB_GLOW_TRANSITION}/>
        <Image src="/logo_white.svg" alt="Cohorts" width={100} height={32} className="relative h-8 w-auto" style={LOGO_AUTO_WIDTH_STYLE} priority/>
      </m.div>
    </LazyMotion>);
}
function ProposalIllustration() {
    return (<div className="mx-4 mt-6 overflow-hidden rounded-t-xl border border-b-0 border-border/50 bg-muted/40 px-4 pt-4">
      <div className="rounded-xl border border-border/60 bg-background p-3.5 shadow-sm">
        ...
        <div className="space-y-1.5">
          {PROPOSAL_LINES_STYLE.map((style) => (<div key={String(style.width)} className="h-[7px] rounded-full bg-muted/80" style={style}/>))}
        </div>
        ...
        <div className="mt-3 flex items-center gap-1.5 border-t border-border/40 pt-3">
          ...
          <div className="ml-auto flex items-center gap-[3px]">
            {PROPOSAL_BOUNCE_DOTS.map((dot) => (<span key={dot.id} className="block size-1.5 animate-subtle-dot-drift rounded-full bg-accent" style={dot.style}/>))}
          </div>
        </div>
      </div>
      ...
      <div className="relative mx-4 mt-6 overflow-hidden rounded-t-[1.25rem] border border-b-0 border-border/50 bg-muted/35 px-4 pt-4">
        <div className="relative overflow-hidden rounded-[1.15rem] border border-border/60 bg-background/95 p-3 shadow-xl backdrop-blur">
          ...
          <div className="relative h-[176px] overflow-hidden rounded-2xl border border-border/50 bg-muted/20 p-2">
            ...
            <m.div className="flex flex-col" animate={prefersReducedMotion ? undefined : PORTAL_SCROLL_ANIMATE} transition={PORTAL_SCROLL_TRANSITION}>
              <ScrollGroup />
              <ScrollGroup />
            </m.div>
          </div>
...
function TeamIllustration() {
    return (<div className="mx-4 mt-6 overflow-hidden rounded-t-xl border border-b-0 border-border/50 bg-muted/40 px-4 pt-6">
      <div className="flex items-center justify-center">
        <TooltipProvider delayDuration={100}>
          <div className="flex">
            {TEAM_MEMBERS.map((member, index) => (<Tooltip key={member.id}>
                <TooltipTrigger asChild>
                  <Avatar className={cn('size-11 cursor-pointer border-2 border-background shadow-sm transition-transform hover:-translate-y-1 hover:z-20', index > 0 && '-ml-2.5', index === 2 ? 'z-10 ring-2 ring-accent/40' : '')}>
                    <AvatarFallback className={cn('text-xs font-bold text-viewer-chrome', member.color)}>
                      {member.initials}
                    </AvatarFallback>
                  </Avatar>
                </TooltipTrigger>
                <TooltipContent side="top" className={cn('border-0 text-viewer-chrome', member.color)}>
                  {member.name}
                </TooltipContent>
              </Tooltip>))}
          </div>
      ...
      <div className="mt-5 space-y-2">
        {TEAM_TASKS.map((task) => (<div key={task.id} className="flex items-center justify-between rounded-xl border border-border/40 bg-background px-3 py-2.5 shadow-sm">
...
export function FeaturesSection() {
    return (<section className="relative overflow-hidden bg-gradient-to-b from-primary/[0.04] via-background to-secondary/[0.03] py-16 sm:py-24">
        ...
        <FadeIn>
          <div className="relative mb-12 flex w-full flex-col items-center sm:mb-16 lg:mb-20">
            <div className="flex items-center rounded-full bg-foreground px-5 py-2.5 shadow-xl lg:hidden">
              <Image src="/logo_white.svg" alt="Cohorts" width={100} height={32} className="h-8 w-auto" style={LOGO_AUTO_WIDTH_STYLE} priority/>
            </div>
            ...
            <div className="relative max-lg:mt-6 max-lg:flex max-lg:flex-wrap max-lg:items-center max-lg:justify-center max-lg:gap-4 lg:grid lg:w-full lg:grid-cols-[auto_minmax(0,1fr)_auto_minmax(0,1fr)_auto] lg:items-center">
              ...
              <div className="relative z-10 flex gap-4 lg:flex-col lg:gap-16">
                {INTEGRATIONS_LEFT.map((chip) => (<IntegrationChip key={chip.id} chip={chip}/>))}
              </div>
              ...
              <div className="relative z-10 flex gap-4 lg:flex-col lg:gap-16">
                {INTEGRATIONS_RIGHT.map((chip) => (<IntegrationChip key={chip.id} chip={chip}/>))}
              </div>
            </div>
          </div>
        </FadeIn>
