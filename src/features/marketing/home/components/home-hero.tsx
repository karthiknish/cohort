      ...
      <FadeInStagger className="grid gap-5 sm:grid-cols-2">
        {highlights.map((item) => (
          <FadeInItem
            key={item.title}
            as="div"
            className="group flex gap-4 rounded-2xl border border-border/50 bg-background/50 p-5 shadow-sm transition-[color,background-color,border-color,text-decoration-color,fill,stroke,opacity,box-shadow,transform,filter,backdrop-filter] hover:border-primary/20 hover:bg-background hover:shadow-md"
          >
