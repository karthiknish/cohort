...
const dotPatternStyle = {
  backgroundImage:
    'radial-gradient(rgb(from var(--primary) r g b / 0.14) 1px, transparent 1px)',
...
function HeroOrb({ orb }: { orb: (typeof orbs)[number] }) {
  ...
  return (
    <m.span
      aria-hidden
      className={cn(
        'absolute inline-block rounded-full bg-gradient-to-br motion-reduce:animate-none',
        orb.gradient,
        orb.blur,
      )}
      style={sizeStyle}
...
export function HeroBackground({ className }: HeroBackgroundProps) {
  return (
    ...
    >
      ...
      <div
        className="absolute inset-0 opacity-[0.55] [mask-image:radial-gradient(ellipse_85%_75%_at_50%_35%,black,transparent)] motion-reduce:opacity-40"
