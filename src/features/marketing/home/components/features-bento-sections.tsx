...
import {
  ...
  CHART_POINTS,
  CLIENT_SPOTLIGHT,
  CLIENT_SPOTLIGHT_STYLES,
  MARQUEE_STYLE,
  PLATFORM_BARS,
  PLATFORM_BAR_HEIGHT_STYLES,
  PLATFORM_STATS,
  SEGMENT_BADGES,
} from './features-bento-data'

type ActivityItemProps = {
  item: { Icon: ComponentType<LucideProps>; name: string; detail: string; value: string; positive: boolean }
}
      ...
      <div className="flex flex-col gap-0 rounded-xl border bg-card pt-0 pb-6 shadow-none">
        <div className="relative flex min-h-72 justify-center overflow-hidden">
          ...
          {SEGMENT_BADGES.map(({ Icon, label, rotation, pos }) => (
            <div key={label} className={cn(pos, rotation)} style={BADGE_BOB_STYLE}>
            ...
            <div className="px-6">
              <svg viewBox="0 0 372 127" className="h-36 w-full" fill="none" aria-hidden="true">
                ...
                {CHART_POINTS.map(({ x, label }) => (
                  <text key={label} x={x} y={122} fill="var(--muted-foreground)" fontSize={11} textAnchor="middle">
                    {label}
                  </text>
                ))}
              </svg>
        ...
        <div className="overflow-hidden px-2 py-1">
          <div className="flex w-max gap-2" style={MARQUEE_STYLE}>
            {ACTIVITY_ITEMS.map((item) => (
              <FeaturesBentoActivityItem key={`first-${item.name}`} item={item} />
            ))}
            {ACTIVITY_ITEMS.map((item) => (
              <FeaturesBentoActivityItem key={`second-${item.name}`} item={item} />
            ))}
          </div>
        </div>
