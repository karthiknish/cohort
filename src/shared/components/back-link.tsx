...
export function BackLink({ label, href, onClick, className, transitionTypes }: BackLinkProps) {
    ...
    if (href) {
        return (<Button variant="ghost" size="sm" asChild className={sharedClasses}>
    ...
    return (<Button variant="ghost" size="sm" type="button" className={sharedClasses} onClick={onClick}>
      <ArrowLeft className="size-4"/>
      {label}
    </Button>);
}
