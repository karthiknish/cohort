...
export function NetworkStatusBanner() {
    ...
    if (isOnline)
        return null;
    return (<div className="border-b border-warning/20 bg-warning/10 px-4 py-2 text-warning md:px-6 lg:px-8">
...
export function SyncStatusBadge({ showLabel = false }: {
    showLabel?: boolean;
}) {
    const isOnline = useOnlineStatus();
    return (<Badge variant={isOnline ? 'outline' : 'secondary'} className={cn('gap-1.5', isOnline
