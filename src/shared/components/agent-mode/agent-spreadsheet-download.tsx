...
export function AgentSpreadsheetDownload({ messageId, conversationId, workspaceId, data, attachments, onStored, }: AgentSpreadsheetDownloadProps) {
    ...
    const alreadyStored = storedExport !== null ||
    ...
    const storeExport = useCallback(async () => {
        ...
        try {
            ...
            reportConvexFailure({
                error,
                context: 'AgentSpreadsheetDownload:storeExport',
                title: 'Could not save Excel file',
                fallbackMessage: 'Upload to storage failed.',
            });
            return null;
        }
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            reportConvexFailure({
                error,
                context: 'AgentSpreadsheetDownload:handleDownload',
    ...
    return (<div className="mt-3 rounded-lg border border-border/60 bg-background/70 px-3 py-2.5">
      <p className="text-xs font-medium text-foreground">{payload.title}</p>
      {payload.subtitle ? (<p className="mt-0.5 text-xs text-muted-foreground">{payload.subtitle}</p>) : null}
      ...
      <Button type="button" size="sm" variant="default" className="mt-2 gap-2" disabled={busy} onClick={handleDownloadClick}>
        {busy ? (<LoaderCircle className="size-3.5 animate-spin" aria-hidden/>) : (<Download className="size-3.5" aria-hidden/>)}
