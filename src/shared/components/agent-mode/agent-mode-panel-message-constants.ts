...
export const AGENT_PANEL_SURFACE = 'relative bg-background before:pointer-events-none before:absolute before:inset-x-0 before:top-0 before:h-24 before:bg-gradient-to-b before:from-primary/[0.04] before:to-transparent';
export const AGENT_MESSAGE_THREAD = 'relative min-h-full bg-[radial-gradient(circle_at_1px_1px,color-mix(in_srgb,var(--foreground)_6%,transparent)_1px,transparent_0)] [background-size:20px_20px]';
...
export function formatMessageDayLabel(date: Date, now: Date): string {
    const todayStart = startOfLocalDay(now);
    const messageStart = startOfLocalDay(date);
    const yesterdayStart = todayStart - 24 * 60 * 60 * 1000;
    if (messageStart === todayStart)
        return 'Today';
    if (messageStart === yesterdayStart)
        return 'Yesterday';
