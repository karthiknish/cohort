...
import { asNumber, asRecord, asRecordArray, asString, compact, formatCurrency, formatLabel, formatPercent, formatWholeNumber, resolveCurrencyCode, resolveMetricsAvailable, resolveTotals, } from './helpers';
export function buildAgentMessageCharts(operation: string | undefined, data: Record<string, unknown> | undefined): AgentChartSeries[] {
    if (!data)
        return [];
    ...
    if (deltaPercent) {
        ...
        const filteredDeltaPoints = deltaPoints.filter((point) => Math.abs(point.value) >= 0.05 && Math.abs(point.value) <= 300);
    ...
    if (topCampaigns.length > 0) {
        const points = compact<AgentChartPoint>(topCampaigns.map((campaign) => {
