...
export function AgentMentionPills({ mentions }: {
    mentions: AgentMentionEntity[];
}) {
    if (mentions.length === 0)
        return null;
    return (<div className="mt-2 flex flex-wrap gap-1.5">
      {mentions.map((mention) => (<span key={`${mention.type}:${mention.id}`} className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-primary/20 bg-primary/10 px-2 py-0.5 text-xs text-primary" title={`${mention.type} · ${mention.id}`}>
          <span className="font-medium">@{mention.name}</span>
          {mention.subtitle ? (<span className="truncate text-[10px] text-muted-foreground">{mention.subtitle}</span>) : null}
...
export function AgentMentionText({ text, mentionLabels = EMPTY_MENTION_LABELS, className, mentionClassName }: AgentMentionTextProps) {
    ...
    return (<span className={className}>
      {segments.map((segment) => {
            ...
            return (<Fragment key={`${segment.text}-${occurrence}`}>
          {segment.isMention ? (<span className={cn('inline rounded-md px-1.5 py-0.5 font-medium ring-1 ring-inset', 'bg-accent/15 text-primary ring-primary/20', mentionClassName)}>
              {segment.text}
            </span>) : (segment.text)}
        </Fragment>);
        })}
