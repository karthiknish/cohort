...
import type { AgentExecutionStep, AgentMessage, AgentPendingConfirmation, } from '@/shared/hooks/use-agent-mode';
import { Button } from '@/shared/ui/button';
import { AgentMessageCard } from './agent-message-card';
import { AGENT_MESSAGE_THREAD, AGENT_PANEL_SURFACE, formatMessageDayLabel, messageDayKey, MOTION_FADE_IN, MOTION_FADE_IN_VISIBLE, stopPropagation, } from './agent-mode-panel-message-constants';
...
export function AgentMessagesSection({ isConversationLoading, isProcessing, mentionLabels, messages, onRetryLastUserTurn, onRetryUserMessage, onConfirmPending, onUndoAction, processingSteps, processingLabel, scrollAreaRef, onMessagesScroll, showJumpToLatest, onJumpToLatest, conversationId, workspaceId, onStoreSpreadsheetExport, }: {
    ...
    return (<div className={cn('relative flex-1 min-h-0', AGENT_PANEL_SURFACE)}>
      <div className={cn('h-full overflow-y-auto px-4 py-5', AGENT_MESSAGE_THREAD)} ref={scrollAreaRef} onScroll={onMessagesScroll} onWheel={stopPropagation}>
        {isConversationLoading ? (<div className="flex h-full min-h-[240px] items-center justify-center">
            ...
            {messages.map((message, index) => {
                ...
                const previousTimestamp = previousMessage
                    ...
                    : null;
                const previousDayKey = previousTimestamp && !Number.isNaN(previousTimestamp.getTime())
                    ? messageDayKey(previousTimestamp)
                    : null;
                const showDayDivider = dayKey !== null && dayKey !== previousDayKey;
                return (<Fragment key={message.clientId}>
                  {showDayDivider && now ? (<AgentMessageDayDivider label={formatMessageDayLabel(timestamp, now)}/>) : null}
                  <AgentMessageCard message={message} mentionLabels={mentionLabels} isProcessing={isProcessing} conversationId={conversationId} workspaceId={workspaceId} onStoreSpreadsheetExport={onStoreSpreadsheetExport} onRetryLastUserTurn={onRetryLastUserTurn} onRetryUserMessage={onRetryUserMessage} onConfirmPending={onConfirmPending} onUndoAction={onUndoAction}/>
                </Fragment>);
            })}

            {isProcessing ? (<m.div initial={MOTION_FADE_IN} animate={MOTION_FADE_IN_VISIBLE} className="flex justify-start pt-1">
