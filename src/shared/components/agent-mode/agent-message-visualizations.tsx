...
export function AgentMessageBarChart({ series }: { series: AgentChartSeries }) {
...
function DeltaPill({ delta, tone }: { delta: string; tone?: MetricItem['deltaTone'] }) {
  return (
    <span
      className={cn(
...
export function AgentMetricTile({ item, emphasized = false }: { item: MetricItem; emphasized?: boolean }) {
  return (
    <div
      className={cn(
        'rounded-xl border border-border/50 bg-background px-3 py-2.5 shadow-sm transition-shadow hover:shadow-md',
        emphasized && 'border-primary/20 bg-linear-to-br from-primary/[0.06] to-background ring-1 ring-primary/10',
      )}
    >
...
export function AgentOverviewChips({ items }: { items: MetricItem[] }) {
  return (
...
export function AgentPerformanceMetricsGrid({ items }: { items: MetricItem[] }) {
  ...
  return (
    <div className="space-y-3">
      {hero.length > 0 ? (
        <div className="grid gap-2 sm:grid-cols-3">{hero.map((item) => <AgentMetricTile key={item.label} item={item} emphasized />)}</div>
      ) : null}
      {rest.length > 0 ? (
        <div className="grid gap-2 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
          {rest.map((item) => (
            <AgentMetricTile key={item.label} item={item} />
          ))}
        </div>
      ) : null}
    </div>
...
export function AgentMetricsSection({ section }: { section: Extract<AgentDataSection, { type: 'metrics' }> }) {
...
export function AgentListSection({
  ...
  section: Extract<AgentDataSection, { type: 'list' }>
  accentClass: string
}) {
  return (
      ...
      {section.items.map((item) => {
          ...
          return (
            <li key={`${item.primary}-${item.secondary ?? ''}`}>
              <Link
                href={item.href}
                className={cn(
                  'block rounded-lg border border-border/50 bg-background px-3 py-2.5 shadow-sm transition-colors hover:border-accent/30 hover:bg-muted/30',
                  accentClass,
                )}
              >
                {inner}
        ...
        return (
          <li
            ...
            className={cn(
              'rounded-lg border border-border/50 bg-background px-3 py-2.5 shadow-sm',
              accentClass,
            )}
          >
            {inner}
