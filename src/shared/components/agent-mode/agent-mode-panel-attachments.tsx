...
export function AgentMessageAttachmentChips({
  ...
  const metadataOnly = attachments.filter(
    (attachment) => !attachment.url?.trim() || attachment.url === 'about:blank',
  )

  return (
    <div className="mt-2.5 space-y-2" aria-label="Attached files">
      {media.length > 0 ? (
        <ChatMediaGallery attachments={media} compact className="[&_img]:ring-primary-foreground/20" />
      ) : null}
