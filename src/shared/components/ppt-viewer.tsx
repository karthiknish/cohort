...
export function PptViewer({ url, refreshUrl, className, title = 'Presentation', embedded = false }: PptViewerProps) {
    ...
    const slideMinHeight = embedded ? 'min-h-[min(42dvh,360px)]' : 'min-h-[min(50dvh,420px)]';
    const slideMaxHeight = embedded ? 'max-h-[min(55dvh,480px)]' : 'max-h-[min(65dvh,560px)]';
    ...
    const viewerBody = (
        <div className={cn('flex min-h-0 min-w-0 max-w-full flex-1 flex-col overflow-hidden', className)}>
            <div className="relative w-full max-w-full overflow-hidden rounded-xl border border-border/60 bg-white shadow-inner ring-1 ring-white/5">
                <div
                    ref={containerRef}
                    className={cn('w-full max-w-full overflow-hidden', slideMinHeight, slideMaxHeight)}
                />
