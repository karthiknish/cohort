...
import { PptViewerCanvas, PptViewerError, PptViewerFilmstrip, PptViewerLoading, } from './ppt-viewer-sections';
...
export function PptViewer({ url, className, title = 'Presentation' }: PptViewerProps) {
    ...
    const viewerBody = (<div className={cn('flex min-h-0 flex-1 flex-col', className)}>
      <PptViewerCanvas title={title} slides={slides} currentSlide={currentSlide} isFullscreen={isFullscreen} onPrevSlide={handlePrevSlide} onNextSlide={handleNextSlide} onToggleFullscreen={handleToggleFullscreen}/>
    ...
    if (isFullscreen) {
        return (<dialog open className="fixed inset-0 z-[100] m-0 flex max-h-none w-full max-w-none flex-col border-0 bg-black/95 p-4 sm:p-8" aria-label={`${title} full screen`}>
