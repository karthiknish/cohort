...
import { countUserMessagesByClientId, filterMessagesForAgentContext, upsertAgentMessage, } from '@/lib/agent-message-lifecycle';
describe('use-agent-mode message reconciliation', () => {
    ...
    it('failed send retry updates in place instead of duplicating', () => {
        ...
        expect(countUserMessagesByClientId(messages.map((m) => ({ type: 'user', clientId: m.clientId }))).get(clientId)).toBe(1);
    });
    it('history reload replaces local ids with persisted ids per row', () => {
        ...
        expect(countUserMessagesByClientId(stored.map((m) => ({ type: 'user', clientId: m.clientId })))).toEqual(new Map([['srv-1', 1], ['srv-2', 1]]));
    });
