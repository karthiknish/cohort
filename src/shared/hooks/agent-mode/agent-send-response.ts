...
export type ApplyAgentResponseParams = {
    ...
    addMessage: (type: 'user' | 'agent', content: string | unknown, route?: string | null, status?: 'success' | 'error' | 'info' | 'warning', metadata?: AgentMessage['metadata'], options?: {
        clientId?: string;
...
export function applyAgentResponse({ response, trimmedText, sentAttachments, conversationId, activeRequestClientId, addMessage, upsertMessage, setConversationId, setPendingAttachments, setLastFailedMessage, router, setOpen, }: ApplyAgentResponseParams): void {
    ...
    if (response.action === 'navigate' && response.route) {
        ...
        const executeRoute = typeof response.route === 'string' && response.route.length > 0
            ? response.route
