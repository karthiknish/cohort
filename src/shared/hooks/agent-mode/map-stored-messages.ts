...
export function mapStoredMessagesToAgentMessages(stored: StoredAgentMessage[]): AgentMessage[] {
    return stored.map((msg) => {
        ...
        const executeResult = msg.executeResult && typeof msg.executeResult === 'object'
            ? (msg.executeResult as Record<string, unknown>)
            : null;
        const executionSuccess = executeResult && typeof executeResult.success === 'boolean'
            ? executeResult.success
            : null;
        const normalizedAction: AgentMessageMetadata['action'] | undefined = action === 'navigate' || action === 'execute' || action === 'clarify' ? action : action ? 'response' : undefined;
        const status: AgentMessage['status'] = normalizedAction === 'execute' && executionSuccess !== null
            ? executionSuccess
                ? 'success'
                : 'error'
            : normalizedAction === 'navigate'
                ? 'success'
                : 'info';
        const pendingConfirmation = type === 'agent'
        ...
        const undoHint = undoHintRaw &&
            ...
            ? {
                ...
                label: typeof (undoHintRaw as Record<string, unknown>).label === 'string'
                    ? ((undoHintRaw as Record<string, unknown>).label as string)
                    : 'Created',
            }
            : undefined;
        const storedMentions = msg.params && typeof msg.params === 'object' && !Array.isArray(msg.params)
            ? parseAgentMentionsFromStored((msg.params as Record<string, unknown>).mentions)
            : undefined;
        const storedAttachmentsFromParams = msg.params && typeof msg.params === 'object' && !Array.isArray(msg.params)
        ...
        const storedAttachments = storedAttachmentsFromParams || storedExportAttachment
            ? [
