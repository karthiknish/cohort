...
export function useAgentMode(): UseAgentModeReturn {
    ...
    const upsertMessage = (message: AgentMessage) => {
        setMessages((prev) => upsertAgentMessage(prev, message));
        return message;
    };
    const addMessage = (type: 'user' | 'agent', content: string | unknown, route?: string | null, status?: 'success' | 'error' | 'info' | 'warning', metadata?: AgentMessageMetadata, options?: {
        clientId?: string;
    ...
    const storeSpreadsheetExportForMessage = (messageId: string, attachment: AgentAttachmentContext) => {
