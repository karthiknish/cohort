...
import { serializeAgentAttachmentsForStorage, toAgentRequestAttachmentContext, type AgentAttachmentContext, } from '@/lib/agent-attachments';
...
type UseAgentSendParams = {
    ...
    addMessage: (type: 'user' | 'agent', content: string | unknown, route?: string | null, status?: 'success' | 'error' | 'info' | 'warning', metadata?: AgentMessageMetadata, options?: {
        clientId?: string;
...
export function useAgentSend({ workspaceId, activeContext, messages, setMessages, conversationId, setConversationId, pendingAttachments, setPendingAttachments, clearAttachments, isExtractingAttachments, isPinnedToBottom, scrollToLatest, upsertMessage, addMessage, setOpen, }: UseAgentSendParams) {
    ...
    const handleError = (err: AgentError, failedMessage?: string) => {
        setError(err);
    ...
    const applyAgentResponse = (response: AgentSendResponse, trimmedText: string, sentAttachments: AgentAttachmentContext[]) => {
    ...
    const buildAgentRequestContext = (overrides?: {
        ...
        attachmentContext: pendingAttachments.length > 0
    ...
    const processInput = async (text: string, options?: {
        ...
        try {
            ...
            setProcessingSteps((steps) => steps.map((step) => (step.id === 'action' || step.id === 'done' ? { ...step, status: 'failed' } : step)));
    ...
    const confirmPendingAction = (pending: AgentPendingConfirmation, decision: 'confirm' | 'cancel' | 'edit') => {
        void processInput('', { confirmation: { pending, decision } });
    };
    const undoAgentAction = async (messageId: string, undoHint: NonNullable<AgentMessageMetadata['undoHint']>) => {
        if (!workspaceId)
            return;
        try {
    ...
    const editLastUserMessage = (text: string) => {
