 ...
 */
export function ResponsiveOverlay({ open, onOpenChange, title, description, children, footer, mobileDirection = 'bottom', dialogClassName, mobileClassName, preferDrawer = false, hideCloseButton = false, }: ResponsiveOverlayProps) {
    ...
    const header = (<>
      {typeof title === 'string' ? (useDrawer ? (<DrawerTitle>{title}</DrawerTitle>) : (<DialogTitle>{title}</DialogTitle>)) : (title)}
      {description ? (useDrawer ? (<DrawerDescription>{description}</DrawerDescription>) : (<DialogDescription>{description}</DialogDescription>)) : null}
    </>);
    if (useDrawer) {
        return (<Drawer open={open} onOpenChange={onOpenChange} direction={mobileDirection}>
        <DrawerContent className={cn(mobileDirection === 'bottom' && 'pb-[max(1rem,env(safe-area-inset-bottom))]', mobileDirection === 'right' && 'pb-[max(0.5rem,env(safe-area-inset-bottom))]', mobileClassName)}>
          <DrawerHeader className="relative shrink-0 border-b border-border/60 text-left">
            {header}
            {!hideCloseButton ? (<Button type="button" variant="ghost" size="icon" className="absolute right-3 top-3 size-8" onClick={handleClose} aria-label="Close">
