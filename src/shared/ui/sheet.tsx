...
function SheetOverlay({ className, ...props }: React.ComponentProps<typeof SheetPrimitive.Overlay>) {
    return (<SheetPrimitive.Overlay data-slot="sheet-overlay" className={cn("fixed inset-0 z-[1500] bg-black/50", surfaceMotionClasses.overlay, className)} {...props}/>);
}
function SheetContent({ className, children, side = "right", showOverlay = true, overlayClassName, ...props }: React.ComponentProps<typeof SheetPrimitive.Content> & {
    ...
    return (<SheetPortal>
      ...
      <SheetPrimitive.Content data-slot="sheet-content" className={cn("bg-background fixed z-[1500] flex flex-col gap-4 shadow-lg", surfaceMotionClasses.sheetContent, side === "right" &&
        ...
        {children}
        <SheetPrimitive.Close className={cn("ring-offset-background focus:ring-ring data-[state=open]:bg-secondary absolute top-4 right-4 rounded-xs opacity-70 hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none", interactiveTransitionClass)}>
