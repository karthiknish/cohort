...
export function VoiceWaveform({ isActive, barCount = 12, className }: VoiceWaveformProps) {
    ...
    return (<LazyMotion features={domAnimation}>
      <div className={`flex items-center justify-center gap-1.5 h-12 ${className || ''}`}>
        {bars.map((bar) => (<m.div key={bar.id} className="w-2 bg-primary rounded-full transition-colors duration-[var(--motion-duration-fast)] ease-[var(--motion-ease-standard)] motion-reduce:transition-none" animate={getWaveformAnimateProps(isActive, bar.height)} transition={getWaveformTransitionProps(bar.delay)}/>))}
      </div>
    </LazyMotion>);
}
