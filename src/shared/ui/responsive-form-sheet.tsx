 ...
 */
export function ResponsiveFormSheet({ open, onOpenChange, children, trigger, contentClassName, sheetSide = 'right', }: ResponsiveFormSheetProps) {
    const isMobile = useIsMobile();
    const kind: OverlayKind = isMobile ? 'drawer' : 'sheet';
    const contextValue = kind;
    if (isMobile) {
        return (<ResponsiveFormSheetContext.Provider value={contextValue}>
        <Drawer open={open} onOpenChange={onOpenChange} direction="bottom" shouldScaleBackground>
          {trigger ? <DrawerTrigger asChild>{trigger}</DrawerTrigger> : null}
          <DrawerContent className={cn('flex max-h-[92dvh] flex-col gap-0 p-0 pb-[max(0.75rem,env(safe-area-inset-bottom))]', contentClassName)}>
            {children}
          </DrawerContent>
        </Drawer>
