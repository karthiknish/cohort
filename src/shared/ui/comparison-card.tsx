...
function calculateChange(current: number, previous: number): {
    ...
    if (change > 0.1)
        return { value: change, type: 'up' };
    if (change < -0.1)
        return { value: Math.abs(change), type: 'down' };
    return { value: 0, type: 'neutral' };
}
    ...
    if (isLoading) {
        ...
        <CardContent>
          <div className="space-y-4">
            {Array.from({ length: 4 }, (_, slot) => `comparison-skeleton-${slot + 1}`).map((slotKey) => (<Skeleton key={slotKey} className="h-16 w-full"/>))}
          </div>
        </CardContent>
      </Card>);
    }
      ...
      <CardContent>
        <div className="space-y-4">
          {metrics.map((metric) => {
            ...
            return (<div key={metricKey} className="flex items-center justify-between py-2 border-b border-muted/10 last:border-0">
...
export function SideBySideComparison({ leftLabel, rightLabel, metrics, isLoading = false, title = 'Side by Side Comparison', className, }: SideBySideComparisonProps) {
    ...
    if (isLoading) {
        ...
        <CardContent>
          <div className="space-y-3">
            {Array.from({ length: 5 }, (_, slot) => `side-by-side-skeleton-${slot + 1}`).map((slotKey) => (<Skeleton key={slotKey} className="h-14 w-full"/>))}
          </div>
        </CardContent>
      </Card>);
    }
    return (<Card className={className}>
      ...
      <CardContent>
        <div className="space-y-2">
          ...
          {/* Data rows */}
          {processedMetrics.map((metric, index) => (<div key={`${metric.label}-${metric.leftValue}-${metric.rightValue}`} className={cn('grid grid-cols-12 gap-2 p-3 rounded-lg transition-colors hover:bg-muted/30', index % 2 === 0 && 'bg-muted/20')}>
              ...
              <div className="col-span-2 text-right">
                <Badge variant={metric.hasImprovement ? 'default' : 'secondary'} className={cn('text-[10px] font-bold', metric.diffPercent > 0
