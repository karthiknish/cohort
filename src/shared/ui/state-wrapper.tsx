...
import { EmptyState, NoDataEmptyState, } from './empty-state';
import { Skeleton, SkeletonCard, SkeletonDashboardCard, SkeletonList, SkeletonTable } from './skeleton';
import { Spinner } from './spinner';
// =============================================================================
// TYPES
// =============================================================================
export type StateWrapperLoadingVariant = 'spinner' | 'skeleton-card' | 'skeleton-table' | 'skeleton-list' | 'skeleton-dashboard' | 'overlay';
...
function ErrorState({ title, description, onRetry, className }: ErrorStateProps) {
    return (<div role="alert" aria-live="assertive" className={cn('flex flex-col items-center justify-center py-12 px-4', className)}>
      ...
      {description && (<p className="mt-1.5 max-w-sm text-sm text-muted-foreground text-center">{description}</p>)}
      {onRetry && (<Button onClick={onRetry} size="sm" variant="outline" className="mt-5">
...
function LoadingState({ variant, message, rows, columns, items }: LoadingStateProps) {
    if (variant === 'spinner') {
        return (<div aria-live="polite" aria-busy="true" className="flex flex-col items-center justify-center py-12 px-4">
        <Spinner size="lg" variant="primary"/>
        {message && (<p className="mt-3 text-sm text-muted-foreground animate-pulse">{message}</p>)}
      </div>);
    }
    if (variant === 'overlay') {
        return (<div aria-live="polite" aria-busy="true" className="flex flex-col items-center justify-center inset-0 bg-background/50 backdrop-blur-sm z-10">
        <Spinner size="lg" variant="primary"/>
        {message && (<p className="mt-3 text-sm text-muted-foreground animate-pulse">{message}</p>)}
      </div>);
    }
...
// =============================================================================
export function StateWrapper({ state, isLoading = false, isEmpty = false, isError = false, children, loadingVariant = 'spinner', loadingMessage, skeletonRows = 5, skeletonColumns = 4, skeletonItems = 5, emptyTitle, emptyDescription, emptyIcon, emptyAction, errorTitle, errorDescription, error, onRetry, className, contentClassName, }: StateWrapperProps) {
    const resolvedState = resolveViewState({ state, isLoading, isEmpty, isError });
    if (resolvedState === 'error') {
        const errorMessage = typeof error === 'string' ? error : error?.message;
        return (<ErrorState title={errorTitle} description={errorDescription || errorMessage} onRetry={onRetry} className={className}/>);
    }
    if (resolvedState === 'loading') {
        return (<div className={className}>
        <LoadingState variant={loadingVariant} message={loadingMessage} rows={skeletonRows} columns={skeletonColumns} items={skeletonItems}/>
      </div>);
    }
...
export function useStateWrapper<Data = unknown>(state: {
    data?: Data | null;
    isLoading?: boolean;
    isError?: boolean;
    error?: Error | null;
}): UseStateWrapperReturn & {
    data: Data | null;
} {
    return {
...
export function AsyncStateWrapper<T>({ data, loading = false, error = null, ...props }: AsyncStateWrapperProps<T>) {
    const isLoading = loading;
    const isEmpty = !loading && !data;
    const isError = !loading && !!error;
    return (<StateWrapper {...props} isLoading={isLoading} isEmpty={isEmpty} isError={isError} error={error}>
      {props.children}
    </StateWrapper>);
}
