...
export function TimePicker({ value, onChange, disabled = false, minuteStep = 15, className, id, }: TimePickerProps) {
    ...
    return (<div id={id} className={cn('grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] gap-2', className)}>
      <Select value={hourValue} onValueChange={handleHourChange} disabled={disabled}>
        <SelectTrigger aria-label="Hour" className="min-w-0">
          <SelectValue placeholder="Hour"/>
        </SelectTrigger>
        <SelectContent>
          {Array.from({ length: 12 }, (_, index) => index + 1).map((hourOption) => (<SelectItem key={hourOption} value={String(hourOption)}>
              {hourOption}
            </SelectItem>))}
        </SelectContent>
      </Select>

      <Select value={minuteValue} onValueChange={handleMinuteChange} disabled={disabled}>
        ...
        <SelectContent>
          {minuteOptions.map((minuteOption) => {
            ...
            return (<SelectItem key={optionValue} value={optionValue}>
                {optionValue}
              </SelectItem>);
        })}
        </SelectContent>
