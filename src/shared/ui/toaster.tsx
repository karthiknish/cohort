...
export function Toaster() {
    ...
    const legacyToasts = toasts.filter((t) => t.variant !== 'loading' && t.variant !== 'info');
    if (legacyToasts.length === 0) {
        return null;
    }
    return (<ToastProvider swipeDirection="right" duration={5000}>
      {legacyToasts.map((toast) => (<LegacyToastItem key={toast.id} dismiss={dismiss} toast={toast}/>))}
      <ToastViewport />
    </ToastProvider>);
}
function LegacyToastItem({ dismiss, toast }: {
    dismiss: (toastId?: string) => void;
    toast: LegacyToast;
}) {
    const { push } = useRouter();
    const { id, title, description, action, onNavigate, href, onMarkRead, persistent, undoLabel, onUndo, duration, ...props } = toast;
    ...
    const onToastNavigate = (event: React.MouseEvent<HTMLLIElement>) => {
        const target = event.target as HTMLElement | null;
        if (target?.closest('[data-toast-action="true"]'))
            return;
        if (onNavigate || href) {
            handleNavigate();
        }
    ...
    const handleMarkReadPointerDown = (event: React.PointerEvent<HTMLButtonElement>) => {
        event.preventDefault();
        event.stopPropagation();
    };
    ...
    return (<Toast {...(restProps as Omit<typeof restProps, 'variant'> & {
        variant?: 'default' | 'success' | 'destructive' | 'warning';
    })} duration={duration ?? (persistent ? 0 : 5000)} onOpenChange={handleOpenChange} onClick={onToastNavigate} role={onNavigate || href ? 'button' : restProps.role} tabIndex={onNavigate || href ? 0 : restProps.tabIndex} onKeyDown={handleKeyDown}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {action}
        {onUndo && (<ToastAction data-toast-action="true" altText={undoLabel ?? 'Undo'} onClick={handleUndoClick}>
            {undoLabel ?? 'Undo'}
          </ToastAction>)}
        {onMarkRead && (<ToastAction data-toast-action="true" altText="Mark as read" onPointerDown={handleMarkReadPointerDown} onClick={handleMarkReadClick}>
            Mark as read
          </ToastAction>)}
