...
export function HoverPreview({ trigger, children, side = 'top', align = 'center', className, }: HoverPreviewProps) {
    return (<HoverCard>
...
function TruncatedTextPreviewTrigger({ text, className }: TruncatedTextPreviewTriggerProps) {
    return (<button type="button" className={cn('block min-w-0 truncate text-left', className)}>
      {text}
    </button>);
}
...
export function TruncatedTextPreview({ text, className, detail }: TruncatedTextPreviewProps) {
    const trigger = <TruncatedTextPreviewTrigger text={text} className={className}/>;
    return (<HoverPreview trigger={trigger}>
...
function MetricHintTrigger({ description, label, className }: MetricHintTriggerProps) {
    return (<button type="button" className={cn('inline-flex size-5 shrink-0 items-center justify-center rounded-full text-muted-foreground/50 transition-colors hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring', className)} aria-label={label ?? description}>
...
export function MetricHint({ description, label, className }: MetricHintProps) {
    const trigger = <MetricHintTrigger description={description} label={label} className={className}/>;
    return (<HoverPreview trigger={trigger} className="max-w-xs">
...
export function MetricCardPreview({ children, description }: MetricCardPreviewProps) {
    const trigger = <MetricCardPreviewTrigger>{children}</MetricCardPreviewTrigger>;
    return (<HoverPreview trigger={trigger} className="max-w-xs">
