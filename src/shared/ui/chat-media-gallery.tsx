...
import { Download, ExternalLink, File, FileArchive, FileSpreadsheet, FileText, Film, ImageIcon, Music2, MonitorPlay, } from 'lucide-react';
...
export function AttachmentKindIcon({ kind, className }: {
    kind: AttachmentKind;
    className?: string;
}) {
...
function AttachmentName({ name, highlightTerms, }: {
    ...
    return (<span className="truncate font-medium" title={name}>
...
function MediaTile({ attachment, kind, highlightTerms, onOpenPdf, compact, }: {
    ...
    if (kind === 'video') {
        return (<div className={cn('overflow-hidden rounded-2xl border border-border/60 bg-card/80 shadow-sm', compact && 'max-w-md')}>
        <div className="flex items-center justify-between gap-2 border-b border-border/50 px-3 py-2">
          <div className="flex min-w-0 items-center gap-2 text-sm">
            ...
            {attachment.size ? (<Badge variant="secondary" className="shrink-0 text-[10px]">
                {attachment.size}
              </Badge>) : null}
          </div>
        ...
        <div className="bg-muted/20 p-1">
          <video src={attachment.url} controls aria-label={attachment.name || 'Video attachment'} className="max-h-72 w-full rounded-xl bg-foreground" preload="metadata">
    ...
    if (kind === 'pdf') {
        return (<div className={cn('group flex items-center gap-3 rounded-2xl border border-border/60 bg-card/90 p-3 shadow-sm transition hover:border-primary/25 hover:shadow-md motion-reduce:transition-none', compact && 'max-w-md')}>
        <span className={cn('flex size-11 shrink-0 items-center justify-center rounded-xl ring-1', KIND_SURFACE.pdf)}>
        ...
        <div className="flex shrink-0 items-center gap-1">
          {onOpenPdf ? (<Button type="button" variant="outline" size="sm" className="h-8 gap-1 rounded-lg" onClick={handleOpenPdf}>
    ...
    return (<a href={attachment.url} target="_blank" rel="noopener noreferrer" className={cn('group flex items-center gap-3 rounded-2xl border border-border/60 bg-card/90 p-3 shadow-sm transition hover:border-primary/25 hover:shadow-md motion-reduce:transition-none', compact && 'max-w-md')}>
      <span className={cn('flex size-11 shrink-0 items-center justify-center rounded-xl ring-1', KIND_SURFACE[kind])}>
...
export function ChatMediaGallery({ attachments, highlightTerms, compact = false, className, }: ChatMediaGalleryProps) {
    ...
    return (<div className={cn('space-y-3', className)}>
      {!compact && attachments.length > 1 ? (<div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
          <span>
            {attachments.length} attachment{attachments.length === 1 ? '' : 's'}
          </span>
          <Button type="button" variant="ghost" size="sm" className="h-7 gap-1 rounded-lg px-2" onClick={handleDownloadAll} disabled={downloadable.length === 0}>
            <Download className="size-3.5" aria-hidden/>
            Download all
          </Button>
        </div>) : null}

      {grouped.images.length > 0 ? (<ImageGallery images={grouped.images.map((a) => ({
                url: a.url,
                name: a.name,
                size: a.size ?? undefined,
            }))}/>) : null}

      {grouped.videos.map((attachment) => (<MediaTile key={`${attachment.url}-video`} attachment={attachment} kind="video" highlightTerms={highlightTerms} compact={compact}/>))}

      {grouped.pdfs.map((attachment) => (<MediaTile key={`${attachment.url}-pdf`} attachment={attachment} kind="pdf" highlightTerms={highlightTerms} onOpenPdf={setActivePdf} compact={compact}/>))}

      {grouped.other.map((attachment) => (<MediaTile key={`${attachment.url}-${attachment.name}`} attachment={attachment} kind={getAttachmentKind(attachment)} highlightTerms={highlightTerms} compact={compact}/>))}

      {grouped.unavailable.map((attachment) => (<div key={`${attachment.name}-pending`} className="flex items-center gap-3 rounded-2xl border border-dashed border-border/70 bg-muted/15 p-3 text-sm">
      ...
      <Dialog open={Boolean(activePdf)} onOpenChange={handlePdfDialogOpenChange}>
        <DialogContent className="flex max-h-[90vh] max-w-4xl flex-col gap-0 overflow-hidden p-0">
          ...
          {activePdf ? (<div className="flex min-h-0 flex-1 flex-col">
              <iframe src={activePdf.url} title={activePdf.name} sandbox="" className="min-h-[60vh] w-full flex-1 bg-muted/20"/>
