...
export function ConfirmDialog({ open, onOpenChange, title, description, confirmLabel = 'Confirm', cancelLabel = 'Cancel', variant = 'default', isLoading = false, onConfirm, onCancel, }: ConfirmDialogProps) {
        ...
        <AlertDialogHeader>
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-2">
              ...
              {description && (<AlertDialogDescription className="text-sm leading-relaxed">
                  {description}
...
export function useConfirmDialog(defaultOptions: UseConfirmDialogOptions) {
    ...
    const confirm = (options?: Partial<UseConfirmDialogOptions>): Promise<boolean> => {
        if (options) {
    ...
    const ConfirmDialogComponent = () => (<ConfirmDialog open={isOpen} onOpenChange={setIsOpen} title={optionsRef.current.title} description={optionsRef.current.description} confirmLabel={optionsRef.current.confirmLabel} cancelLabel={optionsRef.current.cancelLabel} variant={optionsRef.current.variant} isLoading={isPending} onConfirm={handleConfirm} onCancel={handleCancel}/>);
    return {
        confirm,
        isOpen,
        isPending,
