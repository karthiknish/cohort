...
export function AnalyticsEmptyState({ variant = 'no-data', title, description, icon, action, className, }: Props) {
    ...
    return (<div className={cn('flex flex-col items-center justify-center py-12 px-4 text-center', className)}>
      ...
      {action && (<Button variant="outline" size="sm" onClick={action.onClick}>
          {action.label}
        </Button>)}
    </div>);
}
// Specialized empty state components for common analytics scenarios
export function NoPerformanceData({ className }: {
    className?: string;
}) {
    return (<AnalyticsEmptyState variant="no-data" title="No Performance Data" description="There is no performance data available for the selected filters and time period." className={className}/>);
}
export function NoCreativeData({ platform = 'Meta' }: {
    platform?: string;
}) {
    return (<AnalyticsEmptyState variant="no-metrics" title={`No ${platform} Creative Data`} description={`Creative-level insights are not available. Ensure ${platform} syncs are configured with active creative data.`}/>);
}
export function NoInsightsData({ className }: {
    className?: string;
}) {
    return (<AnalyticsEmptyState variant="no-filters" title="No Insights Available" description="No specific optimizations identified for the current data set. Try adjusting your filters or time period." className={className}/>);
}
export function NoIntegrationConnected({ onConnect, platform, className, }: {
    onConnect?: () => void;
    platform?: string;
    className?: string;
}) {
    return (<AnalyticsEmptyState variant="no-integration" title={`Connect Your ${platform || 'Ad'} Account`} description={`Connect your ${platform || 'ad platform'} account to start seeing performance data and insights.`} action={createConnectAction(onConnect, platform)} className={className}/>);
}
export function AnalyticsError({ message, onRetry, }: {
    message?: string;
    onRetry?: () => void;
}) {
    return (<AnalyticsEmptyState variant="error" title="Unable to Load Analytics" description={message || 'There was a problem loading the analytics data. Please try again.'} action={createRetryAction(onRetry)}/>);
}
