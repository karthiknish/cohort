...
function UrlSearchParamsReader({ children }: { children: ReactNode }) {
  const searchParams = useSearchParams()
  const value = useMemo(() => new URLSearchParams(searchParams.toString()), [searchParams])

  return (
    <UrlSearchParamsContext.Provider value={value}>{children}</UrlSearchParamsContext.Provider>
  )
}

export function UrlSearchParamsProvider({ children }: { children: ReactNode }) {
  return (
