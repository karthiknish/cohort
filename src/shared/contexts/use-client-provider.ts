...
export function useClientProvider(): ClientContextValue {
    ...
    const [{ selectedClientId, loading, error }, dispatch] = useReducer(clientProviderReducer, undefined, createInitialClientProviderState);
    const hasInitializedRef = useRef(false);
    const previewEnabled = useSyncExternalStore((onStoreChange) => {
    ...
    useEffect(() => {
        ...
        if (!selectedClientId)
            return;
        if (resolvedClients.some((client) => client.id === selectedClientId))
            return;
