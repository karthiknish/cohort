...
function ProfileMenuItem({ icon: Icon, label, destructive, onSelect, href, onNavigate, }: {
    ...
    const iconClass = cn(HEADER_DROPDOWN_THEME.menuItemIcon, destructive && HEADER_DROPDOWN_THEME.menuItemIconDestructive);
    const content = (<>
    ...
    if (href) {
        return (<DropdownMenuItem asChild className={HEADER_DROPDOWN_THEME.menuItem}>
        <Link href={href} onClick={onNavigate} className="flex w-full items-center gap-2.5">
          {content}
        </Link>
      </DropdownMenuItem>);
    }
    return (<DropdownMenuItem variant={destructive ? 'destructive' : 'default'} onSelect={onSelect} className={HEADER_DROPDOWN_THEME.menuItem}>
      {content}
    </DropdownMenuItem>);
}
export function ProfileDropdown({ displayedName, displayedEmail, initials, isPreviewMode, roleLabel, isAdmin, onNavigate, onOpenHelp, onSignOut, }: ProfileDropdownProps) {
    const accent = useDashboardRoleAccent();
    const badgeLabel = isPreviewMode ? 'Preview' : roleLabel;
    return (<DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" className={HEADER_DROPDOWN_THEME.triggerAvatar} aria-label={`Open account menu for ${displayedName}`}>
      ...
      <DropdownMenuContent align="end" sideOffset={8} className={cn(HEADER_DROPDOWN_THEME.panel, HEADER_DROPDOWN_THEME.panelProfile)}>
            ...
            <div className={HEADER_DROPDOWN_THEME.profileIdentity}>
              ...
              <div className="min-w-0 flex-1">
                <div className="flex min-w-0 flex-wrap items-center gap-1.5">
                  <TruncatedTextPreview text={displayedName} className="text-sm font-semibold text-foreground" detail={displayedEmail}/>
                  {badgeLabel ? (<Badge variant="secondary" className={cn('h-5 shrink-0 px-1.5 text-[10px] font-semibold uppercase tracking-wide', accent.accountBadgeClass)}>
                      {badgeLabel}
                    </Badge>) : null}
                </div>
                {displayedEmail ? (<p className="truncate text-xs text-muted-foreground">{displayedEmail}</p>) : null}
              </div>
            </div>
          </DropdownMenuLabel>
        </div>
        ...
        <div className={HEADER_DROPDOWN_THEME.menuBody}>
          ...
          {isAdmin ? (<ProfileMenuItem icon={Shield} label="Admin panel" href="/admin" onNavigate={onNavigate}/>) : null}
