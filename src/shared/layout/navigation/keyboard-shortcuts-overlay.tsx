...
export function KeyboardShortcutsOverlay({ open, onOpenChange, includeContexts = ['global', 'proposal-builder'], }: KeyboardShortcutsOverlayProps) {
    const { user } = useAuth();
    const shortcuts = includeContexts.flatMap((context) => getShortcutsForRole(user?.role, context));
    const groupedShortcuts = shortcuts.reduce<Record<string, typeof shortcuts>>((accumulator, shortcut) => {
      ...
      <DialogContent className="sm:max-w-2xl overflow-hidden p-0">
        ...
        <ScrollArea className="max-h-[70vh] px-6 py-5">
          <div className="space-y-6">
            {Object.entries(groupedShortcuts).map(([group, items]) => (<section key={group} className="space-y-3">
                ...
                <div className="grid gap-2">
                  {items.map((shortcut) => (<div key={`${shortcut.context}-${shortcut.combo}-${shortcut.description}`} className="flex items-center justify-between gap-4 rounded-xl border border-muted/40 bg-muted/10 px-4 py-3">
