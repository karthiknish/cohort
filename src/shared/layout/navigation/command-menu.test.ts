...
describe('getNavigationItemsForUserRole', () => {
    ...
    it('hides admin Clients for non-admins', () => {
        expect(getNavigationItemsForUserRole('client').some((i) => i.name === 'Clients' && i.href === '/admin/clients')).toBe(false);
        expect(getNavigationItemsForUserRole('team').some((i) => i.name === 'Clients' && i.href === '/admin/clients')).toBe(false);
    });
...
describe('getQuickActionsForUserRole', () => {
    it('hides create proposal for client', () => {
        expect(getQuickActionsForUserRole('client').some((a) => a.action === '/dashboard/proposals' && a.name === 'Create proposal')).toBe(false);
    });
    it('shows create proposal for team', () => {
        expect(getQuickActionsForUserRole('team').some((a) => a.action === '/dashboard/proposals' && a.name === 'Create proposal')).toBe(true);
    });
});
