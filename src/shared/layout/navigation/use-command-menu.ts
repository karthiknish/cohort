...
import { includesQuery, limitResults, type CommandMenuProps, type SearchEntityResult, type SearchableClient, type SearchableProject, type SearchableProposal, type SearchableTask, } from './command-menu-types';
export function useCommandMenu({ onOpenHelp, onOpenShortcuts }: CommandMenuProps) {
    ...
    const clientRows = useQuery(clientsApi.list, canQueryWorkspace
        ...
        : 'skip') as Array<SearchableClient> | undefined;
    const taskRows = useQuery(selectedClientId ? tasksApi.listByClient : tasksApi.listForUser, canQueryWorkspace
        ? selectedClientId
            ? { workspaceId: workspaceId!, clientId: selectedClientId, limit: 100 }
            : user?.id
                ? { workspaceId: workspaceId!, userId: user.id }
                : 'skip'
        : 'skip') as Array<SearchableTask> | undefined;
    const projectRows = useQuery(projectsApi.list, canQueryWorkspace
        ...
        : 'skip') as Array<SearchableProject> | undefined;
    const proposalRows = useQuery(proposalsApi.list, canQueryWorkspace && selectedClientId
        ...
        : 'skip') as Array<SearchableProposal> | undefined;
