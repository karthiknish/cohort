...
type WorkspaceLayoutProps = {
  ...
  /** When set (TanStack Start route loader), skips async `headers()` lookup. */
...
/** Next.js App Router layout — resolves preview header on the server. */
export async function WorkspaceLayout({ children, allowPreviewAccess }: WorkspaceLayoutProps) {
  ...
  if (resolvedPreviewAccess === undefined) {
    ...
    resolvedPreviewAccess =
      isScreenRecordingAuthBypassEnabled() ||
      requestHeaders.get(PREVIEW_ROUTE_REQUEST_HEADER) === '1'
  }
  return (
    <WorkspaceLayoutInner allowPreviewAccess={resolvedPreviewAccess}>
      {children}
