...
import { fetchGoogleCampaignInsights } from '@/services/integrations/google-ads/campaign-insights'
...
type CampaignInsightsResult = {
  ...
  series: SeriesRow[]
  insights: ReturnType<typeof calculateMetaAdsInsights> | PlatformInsightResult
  currency: string
}
...
export const getCampaignInsights = action({
  ...
  handler: async (ctx, args): Promise<CampaignInsightsResult> =>
    withErrorHandling(async (): Promise<CampaignInsightsResult> => {
      ...
      if (args.providerId === 'google') {
        ...
        if (!accountId) {
          throw Errors.integration.notConfigured('Google', 'Ad account ID not configured')
        }
        ...
        const result = await fetchGoogleCampaignInsights({
          accessToken,
          developerToken,
      ...
      if (args.providerId !== 'facebook') {
        throw Errors.base.notImplemented(`Campaign insights for ${args.providerId}`)
      }
